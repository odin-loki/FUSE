...
| BRDF look-up tables (ported: `Renderer/include/fuse/renderer/lighting/ltc/ltc_kernel.hpp`) | Item, 8x8 | `brdf_lut.dfg`: one DFG texel (the split A, B of the GGX lobe's albedo at N.V and roughness) per item, 1,024 VNDF Hammersley samples; `brdf_lut.sphere`: one horizon-clipped cap texel per item. `ltc::bakeBrdfLut` runs both once per process (`sharedBrdfLut`, CpuParallel; about 220 ms serial). CPU backends only (no `.cu` entry yet). Gate: `fuse_rp_brdf_dfg` (bit-exact at 0, 2 and 4 workers, stats, fallback, f64 references). |
