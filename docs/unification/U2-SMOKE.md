...
| 18 | `core/util/zip/*` archive core (`zipArchive`, `zipSubStream`, `zipCryptStream`, `zipTempStream`, `compressor` + deflate/stored, `fileHeader`, `centralDir`, `extraField`), `version_probe_stub.cpp`, deepen `string_stub` (`getHashCaseInsensitive`), deepen `fs_volume_stub` (`Remove`/`Rename`) | `ZipArchive` MemStream deflate write/read round-trip (`fuse_u2_probe.txt`); **not** `zipVolume.cpp` (FS mount + `console`) or `zipObject.cpp` (`engineAPI`) |
...
**Next work should harden docs/stubs and deepen existing smoke** (e.g. zip ReadWrite disk rebuild path, PNG when libpng present) rather than force-linking blocked TUs above.
