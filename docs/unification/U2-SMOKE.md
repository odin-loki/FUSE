...
| Engine `.cpp` probe | ✅ **opt-in** `-DFUSE_T3D_LEGACY_ENGINE_PROBE=ON` | Batch 1–6 (see table); batch 7: `writeBitmap`/`writeBitmapStream` + `Face::getAddress` on gBitmap stub, `bitmapPng.cpp` (when libpng+zlib found), `crc.cpp`, `bitVector.cpp`, `idGenerator.cpp`, `tDictionary.cpp`, `platform_assert_stub.cpp` |
...
**Still blocked:** **`gBitmap.cpp` / `ddsFile.cpp` / `resourceManager.cpp`** — full 3k+ LOC bitmap I/O, `Resource<GBitmap>` load path, DDS encode/decode, `engineAPI` registration; probe uses **gBitmap subset stub** with registration dispatch only. **`rgb2xyz.cpp` / `rgb2luv.cpp`** — pull `math_backend` dispatch tables (not low-dep). **`bitmapSTB.cpp` IES branch** compiles but is not smoke-tested (needs FS + IES sidecar PNG). **STB `writeStreamFunc`** prefixes chunks via `stbiWriteFunc` — not raw image bytes; stream round-trip smoke uses PNG (`pngWriteDataFn`) not STB BMP.
