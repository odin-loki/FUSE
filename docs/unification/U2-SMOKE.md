...
| Engine `.cpp` probe | ✅ **opt-in** `-DFUSE_T3D_LEGACY_ENGINE_PROBE=ON` | Batch 1–17 (see table); batch 17: `StringUnit::setUnit`/`removeUnit`, deepen `gbitmap_probe_stub` `deleteImage`/`allocateBitmapWithMips` |
...
**Remaining Engine probe batches (post-17):** `memVolume.cpp` / `volume.cpp` (FS/platform closure), `streamObject.cpp` (SimObject), full `gBitmap.cpp` I/O, `str.cpp`/`uuid.cpp` real TUs, zip archive subtree, Gui/script VM stacks.
