...
| Engine `.cpp` probe | ✅ **opt-in** `-DFUSE_T3D_LEGACY_ENGINE_PROBE=ON` | Batch 1: `bitmapUtils.cpp`; batch 2: `ies_loader.cpp`, `md5.cpp`; batch 3: `hashFunction.cpp`, `commonSwizzles.cpp`; batch 4: `stream.cpp`, `memStream.cpp`, `fileStream.cpp`, `path.cpp`, `byteBuffer.cpp`, `refBase.cpp`, `stringFunctions.cpp`; batch 5: `bitmapSTB.cpp`, `tVector.cpp` + Con/profiler/resource/gBitmap subset stubs; batch 6: `readBitmap`/`readBitmapStream` on gBitmap stub, `timeClass.cpp`, `tSignal.cpp` |
...
Under `-std=c++17` GCC drops the legacy `linux` macro; probe compile adds `-DLINUX=1` so `types.gcc.h` pulls `types.posix.h` (`dsize_t`, `FileTime`). Quarantine stubs in `engine_probe/`: `platform_stub.cpp` (`dMem*`, `dMalloc`, `dMalloc_r`), extended `string_stub.cpp` (path + hash String surface), `frame_allocator_stub.cpp`, shadow headers (`console/console.h`, `console/engineAPI.h`, `core/color.h`, `core/resource.h`, `core/stringTable.h`, `gfx/bitmap/imageUtils.h`, `platform/profiler.h`, `platform/platformNet.h`, `platform/threads/threadPool.h`), `fs_volume_stub.cpp` (POSIX `Torque::FS::OpenFile`/`CreatePath`), `thread_pool_stub.cpp` (sync `WorkItem` dispatch), `console_stub.cpp` (`Con::printf`/`errorf`), `gbitmap_probe_stub.cpp` (minimal `GBitmap` alloc/register/transparency/**readBitmap dispatch** — **not** full `gBitmap.cpp` / `resourceManager.cpp`). **`fourcc.h` is header-only** (no `fourcc.cpp` in Engine tree) — batch 4 smoke exercises `MakeFourCC` directly. **`PROFILE_START_IF` shadow** expands to `if (false) { (void)0; }` so bitmapSTB's if/else profiler chain parses with profiler disabled.

**Still blocked (batch 6 audit):** `bitmapPng.cpp` — skipped when system `libpng`/`lpng` dev headers unavailable (CMake `find_package(PNG)` gate; bundled `lpng/` still needs dev install). **`gBitmap.cpp` / `ddsFile.cpp` / `resourceManager.cpp`** — full 3k+ LOC bitmap I/O, `Resource<GBitmap>` load path, DDS encode/decode, `engineAPI` registration; probe uses **gBitmap subset stub** with registration dispatch only. **`bitmapSTB.cpp` IES branch** compiles but is not smoke-tested (needs FS + IES sidecar PNG).
