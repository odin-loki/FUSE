...
| Engine `.cpp` probe | ✅ **opt-in** `-DFUSE_T3D_LEGACY_ENGINE_PROBE=ON` | Batch 1–18 (see table); batch 18: `zipArchive` core subtree (MemStream deflate write/read — no `zipVolume`/`zipObject`) |
...
| 17 | deepen `stringUnit` smoke (`setUnit`/`removeUnit`), deepen `gbitmap_probe_stub` (`deleteImage`, `allocateBitmapWithMips`) | `StringUnit::setUnit` middle-unit replace + `removeUnit` middle/tail; `GBitmap::deleteImage` clears byte size then reallocate; `allocateBitmapWithMips(4×4, 2 mips)` caps mip chain at 2 levels |
| 18 | `core/util/zip/*` archive core (`zipArchive`, `zipSubStream`, `zipCryptStream`, `zipTempStream`, `compressor` + deflate/stored, `fileHeader`, `centralDir`, `extraField`), `version_probe_stub.cpp`, deepen `string_stub` (`getHashCaseInsensitive`), deepen `fs_volume_stub` (`Remove`/`Rename`) | `ZipArchive` MemStream deflate write/read round-trip (`fuse_u2_probe.txt`); disk **ReadWrite** rebuild (`/tmp/fuse_u2_zip_disk_probe.zip` write → ReadWrite add → read both entries); **not** `zipVolume.cpp` (FS mount + `console`) or `zipObject.cpp` (`engineAPI`) |
...
Under `-std=c++17` GCC drops the legacy `linux` macro; probe compile adds `-DLINUX=1` so `types.gcc.h` pulls `types.posix.h` (`dsize_t`, `FileTime`). Quarantine stubs in `engine_probe/`: `platform_stub.cpp` (`dMem*`, `dMalloc`, `dMalloc_r`), `platform_assert_stub.cpp` (`avar` **`thread_local`**, `PlatformAssert::processAssert`, `Platform::debugBreak` — for `bitmapPng.cpp` AssertISV paths), extended `string_stub.cpp` (path + hash + **`startsWith`/`endsWith`** + **`getHashCaseInsensitive`** — **not** full `str.cpp`), **`string_table_stub.cpp`** (owned-copy intern set — **stable `StringTableEntry` pointers**), `frame_allocator_stub.cpp`, shadow headers (`console/console.h`, `console/consoleTypes.h`, `console/consoleInternal.h`, `console/consoleObject.h` — `AbstractClassRep` static tables for `BitStream::writeClassId`, `console/engineTypeInfo.h` — minimal `EngineFieldTable` for `uuid.h`, `console/engineAPI.h`, `console/enginePrimitives.h`, `core/color.h` shim for non-color TUs, `core/resource.h`, `core/stringTable.h`, `gfx/bitmap/imageUtils.h`, `platform/profiler.h`, `platform/platformNet.h` + `Net::sendto` stub, `platform/threads/threadPool.h`), `color_cpp_prelude.h` + `color_real_wrapper.h` (force-include on `color.cpp` / `color_probe_smoke.cpp` only — real `core/color.h` + `DefineEngineFunction` stub + MODULE macro no-op), `fs_volume_stub.cpp` (POSIX `Torque::FS::OpenFile`/`CreatePath`/`Remove`/`Rename`), `thread_pool_stub.cpp` (sync `WorkItem` dispatch), `console_stub.cpp` (`Con::printf`/`errorf`/`warnf` + **`getBoolVariable`/`setBoolVariable`** for `$pref::Zip::Verbose`), `console_object_stub.cpp` (`AbstractClassRep::NetClassCount`/`NetClassBitSize`), `math_backend_probe_stub.cpp` (scalar `float3`/`float4`/`mat44` dispatch for `bitStream.cpp` — not full `math_backend.cpp` SIMD paths), `bitstream_prelude.h` (force-include `tVector.h` for Huffman `Vector<>` members), `gbitmap_probe_stub.cpp` (minimal `GBitmap` alloc/register/transparency/**read/writeBitmap dispatch** + `Face::getAddress` + **`copyRect` RGB/RGBA8** + **`fillWhite`/`getSurfaceSize`/`sGetExtensionList`/`getColor`/`setColor` RGB/RGBA8** + **`allocateBitmap` `extrudeMipLevels` mip chain** + **`allocateBitmapWithMips` explicit mip cap** + **`deleteImage` reallocate** + **`chopTopMips`** + **`getSurfaceSize` BC1/BC3 block layout** — **not** full `gBitmap.cpp` / `resourceManager.cpp`), `rgb2xyz_probe_stub.cpp` (sRGB↔XYZ matrix multiply — **not** full `rgb2xyz.cpp` / `math_backend`), `rgb2luv_probe_stub.cpp` (`toLUV`/`toLUVScaled` — **not** full `rgb2luv.cpp` / `mPoint2`/`mPoint3`), **`uuid_probe_stub.cpp`** (v1 `UUID::generate`/`toString`/`fromString`/`getHash` — **not** full `uuid.cpp` / `UUIDEngineExport`), **`version_probe_stub.cpp`** (`getVersionNumber` for `zipArchive.cpp` — **not** full `version.cpp` / `DefineEngineFunction`). **`fourcc.h` is header-only** (no `fourcc.cpp` in Engine tree) — batch 4 smoke exercises `MakeFourCC` directly. **`PROFILE_START_IF` shadow** expands to `if (false) { (void)0; }` so bitmapSTB's if/else profiler chain parses with profiler disabled. **`dataChunker.cpp`** is an include anchor — implementation is header-only templates in `dataChunker.h`.

**`bitmapPng.cpp` gate (batch 7):** CMake `find_package(PNG)` + `find_package(ZLIB)`; when both found, links `bitmapPng.cpp` with bundled `Engine/lib/lpng` headers + system libs, defines `FUSE_T3D_LEGACY_ENGINE_PROBE_PNG=1` on `fuse_t3d_legacy` and `fuse_runtime_smoke`. `bitmapPngRegisterAnchor()` defeats static-lib GC of PNG format registration. Skipped on hosts without dev packages. Deepen: `writeBitmapPngRoundTripSmoke` verifies RGBA8 pixel at (1,0) after MemStream encode/decode when libpng present.

**`zipArchive` core gate (batch 18):** CMake `find_package(ZLIB)`; when found, links zip archive core subtree (10 TUs) and defines `FUSE_T3D_LEGACY_ENGINE_PROBE_ZIP=1`. Uses `version_probe_stub.cpp` (avoids `version.cpp` `DefineEngineFunction` registration) and extended `fs_volume_stub`/`string_stub` for `ZipTempStream`/`tDictionary` link closure. **`zipVolume.cpp`** (Torque::FS mount + `ZipFileSystem`) and **`zipObject.cpp`** (script wrapper / `engineAPI`) remain blocked. Zip encrypted/AES paths not smoke-tested.

**Probe surface exhaustion (batch 18 honest assessment):** The low-risk util/stream/core band is largely covered (batches 1–18). Remaining safe TUs are scarce without crossing into blocked closures:

| Candidate | Blocker |
|-----------|---------|
| `zipVolume.cpp` | `Torque::FS::Mount`, `console`, full volume mount system |
| `memVolume.cpp` / `volume.cpp` | Virtual mount system, `console`, platform FS trees |
| `streamObject.cpp` / `fileStreamObject.cpp` | `SimObject` / `IMPLEMENT_CONOBJECT` |
| `gBitmap.cpp` / `resourceManager.cpp` | `Resource<GBitmap>`, `engineAPI`, 3k+ LOC I/O |
| `str.cpp` / `uuid.cpp` | `engineAPI`, `math/mMathFn.h`, mutex/profiler/`StrFormat` |
| `frameAllocator.cpp` | Real allocator + profiler; probe uses `frame_allocator_stub.cpp` |
| `module.cpp` / `iTickable.cpp` / `dnet.cpp` | SimObject / net / tick registration |
| `virtualMountSystem.cpp` | Full VFS mount stack |
| `resource.cpp` | Resource manager singleton |
| `fileObject.cpp` | ConsoleObject / script surface |
| `stringTable.cpp` | Real singleton; probe uses `string_table_stub.cpp` |
| `ogg/*` | Decoder + stream + resource closure |
| `journal/*` | Process/platform test harness |

**Deepen-only policy (post-18):** Harden docs/stubs and deepen existing smoke rather than force-linking blocked TUs above. Zip disk ReadWrite rebuild + PNG pixel verify (when libpng present) landed; no new probe batch number.

**Still blocked:** **`gBitmap.cpp` / `ddsFile.cpp` / `resourceManager.cpp`** — full 3k+ LOC bitmap I/O, `Resource<GBitmap>` load path, DDS encode/decode, `engineAPI` registration; probe uses **gBitmap subset stub** with registration dispatch + batch-10/17 pixel helpers (`fillWhite`, `getSurfaceSize` **BC1/BC3 block layout + mip-level sizes**, `sGetExtensionList`, **`copyRect` RGB/RGBA8** (batch 16 deepens RGB8), **`getColor`/`setColor` RGB/RGBA8**, **`allocateBitmap` `extrudeMipLevels`**, **`allocateBitmapWithMips` explicit mip cap**, **`deleteImage`**, **`chopTopMips`**) — still no `extrudeMipLevels()` filter pass, cubemap face paths, or compressed-bitmap pixel I/O. **`core/util/str.cpp`** — pulls `console/engineAPI.h`, `math/mMathFn.h`, mutex/profiler/unicode/`StrFormat` closure; probe keeps **`string_stub.cpp`** (batch 12 adds `startsWith`/`endsWith`; not full `str.cpp` / `StringBuilder`). **`core/util/uuid.cpp`** — full TU pulls `UUIDEngineExport` / `engineTypeInfo.cpp`; probe uses **`uuid_probe_stub.cpp`** (v1 generate/parse only). **`rgb2xyz.cpp` / `rgb2luv.cpp`** — full Engine TUs pull `math/mMatrix.h` / `math_backend`; probe uses **`rgb2xyz_probe_stub.cpp`** for `toXYZ`/`fromXYZ` and **`rgb2luv_probe_stub.cpp`** for `toLUV`/`toLUVScaled` only (no `fromLUV`). **`bitmapSTB.cpp` IES branch** compiles but is not smoke-tested (needs FS + IES sidecar PNG). **STB `writeStreamFunc`** prefixes chunks via `stbiWriteFunc` — not raw image bytes; stream round-trip smoke uses PNG (`pngWriteDataFn`) when libpng present, else TGA encode-only via `writeBitmapStreamRoundTripSmoke`. **STB JPG/HDR file-path write** not smoke-tested (BMP path round-trip covered in batch 9). **`stringBuffer.cpp`** debug `DefineEngineFunction` paths skipped (no `TORQUE_DEBUG`).
...
| Metric | Value |
|--------|-------|
| **HEAD** | *(updated at push)* — batch 18 deepen (zip disk ReadWrite + PNG pixel verify) |
...
**Remaining Engine probe batches (post-18):** `zipVolume.cpp` (FS mount), `memVolume.cpp` / `volume.cpp` (virtual mount), `streamObject.cpp` (SimObject), full `gBitmap.cpp` I/O, `str.cpp`/`uuid.cpp` real TUs, `zipObject.cpp`, Gui/script VM stacks. See **Probe surface exhaustion** table in §3.1 — util/stream safe band largely covered; future batches should deepen existing smoke/stubs rather than force-link blocked TUs.
