...
Regression tests: `fuse_core_jobs` — `testParallelSerialFallbackParity`, `testNestedParallelForParity`, `testNestedParallelForSerialFallbackParity`, `testNestedParallelForWithCooperativeWait`, `testNestedParallelForMultiWorkerVisitCount`, `testNestedParallelForStressVisitCoverage` (per-index `atomic<bool>` visit matrix; avoids aggregate-count flakes), work-steal helper/integration tests (`testWorkSteal*`), `testNestedParallelForVisitCountSingleThread`; `fuse_core_jobs_single_thread` — nested parity + visit stress under `FUSE_JOBS_SINGLE_THREAD`.
