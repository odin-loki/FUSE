...
| `particle_soa::*` | `Source/FUSE/VFX/include/fuse/vfx/particle_soa_ops.hpp` | Standalone burst/rate emit, `clamp_burst_count` / `is_at_capacity` / `has_live_particles`, `free_slot_count` / `sync_alive_count`, batch `recycle_slots`, `lifetime_cull`, and `simulate_step` CPU stubs |
...
- **SoA helpers** — `particle_soa::burst_emit` and `accumulate_rate_emit` fill `ParticleSoA` directly with deterministic `seed` RNG (splitmix-style `mix_seed`). `BurstEmitResult::requested` / `emitted` / `clamped` report requested vs actual fill (clamped at capacity; zero-capacity / `burst_emit(0)` are no-ops). `clamp_burst_count`, `is_at_capacity`, and `has_live_particles` expose capacity guards ahead of emission. `free_slot_count` and `sync_alive_count` expose free-list and alive diagnostics. `ParticleEmitter::burst` returns the emitted count. `ParticleEmitter` delegates to these helpers.
...
- **Rate** — accumulator driven by `emit_rate` (particles/second) during `simulate`; accumulator clears when the free list is exhausted at capacity (`RateEmitResult::at_capacity`).
...
`particle_soa::lifetime_cull` ages live slots and recycles expired particles without integrating motion or attributes — a standalone stub for future GPU kill passes. Empty emitters (`count == 0`) and non-positive `dt` short-circuit without scanning slots. `LifetimeCullResult::aged` / `culled` / `alive_after` report per-pass bookkeeping; `recycle_slots` batch-returns dead indices to the free list. `particle_soa::simulate_step` (used by `ParticleEmitter::simulate`) jobifies per-slot integration via `fuse::jobs::parallel_for` with grain size 64. Empty emitters short-circuit the same way. `SimStepResult::integrated` / `culled` / `alive_after` report per-pass bookkeeping. Each live slot:
...
| Test area | Coverage |
|-----------|----------|
| SoA ops | Direct `burst_emit` requested/emitted/clamped counts, `clamp_burst_count` / `is_at_capacity` / `has_live_particles`, `burst_emit(0)` and zero-capacity guards, `free_slot_count` / `sync_alive_count`, deterministic seed parity, `lifetime_cull` mixed-lifetime partial/full expiry + empty-emitter guard + parallel parity, `simulate_step` integrated/culled counts + empty-emitter guard, rate accumulator empty paths + `at_capacity` flag |
| Burst | Requested count, `burst(0)`, pre-init no-op, returned emitted count, capacity clamp, slot recycling |
