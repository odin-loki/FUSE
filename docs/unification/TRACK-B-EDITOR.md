...
| `material_property_inspect` | `material_property_inspect.hpp` | B6.7 deepen — clamp helpers, `MaterialPropertyDescriptor` metadata for inspector widgets; follow-up — `materialPropertyIdAt`, `materialPropertyIsVec3`, `clampMaterialEditState` |
...
- **Push / undo / redo** — `CommandStack::push` aliases `execute`; `push(cmd, before)` seeds scene baseline; empty-stack undo/redo are no-ops; new `push` after undo clears the redo branch; `peekUndo()` / `peekRedo()` expose menu labels; undo posts inverse `SetProperty` to `pendingQueue()` (symmetric with redo).
- **Coalescing** — consecutive `SetProperty` commands with the same `target` + `propertyName` collapse into one undo step (slider/drag edits); caller-supplied `propertyValueBefore` is preserved (otherwise defaults to first value); `coalescedCount()` tracks merges and round-trips in snapshots.
...
| `fuse_editor_command_stack` | `fuse_editor_command_stack_tests` | B6.2 `UndoStack` LIFO, merge, rename/reparent, `MAX_HISTORY`, 100-step chain, empty-stack no-ops, dirty stub, depth-rewind snapshot restore, `push` alias; `CommandStack` push/undo/redo, coalescing baseline (`push(cmd, before)` + caller baseline), undo inverse pending post, `peekUndo`/`peekRedo`, redo-branch clear, dirty tracking, snapshot restore, `evictedCount`, depth round-trip |
...
- [x] `CommandStack` `push(cmd, before)` overload + caller-supplied `propertyValueBefore` preservation
- [x] `CommandStack` undo posts inverse `SetProperty` to pending queue (symmetric with redo)
- [x] `UndoStack::push` alias for `execute`
...
#### B6.7 deepen — material inspector enumeration + bulk clamp

- **Property enumeration** — `materialPropertyIdAt` exposes stable inspector widget order; descriptors carry `isVec3` for base-color rows.
- **Bulk clamp** — `clampMaterialEditState` pins all authoring fields; `MaterialPropertyBinding::refreshFromEditState` applies it before clearing dirty flags.
- **Tests** — `fuse_editor_panels` adds descriptor enumeration, bulk clamp, and refresh-clamp coverage.
