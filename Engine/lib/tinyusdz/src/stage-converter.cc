...
bool CrateWriter::ConvertPropertyToFields(
  ...
  /* FUSE-TINYUSDZ begin: writer-declared-attributes */
  if (prop.is_empty() && prop.is_attribute()) {
    // An attribute declared without a value (`token outputs:out`): keep its spec (type, variability, custom).
...
bool CrateWriter::ConvertAttributeToFields(
  ...
  /* FUSE-TINYUSDZ begin: writer-attr-rendertype */
