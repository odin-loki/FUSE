...
bool CrateWriter::ConvertLayerToSpecs(const Layer& layer, std::string* err) {
  ...
  // Layer metadata (upAxis, metersPerUnit, etc.) is stored as fields on PseudoRoot
  {
    ...
    /* FUSE-TINYUSDZ begin: layer-writer-sublayers */
    if (!metas.subLayers.empty()) {
      // Paths only: the crate writer cannot encode LayerOffset[] (subLayerOffsets) out of line.
    ...
    if (!metas.primChildren.empty()) {
      ...
      pc_value.Set(metas.primChildren);
...
bool CrateWriter::ConvertSinglePrimSpec(
  ...
  /* FUSE-TINYUSDZ begin: layer-writer-primchildren */
  if (!primspec.children().empty()) {
    ...
    fields.push_back({"primChildren", children_value});
  }
  /* FUSE-TINYUSDZ end */
  ...
  if (metas.has_kind()) {
    ...
    /* FUSE-TINYUSDZ begin: layer-writer-kind-token */
    kind_value.Set(value::token(kind_str));  // a token, not the token index (the reader wants `token`)
    /* FUSE-TINYUSDZ end */
  ...
  /* FUSE-TINYUSDZ begin: layer-writer-clips */
  if (metas.has_clips()) {
    crate::CrateValue clips_value;
    clips_value.Set(metas.get_clips());
    fields.push_back({"clips", clips_value});
  }
  /* FUSE-TINYUSDZ end */
  ...
  if (metas.has_apiSchemas()) {
    ...
    /* FUSE-TINYUSDZ begin: layer-writer-unknown-apischemas */
    for (const auto& item : api_schemas.unknownSchemas) {
      api_tokens.push_back(value::token(item.second.empty() ? item.first : item.first + ":" + item.second));
    }
    /* FUSE-TINYUSDZ end */
...
bool CrateWriter::ConvertVariantSpecToFields(
  ...
  /* FUSE-TINYUSDZ begin: layer-writer-variant-arcs */
  {
    auto to_listop = [](const auto& ops, auto& listop) {
      for (const auto& op : ops) {
        switch (op.first) {
          case ListEditQual::Append: listop.SetAppendedItems(op.second); break;
          case ListEditQual::Prepend: listop.SetPrependedItems(op.second); break;
          case ListEditQual::Add: listop.SetAddedItems(op.second); break;
          case ListEditQual::Delete: listop.SetDeletedItems(op.second); break;
          default: listop.ClearAndMakeExplicit(); listop.SetExplicitItems(op.second); break;
        }
      }
    };
    if (metas.references) {
      ListOp<Reference> ref_listop;
      to_listop(metas.references.value(), ref_listop);
      crate::CrateValue ref_value;
      ref_value.Set(ref_listop);
      v_fields.push_back({"references", ref_value});
    }
    if (metas.payload) {
      ListOp<Payload> payload_listop;
      to_listop(metas.payload.value(), payload_listop);
      crate::CrateValue payload_value;
      payload_value.Set(payload_listop);
      v_fields.push_back({"payload", payload_value});
    }
    if (metas.variants) {
      crate::CrateValue variant_value;
      variant_value.Set(metas.variants.value());
      v_fields.push_back({"variantSelection", variant_value});
    }
  }
  /* FUSE-TINYUSDZ end */
