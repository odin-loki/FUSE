...
namespace dxvk {
  ...
  void DxvkDeviceCapabilities::initSupportedExtensions(
    ...
    // FUSE-DXVK begin: RL-0.2-01 accept VK_EXT_load_store_op_none as VK_KHR_load_store_op_none
    // Same enums and semantics (the KHR extension promoted the EXT one). Some Vulkan layers, e.g.
    // Wine 9.0's winevulkan, expose only the EXT name. When the KHR name is absent, point the
    // KHR entry at the EXT name, so that it is detected below and enabled on the device.
    {
      auto fuseHasExt = [&extensions] (const VkExtensionProperties& e) {
        auto it = std::lower_bound(extensions.begin(), extensions.end(), e, vk::SortExtension());
        return it != extensions.end() && !vk::SortExtension()(e, *it);
      };

      if (!fuseHasExt(vk::makeExtension(VK_KHR_LOAD_STORE_OP_NONE_EXTENSION_NAME))
       && fuseHasExt(vk::makeExtension(VK_EXT_LOAD_STORE_OP_NONE_EXTENSION_NAME))) {
        m_extensionsSupported.khrLoadStoreOpNone = vk::makeExtension(VK_EXT_LOAD_STORE_OP_NONE_EXTENSION_NAME);
        m_extensionsEnabled.khrLoadStoreOpNone = vk::makeExtension(VK_EXT_LOAD_STORE_OP_NONE_EXTENSION_NAME);
      }
    }
    // FUSE-DXVK end
