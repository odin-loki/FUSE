...
ProfileRecordSkipReason diagnoseProfileRecordSkipReason(const char* name) {
    if (!enabled()) {
        return ProfileRecordSkipReason::ProfilerDisabled;
...
bool wouldSkipProfileRecord(const char* name,
    ...
    const ProfileRecordSkipReason skipReason = classifyProfileRecordSkip(name, requireOpenAsyncFlow);
    if (reason != nullptr) {
        *reason = skipReason;
    return skipReason != ProfileRecordSkipReason::None;
...
ProfileRecordSkipReason classifyCounterSampleSkip(const char* track) {
    ...
    case ProfileRecordSkipReason::EmptyName:
        return "empty_name";
    case ProfileRecordSkipReason::NoOpenFlows:
        return "no_open_flows";
    case ProfileRecordSkipReason::UnbalancedScopeNesting:
        return "unbalanced_scope_nesting";
    case ProfileRecordSkipReason::UnbalancedFlowNesting:
        return "unbalanced_flow_nesting";
    case ProfileRecordSkipReason::FlowDepthDetached:
        return "flow_depth_detached";
    case ProfileRecordSkipReason::CrossThreadHandoffPending:
        return "cross_thread_handoff_pending";

bool wouldSkipProfileScope(const char* name, ProfileRecordSkipReason* reason) {
    const ProfileRecordSkipReason skipReason = diagnoseProfileRecordSkipReason(name);
...
InvalidEventNameReason classifyInvalidEventName(const char* name) {
        return InvalidEventNameReason::NullPointer;
        return InvalidEventNameReason::EmptyString;
    return InvalidEventNameReason::None;

const char* describeInvalidEventNameReason(InvalidEventNameReason reason) {
    case InvalidEventNameReason::None:
    case InvalidEventNameReason::NullPointer:
        return "null_pointer";
    case InvalidEventNameReason::EmptyString:
        return "empty_string";
...
bool eventNameMatches(const char* eventName, const char* searchName) {
    ...
    return std::strcmp(eventName, searchName) == 0;
        ...
        if (isValidEventName(event.name) && event.scopeId == flowId
            && (event.phase == EventPhase::FlowStart || event.phase == EventPhase::FlowFinish)) {
        ...
        if (eventNameMatches(name, event.name)) {





        if (isFlowEvent(event) && event.scopeId == flowId && isValidEventName(event.name)) {
...
ScopeRecordingPreflight preflightScopeRecording(const char* name) {
    ...
    preflight.activeNestingDepth = scopeNestingDepth();
    preflight.maxNestingDepth = maxNestingDepth();
...
bool wouldSkipChromeTraceExportSafely() {
    const ChromeTraceExportPreflight preflight = preflightChromeTraceExport();
    return !preflight.canExportSafely();
...
bool wouldSkipSafeChromeTraceExport(ProfileRecordSkipReason* reason) {
            *reason = ProfileRecordSkipReason::ProfilerDisabled;

            *reason = ProfileRecordSkipReason::UnbalancedScopeNesting;

            *reason = ProfileRecordSkipReason::UnbalancedFlowNesting;

            *reason = ProfileRecordSkipReason::NoOpenFlows;

            *reason = ProfileRecordSkipReason::FlowDepthDetached;

    if (isCrossThreadFlowHandoffPending()) {
            *reason = ProfileRecordSkipReason::CrossThreadHandoffPending;
