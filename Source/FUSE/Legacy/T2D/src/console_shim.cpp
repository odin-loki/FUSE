...
extern "C" void fuse_t2d_Con_addConstant(const char* name, int type, const void* pointer,
                                         const char* /*usage*/) {
    if (!name || !pointer) {
        return;
    }
...
extern "C" int fuse_t2d_Con_removeVariable(const char* name) {
    if (!name) {
        return 0;
    }
    const std::string key = normalizeVariableName(name);
...
extern "C" void fuse_t2d_Con_addVariableNotify(const char* name, void (*callback)(void*),
                                               void* userdata) {
    if (!name || !callback) {
        return;
    }
...
extern "C" void fuse_t2d_Con_removeVariableNotify(const char* name, void (*callback)(void*),
                                                  void* userdata) {
    if (!name || !callback) {
        return;
    }
...
extern "C" unsigned fuse_t2d_Con_tabComplete(char* inputBuffer, unsigned cursorPos,
...
extern "C" const char* fuse_t2d_Con_evaluate(const char* string, int echo, const char* fileName) {
    if (echo) {
        fuse::log::info("[t2d] Con::evaluate: %s", string ? string : "(null)");
    }
    (void)fileName;
    return emptyReturnBuffer();
}

extern "C" const char* fuse_t2d_Con_evaluatef(const char* fmt, ...) {
    char buffer[1024];
    va_list args;
    va_start(args, fmt);
    std::vsnprintf(buffer, sizeof(buffer), fmt, args);
    va_end(args);
    return fuse_t2d_Con_evaluate(buffer, 1, nullptr);
}

extern "C" const char* fuse_t2d_Con_executeArgv(int argc, const char** argv) {
    char script[1024];
...
extern "C" const char* fuse_t2d_Con_executefArgv(int argc, ...) {
    char script[1024];
    script[0] = '\0';
    va_list args;
    va_start(args, argc);
...
extern "C" void fuse_t2d_Con_removePathExpando(const char* expandoName) {
    if (!expandoName) {
        return;
    }
...
extern "C" int fuse_t2d_Con_isPathExpando(const char* expandoName) {
    if (!expandoName) {
        return 0;
    }
...
extern "C" unsigned fuse_t2d_Con_getPathExpandoCount(void) {
    return static_cast<unsigned>(g_pathExpandos.size());
}

extern "C" float fuse_t2d_Con_getFloatVariable(const char* name, float def) {
    const char* value = fuse_t2d_Con_getVariable(name);
    if (!value || value[0] == '\0') {
        return def;
    }
...
extern "C" void fuse_t2d_Con_setFloatVariable(const char* name, float value) {
    char buffer[64];
...
void addConstant(const char* name, int type, const void* pointer, const char* usage) {
    fuse_t2d_Con_addConstant(name, type, pointer, usage);
}

bool removeVariable(const char* name) {
    return fuse_t2d_Con_removeVariable(name) != 0;
}

void addVariableNotify(const char* name, void (*callback)(void*), void* userdata) {
    fuse_t2d_Con_addVariableNotify(name, callback, userdata);
}

void removeVariableNotify(const char* name, void (*callback)(void*), void* userdata) {
    fuse_t2d_Con_removeVariableNotify(name, callback, userdata);
}

fuse::u32 tabComplete(char* inputBuffer, fuse::u32 cursorPos, fuse::u32 maxResultLength, bool forwardTab) {
    return fuse_t2d_Con_tabComplete(inputBuffer, cursorPos, maxResultLength, forwardTab ? 1 : 0);
}

const char* evaluate(const char* string, bool echo, const char* fileName) {
    return fuse_t2d_Con_evaluate(string, echo ? 1 : 0, fileName);
}
...
const char* executeArgv(int argc, const char** argv) {
    return fuse_t2d_Con_executeArgv(argc, argv);
}
...
void removePathExpando(const char* expandoName) {
    fuse_t2d_Con_removePathExpando(expandoName);
}

bool isPathExpando(const char* expandoName) {
    return fuse_t2d_Con_isPathExpando(expandoName) != 0;
}

fuse::u32 getPathExpandoCount() {
    return fuse_t2d_Con_getPathExpandoCount();
}

float getFloatVariable(const char* name, float def) {
    return fuse_t2d_Con_getFloatVariable(name, def);
}

void setFloatVariable(const char* name, float value) {
    fuse_t2d_Con_setFloatVariable(name, value);
}
