...
FUSE Relight itself is AGPL-3.0 (see the repository root `LICENSE.md`).
...
| GDeflate CPU codec: NVIDIA libdeflate `gdeflate` branch (`libdeflate/`) plus Microsoft DirectStorage `GDeflate/` | libdeflate: MIT (Eric Biggers 2016); GDeflate additions and DirectStorage wrapper: Apache-2.0 (NVIDIA 2020–2022, Microsoft) | libdeflate `8ba9502fb30d` (no tags; `VERSION` records `libdeflate.h`'s 1.8.0), DirectStorage `c53f1499d5f6` | vendored, one marked patch (`FUSE-GDEFLATE` input-bounds fix, see `PATCHES.md`) | `Engine/lib/gdeflate` | Keep `COPYING` and `LICENSE`. Apache-2.0: ship the licence text and mark modified files (the patch carries markers). No NOTICE file upstream. |
...
| TinyUSDZ (`lighttransport/tinyusdz`; shared with Remaster W2.2), USDA + USDC layer-reading subset of `src/` | Apache-2.0 (Syoyo Fujita 2020–2023, Light Transport Entertainment 2024–). Bundled inside: Pixar-derived crate code under Pixar's modified Apache-2.0 (TOST, per-file headers); lz4 BSD-2 (Yann Collet); xxHash BSD-2 (Yann Collet); ghc::filesystem MIT; glob MIT; fast_float MIT / Apache-2.0 / BSL-1.0; dragonbox Apache-2.0 WITH LLVM-exception / BSL-1.0; jsteemann atoi Apache-2.0; mapbox eternal ISC; linalg Unlicense | 0.9.4, tag `v0.9.4` = `dc7684519883` (sha256 per file in `Engine/lib/tinyusdz/VERSION`) | vendored, with marked patches (`FUSE-TINYUSDZ`: one USDC reader fix, the rest in the crate writer used only to generate test fixtures; see `PATCHES.md`) | `Engine/lib/tinyusdz` | Keep `LICENSE` and every bundled licence file (`src/lz4/LICENSE`, `src/external/*LICENSE*`, `src/external/*/LICENSE*`) and the Pixar headers in the sources. Apache-2.0: ship the licence text and state modifications (the patched files carry markers; `PATCHES.md`). No NOTICE file upstream. |
