...
//
//   relight.frame.mode          off | passthrough | solid (env FUSE_RELIGHT_FRAME_MODE, default off). Needs
//                               relight.tap.mode = capture (the classifier finds the injection point).
...
enum class FrameMode : std::uint8_t { Off = 0, Passthrough, Solid };

/// "off" / "passthrough" / "solid" (case-insensitive). False (out unchanged) for anything else.
...
struct FrameOptions {
    FUSE_RELIGHT_OPTION_ENV("relight.frame", std::string, mode, "off", "FUSE_RELIGHT_FRAME_MODE",
                            ...
                            "injection point, composited back) or solid (FUSE's image is relight.frame.solidColor). "
                            "Needs relight.tap.mode = capture.");
