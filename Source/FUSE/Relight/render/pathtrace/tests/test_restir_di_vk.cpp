// FUSE Relight RL-5.2 Vulkan gates (Lavapipe): ReSTIR DI on the GPU (RestirDiGpu, restir_di_gpu.hpp: the
// "relight.restir_di.*" passes before PathTracerGpu's "relight.pt.trace") against the CPU oracle (RestirDiCpu, the same
// single-source core restir_di_core.h) on the RL-5.1 Cornell scene.
//
//   parity       per kernel language, three frames with deterministic seeds (history on from the second):
//                presample  GPU tiles == CPU tiles bit for bit;
//                surface    GPU surface records == CPU records (same instance / primitive, barycentrics and position
//                           within 1e-3) on >= 99% of the pixels (the GPU traverses the TLAS in float, the CPU oracle
//                           in double);
//                initial / temporal / spatial   each GPU pass replayed by the CPU oracle on the GPU's own read-back
//                           inputs (surfaces, tiles, source and history reservoirs): the same light and sample point
//                           (1e-3) and W within 1e-3 relative on >= 97% of the pixels;
//                the frame   GPU ReSTIR accumulation (64 frames) image mean within 3% of the CPU reference path tracer
//                           (plain NEE, 1024 spp), finite, and every ReSTIR surface matched by the trace pass.
//   determinism  two GPU runs of the same frames give bit-identical ReSTIR output and radiance.
//   zero_alloc   64 steady-state frames: no operator-new call in PathTracerGpu / RestirDiGpu beginFrame, the graph
...
int setup(Context& ctx, Validation validation) {
    ...
    instanceDesc.appName = "rl_restir_di_vk";
...
bool runFrame(Context& ctx, Gpus& g, rg::Graph& graph, pt::PtCompiledScene& c, const pt::PtFrameDesc& fd,
              const pt::RestirDiSettings& di, bool measure = false, FrameAllocs* allocs = nullptr) {
    ++ctx.serial;
    ctx.bindless.setFrameSerial(ctx.serial);
    t_allocations = 0;
    t_count = measure;
    bool ok = g.pt.setScene(c) && g.pt.beginFrame(ctx.serial, c, fd) && g.di.beginFrame(ctx.serial, c, fd, di);
    t_count = false;
    ...
    g.di.collectRetired(ctx.serial);
...
std::vector<pt::Word> readWords(const Gpus& g, u32 section, std::size_t words) {
    const auto* p = static_cast<const pt::Word*>(g.di.mappedSection(section));
...
/// Reservoir agreement: same light / kind, sample point within 1e-3, W within 1e-3 relative (empty == empty).
double reservoirAgreement(const std::vector<pt::Word>& gpu, const std::vector<pt::Word>& cpu, u32 pixels, u32* live) {
    u32 same = 0;
    u32 withSample = 0;
    for (u32 i = 0; i < pixels; ++i) {
        const pt::Word& g0 = gpu[i * 2u];
        const pt::Word& g1 = gpu[i * 2u + 1u];
        const pt::Word& c0 = cpu[i * 2u];
        const pt::Word& c1 = cpu[i * 2u + 1u];
        withSample += c1.x >= 0.f ? 1u : 0u;
        bool ok = g1.x == c1.x && g1.w == c1.w && closeTo(g1.y, c1.y, 1e-4f);
        if (ok && c1.x >= 0.f) {
            ok = closeTo(g0.x, c0.x, 1e-3f) && closeTo(g0.y, c0.y, 1e-3f) && closeTo(g0.z, c0.z, 1e-3f) &&
                 closeTo(g0.w, c0.w, 1e-3f);
        }
        same += ok ? 1u : 0u;
    }
    if (live != nullptr) {
...
void parityCase(Context& ctx, Gpus& g, rg::Graph& graph) {
    ...
    for (u32 f = 0; f < 3u; ++f) {
        ...
        fd.accumulate = f != 0u;
        fd.settings = st;
        if (!runFrame(ctx, g, graph, c, fd, di)) {
            check(false, "parity: GPU frame");
            return;
        }
        check(cpu.frame(c, st, kW, kH, kSeed, f, di, kernel::Backend::CpuParallel), "parity: CPU frame");
        const u32 flags = g.di.frameFlags();
        const std::vector<pt::Word> surf = readWords(g, g.di.currentSurfaceSection(), std::size_t(pixels) * 4u);
        const std::vector<pt::Word> init = readWords(g, pt::kRdiSecInitial, std::size_t(pixels) * 2u);
        const std::vector<pt::Word> resA = readWords(g, pt::kRdiSecA, std::size_t(pixels) * 2u);
        const std::vector<pt::Word> fin = readWords(g, g.di.finalSection(), std::size_t(pixels) * 2u);
        const auto* tp = static_cast<const u32*>(g.di.mappedSection(pt::kRdiSecTiles));
        const std::vector<u32> tiles(tp, tp + std::size_t(g.di.tileCount()) * di.tileSize);
        // presample: bit-exact.
        const bool tilesEqual = tiles == cpu.tiles();
        ...
        std::vector<pt::Word> outInit, outTemporal, outSpatial;
        const bool r0 = cpu.replayStage(c, st, kW, kH, kSeed, f, di, flags, pt::RestirDiCpu::kStageInitial, 0u, surf,
                                        prevSurf, {}, {}, tiles, outInit);
        const bool r1 = cpu.replayStage(c, st, kW, kH, kSeed, f, di, flags, pt::RestirDiCpu::kStageTemporal, 0u, surf,
                                        prevSurf.empty() ? surf : prevSurf, init,
                                        prevHistory.empty() ? init : prevHistory, tiles, outTemporal);
        const bool r2 = cpu.replayStage(c, st, kW, kH, kSeed, f, di, flags, pt::RestirDiCpu::kStageSpatial, 0u, surf,
                                        surf, resA, resA, tiles, outSpatial);
        u32 liveI = 0, liveT = 0, liveS = 0;
        const double aI = r0 ? reservoirAgreement(init, outInit, pixels, &liveI) : 0.0;
        const double aT = r1 ? reservoirAgreement(resA, outTemporal, pixels, &liveT) : 0.0;
        const double aS = r2 ? reservoirAgreement(fin, outSpatial, pixels, &liveS) : 0.0;
        std::printf("  frame %u (history %s): tiles %s (%zu), surfaces %u/%u, initial %.1f%% (%u live), temporal "
                    "%.1f%% (%u), spatial %.1f%% (%u)\n",
                    f, (flags & pt::kRdiFlagHistory) != 0u ? "on" : "off", tilesEqual ? "bit-identical" : "DIFFER",
                    tiles.size(), surfSame, pixels, 100.0 * aI, liveI, 100.0 * aT, liveT, 100.0 * aS, liveS);
        check(!tiles.empty() && tilesEqual, "parity: presample GPU == CPU bit for bit");
        ...
        if (f > 0u) {
            check((flags & pt::kRdiFlagHistory) != 0u, "parity: history on after the first frame");
        }
        prevSurf = surf;
        prevHistory = readWords(g, pt::kRdiSecHistory, std::size_t(pixels) * 2u);
    }
}
...
void convergeCase(Context& ctx, Gpus& g, rg::Graph& graph) {
    ...
    for (u32 f = 0; f < kFrames && ran; ++f) {
        ...
        fd.accumulate = f != 0u;
        fd.settings = st;
        ran = runFrame(ctx, g, graph, c, fd, di);
        if (ran && f + 1u == kFrames) {
            // Every ReSTIR surface is the trace pass's G-buffer vertex: its instance section equals the key's.
            const auto* out = static_cast<const pt::Word*>(g.di.mappedSection(pt::kRdiSecOutput));
    ...
    plain.flags &= ~pt::kPtFlagRestirDi;
...
void determinismCase(Context& ctx, Gpus& g, rg::Graph& graph) {
    ...
    for (int k = 0; k < 2; ++k) {
        ...
        const auto* out = static_cast<const u8*>(g.di.mappedSection(pt::kRdiSecOutput));
...
int runParity(Context& ctx, const std::string& language) {
    ...
    for (const Lang& lang : kLangs) {
        ...
        if (!initGpus(ctx, g, lang.language)) {
            if (std::strstr(g.pt.reason(), "no path-tracing kernel") != nullptr ||
                std::strstr(g.di.reason(), "no ReSTIR DI kernel") != nullptr) {
    ...
    if (languages == 0u) {
        std::printf("SKIP: no ReSTIR DI kernel built\n");
        return kSkip;
    }
    return 0;
}
...
int runZeroAlloc(Context& ctx, bool countAllocations) {
    ...
    if (!ok) {
        std::printf("SKIP: GPU path tracer / ReSTIR DI unavailable: %s / %s\n", g.pt.reason(), g.di.reason());
        return kSkip;
    }
    ...
    for (u32 frame = 0; frame < kTotal; ++frame) {
        ...
        FrameAllocs fa;
        t_allocations = 0;
        const bool ran = runFrame(ctx, g, graph, c, fd, di, measure, &fa);
        ...
        if (frame == kWarmup) {
            reallocations = g.pt.stats().reallocations + g.di.stats().reallocations;
        }
    }
    ctx.executor->setPassHooks(rg::PassHooks{});
    check(g.pt.stats().reallocations + g.di.stats().reallocations == reallocations,
    ...
    if (countAllocations) {
        std::printf("zero_alloc (%s kernels, %u steady-state frames, %u ReSTIR passes / frame):\n"
                    ...
                    "  PathTracerGpu + RestirDiGpu setScene / beginFrame: %llu\n"
                    "  graph build (importInto + addPasses + addTracePass + read-back): %llu\n"
                    "  relight.* pass callbacks: %llu\n",
                    g.di.kernelLanguage(), kTotal - kWarmup, g.di.stats().passes, updates, allocs.begin, allocs.graph,
                    callbacks);
        check(updates == 0u && allocs.begin == 0u && allocs.graph == 0u && callbacks == 0u,
              "steady-state ReSTIR DI frames make no heap allocation");
    } else {
        std::printf("zero_alloc frames under validation: %u frames ok (%s)\n", kTotal, g.di.kernelLanguage());
    }
    ctx.executor->waitIdle();
    g.di.destroy();
    g.pt.destroy();
