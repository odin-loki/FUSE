...
void testContactIslandGraphSkipsOutOfRangeRefs() {
    ...
    graph.build(4, contacts, constraints);
    expectTrue(graph.constrainedIslandCount() == 1u,
               "build skips out-of-range contacts and distance constraints");
    expectTrue(graph.island(0).contactIndices.size() == 1u,
               "in-range contact is assigned to island");
    expectTrue(graph.island(0).distanceIndices.size() == 1u,
               "in-range distance constraint is assigned to island");
}
...
void testSolveIslandJobGuardedSkipsAllSleeping() {
    ...
    expectTrue(solve_island_job(bodies, *job.island, work, constraints, 1.f / 60.f, 0.f, invMassFn),
               "unguarded solve still runs for all-sleeping island");
    expectTrue(!solve_island_job_guarded(bodies, *job.island, work, constraints, 1.f / 60.f, 0.f, invMassFn),
               "guarded solve skips all-sleeping island");
}
...
void testPreflightIslandConstraintSolveByIndex() {
    ...
    const IslandConstraintSolvePreflight preflight = preflight_island_constraint_solve_by_index(
        graph, 0u, bodies, work.contactManifolds(), constraints);
    expectTrue(!preflight.skipped, "constraint solve by-index preflight does not skip valid island");
    expectTrue(preflight.can_solve(), "valid island passes constraint solve by-index preflight");
    ...
    const IslandConstraintSolvePreflight outOfRange =
        preflight_island_constraint_solve_by_index(graph, graph.islandCount() + 1u, bodies, {}, constraints);
    expectTrue(outOfRange.skipped, "constraint solve by-index preflight skips out-of-range index");
    expectTrue(should_skip_island_constraint_solve_by_index(
                   graph, graph.islandCount() + 1u, bodies, {}, constraints),
...
void testPreflightIslandSolveableGraph() {
    ...
    const IslandSolveableGraphPreflight preflight = preflight_island_solveable_graph(
        graph, bodies, work.contactManifolds(), constraints, 1.f / 60.f);
    expectTrue(!preflight.skipped, "solveable graph preflight has one active island");
    expectTrue(preflight.can_dispatch(), "solveable graph can dispatch mixed/active island");
    expectTrue(preflight.stats.solveableCount == 1u, "solveable graph counts one solveable island");
    expectTrue(preflight.stats.allSleepingCount == 1u, "solveable graph counts all-sleeping island");
    expectTrue(collect_solveable_island_indices(graph, bodies, work.contactManifolds(), constraints).size() ==
                   1u,
               "collect solveable indices returns active island only");
    expectTrue(!should_skip_island_solveable_graph(
                   graph, bodies, work.contactManifolds(), constraints, 1.f / 60.f),
    ...
    const IslandSolveableGraphPreflight allActive = preflight_island_solveable_graph(
        graph, bodies, work.contactManifolds(), constraints, 1.f / 60.f);
    expectTrue(allActive.stats.solveableCount == 2u, "both islands solveable when all awake");
}
...
void testDispatchSolveIslandWithWakeGuarded() {
    ...
    const IslandDispatchResult result = dispatch_solve_island_with_wake_result(bodies,
                                                                               graph,
                                                                               0u,
                                                                               work,
                                                                               constraints,
                                                                               ...
                                                                               0.f,
                                                                               invMassFn);
    expectTrue(result.solved, "wake+dispatch solves mixed-sleep island");
    expectTrue((bodies.flags[1] & RB_SLEEPING) == 0u, "wake+dispatch clears sleeping neighbor");
    expectTrue(dispatch_solve_island_with_wake_guarded(bodies,
                                                       ...
                                                       0.f,
                                                       invMassFn),
               "guarded wake+dispatch entry succeeds");
}
...
void testDispatchAllSolveableIslandsResult() {
    ...
    RigidBodySoA bodies;
    bodies.addBody({0.f, 0.f, 0.f}, 1.f, 0);
    bodies.addBody({2.2f, 0.f, 0.f}, 1.f, 0);
    bodies.addBody({20.f, 0.f, 0.f}, 1.f, RB_SLEEPING);
    bodies.addBody({22.2f, 0.f, 0.f}, 1.f, RB_SLEEPING);
    bodies.predictedPositions = bodies.positions;
    ...
    const IslandBatchDispatchResult batch = dispatch_all_solveable_islands_result(bodies,
                                                                                ...
                                                                                0.f,
                                                                                invMassFn);
    expectTrue(!batch.skipped, "solveable batch does not skip when active island exists");
    expectTrue(batch.solvedCount == 1u, "solveable batch solves only awake island");
    expectTrue(batch.dispatchableCount == 1u, "solveable batch records one solveable island");

    bodies.flags[2] &= ~RB_SLEEPING;
    bodies.flags[3] &= ~RB_SLEEPING;
    const IslandBatchDispatchResult bothAwake = dispatch_all_solveable_islands_result(bodies,
                                                                                      graph,
                                                                                      work,
                                                                                      constraints,
                                                                                      1.f / 60.f,
                                                                                      0.f,
                                                                                      invMassFn);
    expectTrue(bothAwake.solvedCount == 2u, "solveable batch solves both islands when awake");
}

void testWakeIslandSleepersResult() {
    ContactIslandGraph graph;
    std::vector<narrowphase::ContactManifold> contacts;
    std::vector<DistanceConstraint> constraints = {
        DistanceConstraint{.bodyA = 0, .bodyB = 1, .restLength = 2.f},
    ...
    const IslandWakeResult woke = wake_island_sleepers_result(bodies, graph, islandIndex);
    expectTrue(woke.woke, "wake result activates mixed-sleep island");
    expectTrue(!woke.skipped, "wake result is not skipped for mixed island");
    expectTrue((bodies.flags[1] & RB_SLEEPING) == 0u, "wake result clears sleeping flag");

    const IslandWakeResult skipped = wake_island_sleepers_result(bodies, graph, islandIndex);
    expectTrue(skipped.skipped, "wake result skips when sleepers already active");
    expectTrue(!skipped.woke, "wake result does not re-wake active island");

    const IslandWakeResult outOfRange = wake_island_sleepers_result(bodies, graph, graph.islandCount() + 1u);
    expectTrue(outOfRange.skipped, "wake result skips out-of-range island index");
}

void testOutOfRangeBodyCountInPreflights() {
    ContactIslandGraph graph;
    ContactIslandGraph::Island island{};
    island.bodyIndices = {0, 1, 9};
    island.distanceIndices = {0};
    ...
    expectTrue(bodiesPreflight.outOfRangeBodyCount == 1u,
               "solve-bodies preflight counts out-of-range body index");
...
void testContactIslandGraphBuildGuardsOorRefs() {
    ...
    graph.build(4, contacts, constraints);
    expectTrue(graph.constrainedIslandCount() == 1u,
               "build guards skip out-of-range contact and distance refs");
    expectTrue(graph.island(0).contactIndices.size() == 1u,
               "build assigns only in-range contact indices");
    expectTrue(graph.island(0).distanceIndices.size() == 1u,
               "build assigns only in-range distance indices");
}
...
void testPreflightIslandSolveDispatchGuards() {
    ...
    const IslandSolveDispatchPreflight mixedPreflight =
        preflight_island_solve_dispatch(graph.island(mixedIsland), bodies, work.contactManifolds(), constraints);
    ...
    const IslandSolveDispatchPreflight sleepingPreflight =
        preflight_island_solve_dispatch(graph.island(sleepingIsland), bodies, work.contactManifolds(), constraints);
    ...
    const IslandSolveDispatchPreflight outOfRange =
        preflight_island_solve_dispatch_by_index(graph, graph.islandCount() + 1u, bodies, work.contactManifolds(), constraints);
    expectTrue(outOfRange.skipped, "solve-dispatch index preflight skips out-of-range island");

    const IslandSolveableStats stats =
        compute_island_solveable_stats(graph, bodies, work.contactManifolds(), constraints);
    expectTrue(stats.solveableCount == 1u, "solveable stats count mixed island");
    expectTrue(stats.allSleepingCount == 1u, "solveable stats count all-sleeping island");
    expectTrue(has_solveable_islands(graph, bodies, work.contactManifolds(), constraints),
               "has_solveable_islands true when mixed island exists");
    expectTrue(collect_solveable_island_indices(graph, bodies, work.contactManifolds(), constraints).size() == 1u,
               "collect solveable indices returns mixed island");
...
void testDispatchSolveIslandGuarded() {
    ...
    const IslandDispatchResult skippedResult = dispatch_solve_island_guarded_result(bodies,
                                                                                    graph,
                                                                                    sleepingIsland,
                                                                                    work,
                                                                                    constraints,
                                                                                    ...
                                                                                    0.f,
                                                                                    invMassFn);
    expectTrue(skippedResult.skipped, "guarded dispatch result skips all-sleeping island");
    expectTrue(!skippedResult.solved, "guarded dispatch result does not solve all-sleeping island");
    ...
    expectTrue((bodies.flags[1] & RB_SLEEPING) != 0u, "sleeping body remains asleep before wake dispatch");
    expectTrue(dispatch_solve_island_guarded(bodies,
                                             graph,
                                             mixedIsland,
                                             work,
                                             ...
                                             invMassFn,
                                             true),
               "guarded dispatch with wake solves mixed island");
    expectTrue((bodies.flags[1] & RB_SLEEPING) == 0u, "guarded wake dispatch clears sleeping flag");
}

void testDispatchSolveableIslandsBatch() {
    ContactIslandGraph graph;
    std::vector<narrowphase::ContactManifold> contacts;
    std::vector<DistanceConstraint> constraints = {
        DistanceConstraint{.bodyA = 0, .bodyB = 1, .restLength = 2.f},
    ...
    const IslandBatchDispatchResult batch =
        dispatch_solveable_islands_result(bodies, graph, work, constraints, dt, 0.f, invMassFn, true);
    expectTrue(!batch.skipped, "solveable batch does not skip mixed graph");
    expectTrue(batch.solvedCount == 1u, "solveable batch solves mixed island only");
    expectTrue(batch.any_solved(), "solveable batch reports solved island");
    expectTrue(dispatch_solveable_islands(bodies, graph, work, constraints, dt, 0.f, invMassFn, true) ==
                   batch.solvedCount,
               "solveable guarded count matches batch result");

    const IslandBatchDispatchResult invalidDt =
        dispatch_solveable_islands_result(bodies, graph, work, constraints, 0.f, 0.f, invMassFn);
    expectTrue(invalidDt.skipped, "solveable batch skips invalid dt");
    expectTrue(!invalidDt.any_solved(), "invalid dt solveable batch reports no solved islands");
}

void testContactIslandGraphBodiesInRange() {
    expectTrue(ContactIslandGraph::bodiesInRange(4, 0, 1), "in-range bodies pass bodiesInRange");
    expectTrue(!ContactIslandGraph::bodiesInRange(4, 3, 4), "out-of-range bodyB fails bodiesInRange");
    expectTrue(!ContactIslandGraph::bodiesInRange(4, 5, 1), "out-of-range bodyA fails bodiesInRange");
}
...
void testPreflightIslandSolvePassGuards() {
    ...
    const IslandSolvePassPreflight mixedPass =
        preflight_island_solve_pass(graph.island(mixedIsland), bodies, contacts, constraints);
    expectTrue(!mixedPass.skipped, "solve-pass preflight does not skip mixed island");
    expectTrue(mixedPass.can_solve(), "mixed island can solve");
    expectTrue(mixedPass.should_wake_first(), "mixed island should wake before solve");
    expectTrue(!should_skip_island_solve_pass(graph.island(mixedIsland), bodies, contacts, constraints),
               "should_skip solve-pass false for mixed island");

    const IslandSolvePassPreflight sleepingPass =
        preflight_island_solve_pass(graph.island(sleepingIsland), bodies, contacts, constraints);
    expectTrue(!sleepingPass.can_solve(), "all-sleeping island cannot solve");
    expectTrue(should_skip_island_solve_pass(graph.island(sleepingIsland), bodies, contacts, constraints),
               "should_skip solve-pass true for all-sleeping island");

    const IslandSolvePassPreflight byIndex =
        preflight_island_solve_pass_by_index(graph, mixedIsland, bodies, contacts, constraints);
    expectTrue(byIndex.can_solve(), "solve-pass by-index matches mixed island");
    const IslandSolvePassPreflight outOfRange =
        preflight_island_solve_pass_by_index(graph, graph.islandCount() + 1u, bodies, contacts, constraints);
    expectTrue(outOfRange.skipped, "solve-pass by-index skips out-of-range island");

    const IslandConstraintSolvePreflight constraintByIndex = preflight_island_constraint_solve_by_index(
        graph, mixedIsland, bodies, contacts, constraints);
    expectTrue(constraintByIndex.can_solve(), "constraint-solve by-index matches mixed island");
}

void testGuardedSolveAndDispatchWithWake() {
    ContactIslandGraph graph;
    std::vector<narrowphase::ContactManifold> contacts;
    std::vector<DistanceConstraint> constraints = {
        DistanceConstraint{.bodyA = 0, .bodyB = 1, .restLength = 2.f},
    ...
    expectTrue(solve_island_job_with_wake_guarded(bodies,
                                                  ...
                                                  0.f,
                                                  invMassFn),
               "guarded solve with wake resolves mixed island");
    expectTrue((bodies.flags[1] & RB_SLEEPING) == 0u, "guarded solve with wake clears sleeper flag");
    ...
    expectTrue(dispatch_solve_island_with_wake_guarded(bodies,
                                                       ...
                                                       0.f,
                                                       invMassFn),
               "guarded dispatch with wake resolves mixed island");
    expectTrue(!dispatch_solve_island_guarded(bodies,
                                              graph,
                                              sleepingIsland,
                                              work,
    ...
    expectTrue(!dispatch_solve_island_guarded(bodies,
                                              ...
                                              0.f,
                                              invMassFn),
               "guarded dispatch rejects non-finite dt");
}

void testNonsleepingIslandDispatchBatch() {
    ContactIslandGraph graph;
    std::vector<narrowphase::ContactManifold> contacts;
    std::vector<DistanceConstraint> constraints = {
        DistanceConstraint{.bodyA = 0, .bodyB = 1, .restLength = 2.f},
    ...
    expectTrue(preflight.can_dispatch(), "nonsleeping dispatch preflight can dispatch mixed graph");
    expectTrue(!should_skip_nonsleeping_island_dispatch(graph, bodies, dt),
               "should_skip nonsleeping dispatch false when mixed island exists");
    ...
    expectTrue(batch.solvedCount == 1u, "nonsleeping batch solves only active island");
    expectTrue(dispatch_nonsleeping_islands_guarded(bodies, graph, work, constraints, dt, 0.f, invMassFn) ==
                   batch.solvedCount,
               "nonsleeping guarded count matches batch result");

    bodies.flags[0] |= RB_SLEEPING;
    bodies.flags[1] |= RB_SLEEPING;
    const IslandNonsleepingDispatchPreflight allSleeping =
        preflight_nonsleeping_island_dispatch(graph, bodies, dt);
    expectTrue(!allSleeping.can_dispatch(), "nonsleeping dispatch preflight skips all-sleeping graph");
    expectTrue(should_skip_nonsleeping_island_dispatch(graph, bodies, dt),
               "should_skip nonsleeping dispatch true when all islands sleep");
}
...
void testWakeIslandSleepersResult() {
    ...
    const IslandWakeResult woke = wake_island_sleepers_result(bodies, graph, 0);
    expectTrue(woke.woke, "wake result reports woke mixed island");
    expectTrue(!woke.skipped, "wake result is not skipped when wake occurs");

    const IslandWakeResult skipped = wake_island_sleepers_result(bodies, graph, graph.islandCount() + 1u);
    expectTrue(skipped.skipped, "wake result skips out-of-range island");
    expectTrue(!skipped.woke, "out-of-range wake result does not wake");
}

void testPreflightIslandDeepenFollowUpRejectGuards() {
    std::vector<narrowphase::ContactManifold> contacts;
    contacts.push_back(narrowphase::ContactManifold{});
    contacts.back().valid = true;
    contacts.back().bodyA = 0;
    ...
    expectTrue(island_build_reject_reason(4, contacts, constraints) == IslandBuildRejectReason::OutOfRangeContact,
               "build reject reason flags out-of-range contact");
    expectTrue(island_build_rejects_for_reason(4, contacts, constraints, IslandBuildRejectReason::OutOfRangeContact),
               "build rejects_for_reason matches out-of-range contact");
    ...
    ContactIslandGraph graph;
    IslandBuildPreflight buildPreflight{};
    expectTrue(!build_island_graph_with_preflight(graph, 4, contacts, constraints, buildPreflight),
               "build_with_preflight rejects unsafe refs");
    expectTrue(buildPreflight.reason == IslandBuildRejectReason::OutOfRangeContact,
               "build_with_preflight reports out-of-range contact reason");

    std::vector<narrowphase::ContactManifold> safeContacts;
    safeContacts.push_back(contacts[0]);
    const std::vector<DistanceConstraint> safeConstraints = {constraints[0]};
    expectTrue(build_island_graph_with_preflight(graph, 4, safeContacts, safeConstraints, buildPreflight),
               "build_with_preflight succeeds for in-range inputs");
    expectTrue(buildPreflight.reason == IslandBuildRejectReason::None,
               "build_with_preflight reports None on valid build");

    ContactIslandGraph constraintGraph;
    std::vector<DistanceConstraint> oorDistance = {
        DistanceConstraint{.bodyA = 0, .bodyB = 1, .restLength = 2.f},
        DistanceConstraint{.bodyA = 6, .bodyB = 7, .restLength = 2.f},
    };
    expectTrue(island_build_reject_reason(4, {}, oorDistance) == IslandBuildRejectReason::OutOfRangeDistance,
               "build reject reason flags out-of-range distance");

    ContactIslandGraph solveGraph;
    const std::vector<DistanceConstraint> solveConstraints = {
        DistanceConstraint{.bodyA = 0, .bodyB = 1, .restLength = 2.f},
        DistanceConstraint{.bodyA = 2, .bodyB = 3, .restLength = 2.f},
    };
    solveGraph.build(4, {}, solveConstraints);
    RigidBodySoA bodies;
    bodies.addBody({0.f, 0.f, 0.f}, 1.f, 0);
    bodies.addBody({2.f, 0.f, 0.f}, 1.f, RB_SLEEPING);
    bodies.addBody({20.f, 0.f, 0.f}, 1.f, RB_SLEEPING);
    ...
    const u32 activeIsland = solveGraph.bodyIsland(2);
    SolverWorkBuffers work;
    work.init(4, 0, 2);
    work.contactManifolds().clear();

    expectTrue(island_constraint_solve_reject_reason(
                   solveGraph.island(activeIsland), bodies, work.contactManifolds(), solveConstraints, 1.f / 60.f) ==
                   IslandConstraintSolveRejectReason::NoMovableBodies,
               "constraint solve reject reason flags all-sleeping island");
    expectTrue(island_constraint_solve_rejects_for_reason(solveGraph.island(activeIsland),
                                                          bodies,
                                                          work.contactManifolds(),
                                                          solveConstraints,
                                                          1.f / 60.f,
               ...
               "constraint solve rejects_for_reason matches all-sleeping island");
    expectTrue(island_constraint_solve_reject_reason(
                   solveGraph.island(activeIsland), bodies, work.contactManifolds(), solveConstraints, 0.f) ==
                   IslandConstraintSolveRejectReason::InvalidDt,
               "constraint solve reject reason flags invalid dt");

    IslandConstraintSolvePreflight solvePreflight{};
    const auto invMass = [](const RigidBodySoA& soa, u32 index) { return soa.invMasses[index]; };
    expectTrue(!solve_island_job_with_preflight(bodies,
                                                solveGraph.island(activeIsland),
                                                work,
                                                solveConstraints,
                                                1.f / 60.f,
                                                0.f,
                                                invMass,
                                                solvePreflight),
               "solve_with_preflight skips all-sleeping island");
    expectTrue(solvePreflight.reason == IslandConstraintSolveRejectReason::NoMovableBodies,
               "solve_with_preflight reports no-movable reason");
    ...
    expectTrue(solve_island_job_with_preflight(bodies,
                                               ...
                                               invMass,
                                               solvePreflight),
               "solve_with_preflight solves island with movable body");
    expectTrue(solvePreflight.reason == IslandConstraintSolveRejectReason::None,
               "solve_with_preflight reports None on successful solve");

    ContactIslandGraph sleepGraph;
    sleepGraph.build(4, {}, solveConstraints);
    bodies.flags[0] = 0;
    bodies.flags[1] = RB_SLEEPING;
    bodies.flags[2] = RB_SLEEPING;
    bodies.flags[3] = RB_SLEEPING;
    const u32 mixedIsland = sleepGraph.bodyIsland(0);
    const u32 sleepingIsland = sleepGraph.bodyIsland(2);

    expectTrue(island_sleep_reject_reason(sleepGraph.island(sleepingIsland), bodies) ==
                   IslandSleepRejectReason::AllSleeping,
               "sleep reject reason flags all-sleeping island");
    expectTrue(island_sleep_rejects_for_reason(sleepGraph.island(sleepingIsland),
                                               bodies,
                                               IslandSleepRejectReason::AllSleeping),
               "sleep rejects_for_reason matches all-sleeping island");
    expectTrue(island_sleep_reject_reason(sleepGraph.island(mixedIsland), bodies) == IslandSleepRejectReason::None,
               "sleep reject reason allows mixed island");

    expectTrue(island_wake_reject_reason(sleepGraph.island(sleepingIsland), bodies) ==
                   IslandWakeRejectReason::NoMixedSleepState,
               "wake reject reason flags all-sleeping island");
    expectTrue(island_wake_rejects_for_reason(sleepGraph.island(mixedIsland),
                                              bodies,
                                              IslandWakeRejectReason::None),
               "wake rejects_for_reason allows mixed island");

    expectTrue(!island_sleep_graph_rejects_all(sleepGraph, bodies),
               "sleep graph rejects_all false when mixed island exists");
    expectTrue(!island_wake_graph_rejects_all(sleepGraph, bodies),
               "wake graph rejects_all false when wakeable island exists");

    IslandWakePreflight wakePreflight{};
    expectTrue(wake_island_sleepers_with_preflight(bodies, sleepGraph.island(mixedIsland), wakePreflight),
               "wake_with_preflight activates mixed island sleepers");
    expectTrue(wakePreflight.reason == IslandWakeRejectReason::None,
               "wake_with_preflight reports None on successful wake");
    ...
    bodies.flags[3] = RB_SLEEPING;
    expectTrue(island_sleep_graph_rejects_all(sleepGraph, bodies),
               "sleep graph rejects_all true when every island is all-sleeping");
    expectTrue(island_wake_graph_rejects_all(sleepGraph, bodies),
               "wake graph rejects_all true when no mixed island remains");
}
...
void testPreflightIslandConstraintSolveGraphGuards() {
    ...
    const IslandConstraintSolveGraphPreflight graphPreflight =
        preflight_island_constraint_solve_graph(graph, bodies, work.contactManifolds(), constraints);
    expectTrue(!graphPreflight.skipped, "graph constraint-solve preflight has solveable islands");
    expectTrue(graphPreflight.can_solve(), "graph constraint-solve preflight can solve");
    expectTrue(graphPreflight.stats.solveableCount == 1u, "graph counts one solveable island");
    expectTrue(graphPreflight.stats.allSleepingCount == 1u, "graph counts all-sleeping island");
    expectTrue(!should_skip_island_constraint_solve_graph(graph, bodies, work.contactManifolds(), constraints),
               "should_skip graph false when mixed island exists");
    expectTrue(collect_solveable_island_indices(graph, bodies, work.contactManifolds(), constraints).size() == 1u,
               "collect solveable indices returns mixed island");
...
void testDispatchSolveIslandGuardedAndWake() {
    ContactIslandGraph graph;
    std::vector<narrowphase::ContactManifold> contacts;
    std::vector<DistanceConstraint> constraints = {
        DistanceConstraint{.bodyA = 0, .bodyB = 1, .restLength = 2.f},
    ...
    const IslandWakeAndSolvePreflight preflight =
        preflight_island_wake_and_solve(graph.island(0), bodies, work.contactManifolds(), constraints);
    expectTrue(preflight.can_dispatch(), "wake-and-solve preflight can dispatch mixed island");
    expectTrue(preflight.should_wake_first(), "wake-and-solve preflight requests wake first");
    ...
    expectTrue(dispatch_solve_island_with_wake_guarded(bodies,
                                                       ...
                                                       0.f,
                                                       invMassFn),
               "wake-then-guarded dispatch solves mixed island");
    expectTrue((bodies.flags[1] & RB_SLEEPING) == 0u, "wake-then-dispatch clears sleeper flag");

    bodies.flags[1] |= RB_SLEEPING;
    bodies.sleepTimers[1] = 0.5f;
    const IslandDispatchResult guardedResult = dispatch_solve_island_guarded_result(
        bodies, graph, 0, work, constraints, dt, 0.f, invMassFn);
    expectTrue(guardedResult.solved, "guarded dispatch solves mixed island without wake");
    expectTrue((bodies.flags[1] & RB_SLEEPING) != 0u, "guarded dispatch alone does not wake sleeper");

    bodies.flags[0] |= RB_SLEEPING;
    const IslandDispatchResult blocked = dispatch_solve_island_guarded_result(
        bodies, graph, 0, work, constraints, dt, 0.f, invMassFn);
    expectTrue(!blocked.solved, "guarded dispatch skips all-sleeping island");
    expectTrue(blocked.skipped, "all-sleeping guarded dispatch is skipped");
}
...
void testDispatchAllSolveableIslandsResult() {
    ...
    const IslandBatchDispatchResult batch =
        dispatch_all_solveable_islands_result(bodies, graph, work, constraints, dt, 0.f, invMassFn);
    expectTrue(!batch.skipped, "solveable batch does not skip when one island can solve");
    expectTrue(batch.dispatchableCount == 1u, "solveable batch counts one dispatchable island");
    expectTrue(batch.solvedCount == 1u, "solveable batch solves mixed island");
    expectTrue(batch.skippedCount == 0u, "solveable batch has no per-island skips");

    const IslandBatchDispatchResult wakeBatch =
        dispatch_all_solveable_islands_with_wake_result(bodies, graph, work, constraints, dt, 0.f, invMassFn);
    expectTrue(wakeBatch.solvedCount == 1u, "wake batch solves mixed island");
    expectTrue((bodies.flags[1] & RB_SLEEPING) == 0u, "wake batch wakes sleeper before solve");

    bodies.flags[0] |= RB_SLEEPING;
    bodies.flags[1] |= RB_SLEEPING;
    bodies.flags[2] |= RB_SLEEPING;
    bodies.flags[3] |= RB_SLEEPING;
    const IslandBatchDispatchResult allSleeping =
        dispatch_all_solveable_islands_result(bodies, graph, work, constraints, dt, 0.f, invMassFn);
    expectTrue(allSleeping.skipped, "solveable batch skips when every island is all-sleeping");
    expectTrue(allSleeping.solvedCount == 0u, "all-sleeping batch solves nothing");
}

void testContactIslandGraphBuildSkipsOutOfRangeRefs() {
    std::vector<narrowphase::ContactManifold> contacts;
    contacts.push_back(narrowphase::ContactManifold{});
    contacts.back().valid = true;
    contacts.back().bodyA = 0;
    ...
    graph.build(4, contacts, constraints);
    expectTrue(graph.constrainedIslandCount() == 1u,
               "build ignores out-of-range contacts and distance constraints");
    expectTrue(graph.island(0).contactIndices.size() == 1u, "in-range contact is assigned");
    expectTrue(graph.island(0).distanceIndices.size() == 1u, "in-range distance constraint is assigned");
}
...
void testSolveIslandJobGuardedSkipsAllSleeping() {
    ...
    expectTrue(!solve_island_job_guarded(bodies,
                                         ...
                                         invMassFn),
               "guarded solve skips all-sleeping island");
    expectTrue(solve_island_job_guarded_result(bodies, graph, 0, work, constraints, 1.f / 60.f, 0.f, invMassFn)
                   .skipped,
               "guarded solve result reports skipped all-sleeping island");
}
...
void testPreflightSolveIslandJobFullGuards() {
    ...
    const IslandFullSolvePreflight preflight =
        preflight_solve_island_job_full(job, bodies, work.contactManifolds(), constraints, 1.f / 60.f);
    expectTrue(!preflight.skipped, "full job preflight does not skip constrained island");
    expectTrue(preflight.can_solve(), "full job preflight can solve movable island");
...
void testWakeAndSolveIslandGuarded() {
    ...
    expectTrue(wake_and_solve_island_guarded(bodies,
                                             graph.island(mixedIsland),
                                             work,
                                             constraints,
                                             dt,
                                             0.f,
                                             invMassFn),
               "wake-and-solve succeeds for mixed island");
    ...
    expectTrue(!wake_and_solve_island_guarded(bodies,
                                              ...
                                              0.f,
                                              invMassFn),
               "wake-and-solve skips all-sleeping island");
    ...
    const IslandSleepWakeSolveGraphPreflight graphPreflight = preflight_island_sleep_wake_solve_graph(
        graph, bodies, work.contactManifolds(), constraints);
    ...
    expectTrue(collect_sleep_wake_solveable_island_indices(graph, bodies, work.contactManifolds(), constraints)
                       .size() == 1u,
               "collect returns mixed island only");

    const IslandSleepWakeSolveBatchResult batch = dispatch_all_islands_sleep_wake_result(
        bodies, graph, work, constraints, dt, 0.f, invMassFn);
    expectTrue(!batch.skipped, "sleep-wake batch does not skip solveable graph");
    expectTrue(batch.solvedCount == 1u, "sleep-wake batch solves mixed island");
    expectTrue(batch.any_solved(), "sleep-wake batch reports solved island");
    expectTrue(dispatch_all_islands_sleep_wake_guarded(bodies, graph, work, constraints, dt, 0.f, invMassFn) ==
                   batch.solvedCount,
               "sleep-wake guarded count matches batch result");
}

void testPreflightIslandSolveBodiesOutOfRangeCount() {
    ContactIslandGraph::Island island{};
    island.bodyIndices = {0, 1, 99u};
    island.distanceIndices = {0};
    ...
    expectTrue(preflight.outOfRangeBodyCount == 1u, "solve-bodies preflight counts out-of-range body index");
    expectTrue(preflight.inRangeBodyCount == 2u, "solve-bodies preflight counts in-range bodies");
    expectTrue(preflight.can_solve(), "island with movable in-range bodies can still solve");
}
...
void testPreflightContactIslandBuildGuards() {
    ...
    expectTrue(!preflight.skipped, "contact build preflight does not skip valid partition inputs");
    expectTrue(preflight.has_unsafe_refs(), "contact build preflight flags out-of-range refs");
    expectTrue(!preflight.can_build(), "contact build preflight cannot build unsafe refs");
    expectTrue(should_skip_contact_island_build(4, contacts, constraints),
               "should_skip_contact_island_build on unsafe refs");

    ContactIslandGraph graph;
    expectTrue(!graph.buildGuarded(4, contacts, constraints), "buildGuarded rejects unsafe refs");
    expectTrue(graph.islandCount() == 0u, "rejected buildGuarded clears graph");

    std::vector<narrowphase::ContactManifold> safeContacts;
    safeContacts.push_back(contacts[0]);
    const std::vector<DistanceConstraint> safeConstraints = {constraints[0]};
    expectTrue(graph.buildGuarded(4, safeContacts, safeConstraints), "buildGuarded succeeds for in-range inputs");
...
void testPreflightIslandConstraintSolveGraphGuards() {
    ...
    const IslandConstraintSolveGraphPreflight allSleeping =
        preflight_island_constraint_solve_graph(graph, bodies, work.contactManifolds(), constraints);
    expectTrue(allSleeping.skipped, "constraint-solve graph preflight skips when all islands blocked");
    expectTrue(should_skip_island_constraint_solve_graph(graph, bodies, work.contactManifolds(), constraints),
...
void testSolveIslandJobGuarded() {
    ...
    const auto invMass = [](const RigidBodySoA& soa, u32 index) { return soa.invMasses[index]; };
    const ContactIslandGraph::Island& island = graph.island(0);
    expectTrue(!solve_island_job_guarded(bodies, island, work, constraints, 1.f / 60.f, 0.f, invMass),
               "guarded solve skips all-sleeping island");
    expectTrue(solve_island_job(bodies, island, work, constraints, 1.f / 60.f, 0.f, invMass),
               "unguarded solve still runs on all-sleeping island");

    bodies.flags[0] &= ~RB_SLEEPING;
    expectTrue(solve_island_job_guarded(bodies, island, work, constraints, 1.f / 60.f, 0.f, invMass),
               "guarded solve runs when mixed island has movable body");
}

void testIslandWakeResultAndBatch() {
    ContactIslandGraph graph;
    std::vector<narrowphase::ContactManifold> contacts;
    const std::vector<DistanceConstraint> constraints = {
        DistanceConstraint{.bodyA = 0, .bodyB = 1, .restLength = 2.f},
    ...
    const u32 mixedIsland = graph.bodyIsland(0);
    const IslandWakeResult wakeResult = wake_island_sleepers_result(bodies, graph.island(mixedIsland), mixedIsland);
    expectTrue(wakeResult.woke, "wake result activates mixed island");
    expectTrue(wakeResult.sleeperCount == 1u, "wake result counts awakened sleepers");
    expectTrue(!wakeResult.skipped, "wake result not skipped for wakeable island");

    const IslandWakeResult byIndex = wake_island_sleepers_by_index_result(bodies, graph, mixedIsland);
    expectTrue(byIndex.skipped, "wake by index skips already active island");

    const IslandBatchWakeResult batch = wake_all_island_sleepers_result(bodies, graph);
    expectTrue(batch.skipped, "batch wake skipped when nothing wakeable remains");
}
...
void testDispatchAfterWakeGuards() {
    ...
    expectTrue(!preflight.skipped, "dispatch-after-wake preflight has dispatchable islands");
    expectTrue(preflight.needs_wake(), "dispatch-after-wake preflight needs wake");
    expectTrue(preflight.can_dispatch(), "dispatch-after-wake preflight can dispatch");

    const IslandBatchDispatchResult batch = dispatch_all_islands_after_wake_result(
        bodies, graph, work, constraints, dt, 0.f, invMass);
    expectTrue(batch.any_solved(), "dispatch-after-wake batch solves mixed island");
    expectTrue((bodies.flags[1] & RB_SLEEPING) == 0u, "dispatch-after-wake wakes sleeper before solve");

    const u32 mixedIsland = graph.bodyIsland(0);
    const IslandDispatchResult single =
        dispatch_solve_island_after_wake_result(bodies, graph, mixedIsland, work, constraints, dt, 0.f, invMass);
    expectTrue(single.solved, "single dispatch-after-wake solves mixed island");

    const IslandSolveJob job = extract_island(graph, mixedIsland);
    const IslandDispatchResult guardedJob = dispatch_solve_island_job_guarded_result(
        bodies, job, work, constraints, dt, 0.f, invMass);
    expectTrue(guardedJob.solved, "guarded job dispatch solves mixed island");
}
...
void testIslandBuildResultAndDegenerateGuards() {
    ...
    expectTrue(preflight.reason == IslandBuildRejectReason::DegenerateRefs,
               "build preflight records DegenerateRefs reason");
    expectTrue(preflight.stats.selfContactCount == 1u, "build preflight counts self-contact");
    expectTrue(preflight.stats.selfDistanceCount == 1u, "build preflight counts self-distance");
    expectTrue(!preflight.can_build(), "build preflight rejects degenerate refs");
    expectTrue(should_skip_island_build(2, contacts, constraints),
...
void testContactIslandGraphRangeHelpers() {
    expectTrue(ContactIslandGraph::isBodyPairInRange(0u, 1u, 4u),
               "in-range body pair passes range helper");
    expectTrue(!ContactIslandGraph::isBodyPairInRange(3u, 4u, 4u),
               "out-of-range body pair fails range helper");

    narrowphase::ContactManifold inRange{};
    inRange.valid = true;
    inRange.bodyA = 0;
    inRange.bodyB = 1;
    narrowphase::ContactManifold outOfRange = inRange;
    outOfRange.bodyA = 8;
    outOfRange.bodyB = 9;
    expectTrue(ContactIslandGraph::isContactInRange(inRange, 4u),
               "in-range contact passes contact range helper");
    expectTrue(!ContactIslandGraph::isContactInRange(outOfRange, 4u),
               "out-of-range contact fails contact range helper");

    const DistanceConstraint inRangeConstraint{.bodyA = 0, .bodyB = 1, .restLength = 2.f};
    const DistanceConstraint outOfRangeConstraint{.bodyA = 4, .bodyB = 5, .restLength = 2.f};
    expectTrue(ContactIslandGraph::isDistanceConstraintInRange(inRangeConstraint, 4u),
               "in-range distance constraint passes range helper");
    expectTrue(!ContactIslandGraph::isDistanceConstraintInRange(outOfRangeConstraint, 4u),
               "out-of-range distance constraint fails range helper");

    std::vector<narrowphase::ContactManifold> contacts = {inRange, outOfRange};
    const std::vector<DistanceConstraint> constraints = {inRangeConstraint, outOfRangeConstraint};
    ContactIslandGraph graph;
    graph.build(4, contacts, constraints);
    expectTrue(graph.constrainedIslandCount() == 1u,
               "build ignores out-of-range contacts and distance constraints on valid body count");
    expectTrue(graph.island(graph.bodyIsland(0)).contactIndices.size() == 1u,
               "build assigns only in-range contact to island");
    expectTrue(graph.island(graph.bodyIsland(0)).distanceIndices.size() == 1u,
               "build assigns only in-range distance constraint to island");
...
void testCollectSolveableIslandIndices() {
    ContactIslandGraph graph;
    std::vector<narrowphase::ContactManifold> contacts;
    std::vector<DistanceConstraint> constraints = {
        DistanceConstraint{.bodyA = 0, .bodyB = 1, .restLength = 2.f},
    ...
    const std::vector<u32> solveable = collect_solveable_island_indices(graph, bodies);
    expectTrue(solveable.size() == 1u, "collect solveable indices skips all-sleeping island");
    expectTrue(count_solveable_islands(graph, bodies) == 1u, "count solveable islands matches collection");
    expectTrue(has_solveable_islands(graph, bodies), "has solveable islands true for mixed graph");

    bodies.flags[2] = 0u;
    bodies.flags[3] = 0u;
    expectTrue(collect_solveable_island_indices(graph, bodies).size() == 2u,
               "collect solveable indices includes fully active islands");
}
...
void testPreflightIslandSleepWakeDispatch() {
    ...
    expectTrue(!preflight.skipped, "sleep/wake dispatch preflight does not skip mixed graph");
    expectTrue(preflight.can_dispatch(), "sleep/wake dispatch preflight can dispatch mixed graph");
    expectTrue(preflight.sleep.has_solveable_islands(), "sleep preflight reports solveable islands");
    expectTrue(preflight.wake.can_wake(), "wake preflight reports wakeable island");
    expectTrue(!should_skip_island_sleep_wake_dispatch(graph, bodies, dt),
               "should_skip false for mixed sleep/wake graph");

    ContactIslandGraph emptyGraph;
    emptyGraph.build(0, {}, {});
    expectTrue(preflight_island_sleep_wake_dispatch(emptyGraph, bodies, dt).skipped,
               "sleep/wake dispatch preflight skips empty graph");
    expectTrue(should_skip_island_sleep_wake_dispatch(emptyGraph, bodies, dt),
               "should_skip true for empty graph");

    bodies.flags[0] |= RB_SLEEPING;
    bodies.flags[1] |= RB_SLEEPING;
    expectTrue(preflight_island_sleep_wake_dispatch(graph, bodies, dt).skipped,
               "sleep/wake dispatch preflight skips when every island is all-sleeping");
}
...
void testSolveIslandJobGuarded() {
    ...
    expectTrue(!solve_island_job_guarded(bodies,
                                         ...
                                         0.f,
                                         invMassFn),
               "guarded solve rejects non-finite dt");
}

void testWakeIslandSleepersResultAndBatch() {
    ContactIslandGraph graph;
    std::vector<narrowphase::ContactManifold> contacts;
    std::vector<DistanceConstraint> constraints = {
        DistanceConstraint{.bodyA = 0, .bodyB = 1, .restLength = 2.f},
    ...
    const IslandWakeResult wakeResult = wake_island_sleepers_result(bodies, graph, mixedIsland);
    expectTrue(wakeResult.woke, "wake result activates mixed island sleepers");
    expectTrue(wakeResult.bodiesWoken == 1u, "wake result counts woken bodies");
    expectTrue((bodies.flags[1] & RB_SLEEPING) == 0u, "wake result clears sleeping flag");

    const IslandWakeResult outOfRange = wake_island_sleepers_result(bodies, graph, graph.islandCount() + 1u);
    expectTrue(outOfRange.skipped, "wake result skips out-of-range island");

    const IslandBatchWakeResult batch = wake_all_island_sleepers_result(bodies, graph);
    expectTrue(batch.skipped, "batch wake skips when no wakeable islands remain");
    expectTrue(!batch.any_woke(), "batch wake reports no additional wakes");
}
...
void testDispatchSolveIslandWithSleepWakeGuards() {
    ...
    const u32 mixedIsland = graph.bodyIsland(0);
    const IslandSleepWakeDispatchResult mixedResult = dispatch_solve_island_with_sleep_wake_result(
        bodies, graph, mixedIsland, work, constraints, dt, 0.f, invMassFn);
    expectTrue(mixedResult.wokeSleepers, "sleep/wake dispatch wakes mixed island before solve");
    expectTrue(mixedResult.solved, "sleep/wake dispatch solves mixed island");
    expectTrue(!mixedResult.skipped, "sleep/wake dispatch does not skip mixed island");

    const u32 sleepingIsland = graph.bodyIsland(2);
    const IslandSleepWakeDispatchResult sleepingResult = dispatch_solve_island_with_sleep_wake_result(
        bodies, graph, sleepingIsland, work, constraints, dt, 0.f, invMassFn);
    expectTrue(sleepingResult.skipped, "sleep/wake dispatch skips all-sleeping island");
    expectTrue(!sleepingResult.solved, "sleep/wake dispatch does not solve all-sleeping island");

    expectTrue(dispatch_solve_island_with_sleep_wake_guards(
                   bodies, graph, mixedIsland, work, constraints, dt, 0.f, invMassFn),
               "guarded sleep/wake dispatch returns true for solved island");
}
...
void testDispatchAllIslandsWithSleepWakeGuards() {
    ...
    const IslandBatchSleepWakeDispatchResult batch =
        dispatch_all_islands_with_sleep_wake_result(bodies, graph, work, constraints, dt, 0.f, invMassFn);
    expectTrue(!batch.skipped, "batch sleep/wake dispatch does not skip mixed graph");
    expectTrue(batch.solveableCount == 1u, "batch sleep/wake dispatch records solveable count");
    expectTrue(batch.wokeCount == 1u, "batch sleep/wake dispatch wakes mixed island");
    expectTrue(batch.solvedCount == 1u, "batch sleep/wake dispatch solves mixed island only");
    expectTrue(batch.any_solved(), "batch sleep/wake dispatch reports solved island");
    expectTrue(dispatch_all_islands_with_sleep_wake_guards(bodies, graph, work, constraints, dt, 0.f, invMassFn) ==
                   batch.solvedCount,
               "guarded batch count matches result struct");

    ContactIslandGraph emptyGraph;
    emptyGraph.build(0, {}, {});
    const IslandBatchSleepWakeDispatchResult emptyBatch =
        dispatch_all_islands_with_sleep_wake_result(bodies, emptyGraph, work, constraints, dt, 0.f, invMassFn);
    expectTrue(emptyBatch.skipped, "batch sleep/wake dispatch skips empty graph");
    expectTrue(!emptyBatch.any_solved(), "empty graph batch reports no solved islands");
}

void testContactIslandBuildSkipsOutOfRangeRefs() {
    std::vector<narrowphase::ContactManifold> contacts;
    contacts.push_back(narrowphase::ContactManifold{});
    contacts.back().valid = true;
    contacts.back().bodyA = 0;
...
void testPreflightIslandSolveDispatchGuards() {
    ...
    const IslandSolveDispatchPreflight mixedPreflight = preflight_island_solve_dispatch(
        graph.island(mixedIsland), bodies, contacts, constraints);
    expectTrue(!mixedPreflight.skipped, "solve dispatch preflight does not skip mixed island");
    expectTrue(mixedPreflight.can_solve(), "mixed island can solve");
    expectTrue(mixedPreflight.should_wake_first(), "mixed island should wake before solve");
    ...
    const IslandSolveDispatchPreflight outOfRange =
        preflight_island_solve_dispatch_by_index(graph, graph.islandCount() + 1u, bodies, contacts, constraints);
    expectTrue(outOfRange.skipped, "solve dispatch index preflight skips out-of-range island");

    const std::vector<u32> solveable = collect_solveable_island_indices(graph, bodies, contacts, constraints);
    expectTrue(solveable.size() == 1u, "collect_solveable_island_indices returns mixed island only");
    expectTrue(solveable[0] == mixedIsland, "solveable index is mixed island");
}
...
void testDispatchSolveIslandGuardedWakesAndSolves() {
    ...
    const IslandGuardedDispatchResult result = dispatch_solve_island_guarded_result(bodies,
                                                                                    graph,
                                                                                    0u,
                                                                                    work,
                                                                                    constraints,
                                                                                    ...
                                                                                    0.f,
                                                                                    invMassFn);
    expectTrue(result.wokeSleepers, "guarded dispatch wakes mixed-island sleeper");
    expectTrue(result.solved, "guarded dispatch solves mixed island");
    expectTrue((bodies.flags[1] & RB_SLEEPING) == 0u, "guarded dispatch clears sleeping flag");
    const f32 dist = (bodies.predictedPositions[0] - bodies.predictedPositions[1]).length();
    expectTrue(dist < 2.1f, "guarded dispatch moves bodies toward rest length");
}
...
void testDispatchAllIslandsGuardedBatch() {
    ...
    const IslandBatchGuardedDispatchResult batch = dispatch_all_islands_guarded_result(bodies,
                                                                                       graph,
                                                                                       work,
                                                                                       constraints,
                                                                                       1.f / 60.f,
                                                                                       0.f,
                                                                                       invMassFn);
    expectTrue(!batch.skipped, "guarded batch does not skip mixed graph");
    expectTrue(batch.dispatchableCount == 1u, "guarded batch dispatches one solveable island");
    expectTrue(batch.solvedCount == 1u, "guarded batch solves mixed island");
    expectTrue(batch.wokeCount == 1u, "guarded batch wakes one island");
    expectTrue(batch.any_solved(), "guarded batch reports solved islands");
    expectTrue(dispatch_all_islands_guarded(bodies,
                                            graph,
                                            work,
                                            constraints,
                                            1.f / 60.f,
                                            0.f,
                                            invMassFn) == batch.solvedCount,
               "guarded count matches batch result");

    ContactIslandGraph allSleepingGraph;
    allSleepingGraph.build(2, contacts, {constraints[0]});
    bodies.flags[0] |= RB_SLEEPING;
    bodies.flags[1] |= RB_SLEEPING;
    const IslandBatchGuardedDispatchResult sleepingBatch =
        dispatch_all_islands_guarded_result(bodies,
                                            allSleepingGraph,
                                            work,
                                            {constraints[0]},
                                            1.f / 60.f,
                                            0.f,
                                            invMassFn);
    expectTrue(sleepingBatch.skipped, "guarded batch skips all-sleeping graph");
    expectTrue(sleepingBatch.solvedCount == 0u, "guarded batch solves zero all-sleeping islands");
}
...
void testPreflightIslandBuildSelfReferentialGuards() {
    ...
    const std::vector<DistanceConstraint> constraints = {
        DistanceConstraint{.bodyA = 0, .bodyB = 0, .restLength = 1.f},
        DistanceConstraint{.bodyA = 0, .bodyB = 1, .restLength = 2.f},
    };

    const IslandBuildPreflight preflight = preflight_island_build(2, contacts, constraints);
    expectTrue(preflight.stats.selfReferentialContactCount == 1u,
               "build preflight counts self-referential contact");
    expectTrue(preflight.stats.selfReferentialDistanceCount == 1u,
               "build preflight counts self-referential distance constraint");
    expectTrue(preflight.has_unsafe_refs(), "self-referential refs are unsafe");
    expectTrue(!preflight.can_build(), "build preflight rejects self-referential refs");
    expectTrue(should_skip_island_build(2, contacts, constraints),
...
void testSolveIslandJobGuardedSkipsAllSleeping() {
    ...
    expectTrue(!solve_island_job_guarded(bodies,
                                         ...
                                         invMassFn),
               "guarded solve skips all-sleeping island");
    expectTrue(solve_island_job_guarded_result(bodies,
                                               graph,
                                               islandIndex,
                                               work,
                                               constraints,
                                               ...
                                               invMassFn)
                   .skipped,
               "guarded solve result marks all-sleeping island skipped");
}
...
void testPreflightIslandSolvePipelineGuards() {
    ...
    const IslandSolvePipelinePreflight mixedPipeline = preflight_island_solve_pipeline(
        graph.island(mixedIsland), bodies, work.contactManifolds(), constraints);
    expectTrue(!mixedPipeline.skipped, "pipeline preflight does not skip mixed island");
    expectTrue(mixedPipeline.should_wake_sleepers(), "pipeline preflight detects wake need");
    expectTrue(mixedPipeline.can_solve(), "pipeline preflight allows mixed island solve");
    expectTrue(!should_skip_island_solve_pipeline(
                   graph.island(mixedIsland), bodies, work.contactManifolds(), constraints),
               "should_skip pipeline false for mixed island");

    const IslandSolvePipelinePreflight sleepingPipeline = preflight_island_solve_pipeline(
        graph.island(sleepingIsland), bodies, work.contactManifolds(), constraints);
    expectTrue(sleepingPipeline.can_skip_sleeping_solve(), "pipeline preflight marks all-sleeping island");
    expectTrue(!sleepingPipeline.can_solve(), "pipeline preflight blocks all-sleeping island");
    expectTrue(should_skip_island_solve_pipeline(
                   graph.island(sleepingIsland), bodies, work.contactManifolds(), constraints),
               "should_skip pipeline true for all-sleeping island");

    const std::vector<u32> pipelineIndices =
        collect_pipeline_solveable_island_indices(graph, bodies, work.contactManifolds(), constraints);
    expectTrue(pipelineIndices.size() == 1u, "pipeline collect returns mixed island only");
    expectTrue(collect_constraint_solveable_island_indices(graph, bodies, work.contactManifolds(), constraints)
                       .size() == 1u,
               "constraint-solveable collect matches pipeline for this graph");

    const IslandSolvePipelinePreflight outOfRange =
        preflight_island_solve_pipeline_by_index(graph, graph.islandCount() + 2u, bodies, work.contactManifolds(), constraints);
    expectTrue(outOfRange.skipped, "pipeline index preflight skips out-of-range island");
}
...
void testDispatchSolveIslandWithWakeGuarded() {
    ...
    const u32 islandIndex = graph.bodyIsland(0);
    const vec3 before = bodies.predictedPositions[1];
    expectTrue(dispatch_solve_island_with_wake_guarded(bodies,
                                                       graph,
                                                       islandIndex,
                                                       work,
                                                       ...
                                                       0.f,
                                                       invMassFn),
               "wake+dispatch solves mixed-sleep island");
    expectTrue((bodies.flags[1] & RB_SLEEPING) == 0u, "wake+dispatch clears sleeping flag");
    expectTrue((bodies.predictedPositions[1] - before).length() > 1e-4f,
               "wake+dispatch moves previously sleeping body");
    ...
    expectTrue(!dispatch_solve_island_with_wake_guarded(bodies,
                                                        ...
                                                        0.f,
                                                        invMassFn),
               "wake+dispatch skips all-sleeping island");
}

void testDispatchAllIslandsWithWakeResultBatch() {
    ContactIslandGraph graph;
    std::vector<narrowphase::ContactManifold> contacts;
    std::vector<DistanceConstraint> constraints = {
        DistanceConstraint{.bodyA = 0, .bodyB = 1, .restLength = 2.f},
    ...
    const IslandBatchWakeDispatchResult batch = dispatch_all_islands_with_wake_result(
        bodies, graph, work, constraints, 1.f / 60.f, 0.f, invMassFn);
    expectTrue(!batch.skipped, "wake batch does not skip mixed graph");
    expectTrue(batch.wokeCount == 1u, "wake batch wakes one mixed island");
    expectTrue(batch.solvedCount == 1u, "wake batch solves one pipeline-solveable island");
    expectTrue(batch.any_solved(), "wake batch reports solved islands");
    expectTrue(dispatch_all_islands_with_wake_guarded(
                   bodies, graph, work, constraints, 1.f / 60.f, 0.f, invMassFn) == batch.solvedCount,
               "wake guarded count matches batch solved count");
}
...
void testContactIslandGraphBuildSkipsOutOfRangeRefs() {
    ...
    graph.build(4, contacts, constraints);
    expectTrue(graph.constrainedIslandCount() == 1u,
               "direct build skips out-of-range contacts and distance constraints");
    expectTrue(graph.island(graph.bodyIsland(0)).contactIndices.size() == 1u,
               "in-range contact is retained after build bounds guard");
    expectTrue(graph.island(graph.bodyIsland(0)).distanceIndices.size() == 1u,
               "in-range distance constraint is retained after build bounds guard");
}
...
void testPreflightIslandConstraintSolveByIndex() {
    ...
    expectTrue(!should_skip_island_constraint_solve_by_index(
                   graph, mixedIsland, bodies, work.contactManifolds(), constraints),
               "should_skip false for mixed island by index");

    const IslandConstraintSolvePreflight sleepingPreflight = preflight_island_constraint_solve_by_index(
        graph, sleepingIsland, bodies, work.contactManifolds(), constraints);
    expectTrue(!sleepingPreflight.can_solve(), "all-sleeping island blocked by constraint-solve preflight");
    expectTrue(should_skip_island_constraint_solve_by_index(
                   graph, sleepingIsland, bodies, work.contactManifolds(), constraints),
               "should_skip true for all-sleeping island by index");
    ...
    expectTrue(graphPreflight.stats.solveableCount == 1u, "graph counts one solveable island");
    expectTrue(graphPreflight.stats.blockedByBodiesCount == 1u,
               "graph counts all-sleeping island blocked by bodies");
    expectTrue(collect_solveable_island_indices(graph, bodies, work.contactManifolds(), constraints).size() ==
                   1u,
               "collect solveable indices returns mixed island only");
    expectTrue(!should_skip_island_constraint_solve_graph(
                   graph, bodies, work.contactManifolds(), constraints),
               "should_skip graph false when one island can solve");
}
...
void testSolveIslandJobGuardedSkipsAllSleeping() {
    ...
    expectTrue(solve_island_job(bodies, *job.island, work, constraints, dt, 0.f, invMassFn),
               "unguarded solve still runs for all-sleeping island on valid refs");
    expectTrue(!solve_island_job_guarded(bodies, *job.island, work, constraints, dt, 0.f, invMassFn),
               "guarded solve skips all-sleeping island");
    expectTrue(!dispatch_solve_island_job_guarded(bodies, job, work, constraints, dt, 0.f, invMassFn),
               "guarded dispatch skips all-sleeping island job");
}

void testDispatchSolveIslandWakeGuarded() {
    ContactIslandGraph graph;
    std::vector<narrowphase::ContactManifold> contacts;
    std::vector<DistanceConstraint> constraints = {
        DistanceConstraint{.bodyA = 0, .bodyB = 1, .restLength = 2.f},
    ...
    expectTrue(preflight.can_dispatch(), "dispatch-with-wake preflight can dispatch mixed island");
    expectTrue(preflight.should_wake_first(), "dispatch-with-wake preflight requests wake first");

    expectTrue((bodies.flags[1] & RB_SLEEPING) != 0u, "sleeper starts sleeping");
    expectTrue(dispatch_solve_island_wake_guarded(bodies,
                                                    graph,
                                                    islandIndex,
                                                    work,
                                                    constraints,
                                                    ...
                                                    0.f,
                                                    invMassFn),
               "wake-guarded dispatch solves mixed island");
    expectTrue((bodies.flags[1] & RB_SLEEPING) == 0u, "wake-guarded dispatch clears sleeping flag");
}

void testWakeAllIslandSleepersResult() {
    ContactIslandGraph graph;
    std::vector<narrowphase::ContactManifold> contacts;
    std::vector<DistanceConstraint> constraints = {
        DistanceConstraint{.bodyA = 0, .bodyB = 1, .restLength = 2.f},
    ...
    const IslandWakeBatchResult batch = wake_all_island_sleepers_result(bodies, graph);
    expectTrue(!batch.skipped, "wake batch does not skip when wakeable island exists");
    expectTrue(batch.wakeableCount == 1u, "wake batch records one wakeable island");
    expectTrue(batch.wokeCount == 1u, "wake batch wakes mixed island");
    expectTrue(batch.any_woke(), "wake batch reports woke islands");

    const IslandWakeBatchResult secondPass = wake_all_island_sleepers_result(bodies, graph);
    expectTrue(secondPass.skipped, "second wake batch skips when nothing remains wakeable");
    expectTrue(secondPass.wokeCount == 0u, "second wake batch does not re-wake");
}

void testDispatchRejectsNonFiniteDt() {
    ContactIslandGraph graph;
    std::vector<narrowphase::ContactManifold> contacts;
    std::vector<DistanceConstraint> constraints = {
        DistanceConstraint{.bodyA = 0, .bodyB = 1, .restLength = 2.f},
    ...
    work.init(2, 0, 1);
    const auto invMassFn = [](const RigidBodySoA&, u32) { return 1.f; };
    const f32 infDt = std::numeric_limits<f32>::infinity();

    expectTrue(!dispatch_solve_island(bodies, graph, 0u, work, constraints, infDt, 0.f, invMassFn),
               "dispatch_solve_island rejects infinite dt");
    expectTrue(!solve_island_job(bodies,
                                 graph.island(0),
                                 work,
                                 constraints,
                                 ...
                                 0.f,
                                 invMassFn),
               "solve_island_job rejects NaN dt");
}
...
void testContactIslandGraphBuildSkipsUnsafeRefs() {
    std::vector<narrowphase::ContactManifold> contacts;
    contacts.push_back(narrowphase::ContactManifold{});
    contacts.back().valid = true;
    contacts.back().bodyA = 0;
    ...
    graph.build(4, contacts, constraints);
    expectTrue(graph.constrainedIslandCount() == 1u,
               "build skips out-of-range and self contacts while keeping in-range constraints");
    const u32 islandA = graph.bodyIsland(0);
    expectTrue(graph.island(islandA).contactIndices.size() == 1u,
               "build assigns only in-range contact to island");
    expectTrue(graph.island(islandA).distanceIndices.size() == 1u,
               "build assigns only in-range distance constraint to island");
}
...
void testPreflightIslandBuildSelfContactGuard() {
    ...
    expectTrue(preflight.stats.selfContactCount == 1u, "build preflight counts self contacts");
    expectTrue(preflight.stats.inRangeContactCount == 0u, "self contact is not in-range for build");
}

void testPreflightIslandConstraintRefsOutOfRangeCounts() {
    ContactIslandGraph::Island staleIsland{};
    staleIsland.bodyIndices = {0, 1};
    staleIsland.contactIndices = {9u, 10u};
    staleIsland.distanceIndices = {5u};

    const IslandConstraintRefsPreflight preflight =
        preflight_island_constraint_refs(staleIsland, {}, {});
    expectTrue(preflight.outOfRangeContactCount == 2u, "constraint refs preflight counts out-of-range contacts");
    expectTrue(preflight.outOfRangeDistanceCount == 1u, "constraint refs preflight counts out-of-range distances");
    expectTrue(!preflight.can_solve(), "all out-of-range refs cannot solve");
}
...
void testSolveIslandJobGuardedSkipsAllSleeping() {
    ...
    expectTrue(solve_island_job_guarded(bodies,
                   ...
                   solve_island_job(bodies,
                                    ...
                                    0.f,
                                    invMassFn),
               "guarded solve matches unguarded path when preflight passes");
}
...
void testDispatchSolveIslandConstraintGuarded() {
    ...
    const u32 activeIsland = graph.bodyIsland(0);
    const u32 sleepingIsland = graph.bodyIsland(2);
    const vec3 activeBefore = bodies.predictedPositions[0];
    ...
    expectTrue(dispatch_solve_island_constraint_guarded(bodies,
                                                        ...
                                                        0.f,
                                                        invMassFn),
               "constraint-guarded dispatch solves active island");
    expectTrue(bodies.predictedPositions[0].x != activeBefore.x,
               "constraint-guarded dispatch moves active island bodies");

    const IslandDispatchResult sleepingResult = dispatch_solve_island_constraint_result(bodies,
                                                                                        graph,
                                                                                        sleepingIsland,
                                                                                        work,
                                                                                        constraints,
                                                                                        ...
                                                                                        0.f,
                                                                                        invMassFn);
    expectTrue(sleepingResult.skipped, "constraint-guarded dispatch skips all-sleeping island");
    expectTrue(!sleepingResult.solved, "all-sleeping island is not solved");
}

void testDispatchSolveIslandAfterWake() {
    ContactIslandGraph graph;
    std::vector<DistanceConstraint> constraints = {
        DistanceConstraint{.bodyA = 0, .bodyB = 1, .restLength = 2.f},
    };
    ...
    const IslandWakeThenSolvePreflight preflight = preflight_wake_then_solve_island_by_index(
        graph, islandIndex, bodies, work.contactManifolds(), constraints);
    expectTrue(preflight.can_wake(), "wake-then-solve preflight sees wakeable mixed island");
    expectTrue(preflight.can_solve(), "wake-then-solve preflight allows solve after wake");
    expectTrue(preflight.should_wake_before_solve(), "wake-then-solve preflight requests wake before solve");

    const vec3 before = bodies.predictedPositions[0];
    expectTrue(dispatch_solve_island_after_wake_guarded(bodies,
                                                        graph,
                                                        islandIndex,
                                                        work,
                                                        constraints,
                                                        ...
                                                        0.f,
                                                        invMassFn),
               "wake-then-solve dispatch succeeds for mixed island");
    expectTrue((bodies.flags[1] & RB_SLEEPING) == 0u, "wake-then-solve dispatch clears sleeping flag");
    expectTrue(bodies.predictedPositions[0].x != before.x, "wake-then-solve dispatch moves bodies");
}
...
void testPreflightIslandSolveableGraph() {
    ...
    const IslandSolveableGraphPreflight preflight = preflight_island_solveable_graph(
        graph, bodies, work.contactManifolds(), constraints);
    expectTrue(!preflight.skipped, "solveable graph preflight has solveable islands");
    expectTrue(preflight.stats.solveableCount == 1u, "solveable graph counts one mixed island");
    expectTrue(preflight.stats.allSleepingCount == 1u, "solveable graph counts all-sleeping island");
    expectTrue(collect_solveable_island_indices(graph, bodies, work.contactManifolds(), constraints).size() ==
                   1u,
               "collect solveable indices returns mixed island only");
...
void testDispatchAllSolveableIslandsBatch() {
    ContactIslandGraph graph;
    std::vector<DistanceConstraint> constraints = {
        DistanceConstraint{.bodyA = 0, .bodyB = 1, .restLength = 2.f},
        DistanceConstraint{.bodyA = 2, .bodyB = 3, .restLength = 2.f},
    ...
    const IslandBatchDispatchResult batch =
        dispatch_all_solveable_islands_result(bodies, graph, work, constraints, dt, 0.f, invMassFn);
    expectTrue(!batch.skipped, "solveable batch dispatch does not skip mixed graph");
    expectTrue(batch.dispatchableCount == 1u, "solveable batch dispatch counts one solveable island");
    expectTrue(batch.solvedCount == 1u, "solveable batch dispatch solves one island");
    expectTrue(batch.skippedCount == 0u, "solveable batch dispatch does not skip solveable island");

    const IslandBatchDispatchResult invalidDt =
        dispatch_all_solveable_islands_result(bodies, graph, work, constraints, 0.f, 0.f, invMassFn);
    expectTrue(invalidDt.skipped, "solveable batch dispatch skips invalid dt");
}
...
void testWakeAllIslandSleepersResult() {
    ...
    const IslandWakeBatchResult batch = wake_all_island_sleepers_result(bodies, graph);
    expectTrue(!batch.skipped, "wake batch result does not skip wakeable graph");
    expectTrue(batch.wakeableCount == 1u, "wake batch result counts one wakeable island");
    expectTrue(batch.wokeCount == 1u, "wake batch result wakes one island");
    expectTrue(batch.any_woke(), "wake batch result reports woke islands");

    const IslandWakeBatchResult secondPass = wake_all_island_sleepers_result(bodies, graph);
    expectTrue(secondPass.skipped, "wake batch result skips when nothing remains wakeable");
}
...
void testBuildIslandGraphInRangeGuarded() {
    ...
    expectTrue(build_island_graph_in_range_guarded(graph, 4, contacts, constraints, &stats),
               "in-range guarded build succeeds with partial unsafe refs");
    expectTrue(stats.any_skipped(), "in-range build stats record skipped out-of-range refs");
    expectTrue(stats.processedValidContactCount == 1u, "in-range build processes one valid contact");
    expectTrue(stats.skippedOutOfRangeContactCount == 1u, "in-range build skips out-of-range contact");
    expectTrue(stats.processedDistanceCount == 1u, "in-range build processes one distance constraint");
    expectTrue(stats.skippedOutOfRangeDistanceCount == 1u,
               "in-range build skips out-of-range distance constraint");
    expectTrue(graph.constrainedIslandCount() == 1u, "in-range build forms one constrained island");

    ContactIslandGraph directGraph;
    directGraph.buildInRange(4, contacts, constraints, &stats);
    expectTrue(directGraph.constrainedIslandCount() == graph.constrainedIslandCount(),
               "buildInRange matches guarded in-range partition");
}
...
void testPreflightIslandConstraintSolveGraphGuards() {
    ...
    const IslandConstraintSolveGraphPreflight preflight =
        preflight_island_constraint_solve_graph(graph, bodies, work.contactManifolds(), constraints);
    expectTrue(!preflight.skipped, "constraint-solve graph preflight has solveable islands");
    expectTrue(preflight.stats.solveableCount == 1u, "constraint-solve graph counts one solveable island");
    expectTrue(preflight.stats.noMovableBodiesCount == 1u,
               "constraint-solve graph counts all-sleeping island as no-movable");
    expectTrue(!should_skip_island_constraint_solve_graph(graph, bodies, work.contactManifolds(), constraints),
               "should_skip constraint-solve graph false when mixed islands exist");

    const std::vector<u32> indices =
        collect_constraint_solveable_island_indices(graph, bodies, work.contactManifolds(), constraints);
    expectTrue(indices.size() == 1u, "collect constraint-solveable indices returns active island");
    expectTrue(indices[0] == graph.bodyIsland(0), "constraint-solveable index maps to active island");

    ContactIslandGraph::Island staleIsland{};
    staleIsland.bodyIndices = {99u};
    ...
    expectTrue(staleBodies.has_unsafe_body_refs(), "solve-bodies preflight flags out-of-range body refs");
    expectTrue(staleBodies.outOfRangeBodyCount == 1u, "solve-bodies preflight counts out-of-range bodies");
}
...
void testSolveIslandJobGuardedAndSleepAwareDispatch() {
    ...
    RigidBodySoA freshBodies;
    freshBodies.addBody({0.f, 0.f, 0.f}, 1.f, 0);
    freshBodies.addBody({2.1f, 0.f, 0.f}, 1.f, 0);
    freshBodies.addBody({20.f, 0.f, 0.f}, 1.f, RB_SLEEPING);
    freshBodies.addBody({22.f, 0.f, 0.f}, 1.f, RB_SLEEPING);
    freshBodies.predictedPositions = freshBodies.positions;

    expectTrue(!dispatch_solve_island_sleep_aware_guarded(freshBodies,
                                                          graph,
                                                          sleepingIsland,
                                                          work,
                                                          constraints,
                                                          ...
                                                          0.f,
                                                          invMassFn),
               "sleep-aware dispatch skips all-sleeping island");

    const IslandSleepAwareDispatchPreflight sleepDispatch =
        preflight_island_sleep_aware_dispatch(graph, freshBodies, dt);
    expectTrue(sleepDispatch.can_dispatch(), "sleep-aware dispatch preflight can dispatch mixed graph");
    expectTrue(!should_skip_island_sleep_aware_dispatch(graph, freshBodies, dt),
               "should_skip sleep-aware dispatch false for mixed graph");
    expectTrue(collect_sleep_aware_dispatchable_island_indices(graph, freshBodies).size() == 1u,
               "sleep-aware dispatchable indices skip all-sleeping island");

    RigidBodySoA mixedBodies;
    mixedBodies.addBody({0.f, 0.f, 0.f}, 1.f, 0);
    mixedBodies.addBody({2.1f, 0.f, 0.f}, 1.f, RB_SLEEPING);
    mixedBodies.addBody({20.f, 0.f, 0.f}, 1.f, RB_SLEEPING);
    mixedBodies.addBody({22.f, 0.f, 0.f}, 1.f, RB_SLEEPING);
    mixedBodies.predictedPositions = mixedBodies.positions;

    expectTrue(wake_and_solve_island_guarded(mixedBodies,
                                             graph,
                                             activeIsland,
                                             work,
                                             constraints,
                                             ...
                                             invMassFn),
               "wake-and-solve succeeds for mixed island");
    expectTrue((mixedBodies.flags[1] & RB_SLEEPING) == 0u, "wake-and-solve clears sleeper flag");

    RigidBodySoA batchBodies;
    batchBodies.addBody({0.f, 0.f, 0.f}, 1.f, 0);
    batchBodies.addBody({2.1f, 0.f, 0.f}, 1.f, RB_SLEEPING);
    batchBodies.addBody({20.f, 0.f, 0.f}, 1.f, RB_SLEEPING);
    batchBodies.addBody({22.f, 0.f, 0.f}, 1.f, RB_SLEEPING);
    batchBodies.predictedPositions = batchBodies.positions;

    const u32 constraintSolved =
        dispatch_all_islands_constraint_guarded(batchBodies,
                                                graph,
                                                work,
                                                constraints,
                                                dt,
                                                0.f,
                                                invMassFn);
    expectTrue(constraintSolved == 1u, "constraint-guarded batch solves one active island");

    RigidBodySoA wakeBatchBodies;
    wakeBatchBodies.addBody({0.f, 0.f, 0.f}, 1.f, 0);
    wakeBatchBodies.addBody({2.1f, 0.f, 0.f}, 1.f, RB_SLEEPING);
    wakeBatchBodies.addBody({20.f, 0.f, 0.f}, 1.f, RB_SLEEPING);
    wakeBatchBodies.addBody({22.f, 0.f, 0.f}, 1.f, RB_SLEEPING);
    wakeBatchBodies.predictedPositions = wakeBatchBodies.positions;

    const u32 wakeSolved = wake_and_dispatch_all_islands_guarded(wakeBatchBodies,
                                                                 graph,
                                                                 work,
                                                                 constraints,
                                                                 dt,
                                                                 0.f,
                                                                 invMassFn);
    expectTrue(wakeSolved == 1u, "wake-and-dispatch batch solves one mixed island");
}

void testBodyPairInRangeHelper() {
    expectTrue(body_pair_in_range(4, 0, 3), "in-range body pair accepted");
    expectTrue(!body_pair_in_range(4, 0, 4), "out-of-range bodyB rejected");
    expectTrue(!body_pair_in_range(4, 5, 1), "out-of-range bodyA rejected");
    expectTrue(!body_pair_in_range(0, 0, 0), "zero-body partition rejects all pairs");
}
...
void testBuildIslandGraphGuardedResult() {
    ...
    inRange.bodyA = 0;
    inRange.bodyB = 1;
    contacts.push_back(inRange);

    narrowphase::ContactManifold outOfRange{};
    outOfRange.valid = true;
    outOfRange.bodyA = 0;
    outOfRange.bodyB = 9;
    contacts.push_back(outOfRange);
    ...
    ContactIslandGraph graph;
    const IslandBuildResult unsafeResult = build_island_graph_guarded_result(graph, 4, contacts, constraints);
    expectTrue(unsafeResult.skipped, "guarded result skips unsafe build inputs");
    expectTrue(unsafeResult.unsafeRefs, "guarded result flags unsafe refs");
    expectTrue(!unsafeResult.built, "guarded result does not build unsafe graph");
    expectTrue(graph.islandCount() == 0u, "unsafe guarded result clears graph");

    const std::vector<DistanceConstraint> safeConstraints = {
        DistanceConstraint{.bodyA = 0, .bodyB = 1, .restLength = 2.f},
    };
    const IslandBuildResult safeResult = build_island_graph_guarded_result(graph, 4, {}, safeConstraints);
    expectTrue(safeResult.built, "guarded result builds safe inputs");
    expectTrue(!safeResult.skipped, "guarded result does not skip safe inputs");
    expectTrue(!safeResult.unsafeRefs, "guarded result has no unsafe refs");
    expectTrue(graph.constrainedIslandCount() == 1u, "guarded result forms one constrained island");
}
...
void testPreflightIslandConstraintSolveByIndex() {
    ...
    const IslandConstraintSolvePreflight sleepingPreflight = preflight_island_constraint_solve_by_index(
        graph, sleepingIsland, bodies, work.contactManifolds(), constraints);
    expectTrue(!sleepingPreflight.can_solve(), "all-sleeping island fails constraint solve preflight");
    expectTrue(should_skip_island_constraint_solve_index(
                   graph, sleepingIsland, bodies, work.contactManifolds(), constraints),
               "should_skip constraint solve index on all-sleeping island");

    const IslandSolveBodiesPreflight bodiesPreflight =
        preflight_island_solve_bodies_by_index(graph, mixedIsland, bodies);
...
void testPreflightIslandConstraintSolveDispatch() {
    ...
    const IslandConstraintSolveDispatchPreflight validPreflight =
        preflight_island_constraint_solve_dispatch(island, bodies, work.contactManifolds(), constraints, dt);
    ...
    const IslandConstraintSolveDispatchPreflight invalidDt =
        preflight_island_constraint_solve_dispatch(island, bodies, work.contactManifolds(), constraints, 0.f);
    expectTrue(invalidDt.invalidDt, "dispatch preflight rejects zero dt");
    expectTrue(!invalidDt.can_solve(), "dispatch preflight cannot solve with invalid dt");
    expectTrue(should_skip_island_constraint_solve_dispatch(
                   island, bodies, work.contactManifolds(), constraints, 0.f),
               "should_skip dispatch on invalid dt");

    const IslandConstraintSolveDispatchPreflight indexPreflight =
        preflight_island_constraint_solve_dispatch_by_index(graph, 0, bodies, work.contactManifolds(), constraints, dt);
    ...
    const IslandConstraintSolveDispatchPreflight outOfRange =
        preflight_island_constraint_solve_dispatch_by_index(
            graph, graph.islandCount() + 1u, bodies, work.contactManifolds(), constraints, dt);
    expectTrue(outOfRange.skipped, "index dispatch preflight skips out-of-range island");
}
...
void testSolveIslandJobGuarded() {
    ...
    work.init(4, 0, 2);
    const f32 dt = 1.f / 60.f;
    const auto invMassFn = [](const RigidBodySoA& soa, u32 index) { return soa.invMasses[index]; };

    const u32 activeIsland = graph.bodyIsland(0);
    const u32 sleepingIsland = graph.bodyIsland(2);

    const IslandConstraintSolveResult activeResult = solve_island_job_guarded_result(bodies,
                                                                                     graph.island(activeIsland),
                                                                                     work,
                                                                                     constraints,
                                                                                     dt,
                                                                                     0.f,
                                                                                     invMassFn);
    expectTrue(activeResult.solved, "guarded solve succeeds for active island");
    expectTrue(!activeResult.skipped, "guarded solve does not skip active island");

    const IslandConstraintSolveResult sleepingResult = solve_island_job_by_index_guarded_result(bodies,
                                                                                                graph,
                                                                                                sleepingIsland,
                                                                                                work,
                                                                                                constraints,
                                                                                                ...
                                                                                                0.f,
                                                                                                invMassFn);
    expectTrue(!sleepingResult.solved, "guarded solve skips all-sleeping island");
    expectTrue(sleepingResult.skipped, "guarded solve marks all-sleeping island skipped");
    expectTrue(sleepingResult.islandIndex == sleepingIsland, "guarded solve records island index");

    const IslandConstraintSolveResult invalidDt = solve_island_job_guarded_result(bodies,
                                                                                  graph.island(activeIsland),
                                                                                  work,
                                                                                  constraints,
                                                                                  0.f,
                                                                                  0.f,
                                                                                  invMassFn);
    expectTrue(!invalidDt.solved, "guarded solve skips invalid dt");
    expectTrue(invalidDt.skipped, "guarded solve marks invalid dt skipped");
}
...
void testPreflightIslandPreSolvePipeline() {
    ...
    const IslandPreSolvePreflight mixedPreflight =
        preflight_island_pre_solve(graph.island(mixedIsland), bodies, work.contactManifolds(), constraints);
    ...
    const IslandPreSolvePreflight sleepingPreflight =
        preflight_island_pre_solve_by_index(graph, sleepingIsland, bodies, work.contactManifolds(), constraints);
    ...
    const IslandPreSolveGraphPreflight graphPreflight =
        preflight_island_pre_solve_graph(graph, bodies, work.contactManifolds(), constraints);
    ...
    expectTrue(collect_pre_solveable_island_indices(graph, bodies, work.contactManifolds(), constraints).size() ==
                   1u,
               "collect pre-solveable returns mixed island");
    expectTrue(collect_solveable_island_indices(graph, bodies, work.contactManifolds(), constraints).size() == 1u,
               "collect solveable returns dispatchable pre-solveable island");

    const f32 dt = 1.f / 60.f;
    const auto invMassFn = [](const RigidBodySoA& soa, u32 index) { return soa.invMasses[index]; };
    const IslandConstraintSolveResult preSolveResult = pre_solve_island_guarded_result(bodies,
                                                                                       graph.island(mixedIsland),
                                                                                       work,
                                                                                       constraints,
                                                                                       dt,
                                                                                       0.f,
                                                                                       invMassFn);
    expectTrue(preSolveResult.solved, "pre-solve guarded pipeline solves mixed island");
    expectTrue((bodies.flags[1] & RB_SLEEPING) == 0u, "pre-solve pipeline wakes sleeping neighbor");
}
...
void testWakeIslandSleepersResult() {
    ...
    const IslandWakeResult wakeResult = wake_island_sleepers_result(bodies, graph.island(0));
    expectTrue(wakeResult.woke, "wake result activates mixed island sleepers");
    expectTrue(!wakeResult.skipped, "wake result does not skip wakeable island");
    expectTrue((bodies.flags[1] & RB_SLEEPING) == 0u, "wake result clears sleeping flag");

    const IslandWakeResult repeatWake = wake_island_sleepers_result(bodies, graph.island(0));
    expectTrue(!repeatWake.woke, "repeat wake result skips already-awake island");
    expectTrue(repeatWake.skipped, "repeat wake result is skipped");

    const IslandWakeResult outOfRange =
        wake_island_sleepers_by_index_result(bodies, graph, graph.islandCount() + 1u);
    expectTrue(outOfRange.skipped, "wake index result skips out-of-range island");

    bodies.flags[0] |= RB_SLEEPING;
    bodies.flags[1] |= RB_SLEEPING;
    const IslandBatchWakeResult batchResult = wake_all_island_sleepers_result(bodies, graph);
    expectTrue(batchResult.skipped, "batch wake result skips when no active dynamic body");
    expectTrue(!batchResult.any_woke(), "batch wake result does not wake all-sleeping graph");
}
...
int main() {
    ...
    testWakeIslandSleepersResult();
    testOutOfRangeBodyCountInPreflights();
    testContactIslandGraphBuildGuardsOorRefs();
    testPreflightIslandSolveDispatchGuards();
    testDispatchSolveIslandGuarded();
    testDispatchSolveableIslandsBatch();
    testContactIslandGraphRangeHelpers();
    testCollectSolveableIslandIndices();
    testPreflightIslandSleepWakeDispatch();
    testWakeIslandSleepersResultAndBatch();
    testDispatchSolveIslandWithSleepWakeGuards();
    testDispatchAllIslandsWithSleepWakeGuards();
    testContactIslandGraphBodiesInRange();
    testPreflightIslandSolvePassGuards();
    testGuardedSolveAndDispatchWithWake();
    testNonsleepingIslandDispatchBatch();
    testPreflightIslandDeepenFollowUpRejectGuards();
    testDispatchSolveIslandGuardedAndWake();
    testContactIslandGraphBuildSkipsOutOfRangeRefs();
    testContactIslandGraphBuildSkipsUnsafeRefs();
    testPreflightIslandBuildSelfContactGuard();
    testPreflightIslandConstraintRefsOutOfRangeCounts();
    testDispatchSolveIslandConstraintGuarded();
    testDispatchSolveIslandAfterWake();
    testDispatchAllSolveableIslandsBatch();
    testWakeAllIslandSleepersResult();
    testPreflightSolveIslandJobFullGuards();
    testWakeAndSolveIslandGuarded();
    testPreflightIslandSolveBodiesOutOfRangeCount();
    testIslandWakeResultAndBatch();
    testDispatchAfterWakeGuards();
    testBodyFlagHelpers();
    ...
    testContactIslandBuildSkipsOutOfRangeRefs();
    testPreflightIslandSolveDispatchGuards();
    testDispatchSolveIslandGuardedWakesAndSolves();
    testDispatchAllIslandsGuardedBatch();
    testPreflightIslandBuildSelfReferentialGuards();
    testDispatchAllIslandsWithWakeResultBatch();
    testContactIslandGraphBuildSkipsOutOfRangeRefs();
    testDispatchSolveIslandWakeGuarded();
    testWakeAllIslandSleepersResult();
    testDispatchRejectsNonFiniteDt();
    testBuildIslandGraphInRangeGuarded();
    testSolveIslandJobGuardedAndSleepAwareDispatch();
    testBodyPairInRangeHelper();
    testBuildIslandGraphGuardedResult();
