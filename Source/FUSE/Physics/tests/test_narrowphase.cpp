...
void testContactPairDeepenPassRejectGuardsV2() {
    fuse::physics::RigidBodySoA bodies;
    fuse::physics::CollisionShapeSoA shapes;
    const fuse::u32 dynamicA = bodies.addBody({0.f, 0.f, 0.f}, 1.f);
    const fuse::u32 dynamicB = bodies.addBody({1.5f, 0.f, 0.f}, 1.f);
    ...
    expectTrue(
        fuse::physics::narrowphase::is_resting_contact_pair({sleepingA, sleepingB}, bodies),
        "resting guard flags both-sleeping pair");
    expectTrue(
        !fuse::physics::narrowphase::is_resting_contact_pair({dynamicA, dynamicB}, bodies),
        "resting guard allows dynamic pair");
    expectTrue(
        fuse::physics::narrowphase::contact_pair_deepen_reject_reason({planeA, planeB}, bodies, shapes) ==
            fuse::physics::narrowphase::ContactPairRejectReason::PlanePlane,
        "deepen reject reason flags plane-plane pair");
    expectTrue(
        ...
        "base reject reason unchanged for plane-plane pair");
    expectTrue(
        fuse::physics::narrowphase::contact_pair_deepen_rejects_for_reason_v2(
            {planeA, planeB},
            bodies,
            shapes,
            fuse::physics::narrowphase::ContactPairRejectReason::PlanePlane),
        "deepen rejects_for_reason_v2 matches plane-plane pair");
    ...
    expectTrue(filtered.size() == 1u, "filter_dispatchable keeps one dynamic pair");
    expectTrue(filtered[0].bodyA == dynamicA && filtered[0].bodyB == dynamicB,
        "filter_dispatchable preserves dynamic pair order");

    const auto summary =
        fuse::physics::narrowphase::summarize_contact_pair_batch_rejects(mixedPairs, bodies, shapes);
    expectTrue(summary.pairCount == 3u, "batch summary reports pair count");
    expectTrue(summary.dispatchableCount == 1u, "batch summary counts dispatchable pairs");
    expectTrue(summary.planePlaneCount == 1u, "batch summary counts plane-plane rejects");
    expectTrue(summary.restingCount == 1u, "batch summary counts resting rejects");
    expectTrue(summary.all_rejected() == false, "batch summary not all rejected with one valid pair");
...
void testManifoldNormalizeAndFinalizeIfNeededGuards() {
    fuse::physics::narrowphase::ContactManifold unnormalized{};
    unnormalized.contactNormal = {0.f, 2.f, 0.f};
    unnormalized.addPoint({0.f, 0.f, 0.f}, 0.25f);
    expectTrue(
        !fuse::physics::narrowphase::can_skip_normalize_contact_manifold_normal(unnormalized),
        "can_skip_normalize false for scaled normal");
    expectTrue(
        fuse::physics::narrowphase::normalize_contact_manifold_normal_if_needed(unnormalized),
        "normalize_if_needed succeeds for scaled normal");
    expectNear(unnormalized.contactNormal.length(), 1.f, 1e-4f, "normalize_if_needed produces unit normal");
    expectTrue(
        fuse::physics::narrowphase::can_skip_normalize_contact_manifold_normal(unnormalized),
        "can_skip_normalize true after normalization");

    fuse::physics::narrowphase::ContactManifold noNormal{};
    noNormal.addPoint({0.f, 0.f, 0.f}, 0.2f);
    expectTrue(
        !fuse::physics::narrowphase::normalize_contact_manifold_normal(noNormal),
        "normalize returns false for invalid normal");
    expectTrue(
        fuse::physics::narrowphase::manifold_prune_rejects_for_reason(
            noNormal, fuse::physics::narrowphase::ManifoldPruneRejectReason::InvalidNormal),
        "prune rejects_for_reason flags invalid normal");
    expectTrue(
        std::strcmp(
            fuse::physics::narrowphase::manifold_prune_reject_reason_name(
                fuse::physics::narrowphase::ManifoldPruneRejectReason::InvalidNormal),
            "InvalidNormal") == 0,
        "prune reject reason name resolves InvalidNormal");

    fuse::physics::narrowphase::ContactManifold manifold =
        fuse::physics::narrowphase::collideSphereSphere({0.f, 0.f, 0.f}, 1.f, {1.5f, 0.f, 0.f}, 1.f, 0u, 1u);
    expectTrue(
        fuse::physics::narrowphase::finalize_contact_manifold_with_preflight(manifold),
        "finalize_with_preflight succeeds on detected manifold");
    expectTrue(
        fuse::physics::narrowphase::is_finalized_contact_manifold(manifold),
        "is_finalized true after finalize_with_preflight");
    expectTrue(
        fuse::physics::narrowphase::can_skip_finalize_contact_manifold_if_finalized(manifold),
        "can_skip_finalize true for finalized manifold");

    const auto cachedTangent1 = manifold.frictionBasis.tangent1;
    expectTrue(
        fuse::physics::narrowphase::finalize_contact_manifold_if_needed(manifold),
        "finalize_if_needed no-ops on finalized manifold");
    expectNear(
        manifold.frictionBasis.tangent1.x,
        cachedTangent1.x,
        1e-4f,
        "finalize_if_needed preserves cached tangent1");
}

void testFrictionBasisNormalizeRebuildGuards() {
    fuse::physics::narrowphase::ContactManifold unnormalized{};
    unnormalized.contactNormal = {0.f, 2.f, 0.f};
    unnormalized.addPoint({0.f, 0.f, 0.f}, 0.2f);
    expectTrue(
        !fuse::physics::narrowphase::can_skip_normalize_contact_normal_for_friction(unnormalized),
        "can_skip_normalize_for_friction false for scaled normal");
    expectTrue(
        fuse::physics::narrowphase::normalize_contact_normal_for_friction(unnormalized),
        "normalize_for_friction succeeds for scaled normal");
    expectNear(unnormalized.contactNormal.length(), 1.f, 1e-4f, "normalize_for_friction produces unit normal");

    expectTrue(
        fuse::physics::narrowphase::rebuild_friction_basis_with_normalize_if_needed(unnormalized),
        "rebuild_with_normalize builds missing basis");
    expectTrue(unnormalized.hasFrictionBasis(), "rebuild_with_normalize stores orthonormal basis");

    const auto cachedTangent1 = unnormalized.frictionBasis.tangent1;
    expectTrue(
        fuse::physics::narrowphase::can_skip_friction_basis_rebuild_with_normalize(unnormalized),
        "can_skip_rebuild_with_normalize true for valid basis");
    expectTrue(
        fuse::physics::narrowphase::rebuild_friction_basis_with_normalize_if_needed(unnormalized),
        "rebuild_with_normalize reuses valid basis");
    expectNear(
        unnormalized.frictionBasis.tangent1.x,
        cachedTangent1.x,
        1e-4f,
        "rebuild_with_normalize preserves cached tangent1");

    unnormalized.contactNormal = {1.f, 0.f, 0.f};
    expectTrue(
        fuse::physics::narrowphase::friction_basis_is_stale(unnormalized),
        "stale guard flags basis built for prior normal");
    expectTrue(
        !fuse::physics::narrowphase::can_skip_friction_basis_rebuild_with_normalize(unnormalized),
        "can_skip_rebuild_with_normalize false for stale basis");
    expectTrue(
        fuse::physics::narrowphase::rebuild_friction_basis_with_normalize_if_needed(unnormalized),
        "rebuild_with_normalize refreshes stale basis");
    expectTrue(
        fuse::physics::narrowphase::friction_basis_matches_normal(unnormalized),
        "rebuild_with_normalize matches current normal");
...
void testContactBufferFrictionRebuildIfNeededGuards() {
    fuse::physics::narrowphase::ContactBufferSoA buffer;
    buffer.preparePairSlots(2u);
    ...
    expectTrue(buffer.compact() == 2u, "buffer compacts two manifolds for friction rebuild test");
    const auto preflightFresh =
        fuse::physics::narrowphase::preflight_contact_buffer_friction_rebuild(buffer);
    expectTrue(preflightFresh.can_skip_rebuild(), "buffer preflight can skip fresh orthonormal slots");
    expectTrue(
        fuse::physics::narrowphase::isOrthonormalTangentBasis(
            {1.f, 0.f, 0.f}, buffer.tangentBasisAt(1u)),
        "writeSlot stores orthonormal basis for second slot");

    buffer.contactNormals[0] = {1.f, 0.f, 0.f};
    const auto preflightStale =
        fuse::physics::narrowphase::preflight_contact_buffer_friction_rebuild(buffer);
    expectTrue(preflightStale.rebuildCount == 1u, "buffer preflight flags stale slot after normal change");
    expectTrue(!preflightStale.can_skip_rebuild(), "buffer preflight cannot skip stale slot");
    expectTrue(buffer.slotFrictionBasisIsStale(0u), "slot stale guard flags mismatched normal");

    buffer.buildFrictionTangentBasesIfNeeded();
    const auto preflightAfter =
        fuse::physics::narrowphase::preflight_contact_buffer_friction_rebuild(buffer);
    expectTrue(preflightAfter.can_skip_rebuild(), "buffer preflight can skip after rebuild_if_needed");
    expectTrue(
        !buffer.slotFrictionBasisIsStale(0u),
        "rebuild_if_needed clears stale flag");
    buffer.rebuildFrictionTangentBasisAt(0u);
    expectTrue(
        !buffer.slotFrictionBasisIsStale(0u),
        "slot rebuild clears stale flag");
    expectTrue(
        fuse::physics::narrowphase::isOrthonormalTangentBasis(
            {1.f, 0.f, 0.f}, buffer.tangentBasisAt(0u)),
        "slot rebuild produces orthonormal basis for updated normal");
}

void testRunNarrowphaseFilteredIntoBufferGuards() {
    fuse::physics::RigidBodySoA bodies;
    fuse::physics::CollisionShapeSoA shapes;
    const fuse::u32 dynamicA = bodies.addBody({0.f, 0.f, 0.f}, 1.f);
    const fuse::u32 dynamicB = bodies.addBody({1.5f, 0.f, 0.f}, 1.f);
    ...
    fuse::physics::narrowphase::ContactBufferSoA filteredBuffer;
    fuse::physics::narrowphase::runNarrowphaseFilteredIntoBuffer(pairs, bodies, shapes, filteredBuffer);
    expectTrue(filteredBuffer.activeCount == 1u, "filtered narrowphase produces one contact");
    const auto restored = filteredBuffer.manifoldAt(0u);
    expectTrue(restored.valid, "filtered narrowphase contact is valid");
    expectTrue(restored.bodyA == dynamicA && restored.bodyB == dynamicB,
        "filtered narrowphase keeps dynamic pair");

    fuse::physics::narrowphase::ContactBufferSoA unfilteredBuffer;
    fuse::physics::narrowphase::runNarrowphaseIntoBuffer(pairs, bodies, shapes, unfilteredBuffer);
    expectTrue(unfilteredBuffer.activeCount == 2u, "unfiltered narrowphase still dispatches sleeping pair");
}

void testContactPairB46DeepenGuards() {
    fuse::physics::RigidBodySoA bodies;
    fuse::physics::CollisionShapeSoA shapes;
    const fuse::u32 dynamicA = bodies.addBody({0.f, 0.f, 0.f}, 1.f);
    const fuse::u32 dynamicB = bodies.addBody({1.5f, 0.f, 0.f}, 1.f);
    ...
    expectTrue(
        fuse::physics::narrowphase::contact_pair_deepen_reject_reason({planeA, planeB}, bodies, shapes) ==
            fuse::physics::narrowphase::ContactPairRejectReason::PlanePlane,
        "deepen reject reason refines plane-plane to PlanePlane");
    expectTrue(
        fuse::physics::narrowphase::contact_pair_reject_reason({planeA, planeB}, bodies, shapes) ==
            fuse::physics::narrowphase::ContactPairRejectReason::UnsupportedShapePair,
        "base reject reason unchanged for plane-plane pair");
    expectTrue(
        fuse::physics::narrowphase::preflight_contact_pair_deepen_rejects_for_reason(
            {planeA, planeB},
            bodies,
            shapes,
            fuse::physics::narrowphase::ContactPairRejectReason::PlanePlane),
        "deepen preflight rejects_for_reason matches plane-plane pair");

    const auto detectPreflight =
        fuse::physics::narrowphase::preflight_detect_contacts_pair({dynamicA, dynamicB}, bodies, shapes);
    expectTrue(detectPreflight.can_detect, "detect preflight allows valid pair");
    const auto detected =
        fuse::physics::narrowphase::detect_contacts_pair_with_preflight({dynamicA, dynamicB}, bodies, shapes);
    expectTrue(detected.valid, "detect_with_preflight produces valid manifold");

    const auto rejected =
        fuse::physics::narrowphase::detect_contacts_pair_with_preflight({planeA, planeB}, bodies, shapes);
    expectTrue(!rejected.valid, "detect_with_preflight rejects plane-plane pair");

    const std::vector<fuse::physics::broadphase::CandidatePair> pairs = {{planeA, planeB}};
    const auto dispatchPreflight =
        fuse::physics::narrowphase::preflight_narrowphase_dispatch(pairs, bodies, shapes);
    expectTrue(dispatchPreflight.can_skip_dispatch, "dispatch preflight skips all-rejected batch");
    expectTrue(
        fuse::physics::narrowphase::should_skip_narrowphase_dispatch(pairs, bodies, shapes),
        "should_skip_narrowphase_dispatch on all-rejected batch");
...
void testManifoldPruneFinalizeB46Guards() {
    ...
    noNormal.addPoint({0.f, 0.f, 0.f}, 0.2f);
    expectTrue(
        fuse::physics::narrowphase::manifold_prune_preflight_rejects_for_reason(
            noNormal, fuse::physics::narrowphase::ManifoldPruneRejectReason::NonUnitNormal),
        "prune preflight rejects_for_reason flags zero-length normal");
    expectTrue(
        std::strcmp(
            fuse::physics::narrowphase::manifold_prune_reject_reason_name(
                fuse::physics::narrowphase::ManifoldPruneRejectReason::NonUnitNormal),
            "NonUnitNormal") == 0,
        "prune reject reason name resolves NonUnitNormal");

    fuse::physics::narrowphase::ContactManifold separated{};
    separated.contactNormal = {0.f, 1.f, 0.f};
    separated.addPoint({0.f, 0.f, 0.f}, -0.1f);
    expectTrue(
        fuse::physics::narrowphase::manifold_finalize_preflight_rejects_for_reason(
            separated,
            fuse::physics::narrowphase::ManifoldFinalizeRejectReason::AllSeparatedAfterPrune),
        "finalize preflight rejects_for_reason flags separated manifold");
    ...
    fuse::physics::narrowphase::ContactManifold dirty =
        fuse::physics::narrowphase::collideSphereSphere({0.f, 0.f, 0.f}, 1.f, {1.5f, 0.f, 0.f}, 1.f, 0u, 1u);
    dirty.addPoint({0.f, 0.f, 0.f}, -0.05f);
    expectTrue(
        fuse::physics::narrowphase::prune_and_finalize_contact_manifold_with_preflight(dirty),
...
void testFrictionBasisB46Guards() {
    ...
    expectTrue(
        fuse::physics::narrowphase::friction_basis_preflight_rejects_for_reason(
            stale, fuse::physics::narrowphase::FrictionBasisRejectReason::EmptyManifold) == false,
        "stale basis does not hard-reject preflight");
    expectTrue(
        fuse::physics::narrowphase::compute_friction_tangents_with_preflight(stale),
        "compute_tangents_with_preflight rebuilds stale basis");
    expectTrue(
        fuse::physics::narrowphase::friction_basis_matches_normal(stale),
        "compute_tangents_with_preflight matches current normal");

    fuse::physics::narrowphase::ContactManifold empty{};
    expectTrue(
        fuse::physics::narrowphase::friction_basis_preflight_rejects_for_reason(
            empty, fuse::physics::narrowphase::FrictionBasisRejectReason::EmptyManifold),
        "friction preflight rejects_for_reason flags empty manifold");
    expectTrue(
        std::strcmp(
            fuse::physics::narrowphase::friction_basis_reject_reason_name(
                fuse::physics::narrowphase::FrictionBasisRejectReason::StaleBasis),
...
void testContactBufferB46Guards() {
    ...
    buffer.writeSlot(0u, shallow);
    buffer.writeSlot(2u, deep);
    expectTrue(!buffer.canSkipCompaction(), "compaction needed with gap in valid slots");

    const auto compactionPreflight = fuse::physics::narrowphase::preflight_contact_buffer_compaction(buffer);
    expectTrue(compactionPreflight.needsCompaction, "compaction preflight flags gap");
    expectTrue(buffer.compactIfNeeded() == 2u, "compactIfNeeded packs valid slots");
    expectTrue(buffer.canSkipCompaction(), "compaction skip true after pack");

    buffer.buildFrictionTangentBases();
    expectTrue(buffer.canSkipFrictionTangentRebuild(), "friction rebuild skip after fresh build");
    buffer.contactNormals[0] = {1.f, 0.f, 0.f};
    expectTrue(!buffer.canSkipFrictionTangentRebuild(), "friction rebuild needed after normal change");

    const auto frictionPreflight =
        fuse::physics::narrowphase::preflight_contact_buffer_friction_rebuild(buffer);
    expectTrue(frictionPreflight.slotsWithStaleBasis > 0u, "buffer friction preflight flags stale slot");
    expectTrue(buffer.rebuildFrictionTangentBasesWithPreflight(), "rebuild_with_preflight refreshes slots");
    expectTrue(buffer.canSkipFrictionTangentRebuild(), "friction rebuild skip after refresh");

    expectTrue(buffer.compactAndClampIfNeeded() == 2u, "compactAndClampIfNeeded no-ops when packed");
}

void testNarrowphaseDispatchB46Guards() {
    fuse::physics::RigidBodySoA bodies;
    fuse::physics::CollisionShapeSoA shapes;
    const fuse::u32 sleepingA = bodies.addBody({0.f, 2.f, 0.f}, 1.f, fuse::physics::RB_SLEEPING);
    const fuse::u32 sleepingB = bodies.addBody({0.f, 3.f, 0.f}, 1.f, fuse::physics::RB_SLEEPING);
    ...
    expectTrue(buffer.isEmpty(), "dispatch_with_preflight early-outs on all-rejected batch");
    ...
    expectTrue(buffer.activeCount == 1u, "dispatch_with_preflight fills valid pair");
    expectTrue(buffer.manifoldAt(0u).hasFrictionBasis(), "dispatch_with_preflight finalizes friction basis");
}
...
void testContactPairBeyondDeepenRejectGuards() {
    ...
    expectTrue(
        ...
        "capsule-capsule guard flags both-capsule pair");
    expectTrue(
        fuse::physics::narrowphase::is_undispatchable_shape_pair({capsuleA, capsuleB}, shapes),
        "undispatchable guard flags capsule-capsule pair");
    expectTrue(
        fuse::physics::narrowphase::contact_pair_beyond_deepen_reject_reason(
            {capsuleA, capsuleB}, bodies, shapes) ==
            fuse::physics::narrowphase::ContactPairRejectReason::UndispatchableShapePair,
        "beyond reject reason flags undispatchable capsule-capsule pair");
    expectTrue(
        fuse::physics::narrowphase::contact_pair_deepen_reject_reason(
            {capsuleA, capsuleB}, bodies, shapes) ==
            fuse::physics::narrowphase::ContactPairRejectReason::None,
        "deepen reject reason unchanged for undispatchable capsule-capsule pair");

    const auto beyondPreflight =
        fuse::physics::narrowphase::preflight_contact_pair_beyond({capsuleA, capsuleB}, bodies, shapes);
    expectTrue(!beyondPreflight.can_dispatch(), "beyond preflight rejects capsule-capsule pair");
    expectTrue(
        beyondPreflight.reason ==
            fuse::physics::narrowphase::ContactPairRejectReason::UndispatchableShapePair,
        "beyond preflight reports UndispatchableShapePair");
    expectTrue(
        fuse::physics::narrowphase::should_skip_contact_pair_beyond_dispatch(
            {capsuleA, capsuleB}, bodies, shapes),
        "beyond skip guard rejects capsule-capsule pair");
    expectTrue(
        fuse::physics::narrowphase::contact_pair_beyond_rejects_for_reason(
            {capsuleA, capsuleB},
            bodies,
            shapes,
            fuse::physics::narrowphase::ContactPairRejectReason::UndispatchableShapePair),
        "beyond rejects_for_reason matches capsule-capsule pair");
    ...
    expectTrue(
        fuse::physics::narrowphase::count_beyond_dispatchable_contact_pairs(mixedPairs, bodies, shapes) == 1u,
        "count_beyond_dispatchable excludes undispatchable pairs");
    expectTrue(
        fuse::physics::narrowphase::has_beyond_dispatchable_contact_pair(mixedPairs, bodies, shapes),
        "has_beyond_dispatchable true when one pair passes beyond preflight");

    const auto beyondBatch =
        fuse::physics::narrowphase::preflight_narrowphase_beyond_batch(mixedPairs, bodies, shapes);
    expectTrue(beyondBatch.dispatchableCount == 1u, "beyond batch preflight counts dispatchable pairs");
    expectTrue(beyondBatch.rejectedCount == 1u, "beyond batch preflight counts rejected pairs");
    expectTrue(
        !fuse::physics::narrowphase::narrowphase_beyond_batch_rejects_all(mixedPairs, bodies, shapes),
        "beyond batch rejects_all false when one pair dispatchable");

    const auto undispatchable =
        fuse::physics::narrowphase::detect_contacts_pair_beyond({capsuleA, capsuleB}, bodies, shapes);
    expectTrue(!undispatchable.valid, "detect_contacts_pair_beyond returns invalid for undispatchable pair");

    expectTrue(
        std::strcmp(
            fuse::physics::narrowphase::contact_pair_reject_reason_name(
                fuse::physics::narrowphase::ContactPairRejectReason::UndispatchableShapePair),
            "UndispatchableShapePair") == 0,
        "reject reason name resolves UndispatchableShapePair");
}
...
void testManifoldBeyondPruneFinalizeGuards() {
    ...
    expectNear(unnormalized.contactNormal.y, 1.f, 1e-4f, "normalized normal is unit length");
    expectTrue(
        !unnormalized.needsNormalNormalization(),
        "normalized manifold no longer needs normalization");
    ...
    expectTrue(
        fuse::physics::narrowphase::prune_contact_manifold_beyond_preflight(dirty),
        "beyond prune keeps penetrating slots");
    expectTrue(dirty.pointCount == 1u, "beyond prune removes separated slot");
    expectNear(dirty.contactNormal.y, 1.f, 1e-4f, "beyond prune normalizes contact normal");
    ...
    fuse::physics::narrowphase::ContactManifold ready =
        fuse::physics::narrowphase::collideSphereSphere({0.f, 0.f, 0.f}, 1.f, {1.5f, 0.f, 0.f}, 1.f, 0u, 1u);
    expectTrue(
        fuse::physics::narrowphase::finalize_contact_manifold_beyond_preflight(ready),
        "beyond finalize finalizes valid manifold");
    expectTrue(ready.valid, "beyond finalize sets validity");
    expectTrue(ready.hasFrictionBasis(), "beyond finalize builds friction basis");
    expectNear(ready.contactNormal.length(), 1.f, 1e-4f, "beyond finalize stores unit normal");

    fuse::physics::narrowphase::ContactManifold alreadyFinalized = ready;
    expectTrue(
        fuse::physics::narrowphase::finalize_contact_manifold_if_needed(alreadyFinalized),
        "finalize_if_needed succeeds on finalizable manifold");
    expectTrue(alreadyFinalized.valid, "finalize_if_needed preserves validity");
    ...
    expectTrue(
        std::strcmp(
            fuse::physics::narrowphase::manifold_prune_reject_reason_name(
                fuse::physics::narrowphase::ManifoldPruneRejectReason::ExceedsMaxPoints),
            "ExceedsMaxPoints") == 0,
        "prune reject reason name resolves ExceedsMaxPoints");
}
...
void testFrictionBasisBeyondRebuildGuards() {
    ...
    unnormalized.addPoint({0.f, 0.f, 0.f}, 0.2f);
    expectTrue(
        fuse::physics::narrowphase::normalize_contact_normal_before_friction_if_needed(unnormalized),
        "normalize before friction normalizes scaled normal");
    expectNear(unnormalized.contactNormal.y, 1.f, 1e-4f, "friction normalize stores unit normal");

    fuse::physics::narrowphase::ContactManifold needsBuild{};
    needsBuild.contactNormal = {0.f, 1.f, 0.f};
    needsBuild.addPoint({0.f, 0.f, 0.f}, 0.2f);
    const auto needsPreflight =
        fuse::physics::narrowphase::preflight_friction_basis_beyond_rebuild(needsBuild);
    ...
    expectTrue(
        fuse::physics::narrowphase::rebuild_friction_basis_beyond_preflight(needsBuild),
        "beyond rebuild builds missing basis");
    expectTrue(needsBuild.hasFrictionBasis(), "beyond rebuild stores orthonormal basis");
    ...
    stale.contactNormal = {1.f, 0.f, 0.f};
    const auto stalePreflight =
        fuse::physics::narrowphase::preflight_friction_basis_beyond_rebuild(stale);
    expectTrue(stalePreflight.stale, "beyond friction preflight flags stale basis");
    expectTrue(
        fuse::physics::narrowphase::friction_basis_beyond_reject_reason(stale) ==
            fuse::physics::narrowphase::FrictionBasisRejectReason::StaleBasis,
        "beyond reject reason reports StaleBasis");
    expectTrue(
        !fuse::physics::narrowphase::should_skip_friction_basis_beyond_rebuild(stale),
        "beyond skip rebuild false for stale basis");
    expectTrue(
        fuse::physics::narrowphase::rebuild_friction_basis_beyond_preflight(stale),
        "beyond rebuild refreshes stale basis");
    expectTrue(
        fuse::physics::narrowphase::friction_basis_matches_normal(stale),
        "beyond rebuild produces basis matching current normal");

    fuse::physics::narrowphase::compute_friction_tangents_beyond_preflight(stale);
    const auto cachedTangent1 = stale.frictionBasis.tangent1;
    fuse::physics::narrowphase::compute_friction_tangents_beyond_preflight(stale);
    expectNear(
        stale.frictionBasis.tangent1.x,
        cachedTangent1.x,
        1e-4f,
        "beyond compute_if_needed preserves valid cached basis");
...
void testRunNarrowphaseBeyondAndBufferGuards() {
    ...
    expectTrue(
        !fuse::physics::narrowphase::can_skip_narrowphase_beyond(mixedPairs, bodies, shapes),
        "can_skip_narrowphase_beyond false when one pair dispatchable");

    fuse::physics::narrowphase::ContactBufferSoA buffer;
    fuse::physics::narrowphase::runNarrowphaseIntoBufferBeyond(mixedPairs, bodies, shapes, buffer);
    expectTrue(buffer.activeCount == 1u, "beyond narrowphase produces one finalized contact");
    const auto restored = buffer.manifoldAt(0u);
    expectTrue(restored.valid, "beyond narrowphase stores valid manifold");
    expectTrue(restored.hasFrictionBasis(), "beyond narrowphase stores friction basis");

    fuse::physics::narrowphase::ContactBufferSoA writeBuffer;
    writeBuffer.preparePairSlots(1u);
    fuse::physics::narrowphase::ContactManifold manifold =
        fuse::physics::narrowphase::collideSphereSphere({0.f, 0.f, 0.f}, 1.f, {1.5f, 0.f, 0.f}, 1.f, 0u, 1u);
    expectTrue(
        writeBuffer.writeSlotWithFinalize(0u, manifold),
        "writeSlotWithFinalize finalizes and writes valid manifold");
    expectTrue(writeBuffer.compact() == 1u, "writeSlotWithFinalize compacts one contact");

    writeBuffer.rebuildFrictionTangentBasesIfNeeded();
    const auto basis = writeBuffer.tangentBasisAt(0u);
    expectTrue(
        fuse::physics::narrowphase::isOrthonormalTangentBasis(
            writeBuffer.manifoldAt(0u).contactNormal, basis),
        "rebuildFrictionTangentBasesIfNeeded preserves orthonormal basis");
}
...
void testShouldRunContactPairDispatchGuards() {
    ...
    expectTrue(
        ...
        "should_run_contact_pair_deepen_dispatch false for both-sleeping pair");

    expectTrue(
        fuse::physics::narrowphase::can_skip_narrowphase_batch({{sleepingA, sleepingB}}, bodies, shapes),
        "can_skip_narrowphase_batch when all pairs deepen-rejected");
    expectTrue(
    ...
    expectTrue(
        fuse::physics::narrowphase::should_run_narrowphase_batch({{bodyA, bodyB}}, bodies, shapes),
        "should_run_narrowphase_batch true when dispatchable pair exists");
}

void testNarrowphaseDispatchRejectReasonGuards() {
    fuse::physics::RigidBodySoA bodies;
    fuse::physics::CollisionShapeSoA shapes;
    const fuse::u32 sleepingA = bodies.addBody({0.f, 0.f, 0.f}, 1.f, fuse::physics::RB_SLEEPING);
    const fuse::u32 sleepingB = bodies.addBody({0.f, 1.f, 0.f}, 1.f, fuse::physics::RB_SLEEPING);
    shapes.addShape(fuse::physics::CollisionShapeType::Sphere, sleepingA, {1.f, 0.f, 0.f});
    shapes.addShape(fuse::physics::CollisionShapeType::Sphere, sleepingB, {1.f, 0.f, 0.f});

    expectTrue(
        fuse::physics::narrowphase::narrowphase_dispatch_rejects_for_reason(
            {},
            bodies,
            shapes,
            fuse::physics::narrowphase::NarrowphaseDispatchRejectReason::EmptyPairList),
        "dispatch reject reason flags empty pair list");
    expectTrue(
        std::strcmp(
            fuse::physics::narrowphase::narrowphase_dispatch_reject_reason_name(
                fuse::physics::narrowphase::NarrowphaseDispatchRejectReason::EmptyPairList),
            "EmptyPairList") == 0,
        "dispatch reject reason name resolves EmptyPairList");
    ...
    expectTrue(
        ...
        "dispatch reject reason flags all deepen-rejected pairs");

    const auto batchPreflight =
        fuse::physics::narrowphase::preflight_narrowphase_batch(sleepingPairs, bodies, shapes);
    expectTrue(
        batchPreflight.reason ==
            fuse::physics::narrowphase::NarrowphaseDispatchRejectReason::AllPairsRejected,
        "batch preflight carries AllPairsRejected reason");
...
void testContactBufferCompactAndClampPreflightGuards() {
    ...
    expectTrue(
        ...
        "compact reject reason flags empty buffer");
    expectTrue(
    ...
    expectTrue(
        ...
        "should_run_contact_buffer_compact false on empty buffer");
    ...
    first.bodyB = 1u;
    first.contactNormal = {0.f, 1.f, 0.f};
    first.addPoint({0.f, 0.f, 0.f}, 0.2f);
    buffer.writeSlot(0u, first);
    ...
    expectTrue(
        ...
        "compact reject reason flags all-valid slots");
    expectTrue(buffer.compact() == 2u, "compact early-out preserves valid count");

    fuse::physics::narrowphase::ContactBufferSoA sparse;
    sparse.preparePairSlots(3u);
    sparse.writeSlot(0u, first);
    sparse.writeSlot(2u, first);
    expectTrue(
        fuse::physics::narrowphase::should_run_contact_buffer_compact(sparse),
        "should_run_contact_buffer_compact true with sparse valid slots");
    expectTrue(sparse.compact() == 2u, "compact gathers sparse valid slots");

    buffer.setMaxCapacity(1u);
    expectTrue(
        fuse::physics::narrowphase::should_run_contact_buffer_clamp(buffer),
        "should_run_contact_buffer_clamp true when valid count exceeds max capacity");
    expectTrue(
        std::strcmp(
            fuse::physics::narrowphase::contact_buffer_compact_and_clamp_reject_reason_name(
                fuse::physics::narrowphase::ContactBufferCompactAndClampRejectReason::NoWork),
    ...
    fuse::physics::narrowphase::ContactBufferSoA clean;
    clean.preparePairSlots(1u);
    clean.writeSlot(0u, first);
    expectTrue(clean.compact() == 1u, "initial compact syncs active count");
    expectTrue(
        fuse::physics::narrowphase::contact_buffer_compact_and_clamp_rejects_for_reason(
            clean, fuse::physics::narrowphase::ContactBufferCompactAndClampRejectReason::NoWork),
        "compact-and-clamp reject reason flags no-op buffer");
    expectTrue(clean.compactAndClamp() == 1u, "compactAndClamp no-op preserves active count");
}
...
void testShouldRunManifoldPruneGuards() {
    ...
    expectTrue(
        fuse::physics::narrowphase::should_run_manifold_finalize(ready),
        "should_run_manifold_finalize true for finalizable manifold");
    expectTrue(
        !fuse::physics::narrowphase::can_skip_manifold_finalize(ready),
        "can_skip_manifold_finalize false when finalize should run");
...
void testContactPairBothPlaneDeepenGuards() {
    ...
    expectTrue(
        ...
        "base reject reason unchanged for both-plane pair");
    expectTrue(
        !fuse::physics::narrowphase::should_run_contact_pair_deepen_dispatch({planeA, planeB}, bodies, shapes),
        "should_run deepen dispatch false for both-plane pair");
    expectTrue(
        std::strcmp(
            fuse::physics::narrowphase::contact_pair_reject_reason_name(
                fuse::physics::narrowphase::ContactPairRejectReason::BothPlane),
...
void testShouldRunNarrowphaseDispatchGuards() {
    ...
    expectTrue(
        fuse::physics::narrowphase::should_run_narrowphase_batch({{bodyA, bodyB}}, bodies, shapes),
        "should_run narrowphase batch true when dispatchable pair exists");
    expectTrue(
        !fuse::physics::narrowphase::should_run_narrowphase_batch({{sleepingA, sleepingB}}, bodies, shapes),
        "should_run narrowphase batch false when all pairs deepen-rejected");
}

void testRunNarrowphaseDeepenDispatchGates() {
    fuse::physics::RigidBodySoA bodies;
    fuse::physics::CollisionShapeSoA shapes;
    const fuse::u32 dynamicA = bodies.addBody({0.f, 0.f, 0.f}, 1.f);
    const fuse::u32 dynamicB = bodies.addBody({1.5f, 0.f, 0.f}, 1.f);
    ...
    fuse::physics::narrowphase::ContactBufferSoA buffer;
    fuse::physics::narrowphase::runNarrowphaseIntoBuffer(mixedPairs, bodies, shapes, buffer);
    expectTrue(buffer.activeCount == 1u, "deepen dispatch gate skips rejected pair in buffer path");
    expectTrue(buffer.manifoldAt(0u).valid, "dispatchable pair still produces finalized contact");

    fuse::physics::narrowphase::ContactBufferSoA sleepingBuffer;
    fuse::physics::narrowphase::runNarrowphaseIntoBuffer({{sleepingA, sleepingB}}, bodies, shapes, sleepingBuffer);
    expectTrue(sleepingBuffer.isEmpty(), "all deepen-rejected batch leaves buffer empty");
}
...
void testManifoldPruneInvalidNormalGuards() {
    ...
    noNormal.addPoint({0.f, 0.f, 0.f}, 0.2f);
    expectTrue(
        fuse::physics::narrowphase::manifold_prune_reject_reason(noNormal) ==
            fuse::physics::narrowphase::ManifoldPruneRejectReason::InvalidNormal,
        "prune reject reason flags invalid normal");
    expectTrue(
        std::strcmp(
            fuse::physics::narrowphase::manifold_prune_reject_reason_name(
                fuse::physics::narrowphase::ManifoldPruneRejectReason::InvalidNormal),
        ...
        "prune reject reason name resolves InvalidNormal");
    expectTrue(
        !fuse::physics::narrowphase::normalize_contact_normal_if_needed(noNormal),
        "normalize returns false for invalid normal");
    ...
    expectTrue(
        fuse::physics::narrowphase::normalize_contact_normal_if_needed(unnormalized),
        "normalize succeeds for scaled normal");
...
void testFrictionBasisShouldRunAndComputePreflightGuards() {
    ...
    fuse::physics::narrowphase::compute_friction_tangents_with_preflight(needsBuild);
    expectTrue(needsBuild.hasFrictionBasis(), "compute_with_preflight builds orthonormal basis");
    expectTrue(
        !fuse::physics::narrowphase::should_run_friction_basis_rebuild(needsBuild),
        "should_run friction rebuild false after valid basis cached");
...
void testContactBufferCompactAndClampPreflightGuards() {
    fuse::physics::narrowphase::ContactBufferSoA buffer;
    expectTrue(
        fuse::physics::narrowphase::contact_buffer_compact_reject_reason(buffer) ==
            fuse::physics::narrowphase::ContactBufferCompactRejectReason::EmptyBuffer,
        "empty buffer reports EmptyBuffer compact reject reason");
    expectTrue(
        fuse::physics::narrowphase::can_skip_contact_buffer_compact(buffer),
        "can_skip compact on empty buffer");
    expectTrue(buffer.compact() == 0u, "compact early-outs via preflight on empty buffer");
    ...
    expectTrue(
        ...
        "synced buffer reports NoWork compact reject reason");
    expectTrue(
        fuse::physics::narrowphase::contact_buffer_compact_rejects_for_reason(
            buffer, fuse::physics::narrowphase::ContactBufferCompactRejectReason::NoWork),
        "synced buffer rejects for NoWork");
    expectTrue(buffer.compact() == 2u, "compact no-op returns synced active count");
    ...
    expectTrue(
        fuse::physics::narrowphase::should_run_contact_buffer_compact(sparse),
        "should_run compact true when invalid gap exists");
    expectTrue(sparse.compact() == 2u, "compact gathers sparse valid slots via preflight gate");
    ...
    clampBuffer.writeSlot(1u, manifold);
    expectTrue(
        fuse::physics::narrowphase::should_run_contact_buffer_compact_and_clamp(clampBuffer),
        "should_run compact-and-clamp true when over capacity");
    expectTrue(
        std::strcmp(
            fuse::physics::narrowphase::contact_buffer_compact_and_clamp_reject_reason_name(
                fuse::physics::narrowphase::ContactBufferCompactAndClampRejectReason::NoWork),
...
void testContactPairDispatchPreflightGuards() {
    ...
    expectTrue(
        fuse::physics::narrowphase::is_canonical_contact_pair({bodyA, bodyB}),
        "canonical guard accepts lower index first");
    expectTrue(
        fuse::physics::narrowphase::is_swapped_contact_pair({bodyB, bodyA}),
        "swapped guard flags reversed pair");
    const auto canonical =
        fuse::physics::narrowphase::canonicalize_contact_pair({bodyB, bodyA});
    expectTrue(canonical.bodyA == bodyA && canonical.bodyB == bodyB, "canonicalize restores ordering");

    const auto basePreflight =
        fuse::physics::narrowphase::preflight_detect_contacts_pair({bodyA, bodyB}, bodies, shapes);
    ...
    const auto deepenPreflight =
        fuse::physics::narrowphase::preflight_detect_contacts_pair(
            {sleepingA, sleepingB}, bodies, shapes, true);
    ...
    const auto baseDetect =
        fuse::physics::narrowphase::detect_contacts_pair_with_preflight({bodyA, bodyB}, bodies, shapes);
    expectTrue(baseDetect.valid, "detect_with_preflight returns valid manifold for valid pair");

    const auto rejectedDetect =
        fuse::physics::narrowphase::detect_contacts_pair_with_preflight(
            {sleepingA, sleepingB}, bodies, shapes, true);
    expectTrue(!rejectedDetect.valid, "detect_with_preflight returns invalid for deepen-rejected pair");
...
void testContactBufferWriteCompactPreflightGuards() {
    ...
    const auto validPreflight =
        fuse::physics::narrowphase::preflight_contact_buffer_write(0u, valid, buffer.pairSlotCount);
    ...
    expectTrue(
        fuse::physics::narrowphase::contact_buffer_write_rejects_for_reason(
            0u, selfPair, buffer.pairSlotCount,
            fuse::physics::narrowphase::ContactBufferWriteRejectReason::SelfPair),
        "write rejects_for_reason flags self pair");
    expectTrue(
        !buffer.writeSlotWithPreflight(1u, selfPair),
    ...
    expectTrue(
        fuse::physics::narrowphase::contact_buffer_write_rejects_for_reason(
            0u, invalid, buffer.pairSlotCount,
            fuse::physics::narrowphase::ContactBufferWriteRejectReason::InvalidManifold),
        "write rejects_for_reason flags invalid manifold");
    expectTrue(
        std::strcmp(
    ...
    expectNear(clampBuffer.manifoldAt(0u).penetrationDepth, 0.9f, 1e-4f, "clamp keeps deepest after preflight");
}
...
void testManifoldNormalizeAndChainFinalizeGuards() {
    ...
    expectTrue(
        !fuse::physics::narrowphase::normalize_contact_normal_if_needed(noNormal),
        "normalize_if_needed fails for invalid normal");
    ...
    expectTrue(
        fuse::physics::narrowphase::prune_and_finalize_contact_manifold_with_preflight(dirty),
        "prune_and_finalize chains prune and finalize");
    ...
    expectTrue(
        !fuse::physics::narrowphase::prune_and_finalize_contact_manifold_with_preflight(separated),
        "prune_and_finalize no-ops on all-separated manifold");
    expectTrue(separated.empty(), "prune_and_finalize clears all-separated manifold");
}
...
void testFrictionBasisComputeWithPreflightGuards() {
    ...
    expectTrue(
        fuse::physics::narrowphase::compute_friction_tangents_with_preflight(needsBuild),
        "compute_with_preflight builds basis and normalizes");
    expectTrue(needsBuild.hasFrictionBasis(), "compute_with_preflight stores orthonormal basis");
    expectNear(needsBuild.contactNormal.length(), 1.f, 1e-4f, "compute_with_preflight normalizes contact normal");

    const auto cachedTangent1 = needsBuild.frictionBasis.tangent1;
    expectTrue(
        fuse::physics::narrowphase::compute_friction_tangents_with_preflight(needsBuild),
        "compute_with_preflight reuses cached basis");
    expectNear(
        needsBuild.frictionBasis.tangent1.x,
        cachedTangent1.x,
        1e-4f,
    ...
    expectTrue(
        !fuse::physics::narrowphase::compute_friction_tangents_with_preflight(noNormal),
        "compute_with_preflight skips invalid normal");
    expectTrue(
        !noNormal.hasFrictionBasis(),
        "compute_with_preflight clears basis on invalid normal");
}
...
void testContactPairDeepenPassB46Guards() {
    ...
    const fuse::u32 dynamicA = bodies.addBody({0.f, 0.f, 0.f}, 1.f);
    const fuse::u32 dynamicB = bodies.addBody({1.5f, 0.f, 0.f}, 1.f);
    const fuse::u32 noGravityA = bodies.addBody({0.f, 2.f, 0.f}, 1.f, fuse::physics::RB_NO_GRAVITY);
    const fuse::u32 noGravityB = bodies.addBody({0.f, 3.f, 0.f}, 1.f, fuse::physics::RB_NO_GRAVITY);
    const fuse::u32 ccdA = bodies.addBody({0.f, 4.f, 0.f}, 1.f, fuse::physics::RB_CCD);
    const fuse::u32 ccdB = bodies.addBody({0.f, 5.f, 0.f}, 1.f, fuse::physics::RB_CCD);
    shapes.addShape(fuse::physics::CollisionShapeType::Sphere, dynamicA, {1.f, 0.f, 0.f});
    shapes.addShape(fuse::physics::CollisionShapeType::Sphere, dynamicB, {1.f, 0.f, 0.f});
    shapes.addShape(fuse::physics::CollisionShapeType::Sphere, noGravityA, {1.f, 0.f, 0.f});
    shapes.addShape(fuse::physics::CollisionShapeType::Sphere, noGravityB, {1.f, 0.f, 0.f});
    shapes.addShape(fuse::physics::CollisionShapeType::Sphere, ccdA, {1.f, 0.f, 0.f});
    shapes.addShape(fuse::physics::CollisionShapeType::Sphere, ccdB, {1.f, 0.f, 0.f});

    expectTrue(
        fuse::physics::narrowphase::is_no_gravity_contact_pair({noGravityA, noGravityB}, bodies),
        "no-gravity guard detects both-no-gravity pair");
    expectTrue(
        !fuse::physics::narrowphase::is_no_gravity_contact_pair({dynamicA, noGravityA}, bodies),
        "no-gravity guard allows dynamic/no-gravity mix");
    expectTrue(
        fuse::physics::narrowphase::is_ccd_contact_pair({ccdA, ccdB}, bodies),
        "ccd guard detects both-ccd pair");
    expectTrue(
        !fuse::physics::narrowphase::is_ccd_contact_pair({dynamicA, ccdA}, bodies),
        "ccd guard allows dynamic/ccd mix");

    expectTrue(
        fuse::physics::narrowphase::contact_pair_deepen_reject_reason({noGravityA, noGravityB}, bodies, shapes) ==
            fuse::physics::narrowphase::ContactPairRejectReason::BothNoGravity,
        "deepen reject reason flags both-no-gravity pair");
    expectTrue(
        fuse::physics::narrowphase::contact_pair_deepen_reject_reason({ccdA, ccdB}, bodies, shapes) ==
            fuse::physics::narrowphase::ContactPairRejectReason::BothCcd,
        "deepen reject reason flags both-ccd pair");
    expectTrue(
        fuse::physics::narrowphase::contact_pair_reject_reason({noGravityA, noGravityB}, bodies, shapes) ==
            fuse::physics::narrowphase::ContactPairRejectReason::None,
        "base reject reason unchanged for no-gravity pair");

    const fuse::u32 meshBody = bodies.addBody({6.f, 0.f, 0.f}, 1.f);
    shapes.addShape(fuse::physics::CollisionShapeType::SdfMesh, meshBody, {1.f, 0.f, 0.f});
    expectTrue(
        fuse::physics::narrowphase::is_mesh_shape_contact_pair({dynamicA, meshBody}, shapes),
        "mesh guard flags sdf mesh shape pair");
    expectTrue(
        !fuse::physics::narrowphase::is_mesh_shape_contact_pair({dynamicA, dynamicB}, shapes),
        "mesh guard allows non-mesh pair");

    const std::vector<fuse::physics::broadphase::CandidatePair> mixedPairs = {
        {noGravityA, noGravityB},
        {dynamicA, dynamicB},
    };
    const auto deepenPreflight =
        fuse::physics::narrowphase::preflight_narrowphase_batch_deepen(mixedPairs, bodies, shapes);
    ...
    expectTrue(
        !fuse::physics::narrowphase::narrowphase_batch_deepen_rejects_all(mixedPairs, bodies, shapes),
        "batch deepen rejects_all false when one pair dispatchable");
    ...
    expectTrue(
        std::strcmp(
            fuse::physics::narrowphase::contact_pair_reject_reason_name(
                fuse::physics::narrowphase::ContactPairRejectReason::BothNoGravity),
            "BothNoGravity") == 0,
        "reject reason name resolves BothNoGravity");
    expectTrue(
        std::strcmp(
            fuse::physics::narrowphase::contact_pair_reject_reason_name(
                fuse::physics::narrowphase::ContactPairRejectReason::BothCcd),
            "BothCcd") == 0,
        "reject reason name resolves BothCcd");
}
...
void testManifoldPruneFinalizeB46Guards() {
    ...
    unnormalized.contactNormal = {0.f, 2.f, 0.f};
    unnormalized.addPoint({0.f, 0.f, 0.f}, 0.3f);
    expectTrue(unnormalized.normalizeContactNormalIfNeeded(), "normalize if needed scales contact normal");
    expectNear(unnormalized.contactNormal.y, 1.f, 1e-4f, "normalize if needed produces unit normal");
    expectTrue(
        unnormalized.normalizeContactNormalIfNeeded(),
        "second normalize if needed is no-op on unit normal");
    ...
    fuse::physics::narrowphase::ContactManifold clean{};
    clean.contactNormal = {0.f, 1.f, 0.f};
    clean.addPoint({0.f, 0.f, 0.f}, 0.25f);
    expectTrue(
        fuse::physics::narrowphase::prune_contact_manifold_if_needed(clean),
        "prune if needed no-ops on clean manifold");
    expectTrue(clean.pointCount == 1u, "prune if needed preserves clean slots");
    ...
    expectTrue(
        !fuse::physics::narrowphase::finalize_contact_manifold_if_needed(separated),
        "finalize if needed no-ops on separated manifold");

    fuse::physics::narrowphase::ContactManifold combined =
        fuse::physics::narrowphase::collideSphereSphere({0.f, 0.f, 0.f}, 1.f, {1.5f, 0.f, 0.f}, 1.f, 0u, 1u);
    expectTrue(
        fuse::physics::narrowphase::prune_and_finalize_contact_manifold(combined),
        "prune and finalize succeeds for valid manifold");
    expectTrue(combined.valid, "prune and finalize sets validity");
    expectNear(combined.contactNormal.length(), 1.f, 1e-4f, "prune and finalize stores unit normal");
}

void testFrictionBasisB46Guards() {
    fuse::physics::narrowphase::ContactManifold stale{};
    stale.contactNormal = {0.f, 2.f, 0.f};
    stale.addPoint({0.f, 0.f, 0.f}, 0.2f);
    stale.buildFrictionBasis();
    stale.contactNormal = {1.f, 0.f, 0.f};
    ...
    expectTrue(
        fuse::physics::narrowphase::friction_basis_rejects_for_reason(
            stale, fuse::physics::narrowphase::FrictionBasisRejectReason::StaleBasis),
        "friction rejects_for_reason flags stale basis via is_stale");
    expectTrue(
        fuse::physics::narrowphase::should_rebuild_friction_basis(stale),
        "should_rebuild true for stale basis");

    expectTrue(
        fuse::physics::narrowphase::normalize_and_rebuild_friction_basis(stale),
        "normalize and rebuild refreshes stale basis");
    expectTrue(
        fuse::physics::narrowphase::friction_basis_matches_normal(stale),
        "normalize and rebuild matches current normal");
    ...
    const auto cachedTangent1 = fresh.frictionBasis.tangent1;
    fuse::physics::narrowphase::compute_friction_tangents_with_preflight(fresh);
    expectNear(
        fresh.frictionBasis.tangent1.x,
        cachedTangent1.x,
        1e-4f,
    ...
    expectTrue(buffer.compact() == 2u, "buffer compacts two manifolds for rebuild test");
    expectTrue(
        !buffer.slotNeedsFrictionBasisRebuild(0u),
        "slot rebuild guard false for valid basis");

    const auto firstBasis = buffer.tangentBasisAt(0u);
    buffer.tangent1[1] = firstBasis.tangent1;
    buffer.tangent2[1] = firstBasis.tangent2;
    expectTrue(
        buffer.slotNeedsFrictionBasisRebuild(1u),
        "slot rebuild guard true when basis mismatches normal");

    buffer.rebuildFrictionTangentBasesIfNeeded();
    expectTrue(
        !buffer.slotNeedsFrictionBasisRebuild(1u),
        "rebuild if needed fixes missing basis slot");
    const auto rebuilt = buffer.tangentBasisAt(1u);
    expectTrue(
        fuse::physics::narrowphase::isOrthonormalTangentBasis({1.f, 0.f, 0.f}, rebuilt),
        "rebuild if needed produces orthonormal basis");
...
void testContactPairDeepenPassDispatchPredicates() {
    fuse::physics::RigidBodySoA bodies;
    fuse::physics::CollisionShapeSoA shapes;
    const fuse::u32 dynamicA = bodies.addBody({0.f, 0.f, 0.f}, 1.f);
    const fuse::u32 dynamicB = bodies.addBody({1.5f, 0.f, 0.f}, 1.f);
    ...
    expectTrue(
        fuse::physics::narrowphase::should_run_narrowphase(mixedPairs, bodies, shapes),
        "should_run_narrowphase true when one pair dispatchable");
    expectTrue(
    ...
    expectTrue(
        ...
        "should_run_narrowphase false when all deepen-rejected");
}

void testManifoldPruneFinalizeDispatchPredicates() {
    fuse::physics::narrowphase::ContactManifold clean{};
    clean.contactNormal = {0.f, 1.f, 0.f};
    clean.addPoint({0.f, 0.f, 0.f}, 0.4f);
    expectTrue(
...
void testFrictionBasisRebuildDispatchPredicates() {
    fuse::physics::narrowphase::ContactManifold empty{};
    expectTrue(
        !fuse::physics::narrowphase::should_run_friction_basis_rebuild(empty),
        "should_run_friction_basis_rebuild false for empty manifold");
...
void testContactBufferDeepenPassGuards() {
    ...
    expectTrue(
        ...
        "friction-basis rejects_for_reason flags empty buffer");
    ...
    buffer.preparePairSlots(1u);
    buffer.writeSlot(0u, manifold);
    expectTrue(!buffer.canSkipSoAIteration(), "non-empty contact buffer does not skip iteration");
    expectTrue(buffer.canSkipCompaction(), "single valid slot skips compaction");
    expectTrue(
        fuse::physics::narrowphase::contact_buffer_write_rejects_for_reason(
    ...
    const auto writePreflight =
        fuse::physics::narrowphase::preflight_contact_buffer_write(buffer, 0u, manifold);
    ...
    expectTrue(buffer.compact() == 1u, "compact prepares active slots for friction rebuild");
    expectTrue(
        fuse::physics::narrowphase::should_run_contact_buffer_friction_basis(buffer),
        "should_run friction-basis rebuild for valid slot");
    buffer.buildFrictionTangentBases();
    expectTrue(
        fuse::physics::narrowphase::isOrthonormalTangentBasis(
            buffer.contactNormals[0],
            {buffer.tangent1[0], buffer.tangent2[0]}),
        "friction-basis rebuild stores orthonormal tangents");
}

void testContactBufferCompactionClampPredicates() {
    fuse::physics::narrowphase::ContactBufferSoA buffer;
    buffer.preparePairSlots(2u);
    ...
    expectTrue(
        fuse::physics::narrowphase::should_run_contact_buffer_compaction(buffer),
        "should_run compaction true with sparse valid slot");
    expectTrue(
        fuse::physics::narrowphase::can_skip_contact_buffer_clamp(buffer),
        "can_skip clamp true when within capacity");
    expectTrue(
        fuse::physics::narrowphase::should_run_contact_buffer_compact_and_clamp(buffer),
        "should_run compact-and-clamp true with sparse valid slot");
    expectEq(buffer.compact(), 1u, "compact gathers sparse valid slot");
    expectEq(buffer.activeCount, 1u, "compact sets active count");
    ...
    expectEq(buffer.activeCount, 2u, "compact early-out leaves contacts intact");

    buffer.setMaxCapacity(1u);
    expectTrue(
        fuse::physics::narrowphase::should_run_contact_buffer_clamp(buffer),
        "should_run clamp true when active exceeds max capacity");
    expectEq(buffer.applyMaxCapacityClamp(), 1u, "clamp drops excess contacts");
    expectEq(buffer.droppedCount, 1u, "clamp records dropped contacts");
}
...
void testNarrowphasePairDispatchPreflightGuards() {
    ...
    const auto validPreflight =
        fuse::physics::narrowphase::preflight_narrowphase_pair_dispatch({dynamicA, dynamicB}, bodies, shapes);
    expectTrue(validPreflight.can_dispatch(), "pair dispatch preflight allows valid pair");
    expectTrue(
        fuse::physics::narrowphase::should_run_narrowphase_pair_dispatch({dynamicA, dynamicB}, bodies, shapes),
        "should_run pair dispatch true for valid pair");
    expectTrue(
        !fuse::physics::narrowphase::can_skip_narrowphase_pair_dispatch({dynamicA, dynamicB}, bodies, shapes),
        "can_skip pair dispatch false for valid pair");

    const auto sleepingPreflight =
        fuse::physics::narrowphase::preflight_narrowphase_pair_dispatch({sleepingA, sleepingB}, bodies, shapes);
    expectTrue(!sleepingPreflight.can_dispatch(), "pair dispatch preflight rejects both-sleeping pair");
    expectTrue(
        sleepingPreflight.reason == fuse::physics::narrowphase::ContactPairRejectReason::BothSleeping,
        "pair dispatch preflight reports BothSleeping");
    expectTrue(
        fuse::physics::narrowphase::can_skip_narrowphase_pair_dispatch({sleepingA, sleepingB}, bodies, shapes),
        "can_skip pair dispatch true for both-sleeping pair");

    const auto deepenDetect =
        fuse::physics::narrowphase::detect_contacts_pair_with_deepen_preflight(
            {sleepingA, sleepingB}, bodies, shapes);
    expectTrue(!deepenDetect.valid, "deepen detect returns invalid for rejected pair");

    const auto overlap =
        fuse::physics::narrowphase::detect_contacts_pair_with_deepen_preflight({dynamicA, dynamicB}, bodies, shapes);
    expectTrue(overlap.valid, "deepen detect returns valid manifold for dispatchable pair");
}
...
void testContactBufferWritePreflightGuards() {
    ...
    const auto validPreflight =
        fuse::physics::narrowphase::preflight_contact_buffer_write(0u, valid, buffer);
    ...
    expectTrue(
        fuse::physics::narrowphase::contact_buffer_write_rejects_for_reason(
            ...
            buffer,
            fuse::physics::narrowphase::ContactBufferWriteRejectReason::SelfPair),
        "buffer write rejects_for_reason flags self pair");
    expectTrue(
        !buffer.writeSlotWithPreflight(1u, selfPair),
        "writeSlotWithPreflight rejects self pair");
    ...
    expectTrue(
        fuse::physics::narrowphase::contact_buffer_write_rejects_for_reason(
            ...
            buffer,
            fuse::physics::narrowphase::ContactBufferWriteRejectReason::InvalidSlot),
        "buffer write rejects_for_reason flags invalid slot");
    expectTrue(
        std::strcmp(
            fuse::physics::narrowphase::contact_buffer_write_reject_reason_name(
                fuse::physics::narrowphase::ContactBufferWriteRejectReason::InvalidManifold),
            "InvalidManifold") == 0,
        "buffer write reject reason name resolves InvalidManifold");
}
...
void testContactBufferFrictionBuildPreflightGuards() {
    ...
    expectTrue(empty.canSkipBuildFrictionTangentBases(), "canSkip friction build on empty buffer");
    expectTrue(!empty.shouldRunBuildFrictionTangentBases(), "shouldRun friction build false on empty buffer");
    empty.buildFrictionTangentBasesIfNeeded();
    ...
    manifold.buildFrictionBasis();
    buffer.writeSlot(0u, manifold);
    expectTrue(buffer.compact() == 1u, "friction build preflight buffer compacts one slot");

    const auto validPreflight = fuse::physics::narrowphase::preflight_contact_buffer_friction_build(buffer);
    expectTrue(
        validPreflight.reason ==
            fuse::physics::narrowphase::ContactBufferFrictionBuildRejectReason::AllBasesValid,
        "friction build preflight reports AllBasesValid");
    expectTrue(buffer.canSkipBuildFrictionTangentBases(), "canSkip friction build when bases valid");
    buffer.buildFrictionTangentBasesIfNeeded();
    expectTrue(
        fuse::physics::narrowphase::isOrthonormalTangentBasis(
            {0.f, 0.f, 1.f}, buffer.tangentBasisAt(0u)),
        "buildFrictionTangentBasesIfNeeded preserves valid basis");

    buffer.tangent1[0u] = {};
    expectTrue(buffer.shouldRunBuildFrictionTangentBases(), "shouldRun friction build when basis stale");
    buffer.buildFrictionTangentBasesIfNeeded();
    expectTrue(
        fuse::physics::narrowphase::isOrthonormalTangentBasis(
            {0.f, 0.f, 1.f}, buffer.tangentBasisAt(0u)),
        "buildFrictionTangentBasesIfNeeded rebuilds stale basis");
}
...
void testManifoldPruneFinalizeDeepenPassGuards() {
    ...
    expectTrue(
        fuse::physics::narrowphase::should_run_manifold_prune(dirty),
        "should_run manifold prune true when separated slot present");
    ...
    expectTrue(
        !fuse::physics::narrowphase::should_run_manifold_prune(clean),
        "should_run manifold prune false for clean manifold");
    ...
    expectTrue(
        fuse::physics::narrowphase::normalize_contact_normal_if_needed(unnormalized),
        "normalize_contact_normal_if_needed applies to scaled normal");
    expectNear(unnormalized.contactNormal.y, 1.f, 1e-4f, "normalize produces unit normal");
    expectTrue(
        !fuse::physics::narrowphase::normalize_contact_normal_if_needed(unnormalized),
        "normalize no-op on already unit normal");
}
...
void testFrictionBasisDeepenPassGuards() {
    ...
    fuse::physics::narrowphase::compute_friction_tangents_with_preflight(needsBuild);
    expectTrue(needsBuild.hasFrictionBasis(), "compute_with_preflight builds basis");
    expectTrue(
        !fuse::physics::narrowphase::should_run_friction_basis_rebuild(needsBuild),
        "should_run friction rebuild false after valid basis");
    ...
    expectNear(
        ...
        cachedTangent1.x,
        1e-4f,
        "compute_with_preflight preserves cached basis");
}
...
void testRunNarrowphaseWithDeepenGuards() {
    ...
    fuse::physics::narrowphase::ContactBufferSoA buffer;
    fuse::physics::narrowphase::runNarrowphaseIntoBufferWithDeepenGuards(pairs, bodies, shapes, buffer);
    expectTrue(buffer.activeCount == 1u, "deepen narrowphase skips rejected pair");
    const auto restored = buffer.manifoldAt(0u);
    expectTrue(restored.valid, "deepen narrowphase produces valid manifold");
    expectTrue(restored.hasFrictionBasis(), "deepen narrowphase finalizes friction basis");
    expectTrue(restored.bodyA == dynamicA && restored.bodyB == dynamicB, "deepen narrowphase keeps dispatchable pair");
}
...
void testContactPairNoDispatchPathGuards() {
    ...
    const fuse::u32 capsuleBody = bodies.addBody({2.f, 0.f, 0.f}, 1.f);
    const fuse::u32 planeBody = bodies.addBody({0.f, -1.f, 0.f}, 0.f, fuse::physics::RB_STATIC);
    shapes.addShape(fuse::physics::CollisionShapeType::Sphere, dynamicA, {1.f, 0.f, 0.f});
    shapes.addShape(fuse::physics::CollisionShapeType::Sphere, dynamicB, {1.f, 0.f, 0.f});
    shapes.addShape(fuse::physics::CollisionShapeType::Box, boxBody, {1.f, 1.f, 1.f});
    shapes.addShape(fuse::physics::CollisionShapeType::Capsule, capsuleBody, {0.5f, 1.f, 0.f});
    ...
    expectTrue(
        ...
        "box-plane guard flags box/plane pair");
    expectTrue(
        fuse::physics::narrowphase::is_capsule_plane_contact_pair({capsuleBody, planeBody}, shapes),
        "capsule-plane guard flags capsule/plane pair");
    expectTrue(
        fuse::physics::narrowphase::is_box_capsule_contact_pair({boxBody, capsuleBody}, shapes),
        "box-capsule guard flags box/capsule pair");

    const fuse::u32 capsuleB = bodies.addBody({3.f, 0.f, 0.f}, 1.f);
    shapes.addShape(fuse::physics::CollisionShapeType::Capsule, capsuleB, {0.5f, 1.f, 0.f});
    expectTrue(
        fuse::physics::narrowphase::is_capsule_capsule_contact_pair({capsuleBody, capsuleB}, shapes),
        "capsule-capsule guard flags both-capsule pair");
    expectTrue(
        fuse::physics::narrowphase::is_undispatched_shape_pair({boxBody, planeBody}, shapes),
        "undispatched guard flags box/plane pair");
    expectTrue(
        fuse::physics::narrowphase::contact_pair_deepen_reject_reason({boxBody, planeBody}, bodies, shapes) ==
            fuse::physics::narrowphase::ContactPairRejectReason::NoDispatchPath,
        "deepen reject reason flags box/plane pair");
    expectTrue(
        fuse::physics::narrowphase::contact_pair_reject_reason({boxBody, planeBody}, bodies, shapes) ==
            fuse::physics::narrowphase::ContactPairRejectReason::None,
        "base reject reason unchanged for box/plane pair");

    const auto undispatched =
        fuse::physics::narrowphase::detect_contacts_pair_deepen({boxBody, planeBody}, bodies, shapes);
    expectTrue(!undispatched.valid, "deepen detect returns invalid for undispatched pair");
    expectTrue(undispatched.empty(), "deepen detect has no points for undispatched pair");
    ...
    expectTrue(
        std::strcmp(
            fuse::physics::narrowphase::contact_pair_reject_reason_name(
                fuse::physics::narrowphase::ContactPairRejectReason::NoDispatchPath),
            "NoDispatchPath") == 0,
        "reject reason name resolves NoDispatchPath");
}
...
void testNarrowphaseSlotPreflightGuards() {
    ...
    expectTrue(
        ...
        "slot skip guard allows valid pair");

    const auto undispatchedPreflight =
        fuse::physics::narrowphase::preflight_narrowphase_slot({boxBody, planeBody}, bodies, shapes);
...
void testManifoldNormalizeAndIfNeededGuards() {
    ...
    unnormalized.addPoint({0.f, 0.f, 0.f}, 0.3f);
    expectTrue(
        !fuse::physics::narrowphase::can_skip_normalize_contact_normal(unnormalized),
        "can skip normalize false for scaled normal");
    expectTrue(
        fuse::physics::narrowphase::normalize_contact_normal_if_needed(unnormalized),
        "normalize if needed succeeds for scaled normal");
    expectNear(unnormalized.contactNormal.y, 1.f, 1e-4f, "normalize if needed produces unit normal");

    fuse::physics::narrowphase::ContactManifold unit = unnormalized;
    const auto cachedNormal = unit.contactNormal;
    expectTrue(
        fuse::physics::narrowphase::can_skip_normalize_contact_normal(unit),
        "can skip normalize true for unit normal");
    expectTrue(
        fuse::physics::narrowphase::normalize_contact_normal_if_needed(unit),
        "normalize if needed no-ops on unit normal");
    expectNear(unit.contactNormal.y, cachedNormal.y, 1e-4f, "normalize if needed preserves unit normal");
...
void testFrictionBasisComputeWithPreflightGuards() {
    ...
    expectTrue(
        fuse::physics::narrowphase::normalize_contact_normal_for_friction(unnormalized),
        "normalize for friction succeeds on scaled normal");
    expectNear(unnormalized.contactNormal.y, 1.f, 1e-4f, "normalize for friction produces unit normal");

    fuse::physics::narrowphase::ContactManifold needsBuild{};
    needsBuild.contactNormal = {0.f, 1.f, 0.f};
    needsBuild.addPoint({0.f, 0.f, 0.f}, 0.2f);
    expectTrue(
        !fuse::physics::narrowphase::can_skip_compute_friction_tangents(needsBuild),
        "can skip compute false without cached basis");
    expectTrue(
        fuse::physics::narrowphase::compute_friction_tangents_with_preflight(needsBuild),
        "compute with preflight builds missing basis");
    ...
    expectTrue(
        fuse::physics::narrowphase::can_skip_compute_friction_tangents(needsBuild),
        "can skip compute true with valid cached basis");
    expectTrue(
        fuse::physics::narrowphase::compute_friction_tangents_with_preflight(needsBuild),
        "compute with preflight reuses valid basis");
    expectNear(
    ...
    fuse::physics::narrowphase::ContactManifold empty{};
    expectTrue(
        !fuse::physics::narrowphase::normalize_contact_normal_for_friction(empty),
        "normalize for friction fails on empty manifold");
}
...
void testContactBufferFrictionRebuildPreflightGuards() {
    ...
    expectTrue(
        buffer.frictionBasisMatchesNormalAt(0u),
        "slot zero friction basis matches normal");
    expectTrue(
        buffer.frictionBasisMatchesNormalAt(1u),
        "slot one friction basis matches normal after write");
    expectTrue(buffer.canSkipBuildFrictionTangentBases(), "can skip rebuild when all bases valid");

    const auto preflight = fuse::physics::narrowphase::preflight_contact_buffer_friction_rebuild(buffer);
    expectTrue(preflight.can_skip_rebuild(), "buffer friction preflight can skip valid bases");
    expectTrue(preflight.staleCount == 0u, "buffer friction preflight reports no stale slots");

    buffer.contactNormals[1] = {0.f, 1.f, 0.f};
    expectTrue(
        !buffer.frictionBasisMatchesNormalAt(1u),
        "stale basis detected after normal change");
    ...
    expectTrue(
        !fuse::physics::narrowphase::can_skip_build_friction_tangent_bases(buffer),
        "can skip rebuild false when slot is stale");

    const auto cachedTangent1 = buffer.tangentBasisAt(0u).tangent1;
    buffer.buildFrictionTangentBasesIfNeeded();
    expectTrue(
        fuse::physics::narrowphase::isOrthonormalTangentBasis(
            {0.f, 1.f, 0.f}, buffer.tangentBasisAt(1u)),
        "build if needed refreshes stale slot basis");
    expectNear(
        buffer.tangentBasisAt(0u).tangent1.x,
        cachedTangent1.x,
        1e-4f,
        "build if needed preserves valid slot basis");
    expectTrue(buffer.canSkipBuildFrictionTangentBases(), "can skip rebuild after refresh");
}
...
void testContactBufferDeepenFollowUpGuards() {
    ...
    expectTrue(
        fuse::physics::narrowphase::contact_buffer_write_rejects_for_reason(
            buffer, 0u, valid, fuse::physics::narrowphase::ContactBufferWriteRejectReason::None),
        "write rejects_for_reason allows valid manifold");
    expectTrue(
        fuse::physics::narrowphase::contact_buffer_write_rejects_for_reason(
    ...
    expectTrue(
        ...
        "write_with_preflight writes valid manifold");
    expectTrue(buffer.slotIsValid(0u), "write_with_preflight marks slot valid");
    ...
    expectTrue(
        ...
        "compact_with_preflight sets active count for all-valid buffer");

    buffer.setMaxCapacity(1u);
    expectTrue(buffer.canApplyMaxCapacityClamp(), "canApplyMaxCapacityClamp true when over max capacity");
    expectTrue(
        fuse::physics::narrowphase::compactAndClampWithPreflight(buffer) == 1u,
        "compactAndClamp_with_preflight truncates to max capacity");
    expectTrue(
        std::strcmp(
            fuse::physics::narrowphase::contact_buffer_compact_and_clamp_reject_reason_name(
                fuse::physics::narrowphase::ContactBufferCompactAndClampRejectReason::NoWork),
...
void testContactPairDeepenFollowUpDispatchGuards() {
    ...
    const auto validManifold = fuse::physics::narrowphase::detect_contacts_pair_deepen(
        {dynamicA, dynamicB}, bodies, shapes);
    expectTrue(validManifold.valid, "detect_contacts_pair_deepen dispatches valid pair");
    ...
    expectTrue(
        fuse::physics::narrowphase::narrowphase_batch_has_dispatchable_count(mixedPairs, bodies, shapes, 1u),
        "batch has_dispatchable_count matches deepen preflight");
    expectTrue(
        fuse::physics::narrowphase::can_skip_narrowphase_into_buffer({{sleepingA, sleepingB}}, bodies, shapes),
        "can_skip_narrowphase_into_buffer true when all deepen-rejected");

    fuse::physics::narrowphase::ContactBufferSoA buffer;
    buffer.reserve(2u);
    fuse::physics::narrowphase::runNarrowphaseIntoBufferDeepen(mixedPairs, bodies, shapes, buffer);
    expectTrue(buffer.activeCount == 1u, "runNarrowphaseIntoBufferDeepen skips deepen-rejected pair");
    expectTrue(buffer.manifoldAt(0u).valid, "deepen dispatch produces valid manifold");
}
...
void testManifoldGenerateDeepenFollowUpGuards() {
    ...
    expectTrue(ready.hasFrictionBasis(), "generate_with_preflight builds friction basis");
...
void testFrictionBasisDeepenFollowUpGuards() {
    ...
    stale.contactNormal = {0.f, 1.f, 0.f};
    stale.addPoint({0.f, 0.f, 0.f}, 0.2f);
    stale.frictionBasis = {{1.f, 0.f, 0.f}, {0.f, 1.f, 0.f}};
    expectTrue(
        fuse::physics::narrowphase::friction_basis_deepen_rejects_for_reason(
            stale, fuse::physics::narrowphase::FrictionBasisRejectReason::StaleBasis),
        "deepen rejects_for_reason flags stale basis");
    expectTrue(
        std::strcmp(
            fuse::physics::narrowphase::friction_basis_reject_reason_name(
                fuse::physics::narrowphase::FrictionBasisRejectReason::StaleBasis),
    ...
    unnormalized.addPoint({0.f, 0.f, 0.f}, 0.2f);
    fuse::physics::narrowphase::normalize_contact_normal_if_needed(unnormalized);
    expectNear(unnormalized.contactNormal.y, 1.f, 1e-4f, "normalize_if_needed unitizes normal");
    expectTrue(
        fuse::physics::narrowphase::compute_friction_tangents_with_preflight(unnormalized),
        "compute_tangents_with_preflight builds missing basis");
    expectTrue(unnormalized.hasFrictionBasis(), "compute_tangents_with_preflight stores orthonormal basis");
    ...
    expectNear(
        ...
        cachedTangent1.x,
        1e-4f,
        "compute_tangents_with_preflight preserves cached tangent1");
}
...
void testContactPairDeepenB46RejectGuards() {
    ...
    const fuse::u32 dynamicA = bodies.addBody({0.f, 0.f, 0.f}, 1.f);
    const fuse::u32 dynamicB = bodies.addBody({1.5f, 0.f, 0.f}, 1.f);
    const fuse::u32 meshBody = bodies.addBody({5.f, 0.f, 0.f}, 1.f);
    const fuse::u32 planeBody = bodies.addBody({0.f, -1.f, 0.f}, 0.f, fuse::physics::RB_STATIC);
    shapes.addShape(fuse::physics::CollisionShapeType::Sphere, dynamicA, {1.f, 0.f, 0.f});
    shapes.addShape(fuse::physics::CollisionShapeType::Sphere, dynamicB, {1.f, 0.f, 0.f});
    shapes.addShape(fuse::physics::CollisionShapeType::SdfMesh, meshBody, {1.f, 0.f, 0.f});
    shapes.addShape(fuse::physics::CollisionShapeType::Plane, planeBody, {0.f, 0.f, 0.f});

    expectTrue(
        fuse::physics::narrowphase::is_mesh_shape_contact_pair({dynamicA, meshBody}, shapes),
        "mesh guard flags SdfMesh shape pair");
    expectTrue(
        !fuse::physics::narrowphase::is_mesh_shape_contact_pair({dynamicA, dynamicB}, shapes),
        "mesh guard allows non-mesh pair");
    expectTrue(
        fuse::physics::narrowphase::is_degenerate_plane_normal_pair({dynamicA, planeBody}, shapes),
        "degenerate plane guard flags zero-length plane normal");
    expectTrue(
        fuse::physics::narrowphase::contact_pair_deepen_reject_reason({dynamicA, meshBody}, bodies, shapes) ==
            fuse::physics::narrowphase::ContactPairRejectReason::UnsupportedMeshPair,
        "deepen reject reason flags mesh pair");
    expectTrue(
        fuse::physics::narrowphase::contact_pair_reject_reason({dynamicA, meshBody}, bodies, shapes) ==
            fuse::physics::narrowphase::ContactPairRejectReason::UnsupportedShapePair,
        "base reject reason unchanged for mesh pair");
    expectTrue(
        fuse::physics::narrowphase::contact_pair_deepen_reject_reason({dynamicA, planeBody}, bodies, shapes) ==
            fuse::physics::narrowphase::ContactPairRejectReason::DegeneratePlaneNormal,
        "deepen reject reason flags degenerate plane normal");

    const std::vector<fuse::physics::broadphase::CandidatePair> meshPairs = {
        {dynamicA, meshBody},
        {dynamicA, meshBody},
    };
    expectTrue(
        fuse::physics::narrowphase::count_contact_pairs_rejected_for_reason(
            meshPairs, bodies, shapes,
            fuse::physics::narrowphase::ContactPairRejectReason::UnsupportedMeshPair) == 2u,
        "count rejected tracks mesh pairs in batch");
    expectTrue(
        fuse::physics::narrowphase::narrowphase_batch_all_reject_for_reason(
            meshPairs, bodies, shapes,
            fuse::physics::narrowphase::ContactPairRejectReason::UnsupportedMeshPair),
        "batch all reject true when every pair is mesh-rejected");

    expectTrue(
        std::strcmp(
            fuse::physics::narrowphase::contact_pair_reject_reason_name(
                fuse::physics::narrowphase::ContactPairRejectReason::UnsupportedMeshPair),
            "UnsupportedMeshPair") == 0,
        "reject reason name resolves UnsupportedMeshPair");
    expectTrue(
        std::strcmp(
            fuse::physics::narrowphase::contact_pair_reject_reason_name(
                fuse::physics::narrowphase::ContactPairRejectReason::DegeneratePlaneNormal),
            "DegeneratePlaneNormal") == 0,
        "reject reason name resolves DegeneratePlaneNormal");
}
...
void testManifoldPruneFinalizeB46Guards() {
    ...
    expectTrue(
        fuse::physics::narrowphase::normalize_contact_normal_if_needed(unnormalized),
        "normalize applies to scaled contact normal");
    expectNear(unnormalized.contactNormal.length(), 1.f, 1e-4f, "normalize produces unit normal");
    expectTrue(
        !fuse::physics::narrowphase::normalize_contact_normal_if_needed(unnormalized),
        "normalize no-ops on unit normal");
    ...
    expectTrue(
        fuse::physics::narrowphase::prune_contact_manifold_if_needed(dirty),
        "prune if needed keeps penetrating slot");
    expectTrue(dirty.pointCount == 1u, "prune if needed removes separated slot");
    ...
    expectTrue(
        ...
        "finalize if needed finalizes valid manifold");
    expectTrue(
        fuse::physics::narrowphase::is_contact_manifold_finalized(ready),
        "is finalized true after finalize if needed");

    const auto cachedTangent1 = ready.frictionBasis.tangent1;
    expectTrue(
        fuse::physics::narrowphase::finalize_contact_manifold_if_needed(ready),
        "finalize if needed no-ops on finalized manifold");
    expectNear(
        ready.frictionBasis.tangent1.x,
        cachedTangent1.x,
        1e-4f,
        "finalize if needed preserves cached friction basis");
    ...
    expectTrue(
        ...
        "finalize rejects_for_reason flags already-finalized manifold");
    expectTrue(
        std::strcmp(
            fuse::physics::narrowphase::manifold_finalize_reject_reason_name(
                fuse::physics::narrowphase::ManifoldFinalizeRejectReason::AlreadyFinalized),
            "AlreadyFinalized") == 0,
        "finalize reject reason name resolves AlreadyFinalized");
    expectTrue(
        std::strcmp(
            fuse::physics::narrowphase::manifold_prune_reject_reason_name(
                fuse::physics::narrowphase::ManifoldPruneRejectReason::ExceedsMaxPoints),
...
void testFrictionBasisB46RejectGuards() {
    ...
    expectTrue(
        fuse::physics::narrowphase::friction_basis_stale_reject_reason(stale) ==
            fuse::physics::narrowphase::FrictionBasisRejectReason::StaleBasis,
        "stale reject reason flags stale cached basis");
    expectTrue(
        fuse::physics::narrowphase::invalidate_friction_basis_if_stale(stale),
        "invalidate if stale clears stale basis");
    expectTrue(
        !fuse::physics::narrowphase::has_cached_friction_basis(stale),
        "invalidate if stale removes cached frame");

    expectTrue(
        fuse::physics::narrowphase::compute_friction_tangents_with_preflight(stale),
        "compute with preflight rebuilds missing basis");
    expectTrue(stale.hasFrictionBasis(), "compute with preflight stores orthonormal basis");
...
void testContactBufferB46Guards() {
    fuse::physics::narrowphase::ContactBufferSoA buffer;
    buffer.preparePairSlots(3u);
    expectTrue(buffer.canSkipBufferCompact(), "can skip compact on empty valid slots");
    ...
    expectTrue(!buffer.hasValidPairSlot(0u), "has valid pair slot false for empty slot");
    expectTrue(buffer.hasValidPairSlot(1u), "has valid pair slot true for written slot");
    expectTrue(buffer.compact() == 1u, "compact gathers valid slot");
    expectTrue(buffer.canSkipBufferCompact(), "can skip compact after compact aligns counts");
}
...
void testContactPairDeepenSecondRejectGuards() {
    ...
    expectTrue(
        fuse::physics::narrowphase::contact_pair_deepen_second_reject_reason({planeA, planeB}, bodies, shapes) ==
            fuse::physics::narrowphase::ContactPairRejectReason::PlanePlane,
        "second reject reason flags plane-plane pair");
    expectTrue(
        fuse::physics::narrowphase::contact_pair_reject_reason({planeA, planeB}, bodies, shapes) ==
            fuse::physics::narrowphase::ContactPairRejectReason::UnsupportedShapePair,
        "base reject reason unchanged for plane-plane pair");
    ...
    expectTrue(
        fuse::physics::narrowphase::is_mesh_shape_contact_pair({dynamicA, meshBody}, shapes),
        "mesh guard flags sdf mesh pair");
    expectTrue(
        fuse::physics::narrowphase::contact_pair_deepen_second_reject_reason({dynamicA, meshBody}, bodies, shapes) ==
            fuse::physics::narrowphase::ContactPairRejectReason::MeshShapePair,
        "second reject reason flags mesh shape pair");

    const fuse::u32 thinBoxBody = bodies.addBody({6.f, 0.f, 0.f}, 1.f);
    shapes.addShape(fuse::physics::CollisionShapeType::Box, thinBoxBody, {1.f, 1.f, 1e-5f});
    expectTrue(
        fuse::physics::narrowphase::is_box_thin_shape_pair({dynamicA, thinBoxBody}, shapes),
        "thin box guard flags near-zero extent");
    expectTrue(
        fuse::physics::narrowphase::contact_pair_deepen_second_reject_reason({dynamicA, thinBoxBody}, bodies, shapes) ==
            fuse::physics::narrowphase::ContactPairRejectReason::BoxThinPair,
        "second reject reason flags thin box pair");
    expectTrue(
        fuse::physics::narrowphase::contact_pair_deepen_reject_reason({dynamicA, thinBoxBody}, bodies, shapes) ==
            fuse::physics::narrowphase::ContactPairRejectReason::None,
        "first deepen reject unchanged for thin box pair");

    const auto secondPreflight =
        fuse::physics::narrowphase::preflight_contact_pair_deepen_second({dynamicA, dynamicB}, bodies, shapes);
    ...
    expectTrue(
        ...
        "second skip guard allows valid pair");
    expectTrue(
        fuse::physics::narrowphase::contact_pair_deepen_second_rejects_for_reason(
            {planeA, planeB},
            bodies,
            shapes,
            fuse::physics::narrowphase::ContactPairRejectReason::PlanePlane),
        "second rejects_for_reason matches plane-plane pair");
    ...
    expectTrue(
        std::strcmp(
            fuse::physics::narrowphase::contact_pair_reject_reason_name(
                fuse::physics::narrowphase::ContactPairRejectReason::MeshShapePair),
            "MeshShapePair") == 0,
        "reject reason name resolves MeshShapePair");
    expectTrue(
        std::strcmp(
            fuse::physics::narrowphase::contact_pair_reject_reason_name(
                fuse::physics::narrowphase::ContactPairRejectReason::BoxThinPair),
            "BoxThinPair") == 0,
        "reject reason name resolves BoxThinPair");
}
...
void testNarrowphaseBatchSecondPreflightGuards() {
    ...
    const std::vector<fuse::physics::broadphase::CandidatePair> mixedPairs = {
        {dynamicA, thinBoxBody},
        {dynamicA, dynamicB},
    };
    const auto batchPreflight =
        fuse::physics::narrowphase::preflight_narrowphase_batch_second(mixedPairs, bodies, shapes);
    ...
    expectTrue(
        fuse::physics::narrowphase::count_dispatchable_contact_pairs_second(mixedPairs, bodies, shapes) == 1u,
        "second count_dispatchable excludes rejected pairs");
    expectTrue(
        fuse::physics::narrowphase::has_dispatchable_contact_pair_second(mixedPairs, bodies, shapes),
        "second has_dispatchable true when one pair passes");
    expectTrue(
        !fuse::physics::narrowphase::can_skip_narrowphase_second(mixedPairs, bodies, shapes),
        "can_skip_narrowphase_second false when one pair dispatchable");
    expectTrue(
        fuse::physics::narrowphase::narrowphase_batch_second_rejects_all({{dynamicA, thinBoxBody}}, bodies, shapes),
        "second batch rejects_all true when all pairs rejected");
    expectTrue(
        fuse::physics::narrowphase::can_skip_narrowphase_second({{dynamicA, thinBoxBody}}, bodies, shapes),
        "can_skip_narrowphase_second true when all pairs rejected");
}

void testManifoldPruneFinalizeSecondGuards() {
    fuse::physics::narrowphase::ContactManifold atCap{};
    atCap.contactNormal = {0.f, 1.f, 0.f};
    atCap.addPoint({0.f, 0.f, 0.f}, 0.4f);
    atCap.addPoint({1.f, 0.f, 0.f}, 0.25f);
    atCap.addPoint({2.f, 0.f, 0.f}, -0.1f);
    atCap.addPoint({3.f, 0.f, 0.f}, 0.15f);
    expectTrue(
        fuse::physics::narrowphase::manifold_prune_second_reject_reason(atCap) ==
            fuse::physics::narrowphase::ManifoldPruneRejectReason::ExceedsMaxPoints,
        "second prune reject flags at-cap manifold needing prune");
    expectTrue(
        std::strcmp(
            fuse::physics::narrowphase::manifold_prune_reject_reason_name(
                fuse::physics::narrowphase::ManifoldPruneRejectReason::ExceedsMaxPoints),
        ...
        "prune reject reason name resolves ExceedsMaxPoints");
    expectTrue(
        fuse::physics::narrowphase::prune_contact_manifold_second_with_preflight(atCap),
        "second prune_with_preflight keeps penetrating slots after cap trim");
    expectTrue(atCap.pointCount <= fuse::physics::narrowphase::kMaxContactPointsPerManifold,
        "second prune_with_preflight trims to max points");
    expectTrue(!atCap.hasSeparatedPoints(), "second prune_with_preflight removes separated slot");

    fuse::physics::narrowphase::ContactManifold unnormalized{};
    unnormalized.contactNormal = {0.f, 2.f, 0.f};
    unnormalized.addPoint({0.f, 0.f, 0.f}, 0.25f);
    expectTrue(
        fuse::physics::narrowphase::manifold_finalize_second_reject_reason(unnormalized) ==
            fuse::physics::narrowphase::ManifoldFinalizeRejectReason::NeedsNormalNormalize,
        "second finalize reject flags non-unit normal");
    expectTrue(
        fuse::physics::narrowphase::normalize_contact_normal_if_needed(unnormalized),
        "normalize_contact_normal_if_needed scales non-unit normal");
    expectNear(unnormalized.contactNormal.length(), 1.f, 1e-4f, "normalized contact normal is unit length");
    expectTrue(
        fuse::physics::narrowphase::finalize_contact_manifold_second_with_preflight(unnormalized),
        "second finalize_with_preflight finalizes after normalize");
    expectTrue(unnormalized.valid, "second finalize_with_preflight sets validity");
    expectTrue(unnormalized.hasFrictionBasis(), "second finalize_with_preflight builds friction basis");
    expectTrue(
        std::strcmp(
            fuse::physics::narrowphase::manifold_finalize_reject_reason_name(
                fuse::physics::narrowphase::ManifoldFinalizeRejectReason::NeedsNormalNormalize),
            "NeedsNormalNormalize") == 0,
        "finalize reject reason name resolves NeedsNormalNormalize");
}
...
void testFrictionBasisSecondRebuildGuards() {
    ...
    expectTrue(
        fuse::physics::narrowphase::friction_basis_second_reject_reason(stale) ==
            fuse::physics::narrowphase::FrictionBasisRejectReason::StaleBasis,
        "second friction reject flags stale basis");
    expectTrue(
        !fuse::physics::narrowphase::should_skip_friction_basis_second_preflight(stale),
        "second friction preflight cannot skip stale basis");
    expectTrue(
        fuse::physics::narrowphase::rebuild_friction_basis_second_with_preflight(stale),
        "second rebuild_with_preflight refreshes stale basis");
    expectTrue(
        fuse::physics::narrowphase::friction_basis_matches_normal(stale),
        "second rebuild produces basis matching current normal");
    expectTrue(
        std::strcmp(
            fuse::physics::narrowphase::friction_basis_reject_reason_name(
                fuse::physics::narrowphase::FrictionBasisRejectReason::StaleBasis),
    ...
    expectTrue(
        fuse::physics::narrowphase::normalize_contact_normal_before_friction_if_needed(scaled),
        "normalize before friction scales non-unit normal");
    expectNear(scaled.contactNormal.length(), 1.f, 1e-4f, "scaled normal normalized before friction");
    expectTrue(
        fuse::physics::narrowphase::rebuild_friction_basis_second_with_preflight(scaled),
        "second rebuild builds basis after normalize");
    expectTrue(scaled.hasFrictionBasis(), "second rebuild stores orthonormal basis");
}
...
void testContactBufferSecondPreflightGuards() {
    ...
    expectTrue(
        fuse::physics::narrowphase::contact_buffer_write_reject_reason(buffer, 0u, selfPair) ==
            fuse::physics::narrowphase::ContactBufferWriteRejectReason::SelfPair,
        "buffer write reject flags self pair");
    expectTrue(
        !buffer.writeSlotWithPreflight(0u, selfPair),
        "writeSlotWithPreflight rejects self pair");
    expectTrue(
        buffer.writeSlotWithPreflight(0u, valid),
    ...
    buffer.writeSlot(2u, valid);
    buffer.validFlags[1u] = 0u;
    expectTrue(!buffer.canSkipCompaction(), "canSkipCompaction false with invalid middle slot");
    expectTrue(buffer.compactWithPreflight() == 2u, "compactWithPreflight gathers valid slots");

    fuse::physics::narrowphase::ContactBufferSoA full{};
    full.preparePairSlots(2u);
    full.writeSlot(0u, valid);
    full.writeSlot(1u, valid);
    expectTrue(full.canSkipCompaction(), "canSkipCompaction true when all slots valid");
    expectTrue(
        fuse::physics::narrowphase::contact_buffer_compaction_rejects_for_reason(
            full, fuse::physics::narrowphase::ContactBufferCompactionRejectReason::AllValid),
        "compaction reject AllValid when all slots valid");

    buffer.setMaxCapacity(1u);
    expectTrue(buffer.canApplyMaxCapacityClamp(), "canApplyMaxCapacityClamp true when over max");
    expectTrue(
        fuse::physics::narrowphase::shouldRunContactBufferClamp(buffer),
        "shouldRunContactBufferClamp when over max capacity");
    ...
    expectTrue(
        std::strcmp(
            fuse::physics::narrowphase::contact_buffer_clamp_reject_reason_name(
                fuse::physics::narrowphase::ContactBufferClampRejectReason::WithinCapacity),
            "WithinCapacity") == 0,
        "buffer clamp reject reason name resolves WithinCapacity");

    fuse::physics::narrowphase::ContactBufferSoA empty{};
    expectTrue(
        fuse::physics::narrowphase::contact_buffer_compact_and_clamp_rejects_for_reason(
            empty, fuse::physics::narrowphase::ContactBufferCompactAndClampRejectReason::EmptyBuffer),
        "compact-and-clamp reject EmptyBuffer on empty buffer");
    expectTrue(
        fuse::physics::narrowphase::canSkipContactBufferCompactAndClamp(empty),
        "canSkipContactBufferCompactAndClamp on empty buffer");
}
...
int main() {
    ...
    testManifoldPruneFinalizeDeepenPassGuards();
    testRunNarrowphaseWithDeepenGuards();
    ...
    testContactPairDeepenPassRejectGuardsV2();
    testManifoldNormalizeAndFinalizeIfNeededGuards();
    testFrictionBasisNormalizeRebuildGuards();
    testContactBufferFrictionRebuildIfNeededGuards();
    testRunNarrowphaseFilteredIntoBufferGuards();
    testContactPairB46DeepenGuards();
    testManifoldPruneFinalizeB46Guards();
    testFrictionBasisB46Guards();
    testContactBufferB46Guards();
    testNarrowphaseDispatchB46Guards();
    testContactPairBeyondDeepenRejectGuards();
    ...
    testShouldRunContactPairDispatchGuards();
    testNarrowphaseDispatchRejectReasonGuards();
    testShouldRunManifoldPruneGuards();
    testShouldRunFrictionBasisRebuildGuards();
    testContactPairBothPlaneDeepenGuards();
    testShouldRunNarrowphaseDispatchGuards();
    testRunNarrowphaseDeepenDispatchGates();
    testManifoldPruneInvalidNormalGuards();
    testShouldRunManifoldPruneFinalizeGuards();
    testFrictionBasisShouldRunAndComputePreflightGuards();
    testContactBufferWriteCompactPreflightGuards();
    testManifoldNormalizeAndChainFinalizeGuards();
    testFrictionBasisComputeWithPreflightGuards();
    testContactPairDeepenPassB46Guards();
    testContactPairDeepenPassDispatchPredicates();
    testManifoldPruneFinalizeDispatchPredicates();
    testFrictionBasisRebuildDispatchPredicates();
    testContactBufferCompactionClampPredicates();
    testContactPairDeepenSecondRejectGuards();
    testNarrowphaseBatchSecondPreflightGuards();
    testManifoldPruneFinalizeSecondGuards();
    testFrictionBasisSecondRebuildGuards();
    testContactBufferSecondPreflightGuards();
    testContactPairNoDispatchPathGuards();
    testNarrowphaseSlotPreflightGuards();
    testManifoldNormalizeAndIfNeededGuards();
    testContactBufferDeepenFollowUpGuards();
    testContactPairDeepenFollowUpDispatchGuards();
    testManifoldGenerateDeepenFollowUpGuards();
    testFrictionBasisDeepenFollowUpGuards();
    testContactPairDeepenB46RejectGuards();
    testFrictionBasisB46RejectGuards();
