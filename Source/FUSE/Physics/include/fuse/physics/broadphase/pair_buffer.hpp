...
/// Preflight push without mutation; optional reject-reason output (B4.2 deepen pass).
...
/// Preflight write-slot skip check without mutating the buffer (B4.2 deepen pass).
bool wouldSkipPairBufferWriteSlot(
/// Early-out when write-slot would reject — same ordering as `pairBufferWriteSlotRejectReason` (B4.2 deepen pass).
...
/// Early-out when write-slot preflight would be rejected — same ordering as `tryPreflightPairBufferWriteSlot`.

/// Diagnose why write-slot preflight would reject; vacuously succeeds when write may proceed.
...
/// Predict whether writeSlot would bail before mutating slot storage (B4.2 deepen pass).
...
/// Early-out when invalidate-slot preflight would reject — same ordering as `pairBufferInvalidateSlotRejectReason` (B4.2 deepen pass).


/// Preflight invalidate-slot without mutation; optional reject-reason output (B4.2 deepen pass).
...
/// Preflight invalidate-slot skip check without mutating the buffer (B4.2 deepen pass).
...
/// Predict whether invalidateSlot would bail before mutating slot storage (B4.2 deepen pass).
...
/// Early-out when invalidate-slot would reject — same ordering as `pairBufferInvalidateSlotRejectReason` (B4.2 deepen pass).
...
/// Early-out when invalidate-slot preflight would be rejected — same ordering as `tryPreflightPairBufferInvalidateSlot`.

/// Diagnose why invalidate-slot preflight would reject; vacuously succeeds when invalidate may proceed.
...
/// Non-mutating slot-reservation skip predicate — inverse of `canReserve` (B4.2 deepen pass).

/// Non-mutating slot-reservation predicate — mirrors `preflightPairBufferSlotReservation` (B4.2 deepen pass).
...
/// Preflight invalidate-slot without mutating storage — optional `reason` out-param (B4.2 deepen follow-up pass).

/// Preflight write-slot without mutating storage — optional `reason` out-param (B4.2 deepen follow-up pass).
...
/// Predict whether max-capacity clamp would skip; optional `reason` output (B4.2 deepen pass).
bool wouldSkipPairBufferClamp(const PairBufferSoA& buffer, PairBufferClampRejectReason* reason = nullptr);

/// Predict whether SoA dedupe would skip; optional `reason` output (B4.2 deepen pass).
bool wouldSkipPairBufferDedupe(const PairBufferSoA& buffer, PairBufferDedupeRejectReason* reason = nullptr);

/// Predict whether canonical sort would skip; optional `reason` output (B4.2 deepen pass).
bool wouldSkipPairBufferSort(const PairBufferSoA& buffer, PairBufferSortRejectReason* reason = nullptr);

/// Predict whether compact-and-clamp would skip; optional `reason` output (B4.2 deepen pass).
