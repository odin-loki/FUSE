...
/// Human-readable labels for island dispatch/solve/sleep/wake reject reasons.
...
/// Diagnose why island dispatch would skip; vacuously succeeds when work may proceed.
...
/// Diagnose why island sleep preflight would skip.
...
/// Diagnose why island wake preflight would skip.
...
/// Returns true when dispatch reject reason matches `expected`.

/// Human-readable label for island dispatch reject reasons (B4.4 deepen follow-up pass).

/// Diagnose why island dispatch would skip; vacuously succeeds when dispatch may proceed (B4.4 deepen follow-up pass).
...
/// Why island solve preflight would early-out (B4.4 deepen follow-up pass).
    NoConstraintRefs,
...
/// Human-readable labels for island reject reasons (B4.4 deepen follow-up pass).
...
/// Returns true when solve/sleep/wake reject reason matches `expected`.
...
/// Why island sleep preflight would not skip solve (B4.4 deepen follow-up pass).

/// Why island wake preflight would not activate sleepers (B4.4 deepen follow-up pass).
...
/// Diagnose why one extracted island job would skip dispatch.
IslandDispatchRejectReason island_dispatch_job_reject_reason(const IslandSolveJob& job, f32 dt);

/// Returns true when job dispatch reject reason matches `expected`.
...
struct IslandDispatchJobPreflight {
    ...
    bool can_dispatch() const {
        ...
        return reason == IslandDispatchRejectReason::None && !skipped && !invalidDt && solve.can_dispatch();
    }
};
...
/// Human-readable label for per-job island solve reject reasons (B4.4 deepen follow-up pass).
    ...
    bool can_dispatch() const {
        ...
        return reason == IslandSolveRejectReason::None && !skipped && !invalidDt && constraintCount > 0u;
    }
};
...
struct IslandConstraintSolveResult {
    ...
    bool can_solve() const { return reason == IslandSolveRejectReason::None && !skipped && refs.can_solve() && bodies.can_solve(); }
        ...
        return reason == IslandSolveRejectReason::None && !skipped && refs.can_solve() && bodies.can_solve();
};
...
struct IslandSleepGraphPreflight {
    ...
    bool has_solveable_islands() const { return !skipped && reason == IslandSleepGraphRejectReason::None; }
        ...
        return !skipped && reason == IslandSleepRejectReason::None &&
        return reason == IslandSleepRejectReason::None && !skipped &&
...
/// Human-readable label for island pipeline dispatch reject reasons (B4.4 deepen follow-up pass).
const char* island_pipeline_dispatch_reject_reason_name(IslandPipelineDispatchRejectReason reason);

/// Combined wake + dispatch preflight for one island solve substep (B4.4 deepen follow-up pass).
struct IslandPipelineDispatchPreflight {
    IslandPipelineDispatchRejectReason reason = IslandPipelineDispatchRejectReason::None;
    IslandWakeGraphPreflight wake{};
    IslandSleepGraphPreflight sleep{};
    ...
    bool can_dispatch() const {
        return reason == IslandPipelineDispatchRejectReason::None && !skipped && dispatch.can_dispatch() &&
               sleep.has_solveable_islands();

/// Batch pipeline dispatch summary for wake-then-solve stubs (B4.4 deepen follow-up pass).
...
/// Batch outcome for pipeline guarded dispatch (wake then solve).
...
/// Combined build + wake + dispatch readiness for island pipeline stubs (B4.4 deepen follow-up pass).
...
/// Guarded pipeline: wake wakeable islands then dispatch all islands (B4.4 deepen follow-up).
...
/// Diagnose why one island job would skip dispatch (B4.4 deepen follow-up pass).
IslandDispatchRejectReason island_dispatch_job_reject_reason(const IslandSolveJob& job, f32 dt);
...
/// Diagnose why island solve would skip; vacuously succeeds when solve may proceed (B4.4 deepen follow-up pass).
IslandSolveRejectReason island_solve_reject_reason(const ContactIslandGraph& graph);

/// Diagnose why constraint refs would block island solve (B4.4 deepen follow-up pass).
IslandSolveRejectReason island_constraint_refs_reject_reason(
    const ContactIslandGraph::Island& island,
    const std::vector<narrowphase::ContactManifold>& contacts,
    const std::vector<DistanceConstraint>& distanceConstraints);

/// Diagnose why body participation would block island solve (B4.4 deepen follow-up pass).
IslandSolveRejectReason island_solve_bodies_reject_reason(const ContactIslandGraph::Island& island,
                                                          const RigidBodySoA& bodies);

/// Diagnose why island sleep preflight would skip solve (B4.4 deepen follow-up pass).
IslandSleepRejectReason island_sleep_reject_reason(const ContactIslandGraph::Island& island,
                                                   const RigidBodySoA& bodies);

/// Diagnose why graph-level sleep preflight has no solveable islands (B4.4 deepen follow-up pass).
IslandSleepRejectReason island_sleep_graph_reject_reason(const ContactIslandGraph& graph,
                                                         const RigidBodySoA& bodies);

/// Diagnose why island wake preflight would skip activation (B4.4 deepen follow-up pass).
IslandWakeRejectReason island_wake_reject_reason(const ContactIslandGraph::Island& island,
                                                 const RigidBodySoA& bodies);

/// Diagnose why graph-level wake preflight has no wakeable islands (B4.4 deepen follow-up pass).
IslandWakeRejectReason island_wake_graph_reject_reason(const ContactIslandGraph& graph,
                                                       const RigidBodySoA& bodies);
...
/// Preflight build + wake + dispatch readiness without mutating bodies or graph (B4.4 deepen follow-up pass).
...
/// Guarded island graph build via `ContactIslandGraph::build_guarded` (B4.4 deepen follow-up pass).
bool pipeline_build_islands_guarded(ContactIslandGraph& graph,
                                    u32 bodyCount,
                                    const std::vector<narrowphase::ContactManifold>& contacts,
                                    const std::vector<DistanceConstraint>& distanceConstraints);

/// Guarded wake across wakeable islands (B4.4 deepen follow-up pass).
u32 pipeline_wake_islands_guarded(RigidBodySoA& bodies, const ContactIslandGraph& graph);

/// Guarded dispatch across dispatchable islands (B4.4 deepen follow-up pass).
...
/// Guarded wake then dispatch pipeline stub (B4.4 deepen follow-up pass).
...
/// Guarded build, wake, and dispatch pipeline stub (B4.4 deepen follow-up pass).
...
/// Preflight for guarded island pipeline dispatch (build → wake → solve).
struct IslandPipelineDispatchPreflight {
    IslandGraphBuildRejectReason buildReason = IslandGraphBuildRejectReason::None;
    IslandDispatchRejectReason dispatchReason = IslandDispatchRejectReason::None;
    IslandBuildPreflight build{};
    ...
    bool can_run() const {
        return !skipped && buildReason == IslandGraphBuildRejectReason::None &&
               dispatchReason == IslandDispatchRejectReason::None;
    }
};

/// Aggregate result for guarded island pipeline dispatch (build → wake → solve).
struct IslandPipelineDispatchResult {
    bool built = false;
    u32 wokeCount = 0;
    IslandBatchDispatchResult solve{};
    IslandGraphBuildRejectReason buildReason = IslandGraphBuildRejectReason::None;
    IslandDispatchRejectReason dispatchReason = IslandDispatchRejectReason::None;
    bool skipped = false;

    bool any_solved() const { return solve.any_solved(); }
};

/// Preflight guarded island pipeline dispatch from build inputs and timestep.
IslandPipelineDispatchPreflight preflight_island_pipeline_dispatch(
    u32 bodyCount,
    const std::vector<narrowphase::ContactManifold>& contacts,
    const std::vector<DistanceConstraint>& distanceConstraints,
...
/// Early-out guard for island pipeline dispatch (unsafe build inputs or invalid dt).
bool should_skip_island_pipeline_dispatch(u32 bodyCount,
                                          const std::vector<narrowphase::ContactManifold>& contacts,
                                          const std::vector<DistanceConstraint>& distanceConstraints,
                                          f32 dt);

/// Guarded pipeline: build graph, wake mixed-island sleepers, then batch-solve islands.
IslandPipelineDispatchResult dispatch_island_pipeline_guarded(
    ContactIslandGraph& graph,
    RigidBodySoA& bodies,
    SolverWorkBuffers& workBuffers,
...
/// Guarded solve pipeline over an existing graph: wake sleepers, then batch-solve islands.
...
/// Preflight wake-then-dispatch pipeline for one island solve substep (B4.4 deepen follow-up pass).
IslandPipelineDispatchPreflight preflight_island_pipeline_dispatch(const ContactIslandGraph& graph,
                                                                   const RigidBodySoA& bodies,
                                                                   f32 dt);

/// Early-out guard when the island pipeline has nothing to wake or solve (B4.4 deepen follow-up pass).
bool should_skip_island_pipeline_dispatch(const ContactIslandGraph& graph,
                                            const RigidBodySoA& bodies,
                                            f32 dt);

/// Guarded wake-then-solve pipeline; returns false when preflight rejects dispatch (B4.4 deepen follow-up pass).
...
/// Guarded wake-then-solve pipeline with explicit skip/solve counts (B4.4 deepen follow-up pass).
...
/// Preflight wake + dispatch for one island-solve pipeline pass.
IslandPipelineDispatchPreflight preflight_island_pipeline_dispatch(const ContactIslandGraph& graph,
                                                                   const RigidBodySoA& bodies,
                                                                   f32 dt);

/// Dispatch preflight with optional reject-reason output (B4.4 deepen follow-up pass).
bool preflight_island_dispatch_ready(const ContactIslandGraph& graph,
                                     f32 dt,
                                     IslandDispatchRejectReason* reason = nullptr);

/// Dispatch preflight with mandatory reject-reason output (B4.4 deepen follow-up pass).
bool try_preflight_island_dispatch(const ContactIslandGraph& graph,
                                   f32 dt,
                                   IslandDispatchRejectReason& reason);

/// Pipeline dispatch preflight with optional reject-reason output (B4.4 deepen follow-up pass).
bool preflight_island_pipeline_dispatch_ready(const ContactIslandGraph& graph,
                                              const RigidBodySoA& bodies,
                                              f32 dt,
                                              IslandDispatchRejectReason* reason = nullptr);

/// Pipeline dispatch preflight with mandatory reject-reason output (B4.4 deepen follow-up pass).
bool try_preflight_island_pipeline_dispatch(const ContactIslandGraph& graph,
                                            const RigidBodySoA& bodies,
                                            f32 dt,
                                            IslandDispatchRejectReason& reason);

/// Guarded pipeline dispatch: wake sleepers then solve dispatchable islands.
IslandPipelineDispatchResult dispatch_island_pipeline_guarded(
    RigidBodySoA& bodies,
    const ContactIslandGraph& graph,
    SolverWorkBuffers& workBuffers,
