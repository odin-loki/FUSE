...
bool slotTangentBasisIsValid(const ContactBufferSoA& buffer, u32 slot) {
    if (slot >= buffer.validFlags.size() || buffer.validFlags[slot] == 0u) {
        return false;
    if (slot >= buffer.contactNormals.size()) {
...
bool ContactBufferSoA::canApplyMaxCapacityClamp() const {
    ...
    if (activeCount >= maxCapacity) {
        return 0u;
    return maxCapacity - activeCount;
...
bool shouldRunContactBufferCompaction(const ContactBufferSoA& buffer) {
    ...
    for (u32 readIndex = 0u; readIndex < buffer.pairSlotCount; ++readIndex) {
        if (readIndex < buffer.validFlags.size() && buffer.validFlags[readIndex] != 0u) {
            if (writeIndex != readIndex) {
                return true;
            ++writeIndex;
    return buffer.activeCount != writeIndex;

bool shouldRunContactBufferClamp(const ContactBufferSoA& buffer) {
    return !buffer.canSkipSoAIteration() && buffer.maxCapacity > 0u && buffer.activeCount > buffer.maxCapacity;


u32 ContactBufferSoA::countValidSlots() const {
    if (canSkipSoAIteration()) {
bool contactHasOrthonormalFrictionBasis(const ContactBufferSoA& buffer, u32 slot) {
    if (slot >= buffer.pairSlotCount || buffer.validFlags[slot] == 0u) {
    return isOrthonormalTangentBasis(buffer.contactNormals[slot], {buffer.tangent1[slot], buffer.tangent2[slot]});
...
bool ContactBufferSoA::canSkipFrictionBasisBuild() const {
    if (canSkipSoAIteration() || activeCount == 0u) {

    for (u32 slot = 0u; slot < activeCount; ++slot) {
            continue;
        if (!slotTangentBasisIsValid(*this, slot)) {


bool contactBufferSlotHasCachedFrictionBasis(const ContactBufferSoA& buffer, u32 slot) {
    if (slot >= buffer.activeCount || buffer.validFlags[slot] == 0u) {

bool contactBufferNeedsFrictionBuild(const ContactBufferSoA& buffer) {
...
bool ContactBufferSoA::canSkipFrictionTangentBuild() const {
...
bool ContactBufferSoA::canSkipCompactAndClamp() const {
    ...
    u32 firstInvalid = scanCount;
        } else if (firstInvalid == scanCount) {
            firstInvalid = slot;

    if (validCount == 0u) {

    for (u32 slot = firstInvalid; slot < scanCount; ++slot) {
    return validCount == activeCount || validCount == scanCount;
    ...
    if (activeCount > 0u && activeCount == validCount) {
        bool packedPrefix = true;
                packedPrefix = false;
                break;
        if (packedPrefix) {
            for (u32 slot = activeCount; slot < scanCount; ++slot) {
...
bool slotHasValidFrictionTangent(
    const ContactBufferSoA& buffer,
    u32 slot,
    f32 epsilon) {
    return isOrthonormalTangentBasis(buffer.contactNormals[slot], basis, epsilon);
...
void writeContactSlot(ContactBufferSoA& buffer, u32 slot, const ContactManifold& manifold) {
...
const char* contactBufferCompactAndClampRejectReasonName(
...
bool ContactBufferSoA::canSkipFrictionTangentBuild(f32 epsilon) const {
...
bool bufferHasNoPairSlots(const ContactBufferSoA& buffer) {




    if (bufferHasNoPairSlots(*this)) {


    return validCount <= maxCapacity;
...
ContactBufferCompactRejectReason contact_buffer_compact_reject_reason(const ContactBufferSoA& buffer) {
    if (bufferHasNoPairSlots(buffer)) {
...
bool contact_buffer_write_rejects_for_reason(
    u32 pairSlotCount,
    return contact_buffer_write_reject_reason(slot, manifold, pairSlotCount) == expected;

ContactBufferWritePreflight preflight_contact_buffer_write(
    preflight.reason = contact_buffer_write_reject_reason(slot, manifold, pairSlotCount);
...
bool hasInvalidPairSlot(const ContactBufferSoA& buffer) {
...
bool ContactBufferSoA::canSkipFrictionTangentBases(f32 epsilon) const {
    return !preflight_contact_buffer_friction_tangent_bases(*this, epsilon).needs_rebuild();
...
u32 ContactBufferSoA::compactWithPreflight() {
    if (!preflight_contact_buffer_compaction(*this).needs_compaction()) {
    return compact();
...
u32 ContactBufferSoA::compactAndClampWithPreflight() {
    if (!preflight_contact_buffer_compact_and_clamp(*this).needs_compact_and_clamp()) {
    return compactAndClamp();

void ContactBufferSoA::buildFrictionTangentBasesIfNeeded(f32 epsilon) {
    if (!preflight_contact_buffer_friction_tangent_bases(*this, epsilon).needs_rebuild()) {
...
bool ContactBufferSoA::applyWarmStartStubWithPreflight(u32 slot, ContactManifold& manifold) const {
    ...
    applyWarmStartStub(slot, manifold);
    const u32 slotCount = std::min(pairSlotCount, static_cast<u32>(validFlags.size()));
...
bool ContactBufferSoA::canSkipBuildFrictionTangentBases() const {
        ...
        if (!contactHasOrthonormalFrictionBasis(*this, slot)) {
...
bool ContactBufferSoA::canSkipCompaction() const {
    return !hasInvalidPairSlot(*this);
...
void ContactBufferSoA::writeSlot(u32 slot, const ContactManifold& manifold) {
    ...
    if (canSkipSoAIteration() || maxCapacity == 0u) {
    const u32 effectiveCount = activeCount > 0u ? activeCount : countValidSlots();
    return effectiveCount > maxCapacity;
...
bool writeContactBufferSlotWithPreflight(
    ...
    if (!preflight_contact_buffer_write_slot(*this, slot, manifold).can_write()) {
    if (!preflight_contact_buffer_write_slot(*this, slot, manifold).canWrite()) {

        return;
    }
    if (pairSlotCount > 0u && slot >= pairSlotCount) {
    validFlags[slot] = 0u;
    writeContactSlot(*this, slot, manifold);
}
...
void ContactBufferSoA::applyWarmStartStub(u32 slot, ContactManifold& manifold) const {
    ...
    if (!preflight_contact_buffer_warm_start(*this, slot).can_apply()) {
    if (!preflight_contact_buffer_warm_start(*this, slot).canApply()) {
        return;
    }
...
void ContactBufferSoA::buildFrictionTangentBasesIfNeeded() {
    ...
    if (!should_run_contact_buffer_friction_rebuild(*this)) {
    ...
    if (!preflight_contact_buffer_friction_basis(*this).needs_friction_basis_build()) {
    if (!preflightContactBufferFrictionBasis(*this).canBuild()) {
    if (!preflight_contact_buffer_friction_bases(*this).needs_rebuild()) {
    if (!preflight_contact_buffer_build_friction(*this).can_build()) {
    if (!preflight_contact_buffer_friction_build(*this).needs_build()) {
    if (canSkipBuildFrictionTangentBases()) {
    if (!should_run_contact_buffer_friction_basis_rebuild(*this)) {
    }
...
void ContactBufferSoA::buildFrictionTangentBasesIfNeeded(f32 epsilon) {
        ...
        manifold.contactNormal = contactNormals[slot];
        manifold.frictionBasis = tangentBasisAt(slot);
        if (can_skip_friction_basis_rebuild(manifold)) {
            continue;

        if (contactBufferSlotHasCachedFrictionBasis(*this, slot)) {
        }
...
void buildContactBufferFrictionTangentBasesWithPreflight(ContactBufferSoA& buffer) {
    ...
    for (u32 slot = 0u; slot < buffer.activeCount; ++slot) {
        if (buffer.validFlags[slot] == 0u) {
        const TangentBasis basis = buildTangentBasis(buffer.contactNormals[slot]);
        buffer.tangent1[slot] = basis.tangent1;
        buffer.tangent2[slot] = basis.tangent2;
    }
}
...
bool ContactBufferSoA::buildFrictionTangentBasesWithPreflight() {
    if (!preflight_contact_buffer_friction_basis(*this).can_rebuild()) {
        return false;
    }
    buildFrictionTangentBases();
    return true;
...
u32 ContactBufferSoA::compactAndClamp() {
    ...
    const ContactBufferCompactAndClampPreflight preflight =
        preflight_contact_buffer_compact_and_clamp(*this);
    ...
    compact();
u32 ContactBufferSoA::applyMaxCapacityClampIfNeeded() {
    if (!canApplyMaxCapacityClamp()) {
    return applyMaxCapacityClamp();
}
...
bool ContactBufferSoA::writeSlotWithPreflight(u32 slot, const ContactManifold& manifold) {
    const ContactBufferWritePreflight preflight =
        preflight_contact_buffer_write(slot, manifold, pairSlotCount);
    if (!preflight.can_write()) {
        return false;
    writeSlot(slot, manifold);
    return true;
...
ContactBufferPreflight preflight_contact_buffer(const ContactBufferSoA& buffer) {
    ...
    preflight.canSkipCompaction = buffer.canSkipCompaction();
    preflight.canSkipClamp = !buffer.canApplyMaxCapacityClamp();
    preflight.canSkipCompactAndClamp = buffer.canSkipCompactAndClamp();

bool should_skip_contact_buffer_iteration(const ContactBufferSoA& buffer) {
    return buffer.canSkipSoAIteration();
}
...
std::vector<ContactManifold> ContactBufferSoA::toVector() const {
    ...
    if (!preflight_contact_buffer_to_vector(*this).canExport()) {
        return {};
    }
...
const char* contactBufferWriteRejectReasonName(ContactBufferWriteRejectReason reason) {
    ...
    case ContactBufferWriteRejectReason::OutOfRange:
        return "OutOfRange";
    return can_skip_contact_buffer_compaction(*this);

    return !can_skip_contact_buffer_clamp(*this);
...
ContactBufferCompactionRejectReason contact_buffer_compaction_reject_reason(
    const ContactBufferSoA& buffer) {
...
void write_contact_buffer_slot_with_preflight(
    ...
    if (buffer.canSkipSoAIteration() || buffer.countValidSlots() == 0u) {
...
bool writeSlotWithPreflight(u32 slot, const ContactManifold& manifold, ContactBufferSoA& buffer) {
    ...
    if (validCount == 0u) {
    if (validCount == buffer.pairSlotCount) {
...
bool contact_buffer_compaction_rejects_for_reason(
    ...
    if (buffer.canSkipCompaction() && buffer.activeCount == buffer.countValidSlots()) {
...
u32 applyMaxCapacityClampWithPreflight(ContactBufferSoA& buffer) {
    if (!preflight_contact_buffer_clamp(buffer).needsClamp()) {
    return buffer.applyMaxCapacityClamp();
...
const char* contact_buffer_friction_rebuild_reject_reason_name(
    ContactBufferFrictionRebuildRejectReason reason) {
    case ContactBufferFrictionRebuildRejectReason::None:
    case ContactBufferFrictionRebuildRejectReason::EmptyBuffer:
    case ContactBufferFrictionRebuildRejectReason::NoValidContacts:
        return "NoValidContacts";
...
ContactBufferFrictionRebuildRejectReason contact_buffer_friction_rebuild_reject_reason(
    ...
    if (buffer.countValidSlots() == 0u) {
        return ContactBufferFrictionRebuildRejectReason::NoValidContacts;
...
bool contact_buffer_friction_rebuild_rejects_for_reason(
    ...
    return contact_buffer_friction_rebuild_reject_reason(buffer) == expected;
...
ContactBufferFrictionRebuildPreflight preflight_contact_buffer_friction_rebuild(
    ...
    preflight.reason = contact_buffer_friction_rebuild_reject_reason(buffer);
...
bool can_skip_contact_buffer_friction_rebuild(const ContactBufferSoA& buffer) {
    return !preflight_contact_buffer_friction_rebuild(buffer).can_rebuild();
...
ContactBufferCompactAndClampPreflight preflight_contact_buffer_compact_and_clamp(
    ...
    if (!preflight_contact_buffer_compaction(buffer).needs_compaction() &&
        !preflight_contact_buffer_clamp(buffer).needs_clamp()) {
...
bool should_run_contact_buffer_compact(const ContactBufferSoA& buffer) {
    ...
    if (can_skip_contact_buffer_compaction(buffer) && can_skip_contact_buffer_clamp(buffer)) {
    ...
    const u32 effectiveCount = buffer.activeCount > 0u ? buffer.activeCount : buffer.countValidSlots();
    if (buffer.maxCapacity == 0u || effectiveCount <= buffer.maxCapacity) {
    ...
    if (buffer.canSkipSoAIteration() || validCount == 0u) {

    const bool needsCompaction = should_run_contact_buffer_compaction(buffer);
    const bool needsClamp = should_run_contact_buffer_clamp(buffer);
...
ContactBufferFrictionBuildRejectReason contact_buffer_friction_build_reject_reason(
    ...
    if (buffer.canSkipSoAIteration()) {
    if (buffer.canSkipFrictionTangentBuild()) {
...
bool can_skip_contact_buffer_friction_tangent_build(
    return !preflight_contact_buffer_friction_tangents(buffer, epsilon).needsFrictionTangentBuild();
...
ContactBufferFrictionBasisPreflight preflight_contact_buffer_friction_basis(
    ...
    return !preflight_contact_buffer_friction_basis(buffer).can_rebuild();
...
bool writeContactSlotWithPreflight(
    ...
    if (!preflight_contact_buffer_compaction(buffer).needs_compaction()) {
        ...
        } else if (preflight_contact_buffer_compaction(buffer).allValid) {
...
u32 clamp_contact_buffer_with_preflight(ContactBufferSoA& buffer) {
    if (!preflight_contact_buffer_clamp(buffer).needs_clamp()) {
    ...
    const ContactBufferWritePreflight preflight = preflightContactBufferWrite(buffer, slot, manifold);
    if (!preflight.canWrite()) {

u32 compactContactBufferWithPreflight(ContactBufferSoA& buffer) {
    const ContactBufferCompactionPreflight preflight = preflightContactBufferCompaction(buffer);
        buffer.activeCount = buffer.pairSlotCount;
...
u32 compact_contact_buffer_if_needed(ContactBufferSoA& buffer) {
    if (can_skip_contact_buffer_compaction(buffer)) {

u32 compact_and_clamp_contact_buffer_if_needed(ContactBufferSoA& buffer) {


    if (buffer.canSkipSoAIteration() || buffer.pairSlotCount == 0u) {
...
ContactBufferFrictionBasisRejectReason contactBufferFrictionBasisRejectReason(const ContactBufferSoA& buffer) {
    if (buffer.canSkipFrictionBasisBuild()) {
...
ContactBufferFrictionBuildRejectReason contactBufferFrictionBuildRejectReason(const ContactBufferSoA& buffer) {
    if (!contactBufferNeedsFrictionBuild(buffer)) {
...
const char* contact_buffer_warm_start_reject_reason_name(ContactBufferWarmStartRejectReason reason) {
    ...
    const u32 scanCount = buffer.activeCount > 0u ? buffer.activeCount : buffer.pairSlotCount;
    for (u32 slot = 0u; slot < scanCount; ++slot) {
        if (!buffer.slotIsValid(slot)) {
        if (!isOrthonormalTangentBasis(buffer.contactNormals[slot], {buffer.tangent1[slot], buffer.tangent2[slot]}, epsilon)) {
...
bool can_skip_contact_buffer_friction_tangent_bases(const ContactBufferSoA& buffer, f32 epsilon) {
    return !preflight_contact_buffer_friction_tangent_bases(buffer, epsilon).needs_rebuild();

bool should_run_contact_buffer_friction_tangent_bases(const ContactBufferSoA& buffer, f32 epsilon) {
    return preflight_contact_buffer_friction_tangent_bases(buffer, epsilon).needs_rebuild();
...
bool contact_buffer_build_friction_rejects_for_reason(
    ...
    return contact_buffer_build_friction_reject_reason(buffer) == expected;
...
ContactBufferBuildFrictionPreflight preflight_contact_buffer_build_friction(const ContactBufferSoA& buffer) {
    ...
    preflight.reason = contact_buffer_build_friction_reject_reason(buffer);
...
bool can_skip_contact_buffer_build_friction(const ContactBufferSoA& buffer) {
    return !preflight_contact_buffer_build_friction(buffer).can_build();
...
void write_contact_buffer_slot_with_preflight(
    if (!preflight_contact_buffer_write_slot(buffer, slot, manifold).can_write()) {

void build_contact_buffer_friction_tangents_with_preflight(ContactBufferSoA& buffer) {
    if (!preflight_contact_buffer_friction_build(buffer).needs_build()) {
...
ContactBufferWarmStartPreflight preflight_contact_buffer_warm_start(const ContactBufferSoA& buffer, u32 slot) {
    ...
    return !preflight_contact_buffer_warm_start(buffer, slot).canApply();

    return preflight_contact_buffer_warm_start(buffer, slot).canApply();

    if (!preflight_contact_buffer_write_slot(buffer, slot, manifold).canWrite()) {

    if (!preflight_contact_buffer_compaction(buffer).needsCompaction()) {
...
bool can_skip_contact_buffer_friction_basis_rebuild(const ContactBufferSoA& buffer) {
    return !preflight_contact_buffer_friction_basis(buffer).canRebuild();

bool should_run_contact_buffer_friction_basis_rebuild(const ContactBufferSoA& buffer) {
    return preflight_contact_buffer_friction_basis(buffer).canRebuild();

    writeContactSlot(buffer, slot, manifold);
