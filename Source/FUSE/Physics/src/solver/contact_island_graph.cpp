...
IslandGraphBuildRejectReason island_graph_build_reject_reason(
    ...
    if (outOfRangeContactBodyCount > 0u) {
        return IslandGraphBuildRejectReason::OutOfRangeContactRefs;
    }
    if (outOfRangeDistanceBodyCount > 0u) {
...
const char* islandGraphBuildRejectReasonName(IslandGraphBuildRejectReason reason) {
    ...
    case IslandGraphBuildRejectReason::EmptyInput:
        return "empty_input";
    case IslandGraphBuildRejectReason::OutOfRangeContactBodies:
        return "out_of_range_contact_bodies";
    case IslandGraphBuildRejectReason::OutOfRangeDistanceBodies:
        return "out_of_range_distance_bodies";
    }
    return "unknown";
}
...
IslandGraphBuildRejectReason island_graph_build_reject_reason(
    ...
    for (const narrowphase::ContactManifold& contact : contacts) {
        if (contact.valid && contact.bodyA < bodyCount && contact.bodyB < bodyCount) {
            ++inRangeContactCount;
        } else if (contact.valid && (contact.bodyA >= bodyCount || contact.bodyB >= bodyCount)) {
            return IslandGraphBuildRejectReason::OutOfRangeContactBody;
        }
    }
...
void ContactIslandGraph::buildInRange(u32 bodyCount,
    ...
    const IslandGraphBuildRejectReason rejectReason =
        island_graph_build_reject_reason(bodyCount, contacts, distanceConstraints);
    if (reason != nullptr) {
        *reason = rejectReason;
