...
| `script_console_command_registry.hpp` | Named built-in/custom command registry with `lookup_kind`, prefix match, `suggest_commands`, name validation, shadowing + sorted `help`/`list` |
| `script_console_history.hpp` | Fixed-capacity history ring buffer with recall navigation, `newest`/`oldest`, and `clear` |
