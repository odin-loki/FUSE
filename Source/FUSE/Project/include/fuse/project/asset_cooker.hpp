...
struct CookUpstreamInvalidateEstimate {
    ...
    /// True when reconcile invalidation planning would be a no-op — mirrors `total() == 0` (B7.9 deepen).
...
public:
    ...
    /// True when upstream invalidation probes report zero removals (B7.9 deepen).
    /// True when stale dependency-hash reconcile probes report zero removals (B7.9 deepen).
    /// True when prune reconcile estimate is empty (B7.9 deepen).
    /// True when combined reconcile estimate is empty (B7.9 deepen).
    /// Convenience guard — `estimate_reconcile_invalidation(manifest).should_skip()` (B7.9 deepen).
    ...
    /// Non-mutating skip predicates — mirror reconcile estimators (B7.9 deepen).
    /// True when `estimate_reconcile_invalidation(manifest).should_skip()` (B7.9 deepen).
    /// True when `estimate_prune_reconcile().should_skip()` (B7.9 deepen).
    /// True when prune reconcile would be a no-op — mirrors `estimate_prune_reconcile().should_skip()` (B7.9 deepen).
    ...
    /// Non-mutating skip predicates — mirror estimate `should_skip` helpers (B7.9 deepen).
    /// True when `estimate_reconcile_invalidation` reports no pending work (B7.9 deepen).
...
/// True when combined reconcile estimate reports nothing to invalidate (B7.9 deepen).
