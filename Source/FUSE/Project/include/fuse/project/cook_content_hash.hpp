...
/// Non-mutating hash preflight predicates — same guards as `preflight_*` (B7.9 deepen).
[[nodiscard]] bool should_skip_file_content_hash(const std::string& path);
[[nodiscard]] bool should_skip_mesh_import_hash(const MeshImportDesc& desc);
[[nodiscard]] bool should_skip_texture_import_hash(const TextureImportDesc& desc);
[[nodiscard]] bool should_skip_audio_import_hash(const AudioImportDesc& desc);
...
/// Convenience guards — `preflight_*_hash(...).should_skip()` (B7.9 deepen).
[[nodiscard]] bool should_skip_file_content_hash(const std::string& path);
[[nodiscard]] bool should_skip_mesh_import_hash(const MeshImportDesc& desc);
[[nodiscard]] bool should_skip_texture_import_hash(const TextureImportDesc& desc);
[[nodiscard]] bool should_skip_audio_import_hash(const AudioImportDesc& desc);
...
/// True when hash preflight would reject inputs — mirrors `preflight_*` guards (B7.9 deepen).
[[nodiscard]] bool should_skip_file_content_hash(const std::string& path);
[[nodiscard]] bool should_skip_mesh_import_hash(const MeshImportDesc& desc);
[[nodiscard]] bool should_skip_texture_import_hash(const TextureImportDesc& desc);
[[nodiscard]] bool should_skip_audio_import_hash(const AudioImportDesc& desc);
...
/// True when hash preflight rejects the input — mirrors `preflight_*` guards (B7.9 deepen).
[[nodiscard]] bool should_skip_file_content_hash(const std::string& path);
[[nodiscard]] bool should_skip_mesh_import_hash(const MeshImportDesc& desc);
[[nodiscard]] bool should_skip_texture_import_hash(const TextureImportDesc& desc);
[[nodiscard]] bool should_skip_audio_import_hash(const AudioImportDesc& desc);
...
/// Non-mutating skip predicates — mirror `CookHashPreflight::should_skip` (B7.9 deepen).
[[nodiscard]] bool should_skip_file_content_hash(const std::string& path);
[[nodiscard]] bool should_skip_mesh_import_hash(const MeshImportDesc& desc);
[[nodiscard]] bool should_skip_texture_import_hash(const TextureImportDesc& desc);
[[nodiscard]] bool should_skip_audio_import_hash(const AudioImportDesc& desc);
...
/// True when the corresponding hash helper would return zero — read-only preflight shortcuts (B7.9 deepen).
[[nodiscard]] bool should_skip_file_content_hash(const std::string& path);
[[nodiscard]] bool should_skip_mesh_import_hash(const MeshImportDesc& desc);
[[nodiscard]] bool should_skip_texture_import_hash(const TextureImportDesc& desc);
[[nodiscard]] bool should_skip_audio_import_hash(const AudioImportDesc& desc);
...
[[nodiscard]] bool should_skip_fnv1a64_bytes(const u8* data, usize size);
[[nodiscard]] bool should_skip_combine_cook_cache_key(u64 source_hash, u64 upstream_hash);
[[nodiscard]] bool should_skip_hash_preflight(const CookHashPreflight& preflight);
...
/// Read-only hash skip probes — mirror `preflight_*` guards without computing keys (B7.9 deepen).
[[nodiscard]] bool should_skip_file_content_hash(const std::string& path);
[[nodiscard]] bool should_skip_mesh_import_hash(const MeshImportDesc& desc);
[[nodiscard]] bool should_skip_texture_import_hash(const TextureImportDesc& desc);
[[nodiscard]] bool should_skip_audio_import_hash(const AudioImportDesc& desc);
...
/// True when hash preflight rejects the input — mirrors `CookHashPreflight::should_skip` (B7.9 deepen).
[[nodiscard]] inline bool should_skip_file_content_hash(const std::string& path) {
    return preflight_file_content_hash(path).should_skip();
}
[[nodiscard]] inline bool should_skip_mesh_import_hash(const MeshImportDesc& desc) {
...
[[nodiscard]] inline bool should_skip_fnv1a64_bytes(const u8* data, usize size) {
...
/// True when hash preflight rejects the input — mirrors `preflight_*` without computing keys (B7.9 deepen).
[[nodiscard]] bool should_skip_file_content_hash(const std::string& path);
[[nodiscard]] bool should_skip_mesh_import_hash(const MeshImportDesc& desc);
[[nodiscard]] bool should_skip_texture_import_hash(const TextureImportDesc& desc);
[[nodiscard]] bool should_skip_audio_import_hash(const AudioImportDesc& desc);
...
/// True when hash preflight rejects the input — read-only skip guards (B7.9 deepen).
[[nodiscard]] bool should_skip_file_content_hash(const std::string& path);
[[nodiscard]] bool should_skip_mesh_import_hash(const MeshImportDesc& desc);
[[nodiscard]] bool should_skip_texture_import_hash(const TextureImportDesc& desc);
[[nodiscard]] bool should_skip_audio_import_hash(const AudioImportDesc& desc);
...
/// True when hash preflight would reject inputs — mirrors `CookHashPreflight::should_skip` (B7.9 deepen).
[[nodiscard]] inline bool should_skip_cook_hash_preflight(const CookHashPreflight& preflight) {
    return preflight.should_skip();
}
...
/// Read-only hash preflight skip guards — mirror `preflight_*` without computing keys (B7.9 deepen).
[[nodiscard]] bool should_skip_file_content_hash(const std::string& path);
[[nodiscard]] bool should_skip_mesh_import_hash(const MeshImportDesc& desc);
[[nodiscard]] bool should_skip_texture_import_hash(const TextureImportDesc& desc);
[[nodiscard]] bool should_skip_audio_import_hash(const AudioImportDesc& desc);
...
[[nodiscard]] bool should_skip_fnv1a64_bytes(const u8* data, usize size);
[[nodiscard]] bool should_skip_combine_cook_cache_key(u64 source_hash, u64 upstream_hash);
/// True when `content_hash` is zero — mirrors lookup/store guards (B7.9 deepen).
[[nodiscard]] bool should_skip_cache_lookup(u64 content_hash);
