...
void testCookHashShouldSkipGuards() {
    ...
    expectTrue(fuse::project::should_skip_cook_cache_key(0, 42u), "should_skip rejects zero source key");
    expectTrue(!fuse::project::should_skip_cook_cache_key(99u, 42u), "should_skip allows valid source key");
    ...
    expectTrue(!fuse::project::should_skip_combine_cook_cache_key(99u, 0),
               "should_skip combine allows zero upstream");

    fuse::project::MeshImportDesc mesh;
    mesh.input_path = source;
    mesh.output_path = "/tmp/fuse_b79_should_skip_mesh.fusemesh";
    expectTrue(!fuse::project::should_skip_mesh_import_hash(mesh), "should_skip mesh import false for valid desc");
    expectTrue(!fuse::project::preflight_mesh_import_hash(mesh).should_skip(),
               "preflight should_skip matches standalone mesh guard");

    fuse::project::CookManifest manifest;
    expectTrue(fuse::project::should_skip_upstream_dependencies_hash({}, manifest),
               "should_skip upstream dependencies for empty list");
    expectTrue(fuse::project::preflight_upstream_dependencies_hash({}, manifest).should_skip(),
...
void testCookHashPreflightShouldSkipGuards() {
    ...
    expectTrue(fuse::project::preflight_mesh_import_hash(desc).should_skip() ==
                   fuse::project::should_skip_mesh_import_hash(desc),
    ...
    expectTrue(cooker.cache().would_prune_all(), "stale entry makes would_prune_all true");
    ...
    fuse::project::MeshImportDesc valid_desc;
    valid_desc.input_path = source;
    valid_desc.output_path = "/tmp/fuse_b79_preflight_mesh.fusemesh";
...
void testCookCacheWouldInvalidateProbes() {
    ...
    expectTrue(cooker.cache().would_prune_all(), "would_prune_all agrees with non-skipping estimate");
...
void testCookHashShouldSkipGuards() {
    ...
    expectTrue(!fuse::project::should_skip_mesh_import_hash(mesh),
               "should_skip mesh import ok for readable source");
    expectTrue(fuse::project::preflight_mesh_import_hash(mesh).should_skip() ==
                   fuse::project::should_skip_mesh_import_hash(mesh),
...
void testCookHashShouldSkipHelpers() {
    expectTrue(fuse::project::should_skip_file_content_hash(""),
...
void testCookCacheShouldSkipAndWouldInvalidateHelpers() {
    fuse::project::CookCache cache;
    ...
    writeTempFile(source, "# should skip cache updated\n");
    const fuse::u64 revised_hash = fuse::project::hash_mesh_import(desc);
    expectTrue(revised_hash != seeded.content_hash, "source change yields new content hash");
    expectTrue(cooker.cache().would_invalidate_stale_content_for_source(source, revised_hash),
               "would_invalidate_stale_content true when current hash differs from stored");
...
void testCookHashShouldSkipHelpers() {
    ...
    const std::string source = writeTempFile("/tmp/fuse_b79_skip_mesh.obj", "# skip mesh\n");
    fuse::project::MeshImportDesc desc;
    desc.input_path = source;
    desc.output_path = "/tmp/fuse_b79_skip_mesh.fusemesh";
    ...
    expectTrue(fuse::project::should_skip_mesh_import_hash(desc)
                   == fuse::project::preflight_mesh_import_hash(desc).should_skip_hash(),
    ...
    fuse::project::CookManifest manifest;
    expectTrue(fuse::project::should_skip_upstream_dependencies_hash({}, manifest),
               "empty dependency list skips upstream hash");
}

void testCookCacheShouldSkipStoreAndLookup() {
    fuse::project::CookCache cache;

    fuse::project::CookCacheEntry invalid;
    invalid.content_hash = 0;
    invalid.source_path = "/tmp/fuse_b79_skip_store.obj";
    invalid.output_path = "/tmp/fuse_b79_skip_store.fusemesh";
    expectTrue(fuse::project::should_skip_cache_store(invalid), "invalid entry skips store");
    cache.store(invalid);
    expectTrue(cache.empty(), "store skipped for invalid entry");
    ...
    valid.source_path = "/tmp/fuse_b79_skip_lookup.obj";
    valid.output_path = "/tmp/fuse_b79_skip_lookup.fusemesh";
    ...
    expectTrue(cache.lookup(valid.content_hash) == fuse::project::CookCacheLookup::Hit,
               "should_skip_cache_lookup false implies lookup hits");
}

void testCookCacheWouldInvalidateHelpers() {
    fuse::project::CookCache cache;
    expectTrue(!cache.would_invalidate_source("/tmp/fuse_b79_would_src.obj"),
               "would_invalidate_source false on empty cache");
    expectTrue(!cache.would_invalidate_output("/tmp/fuse_b79_would_out.fusemesh"),
...
void testCookHashShouldSkipGuards() {
    ...
    expectTrue(!fuse::project::should_skip_combine_cook_cache_key(99u, 0),
               "combine cache key allows zero upstream");
    expectTrue(fuse::project::should_skip_fnv1a64_bytes(nullptr, 4u),
               "null bytes with non-zero size should_skip");
    expectTrue(!fuse::project::should_skip_fnv1a64_bytes(nullptr, 0),
...
void testCookHashPreflightShouldSkipHelpers() {
    ...
    expectTrue(fuse::project::should_skip_mesh_import_hash(desc) ==
               ...
               "should_skip_mesh_import_hash matches preflight should_skip");
...
void testCookHashPreflightShouldSkipHelpers() {
    ...
    desc.output_path = "/tmp/fuse_b79_should_skip_mesh.fusemesh";
    expectTrue(!fuse::project::should_skip_mesh_import_hash(desc),
               "readable mesh import should not skip hash preflight");
    expectTrue(!fuse::project::preflight_mesh_import_hash(desc).should_skip(),
    ...
    expectTrue(fuse::project::should_skip_hash_preflight(
    ...
    expectTrue(!fuse::project::should_skip_fnv1a64_bytes(nullptr, 0),
               "zero-size null bytes should not skip fnv1a64 preflight");
...
void testCookCacheWouldInvalidateProbes() {
    ...
    expectTrue(cooker.cache().would_invalidate_output(desc.output_path),
               "would_invalidate_output true for seeded entry");
    expectTrue(cooker.cache().would_invalidate_all(),
               "would_invalidate_all true when cache populated");
    expectTrue(!cooker.cache().would_invalidate_stale_content_for_source(source, seeded.content_hash),
               "would_invalidate_stale_content false when hash matches");
    expectTrue(cooker.cache().would_invalidate_stale_content_for_source(source, seeded.content_hash + 1u),
               "would_invalidate_stale_content true when hash mismatches");
...
void testCookHashPreflightShouldSkipHelpers() {
    ...
    expectTrue(fuse::project::should_skip_cook_cache_key(0, 42u),
               "zero source cache key should_skip");
    expectTrue(!fuse::project::should_skip_combine_cook_cache_key(99u, 0),
               "valid source with zero upstream does not skip combine key");
    expectTrue(fuse::project::should_skip_fnv1a64_bytes(nullptr, 4u),
               "null bytes with non-zero size should_skip");
    expectTrue(!fuse::project::should_skip_fnv1a64_bytes(nullptr, 0),
               "zero-size null bytes does not skip");
}
...
void testCookCacheWouldInvalidateAndShouldSkipHelpers() {
    ...
    const std::string source = writeTempFile("/tmp/fuse_b79_skip_inv_mesh.obj", "# skip inv\n");
    fuse::project::MeshImportDesc desc;
    desc.input_path = source;
    desc.output_path = "/tmp/fuse_b79_skip_inv_mesh.fusemesh";
    ...
    writeTempFile(source, "# skip inv updated\n");
    expectTrue(cooker.cache().would_prune_all(), "stale entry would_prune_all");
    expectTrue(!cooker.cache().should_skip_prune_all(), "stale entry should not skip prune_all");
    ...
    const fuse::project::CookCachePruneEstimate estimate = cooker.cache().estimate_prune_removals();
    expectTrue(estimate.would_prune(), "prune estimate would_prune on stale entry");
...
void testCookHashPreflightFnvAndCombineGuards() {
    ...
    expectTrue(!fuse::project::should_skip_combine_cook_cache_key(99u, 0),
               "combine cache key should not skip valid source-only fold");
    expectTrue(fuse::project::preflight_combine_cook_cache_key(99u, 42u).ok(),
...
void testCookCacheWouldInvalidationProbes() {
    ...
    expectTrue(fuse::project::should_skip_invalidate(42u, cache),
               "should_skip_invalidate on empty cache is true");
    ...
    fuse::project::CookCacheEntry zero_entry;
    zero_entry.content_hash = 0;
    zero_entry.source_path = source;
    zero_entry.output_path = desc.output_path;
    expectTrue(fuse::project::should_skip_store_cache_entry(zero_entry),
               "should_skip_store rejects zero-hash entry");

    expectTrue(updated_hash != seeded.content_hash, "content change yields new hash for stale probe");
    expectTrue(cooker.cache().would_invalidate_stale_content_for_source(source, updated_hash),
...
void testCookCacheEntryPreflightGuards() {
    ...
    expectTrue(!fuse::project::should_skip_store_cache_entry(valid),
               "valid cache entry should not skip store preflight");
    ...
    expectTrue(fuse::project::should_skip_store_cache_entry(zero_hash),
               "zero-hash entry should skip store preflight");
...
void testCookHashShaderAndManifestUpstreamPreflights() {
    ...
    expectTrue(!fuse::project::preflight_shader_entry_hash(shader).should_skip(),
               "shader preflight should not skip with valid paths");
...
void testCookCacheProbeStaleUpstreamSourcePaths() {
    ...
    expectTrue(!cache.would_invalidate_stale_upstream_hashes({}), "empty upstream list guarded");
               "cacheable key preflight rejects zero fold");
               "cacheable key preflight ok for valid source-only fold");
               "cacheable key preflight ok for valid combined fold");
    ...
    expectTrue(zero_hash.reason == fuse::project::CookCacheRejectReason::ZeroContentHash,
               ...
               "empty output path entry preflight reason");

    expectTrue(fuse::project::preflight_cook_cache_entry(invalid).ok(), "valid entry passes preflight");
    expectTrue(std::string(fuse::project::cookCacheRejectReasonLabel(
               ...
               "cache entry reject reason label");
    ...
    expectTrue(cache.count_invalidate_all() == 0u, "count_invalidate_all on empty cache is zero");
    ...
    expectTrue(cooker.cache().count_invalidate_all() == 1u, "count_invalidate_all matches entry count");
...
void testCookCacheWouldInvalidateSourceAndOutput() {
    ...
    const std::string source = writeTempFile("/tmp/fuse_b79_store_preflight.obj", "# store preflight\n");

    valid.output_path = "/tmp/fuse_b79_store_preflight.fusemesh";

    expectTrue(ok.ok(), "readable mesh cache entry passes store preflight");

    expectTrue(!cache.preflight_store_entry(zero_key).ok(), "zero content hash fails store preflight");
    expectTrue(cache.preflight_store_entry(zero_key).reason == fuse::project::CookHashRejectReason::ZeroSourceHash,

    expectTrue(!cache.preflight_store_entry(empty_source).ok(), "empty source fails store preflight");

    fuse::project::CookCacheEntry missing_source = valid;
    missing_source.source_path = "/tmp/fuse_b79_missing_store_preflight.obj";
    expectTrue(!cache.preflight_store_entry(missing_source).ok(), "missing source fails store preflight");
    expectTrue(cache.preflight_store_entry(missing_source).reason ==

    expectTrue(cache.preflight_store_entry(shader_entry).ok(), "shader entry passes structural store preflight");


    entry_a.content_hash = 1001;

    entry_b.content_hash = 1002;

    fuse::project::CookCacheEntry entry_dup = entry_a;
    entry_dup.content_hash = 1003;
    entry_dup.output_path = "/tmp/fuse_b79_unique_upstream_a_alt.fusemesh";
    cache.store(entry_dup);

        {entry_b.source_path, 99u},

    expectTrue(per_entry.size() == 3u, "per-entry stale upstream probe counts duplicate source paths");

    expectTrue(unique[0] == entry_a.source_path || unique[1] == entry_a.source_path,
               "unique probe includes first stale source");
    expectTrue(unique[0] == entry_b.source_path || unique[1] == entry_b.source_path,
               "unique probe includes second stale source");
...
void testCookCacheInvalidationReconcileEstimate() {
        cache.estimate_invalidation_reconcile({{"/tmp/fuse_b79_est_up.obj", 1u}});
    expectTrue(empty.total() == 0u, "empty cache invalidation reconcile estimate is zero");

    shader_entry.content_hash = 909;
    shader_entry.source_path = "/tmp/fuse_b79_est_inv_shader.obj";
    shader_entry.output_path = "/tmp/fuse_b79_est_inv_shader.fuseshader";
    expectTrue(cache.entry_count() == 1u, "shader entry stored for invalidation reconcile estimate");

        cache.estimate_invalidation_reconcile({});
    expectTrue(estimate.prune_stale_entries == 1u, "shader entry counted in prune stale bucket");
    expectTrue(estimate.stale_upstream_entries == 0u, "no stale upstream without upstream probe list");
    expectTrue(estimate.total() == estimate.prune_stale_entries + estimate.prune_invalid_entries +
                                    estimate.stale_upstream_entries,
               "invalidation reconcile total sums buckets");

        {shader_entry.source_path, 99u},
    const fuse::project::CookCacheInvalidationEstimate upstream_estimate =
        cache.estimate_invalidation_reconcile(stale_upstream);
    expectTrue(upstream_estimate.stale_upstream_entries == 1u,
               "stale upstream hash counted in invalidation reconcile estimate");
    expectTrue(upstream_estimate.total() >= 1u, "stale upstream increases reconcile total");

void testCookHashPreflightCookCacheEntry() {
    expectTrue(!invalid_preflight.ok(), "zero-hash cache entry fails preflight");
    expectTrue(invalid_preflight.reason == fuse::project::CookHashRejectReason::InvalidCacheEntry,
               "zero-hash cache entry preflight reason is InvalidCacheEntry");

    valid.content_hash = 77;
    expectTrue(valid_preflight.ok(), "readable mesh cache entry passes preflight");

    shader_entry.output_path = "/tmp/fuse_b79_preflight_entry_shader.fuseshader";
               "shader cache entry preflight reason is UnsupportedAssetKind");


    expectTrue(fuse::project::preflight_cook_cache_entry(valid).ok(), "valid entry passes store preflight");

    fuse::project::CookCacheEntry zero_key;
    zero_key.source_path = valid.source_path;
    zero_key.output_path = valid.output_path;
    expectTrue(!fuse::project::preflight_cook_cache_entry(zero_key).ok(), "zero key fails store preflight");
                   fuse::project::CookCacheEntryRejectReason::ZeroContentHash,
               "zero key preflight reason");

    empty_source.output_path = valid.output_path;
    ...
    expectTrue(std::string(fuse::project::cookCacheEntryRejectReasonLabel(
               ...
               "entry reject reason label");

void testCookHashPreflightUnknownDependencyAndShaderGuards() {
    entry.source_path = writeTempFile("/tmp/fuse_b79_unknown_dep.obj", "# unknown dep\n");
    entry.output_path = "/tmp/fuse_b79_unknown_dep.fusemesh";

    const fuse::project::CookHashPreflight unknown_dep =
        fuse::project::preflight_upstream_dependencies_hash({"/tmp/fuse_b79_missing_output.fusemesh"}, manifest);
    expectTrue(!unknown_dep.ok(), "unknown dependency output fails upstream preflight");
    expectTrue(unknown_dep.reason == fuse::project::CookHashRejectReason::UnknownDependency,
               "unknown dependency preflight reason");

    fuse::project::CookManifestEntry shader_entry;
    shader_entry.source_path = "/tmp/fuse_b79_shader_preflight.glsl";
    shader_entry.output_path = "/tmp/fuse_b79_shader_preflight.spv";
    expectTrue(!fuse::project::preflight_shader_manifest_hash(shader_entry).ok(),
               "shader manifest entry fails hash preflight");
    expectTrue(fuse::project::preflight_manifest_entry_hash(shader_entry).reason ==
                   fuse::project::CookHashRejectReason::UnsupportedKind,
               "shader manifest entry preflight reason");
                   fuse::project::CookHashRejectReason::UnknownDependency)) == "unknown_dependency",
               "unknown dependency reject label");

void testCookCacheInvalidationWouldProbes() {
    ...
    expectTrue(cooker.cook_mesh(desc).ok, "seed cook for would probes ok");
    expectTrue(cooker.cache().would_invalidate_all(), "would_invalidate_all true when cache populated");
    expectTrue(!cooker.cache().probe_stale_content_for_source(source),
               "fresh source not probed as stale content");

    writeTempFile(source, "# would probe revised\n");
    expectTrue(cooker.cache().probe_stale_content_for_source(source),
               "revised source probed as stale content");

void testCookCacheStaleUpstreamUniqueProbes() {

    desc_a.input_path = source;
    desc_b.input_path = source;
    const fuse::u64 hash_a = fuse::project::hash_mesh_import(desc_a);
    const fuse::u64 hash_b = fuse::project::hash_mesh_import(desc_b);
    expectTrue(hash_a != 0 && hash_b != 0, "mesh hashes non-zero for unique upstream probe");

    entry_a.content_hash = fuse::project::combine_cook_cache_key(hash_a, 1u);
    entry_a.upstream_hash = 1u;
    entry_a.output_path = desc_a.output_path;
    entry_a.kind = fuse::project::CookAssetKind::Mesh;

    entry_b.content_hash = fuse::project::combine_cook_cache_key(hash_b, 1u);
    entry_b.upstream_hash = 1u;
    entry_b.source_path = source;
    entry_b.output_path = desc_b.output_path;
    entry_b.kind = fuse::project::CookAssetKind::Mesh;
    expectTrue(cache.entry_count() == 2u, "two entries share source for dedup probe");

    const std::vector<std::pair<std::string, fuse::u64>> stale_upstream{{source, 999u}};
    const std::vector<std::string> duplicated = cache.probe_stale_upstream_sources(stale_upstream);
    expectTrue(duplicated.size() == 2u, "non-unique upstream probe duplicates per entry");

    expectTrue(unique.size() == 1u, "unique upstream probe deduplicates sources");
    expectTrue(cache.count_stale_upstream_sources_unique(stale_upstream) == 1u,
               "unique upstream count matches deduplicated probe");
    expectTrue(cache.count_reconcile_overlap_entries(stale_upstream) == 0u,
               "no overlap when content is fresh");
    ...
    const std::string source = writeTempFile("/tmp/fuse_b79_preflight_store_ok.obj", "# store ok\n");
    ...
    const std::vector<std::string> duplicated =
    expectTrue(duplicated.size() >= 2u, "non-unique probe may repeat stale sources");

        cooker.cache().probe_unique_stale_upstream_sources(stale_upstream);
    expectTrue(unique.size() == 2u, "unique probe deduplicates stale upstream sources");
               "would_invalidate_stale_upstream true when stale upstream present");

void testCookHashWouldAndTryPreflightGuards() {

    expectTrue(!fuse::project::wouldHashFileContent(""), "wouldHashFileContent rejects empty path");
    ...
    expectTrue(fuse::project::classifyCookHashReject(
                   fuse::project::preflight_file_content_hash("")) ==
                   fuse::project::CookHashRejectReason::EmptyPath,
               "classifyCookHashReject maps empty path");

    const std::string source = writeTempFile("/tmp/fuse_b79_would_hash_mesh.obj", "# would hash\n");
    desc.output_path = "/tmp/fuse_b79_would_hash_mesh.fusemesh";
    expectTrue(fuse::project::wouldHashMeshImport(desc), "wouldHashMeshImport accepts readable source");
    ...
    desc.input_path = "";
    expectTrue(!fuse::project::wouldHashMeshImport(desc), "wouldHashMeshImport rejects empty input");
    expectTrue(!fuse::project::tryPreflightMeshImportHash(desc, reason),
               "tryPreflightMeshImportHash rejects empty input");
    expectTrue(reason == fuse::project::CookHashRejectReason::EmptyInputPath,
               "empty mesh input tryPreflight reason");

    tex.output_path = "/tmp/fuse_b79_would_hash_tex.fusetex";
    expectTrue(fuse::project::wouldHashTextureImport(tex), "wouldHashTextureImport ok");
    expectTrue(fuse::project::tryPreflightTextureImportHash(tex, reason), "tryPreflightTextureImportHash ok");

    audio.output_path = "/tmp/fuse_b79_would_hash_audio.fuseaudio";
    expectTrue(fuse::project::wouldHashAudioImport(audio), "wouldHashAudioImport ok");
    expectTrue(fuse::project::tryPreflightAudioImportHash(audio, reason), "tryPreflightAudioImportHash ok");

    entry.output_path = "/tmp/fuse_b79_would_hash_manifest.fusemesh";
    expectTrue(fuse::project::wouldHashManifestEntry(entry), "wouldHashManifestEntry ok");
    expectTrue(fuse::project::tryPreflightManifestEntryHash(entry, reason),
               "tryPreflightManifestEntryHash ok");

    expectTrue(fuse::project::wouldHashUpstreamDependencies({entry.output_path}, manifest),
               "wouldHashUpstreamDependencies ok");
    ...
    expectTrue(!fuse::project::wouldHashCookCacheKey(0, 42u), "wouldHashCookCacheKey rejects zero source");
    ...
    expectTrue(fuse::project::wouldHashCookCacheKey(99u, 0), "wouldHashCookCacheKey allows zero upstream");
    ...
    writeTempFile(source, "# would inv revised\n");
    expectTrue(cooker.cache().would_prune_all(), "would_prune_all true after source change");
...
void testCookHashPreflightDependencyGuards() {
    const std::string source = writeTempFile("/tmp/fuse_b79_preflight_dep.obj", "# preflight dep\n");
    const std::string dependency = writeTempFile("/tmp/fuse_b79_preflight_dep_upstream.obj", "# upstream dep\n");

    entry.dependencies.push_back(dependency);
               "manifest entry preflight walks readable dependencies");
    expectTrue(!fuse::project::should_skip_manifest_entry_hash(entry),
               "readable manifest entry should not skip hash");

    entry.dependencies.push_back("/tmp/fuse_b79_missing_preflight_dep.obj");
    const fuse::project::CookHashPreflight missing_dep =
        fuse::project::preflight_manifest_entry_hash(entry);
    expectTrue(!missing_dep.ok(), "unreadable manifest dependency fails preflight");
    expectTrue(missing_dep.reason == fuse::project::CookHashRejectReason::SourceUnreadable,

    upstream.source_path = dependency;
    upstream.output_path = "/tmp/fuse_b79_preflight_dep_upstream.fusemesh";

    expectTrue(fuse::project::preflight_upstream_dependencies_hash({upstream.output_path}, manifest).ok(),
               "known manifest dependency output passes upstream preflight");
    expectTrue(fuse::project::should_skip_manifest_entry_hash(entry),
    ...
    expectTrue(!fuse::project::should_skip_upstream_dependencies_hash({upstream.output_path}, manifest),
               "known dependency output should not skip upstream hash");

    const fuse::project::CookHashPreflight unknown_output =
        fuse::project::preflight_upstream_dependencies_hash({"/tmp/fuse_b79_unknown_dep.fusemesh"}, manifest);
    expectTrue(!unknown_output.ok(), "unknown dependency output fails upstream preflight");
    expectTrue(unknown_output.reason == fuse::project::CookHashRejectReason::UnknownDependencyOutput,
               "unknown dependency output preflight reason");
                   fuse::project::CookHashRejectReason::UnknownDependencyOutput)) == "unknown_dependency_output",
               "reject reason label for unknown dependency output");

void testCookCacheStaleUpstreamProbeDedup() {

    entry_a.content_hash = 901u;
    entry_a.upstream_hash = 42u;
    entry_a.output_path = "/tmp/fuse_b79_upstream_dedup_a.fusemesh";

    entry_b.content_hash = 902u;
    entry_b.upstream_hash = 42u;
    entry_b.output_path = "/tmp/fuse_b79_upstream_dedup_b.fusemesh";
    expectTrue(cache.entry_count() == 2u, "two entries seeded for upstream dedup probe");

    const std::vector<std::pair<std::string, fuse::u64>> stale_upstream = {{source, 1u}};
    ...
    expectTrue(probed.size() == 1u, "stale upstream probe deduplicates shared source path");
    expectTrue(probed[0] == source, "deduplicated stale upstream probe reports shared source");
    expectTrue(cache.count_stale_upstream_sources(stale_upstream) == 1u,
               "stale upstream count matches deduplicated probe size");
...
void testCookCacheEntryPreflightAndStoreSkip() {
    ...
    expectTrue(valid_preflight.ok(), "structurally valid cache entry passes preflight");
...
void testCookCacheShouldSkipAndWouldInvalidateGuards() {
    ...
    const fuse::u64 new_hash = fuse::project::hash_mesh_import(desc);
    expectTrue(new_hash != seeded.content_hash, "content change yields new hash key");
...
void testCookHashPreflightShouldSkipHelpers() {
    expectTrue(!fuse::project::should_skip_cook_cache_key(99u, 0),
...
void testCookHashPreflightShouldSkipGuards() {
               "preflight should_skip for empty path");
               "should_skip_file_content_hash mirrors preflight for empty path");
    ...
    expectTrue(fuse::project::should_skip_mesh_import_hash(desc), "empty mesh input should skip hash");

               "zero source hash should skip cache key");
               "valid source with zero upstream should not skip combine key");
               "null bytes with non-zero size should skip FNV hash");
               "zero-size null bytes should not skip FNV hash");

               "empty dependency list should skip upstream hash");

void testCookCacheEntryPreflightAndStoreSkipGuards() {
    expectTrue(valid_preflight.ok(), "valid cache entry passes preflight");
    expectTrue(!fuse::project::should_skip_cook_cache_store(valid), "valid entry should not skip store");

    expectTrue(fuse::project::preflight_cook_cache_entry(invalid).should_skip(),
               "zero hash cache entry preflight should skip");
    expectTrue(fuse::project::should_skip_cook_cache_store(invalid), "invalid entry should skip store");

    expectTrue(cache.entry_count() == 0u, "store still rejects invalid entry via existing guard");

void testCookCacheWouldInvalidateHelpers() {
    expectTrue(cache.should_skip_prune_all(), "empty cache should skip prune_all");



               "would_invalidate_source true for seeded source");
               "would_invalidate_output true for seeded output");

    expectTrue(!cooker.cache().should_skip_prune_all(), "stale entry should not skip prune_all");

    expectTrue(estimate.would_prune(), "estimate would_prune mirrors would_prune_all");
    ...
    expectTrue(!fuse::project::preflight_cook_cache_key(99u, 42u).should_skip(),
               "valid cache key preflight should not skip");
...
void testCookCacheWouldInvalidateAndShouldSkipGuards() {
    ...
    invalid.source_path = "/tmp/fuse_b79_skip_invalid.obj";
    invalid.output_path = "/tmp/fuse_b79_skip_invalid.fusemesh";

    const std::string source = writeTempFile("/tmp/fuse_b79_skip_mesh.obj", "# skip mesh\n");
    desc.output_path = "/tmp/fuse_b79_skip_mesh.fusemesh";

    expectTrue(seeded.ok, "seed cook for skip/would guards ok");
    ...
    expectTrue(revised_hash != seeded.content_hash, "content change yields new hash for stale probe");
               "would_invalidate_stale_content true when current hash differs from stored entry");
    ...
    fuse::project::CookCacheEntry valid_entry;
    valid_entry.content_hash = seeded.content_hash;
    valid_entry.source_path = source;
    valid_entry.output_path = desc.output_path;
    valid_entry.kind = fuse::project::CookAssetKind::Mesh;
    expectTrue(!cooker.cache().should_skip_store(valid_entry), "should_skip_store accepts valid entry");

               "matching hash should skip stale-content invalidation");
               "mismatched hash would invalidate stale content");

    writeTempFile(source, "# skip mesh updated\n");
    expectTrue(cooker.cache().would_prune_all(), "stale entry would_prune_all");
    expectTrue(cooker.cache().estimate_prune_removals().should_skip() == false,
               "stale prune estimate should not skip");
    ...
    fuse::project::CookCacheEntry invalid_entry;
    invalid_entry.content_hash = 0;
    invalid_entry.source_path = source;
    invalid_entry.output_path = desc.output_path;
    expectTrue(cooker.cache().should_skip_store(invalid_entry),
               "should_skip_store true for invalid entry");

    expectTrue(!cooker.cache().should_skip_store(valid_entry),
               "should_skip_store false for valid entry");

void testCookCacheShouldSkipStoreGuard() {
    valid.source_path = "/tmp/fuse_b79_skip_store.obj";
    valid.output_path = "/tmp/fuse_b79_skip_store.fusemesh";
...
void testCookHashShouldSkipHelpers() {
    ...
    desc.output_path = "/tmp/fuse_b79_should_skip.fusemesh";
               "readable mesh import does not skip hash preflight");
               "should_skip_mesh mirrors preflight should_skip");

               "empty dependency list skips upstream hash preflight");
    ...
    expectTrue(cooker.cache().estimate_prune_removals().should_skip() ==
                   cooker.cache().should_skip_prune_all(),
    ...
    const std::string source = writeTempFile("/tmp/fuse_b79_would_mesh.obj", "# would invalidate mesh\n");



    writeTempFile(source, "# would invalidate mesh revised\n");
    expectTrue(revised_hash != seeded.content_hash, "revised source produces new content hash");
...
void testCookCacheWouldInvalidateAndShouldSkipProbes() {
    ...
    expectTrue(revised_hash != seeded.content_hash, "disk change alters content hash");
    ...
    expectTrue(cooker.cache().would_prune_all(),
               "would_prune_all true after source revision makes entry stale");

    const fuse::project::CookCachePruneEstimate prune = cooker.cache().estimate_prune_removals();
...
void testCookCachePruneReconcileShouldSkipGuards() {
    ...
    shader_entry.content_hash = 909u;
    shader_entry.source_path = "/tmp/fuse_b79_should_skip_shader.obj";
    shader_entry.output_path = "/tmp/fuse_b79_should_skip_shader.fuseshader";
    ...
    expectTrue(cache.would_prune_all(), "shader stale cache would_prune_all is true");
    ...
    expectTrue(cooker.cache().would_prune_all(), "would_prune_all true when entry is stale on disk");
    expectTrue(null_bytes.should_skip(), "null bytes preflight should_skip");
    expectTrue(!empty_bytes.should_skip(), "zero-size null bytes preflight should not skip");
               "should_skip_combine_cook_cache_key rejects zero source");
...
void testCookCacheWouldInvalidateNamedProbes() {

    const std::string source = writeTempFile("/tmp/fuse_b79_would_probe_mesh.obj", "# would probe\n");
    desc.output_path = "/tmp/fuse_b79_would_probe_mesh.fusemesh";
...
void testCookHashPreflightShouldSkipGuards() {
    ...
    desc.output_path = "/tmp/fuse_b79_should_skip_mesh.fusemesh";
    expectTrue(!fuse::project::preflight_mesh_import_hash(desc).should_skip(),
               "valid mesh preflight should not skip");
    expectTrue(!fuse::project::should_skip_mesh_import_hash(desc),
               "should_skip_mesh_import_hash false for valid mesh");
...
void testCookHashShouldSkipGuards() {
    ...
    expectTrue(cooker.cache().invalidate(seeded.content_hash), "invalidate removes seeded entry");
    expectTrue(cooker.cache().should_skip_invalidate(seeded.content_hash),
               "should_skip_invalidate true after removal");
    expectTrue(!cooker.cache().would_invalidate(seeded.content_hash),
               "would_invalidate false after removal");
void testCookCacheWouldInvalidateUpstreamAndDownstreamProbes() {
    ...
    fuse::project::CookCacheEntry cached_b;
    expectTrue(cooker.cache().lookup(cooked.records[1].content_hash, &cached_b) ==
                   fuse::project::CookCacheLookup::Hit,
               "downstream entry cached for upstream probe");
    const fuse::u64 matching_upstream = cached_b.upstream_hash;

    expectTrue(!cooker.cache().would_invalidate_stale_upstream_hashes({}),
               "empty upstream list would_invalidate is false");
    ...
    const std::vector<std::pair<std::string, fuse::u64>> stale_upstream = {{source_b, matching_upstream + 1u}};
    expectTrue(cooker.cache().would_invalidate_stale_upstream_hashes(stale_upstream),
    ...
    expectTrue(!cooker.cache().would_invalidate_source(""),
               "would_invalidate_source guarded on empty path");
    expectTrue(!cooker.cache().would_invalidate(0),
               "would_invalidate guarded on zero hash");

    expectTrue(cooker.cache().would_invalidate(seeded.content_hash) ==
                   cooker.cache().count_by_source(source) >= 1u,
               "would_invalidate hash aligns with populated cache");
}

void testCookCachePruneEstimateShouldSkipHelpers() {
    fuse::project::CookCache cache;
    const fuse::project::CookCachePruneEstimate empty = cache.estimate_prune_removals();
    expectTrue(empty.should_skip(), "empty prune estimate should_skip is true");
    ...
    fuse::project::CookCacheEntry shader_entry;
    shader_entry.content_hash = 909;
    shader_entry.source_path = "/tmp/fuse_b79_skip_shader.obj";
    shader_entry.output_path = "/tmp/fuse_b79_skip_shader.fuseshader";
    cache.store(shader_entry);
    ...
    expectTrue(cache.would_prune_all(), "stale cache would_prune_all is true");
    expectTrue(estimate.should_skip() == !cache.would_prune_all(),
...
int main() {
    ...
    testCookHashShouldSkipHelpers();
    testCookCacheShouldSkipStoreAndLookup();
    testCookCacheWouldInvalidateHelpers();
    ...
    testCookCacheShouldSkipAndWouldInvalidateHelpers();
    testCookCacheWouldInvalidateAndShouldSkipHelpers();
    testCookCacheShouldSkipAndWouldInvalidateGuards();
    ...
    testCookCacheStaleUpstreamProbeDedup();
    testCookCacheWouldInvalidateSourceOutputProbes();
    testCookCacheWouldInvalidationProbes();
    testCookHashPreflightShouldSkipGuards();
    testCookCacheWouldInvalidateAndShouldSkipGuards();
    ...
    testCookCacheWouldInvalidateSourceAndOutput();
    testCookCachePreflightStoreEntry();
    testCookCacheProbeUniqueStaleUpstreamSources();
    testCookCacheInvalidationReconcileEstimate();
    testCookHashPreflightCookCacheEntry();
    testCookHashPreflightUnknownDependencyAndShaderGuards();
    testCookCacheInvalidationWouldProbes();
    testCookCacheStaleUpstreamUniqueProbes();
    testCookCacheUniqueStaleUpstreamProbe();
    testCookHashShouldSkipGuards();
    testCookCacheShouldSkipAndWouldInvalidateGuards();
    testCookHashPreflightShouldSkipGuards();
    ...
    testCookCacheWouldInvalidateHelpers();
    testCookCacheShouldSkipStoreGuard();
    testCookHashShouldSkipHelpers();
    testCookCacheWouldInvalidateAndShouldSkipGuards();
    testCookCacheWouldInvalidateAndShouldSkipProbes();
    ...
    testCookCacheWouldInvalidateUpstreamAndDownstreamProbes();
    testCookHashPreflightShouldSkipHelpers();
    testCookCachePruneEstimateShouldSkipHelpers();
