...
u32 CookCache::count_unique_stale_upstream_hashes(
...
std::vector<std::string> CookCache::probe_stale_upstream_sources_unique(
    ...
    std::vector<std::string> unique_sources;
    for (const std::string& source_path : per_entry) {
        append_unique_source_(unique_sources, source_path);
    return unique_sources;

namespace {

void append_unique_source_path_(std::vector<std::string>& sources, const std::string& source_path) {
    if (!is_valid_cook_cache_path(source_path)) {
        return;
    for (const std::string& recorded : sources) {
        if (recorded == source_path) {
...
u32 CookCache::count_unique_stale_upstream_sources(
    ...
    for (const auto& pair : source_upstream_by_path) {
        ...
        for (const CookCacheEntry& entry : m_entries) {
            if (entry.source_path == source_path && entry.upstream_hash != current_upstream) {
                append_unique_source_path_(stale_sources, source_path);
                break;
    return stale_sources;
...
CookCacheInvalidationEstimate CookCache::estimate_invalidation_reconcile(
    const std::vector<std::pair<std::string, u64>>& source_upstream_by_path) const {
    CookCacheInvalidationEstimate estimate;
    estimate.stale_upstream_entries = count_stale_upstream_hashes(source_upstream_by_path);

    const CookCachePruneEstimate prune = estimate_prune_removals();
...
CookCacheInvalidationSurface CookCache::estimate_invalidation_surface() const {
    CookCacheInvalidationSurface surface;
    surface.entry_count = static_cast<u32>(m_entries.size());
    surface.prunable_entries = count_prunable_entries();
...
namespace {
std::vector<std::string> CookCache::probe_unique_stale_upstream_sources(
    const std::vector<std::string> raw = probe_stale_upstream_sources(source_upstream_by_path);
    if (raw.empty()) {
        return {};

    std::vector<std::string> unique;
    for (const std::string& source_path : raw) {
...
std::vector<std::string> CookCache::probe_invalid_entry_sources() const {
    if (m_entries.empty()) {
        return {};
    }
    ...
    for (const CookCacheEntry& entry : m_entries) {
        ...
        bool already_recorded = false;
        for (const std::string& recorded : invalid_sources) {
            if (recorded == entry.source_path) {
                already_recorded = true;
                break;
        if (!already_recorded && is_valid_cook_cache_path(entry.source_path)) {
            invalid_sources.push_back(entry.source_path);
...
bool CookCache::would_invalidate_downstream_of(const std::string& output_path,
    ...
    return count_downstream_of(output_path, edges, jobs) != 0;
std::vector<std::string> CookCache::probe_unique_stale_upstream_sources(
    for (const std::string& source_path : probe_stale_upstream_sources(source_upstream_by_path)) {
    return unique_sources;
...
CookCacheUpstreamInvalidationEstimate CookCache::estimate_upstream_invalidation(
    ...
    estimate.direct_entries = count_by_source(changed_source);
    for (const CookJob& job : jobs) {
        if (job.source_path != changed_source) {
        estimate.downstream_entries += count_downstream_of(job.output_path, edges, jobs);
bool CookCache::should_skip_prune_reconcile() const {
    return estimate_prune_removals().should_skip();
}
