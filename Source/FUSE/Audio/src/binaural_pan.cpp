...
bool preflight_hrtf_ir_ready(const HrtfIrPreflight& preflight, HrtfIrRejectReason* reason) {
    if (reason != nullptr) {
        *reason = preflight.reason;


bool try_preflight_hrtf_ir(const HrtfIrPreflight& preflight, HrtfIrRejectReason& reason) {
    return preflight_hrtf_ir_ready(preflight, &reason);
...
bool try_preflight_hrtf_pan_path(const HrtfPanPathPreflight& preflight,
    return preflight_hrtf_pan_path_ready(preflight, &reason);
...
bool preflight_hrtf_pan_path_convolve_ready(bool hrtf_enabled, const HrtfIrStub& ir,
        *reason = classify_hrtf_pan_path_convolve_reject(pan_preflight, ir_preflight);
...
bool try_preflight_hrtf_attenuation_coupling(const HrtfAttenuationCouplingPreflight& preflight,
                                             HrtfAttenuationCouplingRejectReason& reason) {
    return preflight_hrtf_attenuation_coupling_ready(preflight, &reason);
}
...
bool try_preflight_hrtf_attenuation_coupling(
    ...
    HrtfAttenuationCouplingRejectReason& reason, const HrtfAttenuationCoupling& coupling,
    const BinauralPanParams& params) {
    return preflight_hrtf_attenuation_coupling_ready(path, distance_attenuation, occlusion_gain, &reason,
                                                     coupling, params);
}
...
bool preflightHrtfAttenuationCouplingReady(

bool tryPreflightHrtfAttenuationCoupling(
    return preflightHrtfAttenuationCouplingReady(path, distance_attenuation, occlusion_gain,
}
...
HrtfBinauralRejectReason classify_hrtf_binaural_convolution_reject(
    const HrtfBinauralPreflight& preflight) {
    const HrtfPanPathRejectReason pan_reason =
        classify_hrtf_pan_path_convolution_reject(preflight.panPath);
    if (pan_reason == HrtfPanPathRejectReason::HrtfDisabled) {
        return HrtfBinauralRejectReason::HrtfDisabled;
    }
    if (pan_reason == HrtfPanPathRejectReason::CoLocated) {
    ...
    if (pan_reason == HrtfPanPathRejectReason::EmptyIr) {
        return HrtfBinauralRejectReason::EmptyIr;
    }
    if (preflight.ir.malformedIr) {
        return HrtfBinauralRejectReason::MalformedIr;
...
bool try_preflight_hrtf_binaural(const HrtfBinauralPreflight& preflight,
                                 HrtfBinauralRejectReason& reason) {
    return preflight_hrtf_binaural_ready(preflight, &reason);
}
...
HrtfBinauralRejectReason classify_hrtf_binaural_narrow_reject(
    const HrtfBinauralPreflight& preflight) {
    if (preflight.panPath.skipped) {
        return HrtfBinauralRejectReason::BypassPath;
    }
...
bool preflight_hrtf_binaural_ready(bool hrtf_enabled, const Vec3& rel_listener,
                                   HrtfBinauralRejectReason* reason) {
    return preflight_hrtf_binaural_ready(hrtf_enabled, make_empty_hrtf_ir(), rel_listener, reason);
}

bool try_preflight_hrtf_binaural(bool hrtf_enabled, const HrtfIrStub& ir, const Vec3& rel_listener,
                                 HrtfBinauralRejectReason& reason) {
    return preflight_hrtf_binaural_ready(hrtf_enabled, ir, rel_listener, &reason);
}

bool try_preflight_hrtf_binaural(bool hrtf_enabled, const Vec3& rel_listener,
                                 HrtfBinauralRejectReason& reason) {
    return preflight_hrtf_binaural_ready(hrtf_enabled, rel_listener, &reason);
}
