...
void testHrtfIrRejectReasonEnums() {
    ...
    expectTrue(fuse::audio::classifyHrtfIrReject(empty)
                   == fuse::audio::HrtfIrRejectReason::NullSamples,
               "canonical empty IR classifies as null_samples");
    ...
    expectTrue(fuse::audio::classifyHrtfIrReject(malformed)
                   == fuse::audio::HrtfIrRejectReason::MalformedIr,
               "malformed IR classifies as malformed_ir");
    expectTrue(fuse::audio::hrtfIrRejectReasonIsBlocking(
                   fuse::audio::HrtfIrRejectReason::MalformedIr),
               "malformed_ir is blocking");

    const fuse::audio::HrtfIrStub null_samples_nonzero{nullptr, 4};
    expectTrue(fuse::audio::classifyHrtfIrReject(null_samples_nonzero)
                   == fuse::audio::HrtfIrRejectReason::NullSamples,
               "null samples with non-zero length classifies as null_samples");

    const fuse::audio::HrtfIrPreflight empty_preflight = fuse::audio::preflight_hrtf_ir(empty);
    expectTrue(empty_preflight.reason == fuse::audio::HrtfIrRejectReason::NullSamples,
               "preflight carries IR reject reason");
    expectTrue(!empty_preflight.ok(), "empty IR preflight is not ok");

    fuse::audio::HrtfIrRejectReason reason = fuse::audio::HrtfIrRejectReason::None;
    expectTrue(!fuse::audio::try_preflight_hrtf_ir(empty, reason),
               "try_preflight_hrtf_ir rejects empty IR");
    expectTrue(reason == fuse::audio::HrtfIrRejectReason::NullSamples,
               "try_preflight_hrtf_ir writes reject reason");
    expectTrue(fuse::audio::try_preflight_hrtf_ir(valid, reason),
               "try_preflight_hrtf_ir accepts valid IR");
    expectTrue(reason == fuse::audio::HrtfIrRejectReason::None,
               "try_preflight_hrtf_ir clears reason on success");
...
void testHrtfPanPathRejectReasonEnums() {
    ...
    expectTrue(fuse::audio::classifyHrtfPanPathReject(false, valid, offset)
                   == fuse::audio::HrtfPanPathRejectReason::HrtfDisabled,
               "disabled HRTF classifies as hrtf_disabled");
    expectTrue(fuse::audio::hrtfPanPathRejectReasonIsBlocking(
                   fuse::audio::HrtfPanPathRejectReason::HrtfDisabled),
               "hrtf_disabled blocks spatial pan");
    ...
    expectTrue(fuse::audio::classifyHrtfPanPathReject(true, empty, offset)
                   == fuse::audio::HrtfPanPathRejectReason::EmptyIr,
               "empty IR classifies as empty_ir pan downgrade");
    expectTrue(!fuse::audio::hrtfPanPathRejectReasonIsBlocking(
                   fuse::audio::HrtfPanPathRejectReason::EmptyIr),
               "empty_ir does not block spatial pan");
    ...
    expectTrue(fuse::audio::classifyHrtfPanPathReject(true, malformed, offset)
                   == fuse::audio::HrtfPanPathRejectReason::MalformedIr,
               "malformed IR classifies as malformed_ir pan downgrade");
    expectTrue(fuse::audio::classifyHrtfPanPathReject(true, valid, offset)
                   == fuse::audio::HrtfPanPathRejectReason::None,
               "valid path has no pan reject reason");
    ...
    fuse::audio::HrtfPanPathRejectReason reason = fuse::audio::HrtfPanPathRejectReason::None;
    expectTrue(fuse::audio::try_preflight_hrtf_pan_path(true, valid, offset, reason),
               "try_preflight_hrtf_pan_path accepts valid spatial path");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::None,
               "valid spatial path leaves reason at none");
    expectTrue(!fuse::audio::try_preflight_hrtf_pan_path(false, valid, offset, reason),
               "try_preflight_hrtf_pan_path rejects disabled HRTF");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::HrtfDisabled,
               "disabled HRTF writes pan reject reason");
}
...
void testHrtfAttenuationCouplingRejectReasonEnums() {
    ...
    expectTrue(narrow_preflight.reason == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "narrowing preflight carries none reject reason");
    expectTrue(narrow_preflight.ok(), "narrowing preflight is ok");

    fuse::audio::HrtfAttenuationCouplingRejectReason reason =
        fuse::audio::HrtfAttenuationCouplingRejectReason::None;
    expectTrue(!fuse::audio::try_preflight_hrtf_attenuation_coupling(
                   fuse::audio::HrtfPanPath::Bypass, 0.1f, 0.1f, reason),
               "try_preflight attenuation coupling rejects bypass path");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::BypassPath,
               "bypass path writes coupling reject reason");
    expectTrue(fuse::audio::try_preflight_hrtf_attenuation_coupling(
                   fuse::audio::HrtfPanPath::Convolution, 0.2f, 0.3f, reason),
               "try_preflight attenuation coupling accepts narrowing path");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "narrowing path clears coupling reject reason");
}
...
void testHrtfBinauralRejectReasonEnums() {
    ...
    const fuse::audio::HrtfBinauralPreflight stub_preflight =
        fuse::audio::preflight_hrtf_binaural(true, empty, offset, 0.2f, 0.3f);
    expectTrue(stub_preflight.reason == fuse::audio::HrtfBinauralRejectReason::EmptyIr,
               "composite preflight primary reason is empty_ir on stub path");
    expectTrue(!fuse::audio::hrtfBinauralRejectReasonIsBlocking(stub_preflight.reason),
               "empty_ir composite reason does not block spatial pan");
    ...
    const fuse::audio::HrtfBinauralPreflight conv_preflight =
        fuse::audio::preflight_hrtf_binaural(true, valid, offset, 1.f, 1.f);
    expectTrue(conv_preflight.reason
                   == fuse::audio::HrtfBinauralRejectReason::UnityAttenuation,
               "unity attenuation surfaces as composite reject reason");
    expectTrue(conv_preflight.ok() == false,
               "unity attenuation composite preflight is not fully ok");

    const fuse::audio::HrtfBinauralPreflight bypass_preflight =
        fuse::audio::preflight_hrtf_binaural(false, valid, offset, 0.1f, 0.1f);
    expectTrue(bypass_preflight.reason == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "disabled HRTF surfaces as composite reject reason");
    expectTrue(fuse::audio::hrtfBinauralRejectReasonIsBlocking(bypass_preflight.reason),
               "hrtf_disabled composite reason blocks spatial pan");
    ...
    const fuse::audio::HrtfBinauralPreflight co_located_preflight =
        fuse::audio::preflight_hrtf_binaural(true, valid, co_located, 0.1f, 0.1f);
    expectTrue(co_located_preflight.reason == fuse::audio::HrtfBinauralRejectReason::CoLocated,
               "co-located source surfaces as composite reject reason");

    const fuse::audio::HrtfIrStub malformed{samples, 0};
    const fuse::audio::HrtfBinauralPreflight malformed_preflight =
        fuse::audio::preflight_hrtf_binaural(true, malformed, offset, 0.2f, 0.3f);
    expectTrue(malformed_preflight.reason == fuse::audio::HrtfBinauralRejectReason::MalformedIr,
               "malformed IR surfaces as composite reject reason");
    ...
    expectTrue(valid_preflight.reason == fuse::audio::HrtfBinauralRejectReason::None,
               "valid IR with reduced attenuation has no composite reject reason");
    expectTrue(valid_preflight.ok(), "fully valid composite preflight is ok");

    fuse::audio::HrtfBinauralRejectReason reason = fuse::audio::HrtfBinauralRejectReason::None;
    expectTrue(fuse::audio::try_preflight_hrtf_binaural(true, valid, offset, 0.2f, 0.3f, reason),
               "try_preflight_hrtf_binaural accepts valid composite path");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::None,
               "valid composite path leaves reason at none");
    expectTrue(!fuse::audio::try_preflight_hrtf_binaural(false, valid, offset, 0.2f, 0.3f, reason),
               "try_preflight_hrtf_binaural rejects disabled composite path");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "disabled composite path writes reject reason");
}
...
void testHrtfIrRejectReasonGuards() {
    ...
    expectTrue(fuse::audio::classify_hrtf_ir_reject(empty_preflight)
                   == fuse::audio::HrtfIrRejectReason::NullSamples,
               "classify_hrtf_ir_reject maps null samples on canonical empty IR");
    expectTrue(std::strcmp(fuse::audio::hrtf_ir_reject_reason_label(
                               fuse::audio::HrtfIrRejectReason::NullSamples),
                           "NullSamples")
                   == 0,
               "hrtf_ir_reject_reason_label returns NullSamples");
    ...
    const fuse::audio::HrtfIrStub valid{samples, 1};
    const fuse::audio::HrtfIrPreflight valid_preflight = fuse::audio::preflight_hrtf_ir(valid);
    expectTrue(fuse::audio::classify_hrtf_ir_reject(valid_preflight)
                   == fuse::audio::HrtfIrRejectReason::None,
               "valid IR reject reason is None");
    ...
    expectTrue(!fuse::audio::try_preflight_hrtf_ir(empty, reason),
               "try_preflight_hrtf_ir rejects empty IR");
    expectTrue(reason == fuse::audio::HrtfIrRejectReason::NullSamples,
               "try_preflight_hrtf_ir sets reject reason on empty IR");
    ...
    expectTrue(fuse::audio::preflight_hrtf_ir_ready(valid, &reason),
               "preflight_hrtf_ir_ready with reason accepts valid IR");
    expectTrue(reason == fuse::audio::HrtfIrRejectReason::None,
               "preflight_hrtf_ir_ready sets None on valid IR");
}
...
void testHrtfPanPathRejectReasonGuards() {
    ...
    const fuse::audio::HrtfPanPathPreflight stub_preflight =
        fuse::audio::preflight_hrtf_pan_path(true, empty, offset);
    expectTrue(fuse::audio::classify_hrtf_pan_path_reject(stub_preflight)
                   == fuse::audio::HrtfPanPathRejectReason::None,
               "ILD/ITD stub path reject reason is None");

    const fuse::audio::HrtfPanPathPreflight disabled_preflight =
        fuse::audio::preflight_hrtf_pan_path(false, valid, offset);
    expectTrue(fuse::audio::classify_hrtf_pan_path_reject(disabled_preflight)
                   == fuse::audio::HrtfPanPathRejectReason::HrtfDisabled,
               "classify_hrtf_pan_path_reject maps disabled HRTF");
    expectTrue(std::strcmp(fuse::audio::hrtf_pan_path_reject_reason_label(
                               fuse::audio::HrtfPanPathRejectReason::HrtfDisabled),
                           "HrtfDisabled")
                   == 0,
               "hrtf_pan_path_reject_reason_label returns HrtfDisabled");

    const fuse::audio::HrtfPanPathPreflight co_located_preflight =
        fuse::audio::preflight_hrtf_pan_path(true, valid, co_located);
    expectTrue(fuse::audio::classify_hrtf_pan_path_reject(co_located_preflight)
                   == fuse::audio::HrtfPanPathRejectReason::CoLocated,
               "classify_hrtf_pan_path_reject maps co-located source");

    fuse::audio::HrtfPanPathRejectReason reason = fuse::audio::HrtfPanPathRejectReason::None;
    expectTrue(fuse::audio::preflight_hrtf_pan_path_ready(true, empty, offset, &reason),
               "preflight_hrtf_pan_path_ready accepts stub path");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::None,
               "stub path preflight ready sets None");
    expectTrue(!fuse::audio::try_preflight_hrtf_pan_path(false, valid, offset, reason),
               "try_preflight_hrtf_pan_path rejects disabled HRTF");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::HrtfDisabled,
               "try_preflight_hrtf_pan_path sets HrtfDisabled reason");
    expectTrue(fuse::audio::preflight_hrtf_pan_path_ready(true, offset),
               "no-IR overload preflight_hrtf_pan_path_ready accepts stub path");
}
...
void testHrtfAttenuationCouplingRejectReasonGuards() {
    ...
    expectTrue(std::strcmp(
                   fuse::audio::hrtf_attenuation_coupling_reject_reason_label(
                       fuse::audio::HrtfAttenuationCouplingRejectReason::BypassPath),
                   "BypassPath")
                   == 0,
               "hrtf_attenuation_coupling_reject_reason_label returns BypassPath");

    const fuse::audio::HrtfAttenuationCouplingPreflight unity_preflight =
        fuse::audio::preflight_hrtf_attenuation_coupling(fuse::audio::HrtfPanPath::Convolution,
                                                         1.f, 1.f);
    expectTrue(fuse::audio::classify_hrtf_attenuation_coupling_reject(unity_preflight)
                   == fuse::audio::HrtfAttenuationCouplingRejectReason::UnityAttenuation,
               "classify_hrtf_attenuation_coupling_reject maps unity attenuation");
    ...
    expectTrue(fuse::audio::classify_hrtf_attenuation_coupling_reject(narrow_preflight)
                   == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "narrowing path reject reason is None");

    fuse::audio::HrtfAttenuationCouplingRejectReason reason =
        fuse::audio::HrtfAttenuationCouplingRejectReason::None;
    expectTrue(fuse::audio::preflight_hrtf_attenuation_coupling_ready(
                   fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f, &reason),
               "preflight_hrtf_attenuation_coupling_ready accepts narrowing");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "narrowing preflight ready sets None");
    expectTrue(!fuse::audio::try_preflight_hrtf_attenuation_coupling(
                   fuse::audio::HrtfPanPath::Bypass, 0.1f, 0.1f, reason),
               "try_preflight_hrtf_attenuation_coupling rejects bypass path");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::BypassPath,
               "try_preflight_hrtf_attenuation_coupling sets BypassPath reason");
}
...
void testHrtfBinauralRejectReasonGuards() {
    ...
    expectTrue(fuse::audio::classify_hrtf_binaural_reject(stub_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::None,
               "composite stub path reject reason is None");
    expectTrue(fuse::audio::classify_hrtf_binaural_convolution_reject(stub_preflight)
                   == fuse::audio::HrtfIrRejectReason::NullSamples,
               "composite convolution reject delegates to IR classification");
    expectTrue(fuse::audio::classify_hrtf_binaural_narrowing_reject(stub_preflight)
                   == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "composite narrowing reject is None when attenuation reduced");

    const fuse::audio::HrtfBinauralPreflight conv_preflight =
        fuse::audio::preflight_hrtf_binaural(true, valid, offset, 1.f, 1.f);
    expectTrue(fuse::audio::classify_hrtf_binaural_convolution_reject(conv_preflight)
                   == fuse::audio::HrtfIrRejectReason::None,
               "valid IR composite convolution reject is None");
    expectTrue(fuse::audio::classify_hrtf_binaural_narrowing_reject(conv_preflight)
                   == fuse::audio::HrtfAttenuationCouplingRejectReason::UnityAttenuation,
               "unity attenuation composite narrowing reject is UnityAttenuation");

    const fuse::audio::HrtfBinauralPreflight bypass_preflight =
        fuse::audio::preflight_hrtf_binaural(false, valid, offset, 0.1f, 0.1f);
    expectTrue(fuse::audio::classify_hrtf_binaural_reject(bypass_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "composite bypass reject maps HrtfDisabled");
    expectTrue(std::strcmp(fuse::audio::hrtf_binaural_reject_reason_label(
                               fuse::audio::HrtfBinauralRejectReason::HrtfDisabled),
                           "HrtfDisabled")
                   == 0,
               "hrtf_binaural_reject_reason_label returns HrtfDisabled");

    const fuse::audio::HrtfBinauralPreflight co_located_preflight =
        fuse::audio::preflight_hrtf_binaural(true, valid, co_located, 0.1f, 0.1f);
    expectTrue(fuse::audio::classify_hrtf_binaural_reject(co_located_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::CoLocated,
               "composite co-located reject maps CoLocated");

    fuse::audio::HrtfBinauralRejectReason reason = fuse::audio::HrtfBinauralRejectReason::None;
    expectTrue(fuse::audio::preflight_hrtf_binaural_ready(true, empty, offset, 0.2f, 0.3f, &reason),
               "preflight_hrtf_binaural_ready accepts stub path");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::None,
               "stub composite preflight ready sets None");
    expectTrue(!fuse::audio::try_preflight_hrtf_binaural(false, valid, offset, 0.1f, 0.1f, reason),
               "try_preflight_hrtf_binaural rejects disabled HRTF");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "try_preflight_hrtf_binaural sets HrtfDisabled reason");
    expectTrue(fuse::audio::preflight_hrtf_binaural_ready(true, offset, 0.25f, 0.35f),
               "no-IR overload preflight_hrtf_binaural_ready accepts stub path");
}
...
void testHrtfIrRejectReason() {
    ...
    expectTrue(fuse::audio::hrtf_ir_rejects_for_reason(empty, fuse::audio::HrtfIrRejectReason::NullSamples),
               "canonical empty IR rejects with NullSamples");
    expectTrue(fuse::audio::hrtf_ir_reject_reason(empty)
                   == fuse::audio::preflight_hrtf_ir(empty).reason,
               "preflight IR reason matches hrtf_ir_reject_reason");
    ...
    const fuse::audio::HrtfIrStub null_samples_nonzero_length{nullptr, 4};
    expectTrue(fuse::audio::hrtf_ir_rejects_for_reason(null_samples_nonzero_length,
                                                       fuse::audio::HrtfIrRejectReason::NullSamples),
...
void testHrtfPanPathRejectReason() {
    ...
    expectTrue(fuse::audio::hrtf_pan_path_rejects_for_reason(false, offset,
                                                             fuse::audio::HrtfPanPathRejectReason::HrtfDisabled),
               "disabled HRTF rejects pan path");
    expectTrue(fuse::audio::hrtf_pan_path_rejects_for_reason(true, co_located,
                                                             fuse::audio::HrtfPanPathRejectReason::CoLocated),
               "co-located source rejects pan path");
    expectTrue(fuse::audio::hrtf_pan_path_rejects_for_reason(true, offset,
                                                             fuse::audio::HrtfPanPathRejectReason::None),
               "enabled offset source has no pan-path reject");
    ...
    const fuse::audio::HrtfPanPathPreflight stub_preflight =
        fuse::audio::preflight_hrtf_pan_path(true, empty, offset);
    expectTrue(stub_preflight.reason == fuse::audio::HrtfPanPathRejectReason::None,
               "ILD/ITD stub preflight has no bypass reject reason");
    expectTrue(stub_preflight.convolutionReason == fuse::audio::HrtfConvolutionRejectReason::NullSamples,
               "empty IR convolution reject is NullSamples");

    const fuse::audio::HrtfPanPathPreflight conv_preflight =
        fuse::audio::preflight_hrtf_pan_path(true, valid, offset);
    expectTrue(conv_preflight.convolutionReason == fuse::audio::HrtfConvolutionRejectReason::None,
               "valid IR convolution reject is None");

    const fuse::audio::HrtfPanPathPreflight disabled_preflight =
        fuse::audio::preflight_hrtf_pan_path(false, valid, offset);
    expectTrue(disabled_preflight.reason == fuse::audio::HrtfPanPathRejectReason::HrtfDisabled,
               "disabled preflight carries HrtfDisabled reject reason");
    expectTrue(disabled_preflight.convolutionReason
                   == fuse::audio::HrtfConvolutionRejectReason::HrtfDisabled,
               "disabled preflight convolution reject is HrtfDisabled");

    expectTrue(fuse::audio::hrtf_convolution_rejects_for_reason(true, valid, offset,
                                                                fuse::audio::HrtfConvolutionRejectReason::None),
               "valid IR convolution rejects for None");
    expectTrue(fuse::audio::hrtf_convolution_rejects_for_reason(true, empty, offset,
                                                                fuse::audio::HrtfConvolutionRejectReason::NullSamples),
               "empty IR convolution rejects for NullSamples");
    expectTrue(std::strcmp(
                   fuse::audio::hrtf_convolution_reject_reason_name(
                       fuse::audio::HrtfConvolutionRejectReason::NullSamples),
                   "NullSamples") == 0,
               "NullSamples convolution reject reason name");
}
...
void testHrtfAttenuationCouplingRejectReason() {
    ...
    expectTrue(bypass_preflight.reason == fuse::audio::HrtfAttenuationCouplingRejectReason::BypassPath,
               "bypass coupling preflight carries BypassPath reject reason");
}
...
void testHrtfBinauralRejectReason() {
    ...
    expectTrue(fuse::audio::hrtf_binaural_rejects_for_reason(false, offset,
                                                               fuse::audio::HrtfBinauralRejectReason::HrtfDisabled),
               "composite bypass rejects for HrtfDisabled");
    expectTrue(fuse::audio::hrtf_binaural_rejects_for_reason(true, co_located,
                                                               fuse::audio::HrtfBinauralRejectReason::CoLocated),
               "composite bypass rejects for CoLocated");
    expectTrue(fuse::audio::hrtf_binaural_rejects_for_reason(true, offset,
                                                               fuse::audio::HrtfBinauralRejectReason::None),
               "enabled offset source has no composite bypass reject");

    const fuse::audio::HrtfBinauralPreflight stub_preflight =
        fuse::audio::preflight_hrtf_binaural(true, empty, offset, 0.2f, 0.3f);
    expectTrue(stub_preflight.reason == fuse::audio::HrtfBinauralRejectReason::None,
               "stub composite preflight has no bypass reject reason");
    expectTrue(fuse::audio::hrtf_binaural_convolution_rejects_for_reason(
                   stub_preflight, fuse::audio::HrtfConvolutionRejectReason::NullSamples),
               "stub composite convolution rejects for NullSamples");
    expectTrue(stub_preflight.ir.reason == fuse::audio::HrtfIrRejectReason::NullSamples,
               "composite preflight carries IR reject reason");
    expectTrue(stub_preflight.attenuationCoupling.reason
                   == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "reduced attenuation composite coupling reject is None");

    const fuse::audio::HrtfBinauralPreflight conv_preflight =
        fuse::audio::preflight_hrtf_binaural(true, valid, offset, 1.f, 1.f);
    expectTrue(fuse::audio::hrtf_binaural_convolution_rejects_for_reason(
                   conv_preflight, fuse::audio::HrtfConvolutionRejectReason::None),
               "valid IR composite convolution has no reject reason");
    expectTrue(conv_preflight.attenuationCoupling.reason
                   == fuse::audio::HrtfAttenuationCouplingRejectReason::UnityAttenuation,
               "unity attenuation composite coupling reject is UnityAttenuation");
    ...
    const fuse::audio::HrtfBinauralPreflight bypass_preflight =
        fuse::audio::preflight_hrtf_binaural(false, valid, offset, 0.1f, 0.1f);
    expectTrue(bypass_preflight.reason == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "disabled composite preflight carries HrtfDisabled reject reason");
    expectTrue(std::strcmp(
                   fuse::audio::hrtf_binaural_reject_reason_name(
                       fuse::audio::HrtfBinauralRejectReason::HrtfDisabled),
                   "HrtfDisabled") == 0,
               "HrtfDisabled composite reject reason name");
}
...
void testHrtfIrRejectReasonGuards() {
    ...
    const fuse::audio::HrtfIrStub malformed{samples, 0};
    expectTrue(!fuse::audio::preflight_hrtf_ir_ready(malformed, &reason),
               "preflight_hrtf_ir_ready rejects malformed IR");
    expectTrue(reason == fuse::audio::HrtfIrRejectReason::MalformedIr,
    ...
    expectTrue(fuse::audio::classify_hrtf_ir_reject(empty_preflight)
                   == fuse::audio::HrtfIrRejectReason::NullSamples,
               "classify_hrtf_ir_reject maps nullSamples flag");
    expectTrue(std::strcmp(fuse::audio::hrtfIrRejectReasonLabel(
                   fuse::audio::HrtfIrRejectReason::MalformedIr),
               "malformed_ir") == 0,
               "IR reject reason label for malformed_ir");
...
void testHrtfPanPathRejectReasonGuards() {
    ...
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::None,
               "valid pan path reject reason is None");
    ...
    expectTrue(!fuse::audio::preflight_hrtf_pan_path_ready(true, valid, co_located, &reason),
               "preflight_hrtf_pan_path_ready rejects co-located source");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::CoLocated,
               "co-located reject reason is CoLocated");
    ...
    expectTrue(fuse::audio::classify_hrtf_pan_path_reject(disabled_preflight)
                   == fuse::audio::HrtfPanPathRejectReason::HrtfDisabled,
               "classify_hrtf_pan_path_reject maps hrtfDisabled flag");
    expectTrue(std::strcmp(fuse::audio::hrtfPanPathRejectReasonLabel(
                   fuse::audio::HrtfPanPathRejectReason::CoLocated),
               "co_located") == 0,
               "pan-path reject reason label for co_located");
    ...
    expectTrue(fuse::audio::preflight_hrtf_pan_path_ready(true, offset, &reason),
               "no-IR pan-path preflight ready on valid offset");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::None,
               "no-IR valid pan path reject reason is None");
}
...
void testHrtfAttenuationCouplingRejectReasonGuards() {
    ...
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "narrowing reject reason is None on spatial path");

    expectTrue(!fuse::audio::tryPreflight_hrtf_attenuation_coupling(
                   fuse::audio::HrtfPanPath::Bypass, 0.1f, 0.1f, reason),
               "tryPreflight_hrtf_attenuation_coupling rejects bypass path");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::BypassPath,
               "bypass path reject reason is BypassPath");

    expectTrue(!fuse::audio::preflight_hrtf_attenuation_coupling_ready(
                   fuse::audio::HrtfPanPath::Convolution, 1.f, 1.f, {}, {}, &reason),
               "preflight_hrtf_attenuation_coupling_ready rejects unity attenuation");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::UnityAttenuation,
               "unity attenuation reject reason is UnityAttenuation");
    ...
    expectTrue(fuse::audio::classify_hrtf_attenuation_coupling_reject(bypass_preflight)
                   == fuse::audio::HrtfAttenuationCouplingRejectReason::BypassPath,
               "classify_hrtf_attenuation_coupling_reject maps bypassPath flag");
    expectTrue(std::strcmp(fuse::audio::hrtfAttenuationCouplingRejectReasonLabel(
                   fuse::audio::HrtfAttenuationCouplingRejectReason::UnityAttenuation),
               "unity_attenuation") == 0,
               "attenuation coupling reject reason label for unity_attenuation");
}
...
void testHrtfBinauralRejectReasonGuards() {
    ...
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::None,
               "valid composite reject reason is None");
    ...
    expectTrue(!fuse::audio::preflight_hrtf_binaural_ready(true, valid, co_located, 0.1f, 0.1f, {},
                                                           {}, &reason),
               "preflight_hrtf_binaural_ready rejects co-located source");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::CoLocated,
    ...
    expectTrue(fuse::audio::classify_hrtf_binaural_reject(bypass_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "classify_hrtf_binaural_reject maps pan-path bypass");
    expectTrue(bypass_preflight.reason == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "composite preflight carries reject reason");
    expectTrue(std::strcmp(fuse::audio::hrtfBinauralRejectReasonLabel(
                   fuse::audio::HrtfBinauralRejectReason::HrtfDisabled),
               "hrtf_disabled") == 0,
               "binaural reject reason label for hrtf_disabled");
}
...
void testRejectReasonMirrorsExistingPreflights() {
    ...
    expectTrue(fuse::audio::preflight_hrtf_pan_path_ready(true, valid, offset)
    ...
    expectTrue(ir_preflight.reason == fuse::audio::classify_hrtf_ir_reject(ir_preflight),
               "IR preflight reason matches classify_hrtf_ir_reject");

    const fuse::audio::HrtfPanPathPreflight pan_preflight =
        fuse::audio::preflight_hrtf_pan_path(true, empty, offset);
    expectTrue(pan_preflight.reason == fuse::audio::classify_hrtf_pan_path_reject(pan_preflight),
               "pan-path preflight reason matches classify");
    ...
    expectTrue(
        coupling_preflight.reason
            == fuse::audio::classify_hrtf_attenuation_coupling_reject(coupling_preflight),
        "attenuation coupling preflight reason matches classify");
}
...
void testHrtfIrRejectReasonGuards() {
    ...
    expectTrue(!fuse::audio::preflight_hrtf_ir_ready(empty, &reason),
               "empty IR preflight is not ready for convolution");
    expectTrue(reason == fuse::audio::HrtfIrRejectReason::NullSamples,
               "canonical empty IR rejects with NullSamples");

    const float samples[] = {0.5f};
    const fuse::audio::HrtfIrStub valid{samples, 1};
    reason = fuse::audio::HrtfIrRejectReason::None;
    expectTrue(fuse::audio::try_preflight_hrtf_ir(valid, reason),
               "valid IR passes try_preflight_hrtf_ir");
    expectTrue(reason == fuse::audio::HrtfIrRejectReason::None,
               "valid IR reject reason is None");
    ...
    reason = fuse::audio::HrtfIrRejectReason::None;
    expectTrue(!fuse::audio::preflight_hrtf_ir_ready(malformed, &reason),
               "malformed IR preflight is not ready");
    expectTrue(reason == fuse::audio::HrtfIrRejectReason::MalformedIr,
               "malformed IR rejects with MalformedIr");
    ...
    expectTrue(fuse::audio::classify_hrtf_ir_reject(valid_preflight)
                   == fuse::audio::HrtfIrRejectReason::None,
               "classify_hrtf_ir_reject returns None for valid IR");
    expectTrue(std::strcmp(fuse::audio::hrtf_ir_reject_reason_label(
                               fuse::audio::HrtfIrRejectReason::MalformedIr),
                           "MalformedIr")
                   == 0,
               "hrtf_ir_reject_reason_label returns stable label");
}
...
void testHrtfPanPathRejectReasonGuards() {
    ...
    fuse::audio::HrtfPanPathRejectReason reason = fuse::audio::HrtfPanPathRejectReason::None;
    expectTrue(fuse::audio::preflight_hrtf_pan_path_ready(true, valid, offset, &reason),
               "enabled offset source passes pan-path preflight");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::None,
               "spatial pan reject reason is None on valid path");

    reason = fuse::audio::HrtfPanPathRejectReason::None;
    expectTrue(!fuse::audio::try_preflight_hrtf_pan_path(false, valid, offset, reason),
               "disabled HRTF fails pan-path try_preflight");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::HrtfDisabled,
               "disabled HRTF rejects with HrtfDisabled");
    ...
    reason = fuse::audio::HrtfPanPathRejectReason::None;
    expectTrue(fuse::audio::preflight_hrtf_pan_path_ready(true, offset, &reason),
               "no-IR overload passes for offset source");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::None,
               "no-IR overload reject reason is None on valid path");
    expectTrue(std::strcmp(fuse::audio::hrtf_pan_path_reject_reason_label(
                               fuse::audio::HrtfPanPathRejectReason::CoLocated),
                           "CoLocated")
                   == 0,
               "hrtf_pan_path_reject_reason_label returns stable label");
}
...
void testHrtfAttenuationCouplingRejectReasonGuards() {
    ...
    expectTrue(!fuse::audio::preflight_hrtf_attenuation_coupling_ready(
                   fuse::audio::HrtfPanPath::Bypass, 0.2f, 0.3f, &reason),
               "bypass path fails attenuation-coupling preflight");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::BypassPath,
               "bypass path rejects with BypassPath");

    reason = fuse::audio::HrtfAttenuationCouplingRejectReason::None;
    expectTrue(!fuse::audio::try_preflight_hrtf_attenuation_coupling(
                   fuse::audio::HrtfPanPath::Convolution, 1.f, 1.f, reason),
               "unity attenuation fails narrowing try_preflight");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::UnityAttenuation,
    ...
    expectTrue(fuse::audio::preflight_hrtf_attenuation_coupling_ready(
                   fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f, &reason),
               "reduced attenuation passes narrowing preflight");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "narrowing reject reason is None when coupling applies");
    ...
    expectTrue(fuse::audio::classify_hrtf_attenuation_coupling_reject(narrow_preflight)
                   == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "classify narrowing reject is None when coupling can apply");
    expectTrue(std::strcmp(
                   fuse::audio::hrtf_attenuation_coupling_reject_reason_label(
                       fuse::audio::HrtfAttenuationCouplingRejectReason::UnityAttenuation),
                   "UnityAttenuation")
                   == 0,
               "hrtf_attenuation_coupling_reject_reason_label returns stable label");
}
...
void testHrtfBinauralRejectReasonGuards() {
    ...
    fuse::audio::HrtfBinauralRejectReason reason = fuse::audio::HrtfBinauralRejectReason::None;
    expectTrue(fuse::audio::preflight_hrtf_binaural_ready(true, empty, offset, 0.2f, 0.3f, &reason),
               "enabled stub path passes composite preflight");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::None,
               "composite reject reason is None on spatial stub path");

    reason = fuse::audio::HrtfBinauralRejectReason::None;
    expectTrue(!fuse::audio::try_preflight_hrtf_binaural(false, valid, offset, 0.1f, 0.1f, reason),
               "disabled HRTF fails composite try_preflight");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "composite rejects disabled HRTF with HrtfDisabled");

    reason = fuse::audio::HrtfBinauralRejectReason::None;
    expectTrue(!fuse::audio::preflight_hrtf_binaural_ready(true, valid, co_located, 0.1f, 0.1f,
                                                           &reason),
               "co-located source fails composite preflight");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::CoLocated,
               "composite rejects co-located source with CoLocated");

    const fuse::audio::HrtfBinauralPreflight stub_preflight =
        fuse::audio::preflight_hrtf_binaural(true, empty, offset, 0.2f, 0.3f);
    expectTrue(fuse::audio::classify_hrtf_binaural_convolution_reject(stub_preflight)
                   == fuse::audio::HrtfIrRejectReason::NullSamples,
               "composite convolution reject delegates to empty-IR diagnostics");
    expectTrue(fuse::audio::classify_hrtf_binaural_narrowing_reject(stub_preflight)
                   == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "composite narrowing reject is None when coupling applies");

    const fuse::audio::HrtfBinauralPreflight bypass_preflight =
        fuse::audio::preflight_hrtf_binaural(false, valid, offset, 0.1f, 0.1f);
    expectTrue(fuse::audio::classify_hrtf_binaural_convolution_reject(bypass_preflight)
                   == fuse::audio::HrtfIrRejectReason::None,
               "bypass composite does not attribute convolution skip to IR");
    expectTrue(fuse::audio::classify_hrtf_binaural_narrowing_reject(bypass_preflight)
                   == fuse::audio::HrtfAttenuationCouplingRejectReason::BypassPath,
               "bypass composite narrowing reject is BypassPath");

    const fuse::audio::HrtfBinauralPreflight unity_preflight =
        fuse::audio::preflight_hrtf_binaural(true, valid, offset, 1.f, 1.f);
    expectTrue(fuse::audio::classify_hrtf_binaural_convolution_reject(unity_preflight)
                   == fuse::audio::HrtfIrRejectReason::None,
               "valid IR composite convolution reject is None");
    expectTrue(fuse::audio::classify_hrtf_binaural_narrowing_reject(unity_preflight)
                   == fuse::audio::HrtfAttenuationCouplingRejectReason::UnityAttenuation,
    ...
    reason = fuse::audio::HrtfBinauralRejectReason::None;
    expectTrue(fuse::audio::preflight_hrtf_binaural_ready(true, offset, 0.25f, 0.35f, &reason),
               "no-IR composite overload passes for offset source");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::None,
               "no-IR composite overload reject reason is None on valid path");
    expectTrue(std::strcmp(fuse::audio::hrtf_binaural_reject_reason_label(
                               fuse::audio::HrtfBinauralRejectReason::HrtfDisabled),
                           "HrtfDisabled")
                   == 0,
               "hrtf_binaural_reject_reason_label returns stable label");
}
...
void testHrtfIrRejectReasonEnums() {
    ...
    expectTrue(fuse::audio::hrtf_ir_rejects_for_reason(valid, fuse::audio::HrtfIrRejectReason::None),
               "valid IR has no reject reason");
    expectTrue(fuse::audio::preflight_hrtf_ir(valid).reason == fuse::audio::HrtfIrRejectReason::None,
               "valid IR preflight carries None reason");

    const fuse::audio::HrtfIrStub malformed{samples, 0};
    expectTrue(fuse::audio::hrtf_ir_rejects_for_reason(malformed, fuse::audio::HrtfIrRejectReason::MalformedIr),
               "non-null zero-length IR rejects for MalformedIr");
    expectTrue(fuse::audio::preflight_hrtf_ir(malformed).reason
                   == fuse::audio::HrtfIrRejectReason::MalformedIr,
               "malformed IR preflight carries MalformedIr reason");

    const fuse::audio::HrtfIrStub null_samples_nonzero_length{nullptr, 4};
    expectTrue(fuse::audio::hrtf_ir_rejects_for_reason(null_samples_nonzero_length,
                                                       fuse::audio::HrtfIrRejectReason::NullSamples),
               "null samples with non-zero length rejects for NullSamples");
}
...
void testHrtfPanPathRejectReasonEnums() {
    ...
    expectTrue(fuse::audio::hrtf_pan_path_rejects_for_reason(true, valid, co_located,
               ...
               "co-located source rejects for CoLocatedSource");
    expectTrue(fuse::audio::hrtf_pan_path_rejects_for_reason(true, empty, offset,
                                                             fuse::audio::HrtfPanPathRejectReason::EmptyIr),
               "empty IR rejects for EmptyIr fallback");
    expectTrue(fuse::audio::hrtf_pan_path_rejects_for_reason(true, valid, offset,
                                                             fuse::audio::HrtfPanPathRejectReason::None),
               "valid IR offset source has no pan-path reject reason");

    expectTrue(fuse::audio::hrtf_pan_path_rejects_for_reason(true, offset,
                                                             fuse::audio::HrtfPanPathRejectReason::EmptyIr),
               "no-IR overload rejects for EmptyIr");
    expectTrue(std::strcmp(
                   fuse::audio::hrtf_pan_path_reject_reason_name(
                       fuse::audio::HrtfPanPathRejectReason::CoLocatedSource),
                   "CoLocatedSource") == 0,
               "CoLocatedSource pan-path reject reason name");

    const fuse::audio::HrtfPanPathPreflight stub_preflight =
        fuse::audio::preflight_hrtf_pan_path(true, empty, offset);
    expectTrue(stub_preflight.reason == fuse::audio::HrtfPanPathRejectReason::EmptyIr,
               "pan-path preflight carries EmptyIr reason for empty IR");
    expectTrue(stub_preflight.reason
                   == fuse::audio::hrtf_pan_path_reject_reason(true, empty, offset),
               "pan-path preflight reason matches reject_reason helper");

    const fuse::audio::HrtfPanPathPreflight conv_preflight =
        fuse::audio::preflight_hrtf_pan_path(true, valid, offset);
    expectTrue(conv_preflight.reason == fuse::audio::HrtfPanPathRejectReason::None,
               "convolution pan-path preflight carries None reason");
}
...
void testHrtfAttenuationCouplingRejectReasonEnums() {
    ...
    expectTrue(unity_preflight.reason
                   == fuse::audio::HrtfAttenuationCouplingRejectReason::UnityAttenuation,
               "unity attenuation preflight carries UnityAttenuation reason");
    expectTrue(unity_preflight.reason
                   == fuse::audio::hrtf_attenuation_coupling_reject_reason(
                          fuse::audio::HrtfPanPath::Convolution, 1.f, 1.f),
               "attenuation preflight reason matches reject_reason helper");

    const fuse::audio::HrtfAttenuationCouplingPreflight narrow_preflight =
        fuse::audio::preflight_hrtf_attenuation_coupling(fuse::audio::HrtfPanPath::IldItdStub, 0.2f,
                                                         0.3f);
    expectTrue(narrow_preflight.reason == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "narrowing preflight carries None reason");
}
...
void testHrtfBinauralRejectReasonEnums() {
    ...
    expectTrue(fuse::audio::hrtf_binaural_rejects_for_reason(
                   disabled_preflight, fuse::audio::HrtfBinauralRejectReason::HrtfDisabled),
               "composite preflight rejects for HrtfDisabled");
    expectTrue(disabled_preflight.reason == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "composite preflight carries HrtfDisabled reason");

    const fuse::audio::HrtfBinauralPreflight co_located_preflight =
        fuse::audio::preflight_hrtf_binaural(true, valid, co_located, 0.1f, 0.1f);
    expectTrue(fuse::audio::hrtf_binaural_rejects_for_reason(
                   co_located_preflight, fuse::audio::HrtfBinauralRejectReason::CoLocatedSource),
               "composite preflight rejects for CoLocatedSource");

    const fuse::audio::HrtfBinauralPreflight stub_preflight =
        fuse::audio::preflight_hrtf_binaural(true, empty, offset, 0.2f, 0.3f);
    expectTrue(fuse::audio::hrtf_binaural_rejects_for_reason(stub_preflight,
                                                             fuse::audio::HrtfBinauralRejectReason::EmptyIr),
               "composite stub path rejects for EmptyIr");
    expectTrue(stub_preflight.reason == fuse::audio::HrtfBinauralRejectReason::EmptyIr,
               "composite stub preflight carries EmptyIr reason");

    const fuse::audio::HrtfBinauralPreflight unity_preflight =
        fuse::audio::preflight_hrtf_binaural(true, valid, offset, 1.f, 1.f);
    expectTrue(fuse::audio::hrtf_binaural_rejects_for_reason(
                   unity_preflight, fuse::audio::HrtfBinauralRejectReason::UnityAttenuation),
               "composite preflight rejects for UnityAttenuation on valid IR");
    expectTrue(unity_preflight.reason == fuse::audio::HrtfBinauralRejectReason::UnityAttenuation,
               "composite unity preflight carries UnityAttenuation reason");

    const fuse::audio::HrtfBinauralPreflight conv_preflight =
        fuse::audio::preflight_hrtf_binaural(true, valid, offset, 0.2f, 0.3f);
    expectTrue(fuse::audio::hrtf_binaural_rejects_for_reason(conv_preflight,
                                                             fuse::audio::HrtfBinauralRejectReason::None),
               "valid IR with reduced attenuation has no composite reject reason");
    expectTrue(conv_preflight.reason == fuse::audio::HrtfBinauralRejectReason::None,
               "convolution composite preflight carries None reason");
    expectTrue(fuse::audio::hrtf_binaural_reject_reason(conv_preflight)
                   == conv_preflight.reason,
               "composite reject_reason helper matches preflight reason field");
    expectTrue(std::strcmp(
                   fuse::audio::hrtf_binaural_reject_reason_name(
                       fuse::audio::HrtfBinauralRejectReason::EmptyIr),
                   "EmptyIr") == 0,
               "EmptyIr composite reject reason name");
}
...
void testHrtfIrRejectReasonGuards() {
    ...
    expectTrue(fuse::audio::hrtf_ir_rejects_for_reason(empty,
                                                       fuse::audio::HrtfIrRejectReason::NullSamples),
               "empty IR rejects_for_reason NullSamples");
    ...
    expectTrue(empty_preflight.reason == fuse::audio::HrtfIrRejectReason::NullSamples,
               "preflight carries IR reject reason");
    expectTrue(fuse::audio::classify_hrtf_ir_reject(empty_preflight)
                   == empty_preflight.reason,
               "classify_hrtf_ir_reject mirrors preflight reason");

    fuse::audio::HrtfIrRejectReason reason = fuse::audio::HrtfIrRejectReason::None;
    expectTrue(!fuse::audio::try_preflight_hrtf_ir(empty, reason),
               "try_preflight_hrtf_ir false on empty IR");
    expectTrue(reason == fuse::audio::HrtfIrRejectReason::NullSamples,
               "try_preflight_hrtf_ir writes reject reason");
    expectTrue(fuse::audio::preflight_hrtf_ir_ready(valid),
               "preflight_hrtf_ir_ready true on valid IR");
    expectTrue(std::strcmp(fuse::audio::hrtf_ir_reject_reason_name(
                   fuse::audio::HrtfIrRejectReason::MalformedIr),
               "MalformedIr")
                   == 0,
...
void testHrtfPanPathRejectReasonGuards() {
    ...
    expectTrue(fuse::audio::hrtf_pan_path_reject_reason(false, offset)
                   == fuse::audio::HrtfPanPathRejectReason::HrtfDisabled,
               "disabled HRTF pan-path reject reason");
    expectTrue(fuse::audio::hrtf_pan_path_reject_reason(true, co_located)
                   == fuse::audio::HrtfPanPathRejectReason::CoLocated,
               "co-located pan-path reject reason");
    expectTrue(fuse::audio::hrtf_pan_path_reject_reason(true, offset)
                   == fuse::audio::HrtfPanPathRejectReason::None,
               "spatial offset has no pan-path reject reason");

    const fuse::audio::HrtfPanPathPreflight bypass_preflight =
        fuse::audio::preflight_hrtf_pan_path(false, valid, offset);
    expectTrue(bypass_preflight.reason == fuse::audio::HrtfPanPathRejectReason::HrtfDisabled,
               "preflight carries pan-path reject reason");
    expectTrue(fuse::audio::classify_hrtf_pan_path_reject(bypass_preflight)
                   == bypass_preflight.reason,
               "classify_hrtf_pan_path_reject mirrors preflight reason");

    fuse::audio::HrtfPanPathRejectReason reason = fuse::audio::HrtfPanPathRejectReason::None;
    expectTrue(!fuse::audio::try_preflight_hrtf_pan_path(false, valid, offset, reason),
               "try_preflight_hrtf_pan_path false when disabled");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::HrtfDisabled,
               "try_preflight_hrtf_pan_path writes reject reason");
    expectTrue(fuse::audio::preflight_hrtf_pan_path_ready(true, offset),
               "preflight_hrtf_pan_path_ready true on spatial offset");
    expectTrue(std::strcmp(fuse::audio::hrtf_pan_path_reject_reason_name(
                   fuse::audio::HrtfPanPathRejectReason::CoLocated),
               "CoLocated")
                   == 0,
...
void testHrtfAttenuationCouplingRejectReasonGuards() {
    ...
    expectTrue(bypass_preflight.reason
                   == fuse::audio::HrtfAttenuationCouplingRejectReason::BypassPath,
               "preflight carries coupling reject reason");
    expectTrue(fuse::audio::classify_hrtf_attenuation_coupling_reject(bypass_preflight)
                   == bypass_preflight.reason,
               "classify coupling reject mirrors preflight reason");

    fuse::audio::HrtfAttenuationCouplingRejectReason reason =
        fuse::audio::HrtfAttenuationCouplingRejectReason::None;
    expectTrue(!fuse::audio::try_preflight_hrtf_attenuation_coupling(
                   fuse::audio::HrtfPanPath::Bypass, 0.1f, 0.1f, reason),
               "try_preflight coupling false on bypass path");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::BypassPath,
               "try_preflight coupling writes reject reason");
    expectTrue(fuse::audio::preflight_hrtf_attenuation_coupling_ready(
                   fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f),
               "preflight coupling ready when narrowing applies");
    expectTrue(std::strcmp(fuse::audio::hrtf_attenuation_coupling_reject_reason_name(
                   fuse::audio::HrtfAttenuationCouplingRejectReason::UnityAttenuation),
               "UnityAttenuation")
                   == 0,
               "coupling reject reason label is readable");
}
...
void testHrtfBinauralRejectReasonGuards() {
    ...
    const fuse::audio::HrtfBinauralPreflight stub_preflight =
        fuse::audio::preflight_hrtf_binaural(true, empty, offset, 0.2f, 0.3f);
    expectTrue(stub_preflight.reason == fuse::audio::HrtfBinauralRejectReason::None,
               "spatial stub path has no binaural reject reason");
    expectTrue(stub_preflight.convolutionReason == fuse::audio::HrtfConvolutionRejectReason::EmptyIr,
               "empty IR composite marks convolution reject reason");
    expectTrue(fuse::audio::classify_hrtf_convolution_reject(stub_preflight)
                   == stub_preflight.convolutionReason,
               "classify convolution reject mirrors preflight reason");

    const fuse::audio::HrtfBinauralPreflight conv_preflight =
        fuse::audio::preflight_hrtf_binaural(true, valid, offset, 1.f, 1.f);
    expectTrue(conv_preflight.convolutionReason == fuse::audio::HrtfConvolutionRejectReason::None,
               "valid IR composite has no convolution reject reason");
    expectTrue(fuse::audio::hrtf_convolution_rejects_for_reason(
                   conv_preflight, fuse::audio::HrtfConvolutionRejectReason::None),
               "convolution rejects_for_reason None on valid IR");

    const fuse::audio::HrtfBinauralPreflight bypass_preflight =
        fuse::audio::preflight_hrtf_binaural(false, valid, offset, 0.1f, 0.1f);
    expectTrue(bypass_preflight.reason == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "disabled composite carries binaural reject reason");
    expectTrue(bypass_preflight.convolutionReason
                   == fuse::audio::HrtfConvolutionRejectReason::HrtfDisabled,
               "disabled composite carries convolution reject reason");
    expectTrue(fuse::audio::classify_hrtf_binaural_reject(bypass_preflight)
                   == bypass_preflight.reason,
               "classify binaural reject mirrors preflight reason");

    const fuse::audio::HrtfBinauralPreflight co_located_preflight =
        fuse::audio::preflight_hrtf_binaural(true, valid, co_located, 0.1f, 0.1f);
    expectTrue(co_located_preflight.reason == fuse::audio::HrtfBinauralRejectReason::CoLocated,
               "co-located composite carries binaural reject reason");
    expectTrue(fuse::audio::hrtf_binaural_rejects_for_reason(
                   co_located_preflight, fuse::audio::HrtfBinauralRejectReason::CoLocated),
               "binaural rejects_for_reason CoLocated");
    ...
    expectTrue(malformed_preflight.convolutionReason
               ...
               "malformed IR composite marks MalformedIr convolution reject");

    fuse::audio::HrtfBinauralRejectReason reason = fuse::audio::HrtfBinauralRejectReason::None;
    expectTrue(!fuse::audio::try_preflight_hrtf_binaural(false, valid, offset, 0.1f, 0.1f, reason),
               "try_preflight binaural false when disabled");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "try_preflight binaural writes reject reason");
    expectTrue(fuse::audio::preflight_hrtf_binaural_ready(true, empty, offset, 0.2f, 0.3f),
               "preflight binaural ready on spatial stub path");
    expectTrue(std::strcmp(fuse::audio::hrtf_binaural_reject_reason_name(
                   fuse::audio::HrtfBinauralRejectReason::HrtfDisabled),
               "HrtfDisabled")
                   == 0,
               "binaural reject reason label is readable");
    expectTrue(std::strcmp(fuse::audio::hrtf_convolution_reject_reason_name(
                   fuse::audio::HrtfConvolutionRejectReason::EmptyIr),
               "EmptyIr")
                   == 0,
               "convolution reject reason label is readable");
}
...
void testHrtfIrRejectReasonGuards() {
    ...
    expectTrue(reason == fuse::audio::HrtfIrRejectReason::NullSamples,
               "empty IR reject reason is null_samples");
    expectTrue(std::strcmp(fuse::audio::hrtf_ir_reject_reason_label(reason), "null_samples") == 0,
               "empty IR reject label is null_samples");

    const float samples[] = {0.5f};
    const fuse::audio::HrtfIrStub valid{samples, 1};
    expectTrue(fuse::audio::try_preflight_hrtf_ir(valid, reason),
               "valid IR try_preflight accepts convolution");
    expectTrue(reason == fuse::audio::HrtfIrRejectReason::None,
               "valid IR reject reason is none");

    const fuse::audio::HrtfIrStub malformed{samples, 0};
    expectTrue(!fuse::audio::try_preflight_hrtf_ir(malformed, reason),
               "malformed IR try_preflight rejects convolution");
    expectTrue(reason == fuse::audio::HrtfIrRejectReason::MalformedIr,
               "malformed IR reject reason is malformed_ir");
    expectTrue(fuse::audio::classify_hrtf_ir_reject(fuse::audio::preflight_hrtf_ir(malformed))
                   == fuse::audio::HrtfIrRejectReason::MalformedIr,
               "classify_hrtf_ir_reject marks malformed IR");

    const fuse::audio::HrtfIrStub null_samples_nonzero_length{nullptr, 4};
    const fuse::audio::HrtfIrPreflight null_preflight =
        fuse::audio::preflight_hrtf_ir(null_samples_nonzero_length);
    expectTrue(null_preflight.reject == fuse::audio::HrtfIrRejectReason::NullSamples,
               "null samples with non-zero length reject as null_samples");
}
...
void testHrtfPanPathRejectReasonGuards() {
    ...
    fuse::audio::HrtfPanPathRejectReason reason = fuse::audio::HrtfPanPathRejectReason::None;
    expectTrue(fuse::audio::try_preflight_hrtf_pan_path(true, empty, offset, reason),
               "enabled offset source try_preflight accepts spatial pan");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::None,
               "spatial pan reject reason is none on valid path");

    expectTrue(!fuse::audio::try_preflight_hrtf_pan_path(false, valid, offset, reason),
               "disabled HRTF try_preflight rejects spatial pan");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::HrtfDisabled,
               "disabled HRTF reject reason is hrtf_disabled");
    expectTrue(std::strcmp(fuse::audio::hrtf_pan_path_reject_reason_label(reason), "hrtf_disabled")
                   == 0,
               "disabled HRTF reject label is hrtf_disabled");

    expectTrue(!fuse::audio::try_preflight_hrtf_pan_path(true, valid, co_located, reason),
               "co-located source try_preflight rejects spatial pan");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::CoLocated,
               "co-located reject reason is co_located");

    expectTrue(fuse::audio::preflight_hrtf_pan_path_ready(true, offset),
               "no-IR overload preflight accepts spatial pan");
    expectTrue(!fuse::audio::preflight_hrtf_pan_path_ready(false, offset, &reason),
               "no-IR overload preflight rejects when disabled");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::HrtfDisabled,
               "no-IR overload reject reason is hrtf_disabled");
}
...
void testHrtfAttenuationCouplingRejectReasonGuards() {
    ...
    expectTrue(fuse::audio::try_preflight_hrtf_attenuation_coupling(
                   fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f, reason),
               "reduced attenuation try_preflight accepts narrowing");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "narrowing reject reason is none on valid path");

    expectTrue(!fuse::audio::try_preflight_hrtf_attenuation_coupling(
                   fuse::audio::HrtfPanPath::Bypass, 0.1f, 0.1f, reason),
               "bypass path try_preflight rejects narrowing");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::BypassPath,
               "bypass path reject reason is bypass_path");
    expectTrue(std::strcmp(fuse::audio::hrtf_attenuation_coupling_reject_reason_label(reason),
                          "bypass_path")
                   == 0,
               "bypass path reject label is bypass_path");

    expectTrue(!fuse::audio::try_preflight_hrtf_attenuation_coupling(
                   fuse::audio::HrtfPanPath::Convolution, 1.f, 1.f, reason),
               "unity attenuation try_preflight rejects narrowing");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::UnityAttenuation,
               "unity attenuation reject reason is unity_attenuation");

    const fuse::audio::HrtfAttenuationCouplingPreflight preflight =
        fuse::audio::preflight_hrtf_attenuation_coupling(fuse::audio::HrtfPanPath::IldItdStub,
                                                         0.2f, 0.3f);
    expectTrue(preflight.reject == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "preflight struct carries none reject on valid narrowing path");
    expectTrue(fuse::audio::classify_hrtf_attenuation_coupling_reject(preflight)
                   == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "classify mirrors preflight reject on valid path");
}
...
void testHrtfBinauralRejectReasonGuards() {
    ...
    expectTrue(fuse::audio::try_preflight_hrtf_binaural(true, empty, offset, 0.2f, 0.3f, reason),
               "enabled stub path try_preflight accepts spatial pan");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::None,
               "composite reject reason is none on valid path");

    expectTrue(!fuse::audio::try_preflight_hrtf_binaural(false, valid, offset, 0.1f, 0.1f, reason),
               "disabled HRTF composite try_preflight rejects spatial pan");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "composite reject reason is hrtf_disabled");
    expectTrue(std::strcmp(fuse::audio::hrtf_binaural_reject_reason_label(reason), "hrtf_disabled")
                   == 0,
               "composite reject label is hrtf_disabled");

    expectTrue(!fuse::audio::try_preflight_hrtf_binaural(true, valid, co_located, 0.1f, 0.1f,
                                                         reason),
               "co-located composite try_preflight rejects spatial pan");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::CoLocated,
               "composite reject reason is co_located");

    const fuse::audio::HrtfBinauralPreflight stub_preflight =
        fuse::audio::preflight_hrtf_binaural(true, empty, offset, 0.2f, 0.3f);
    expectTrue(stub_preflight.convolveReject
                   == fuse::audio::HrtfBinauralConvolveRejectReason::NullSamples,
               "empty IR composite convolve reject is null_samples");
    expectTrue(std::strcmp(fuse::audio::hrtf_binaural_convolve_reject_reason_label(
                               stub_preflight.convolveReject),
                           "null_samples")
                   == 0,
               "empty IR convolve reject label is null_samples");

    const float samples_malformed[] = {1.f};
    const fuse::audio::HrtfIrStub malformed{samples_malformed, 0};
    const fuse::audio::HrtfBinauralPreflight malformed_preflight =
        fuse::audio::preflight_hrtf_binaural(true, malformed, offset, 0.2f, 0.3f);
    expectTrue(malformed_preflight.convolveReject
                   == fuse::audio::HrtfBinauralConvolveRejectReason::MalformedIr,
               "malformed IR composite convolve reject is malformed_ir");

    const fuse::audio::HrtfBinauralPreflight conv_preflight =
        fuse::audio::preflight_hrtf_binaural(true, valid, offset, 1.f, 1.f);
    expectTrue(conv_preflight.convolveReject == fuse::audio::HrtfBinauralConvolveRejectReason::None,
               "valid IR composite convolve reject is none");
    expectTrue(fuse::audio::classify_hrtf_binaural_convolve_reject(conv_preflight)
                   == fuse::audio::HrtfBinauralConvolveRejectReason::None,
               "classify_hrtf_binaural_convolve_reject is none on valid IR");

    const fuse::audio::HrtfBinauralPreflight bypass_preflight =
        fuse::audio::preflight_hrtf_binaural(false, valid, offset, 0.1f, 0.1f);
    expectTrue(bypass_preflight.convolveReject
                   == fuse::audio::HrtfBinauralConvolveRejectReason::Bypass,
               "bypass composite convolve reject is bypass");
    expectTrue(fuse::audio::classify_hrtf_binaural_reject(bypass_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
...
void testRejectReasonMirrorsExistingPreflights() {
    ...
    const fuse::audio::HrtfIrPreflight ir_preflight = fuse::audio::preflight_hrtf_ir(empty);
    expectTrue(ir_preflight.reject
                   == fuse::audio::classify_hrtf_ir_reject(ir_preflight),
               "preflight IR reject matches classify_hrtf_ir_reject");

    const fuse::audio::HrtfPanPathPreflight pan_preflight =
        fuse::audio::preflight_hrtf_pan_path(false, valid, offset);
    expectTrue(pan_preflight.reject
                   == fuse::audio::classify_hrtf_pan_path_reject(pan_preflight),
               "preflight pan-path reject matches classify_hrtf_pan_path_reject");

    const fuse::audio::HrtfBinauralPreflight composite =
        fuse::audio::preflight_hrtf_binaural(true, empty, offset, 0.2f, 0.3f);
    expectTrue(composite.reject
                   == fuse::audio::classify_hrtf_binaural_reject(composite),
               "composite reject matches classify_hrtf_binaural_reject");
    expectTrue(composite.convolveReject
                   == fuse::audio::classify_hrtf_binaural_convolve_reject(composite),
               "composite convolve reject matches classify_hrtf_binaural_convolve_reject");
}
...
void testHrtfIrRejectReasons() {
    ...
    expectTrue(fuse::audio::hrtf_ir_reject_reason(empty)
                   == fuse::audio::HrtfIrRejectReason::NullSamples,
               "empty IR rejects with NullSamples");
    expectTrue(fuse::audio::hrtf_ir_rejects_for_reason(empty,
                                                       fuse::audio::HrtfIrRejectReason::NullSamples),
               "hrtf_ir_rejects_for_reason matches NullSamples");
    ...
    expectTrue(fuse::audio::hrtf_ir_rejects_for_reason(malformed,
                                                       fuse::audio::HrtfIrRejectReason::MalformedIr),
               "hrtf_ir_rejects_for_reason matches MalformedIr");

    const fuse::audio::HrtfIrPreflight empty_preflight = fuse::audio::preflight_hrtf_ir(empty);
    expectTrue(empty_preflight.reason == fuse::audio::HrtfIrRejectReason::NullSamples,
               "preflight carries NullSamples reject reason");
    expectTrue(fuse::audio::classify_hrtf_ir_reject(empty_preflight)
                   == fuse::audio::HrtfIrRejectReason::NullSamples,
               "classify_hrtf_ir_reject maps nullSamples flag");
    ...
    const fuse::audio::HrtfIrPreflight malformed_preflight =
        fuse::audio::preflight_hrtf_ir(malformed);
    expectTrue(malformed_preflight.reason == fuse::audio::HrtfIrRejectReason::MalformedIr,
               "preflight carries MalformedIr reject reason");
    expectTrue(fuse::audio::classify_hrtf_ir_reject(malformed_preflight)
                   == fuse::audio::HrtfIrRejectReason::MalformedIr,
               "classify_hrtf_ir_reject maps malformedIr flag");

    expectTrue(std::strcmp(fuse::audio::hrtf_ir_reject_reason_name(
                               fuse::audio::HrtfIrRejectReason::MalformedIr),
                           "MalformedIr")
                   == 0,
               "hrtf_ir_reject_reason_name returns MalformedIr label");
}
...
void testHrtfPanPathRejectReasons() {
    ...
    expectTrue(fuse::audio::hrtf_pan_path_rejects_for_reason(
                   false, offset, fuse::audio::HrtfPanPathRejectReason::HrtfDisabled),
               "hrtf_pan_path_rejects_for_reason matches HrtfDisabled");
    ...
    expectTrue(fuse::audio::hrtf_pan_path_reject_reason(true, offset)
                   == fuse::audio::HrtfPanPathRejectReason::None,
               "enabled offset source has no reject reason");
    ...
    const fuse::audio::HrtfPanPathPreflight co_located_preflight =
        fuse::audio::preflight_hrtf_pan_path(true, co_located);
    expectTrue(co_located_preflight.reason == fuse::audio::HrtfPanPathRejectReason::CoLocated,
               "pan-path preflight carries CoLocated reject reason");
    expectTrue(fuse::audio::classify_hrtf_pan_path_reject(co_located_preflight)
                   == fuse::audio::HrtfPanPathRejectReason::CoLocated,
               "classify_hrtf_pan_path_reject maps coLocated flag");

    const fuse::audio::HrtfIrStub empty{};
    const fuse::audio::HrtfPanPathPreflight stub_preflight =
        fuse::audio::preflight_hrtf_pan_path(true, empty, offset);
    expectTrue(stub_preflight.reason == fuse::audio::HrtfPanPathRejectReason::None,
               "empty IR does not reject pan-path routing");
    expectTrue(stub_preflight.can_spatial_pan(), "empty IR still allows spatial pan");

    expectTrue(std::strcmp(fuse::audio::hrtf_pan_path_reject_reason_name(
                               fuse::audio::HrtfPanPathRejectReason::CoLocated),
                           "CoLocated")
                   == 0,
               "hrtf_pan_path_reject_reason_name returns CoLocated label");
}
...
void testHrtfAttenuationCouplingRejectReasons() {
    ...
    expectTrue(bypass_preflight.reason
                   == fuse::audio::HrtfAttenuationCouplingRejectReason::BypassPath,
               "coupling preflight carries BypassPath reject reason");
    expectTrue(fuse::audio::classify_hrtf_attenuation_coupling_reject(bypass_preflight)
                   == fuse::audio::HrtfAttenuationCouplingRejectReason::BypassPath,
               "classify_hrtf_attenuation_coupling_reject maps bypassPath flag");
    ...
    expectTrue(unity_preflight.reason
                   == fuse::audio::HrtfAttenuationCouplingRejectReason::UnityAttenuation,
               "coupling preflight carries UnityAttenuation reject reason");
    expectTrue(fuse::audio::classify_hrtf_attenuation_coupling_reject(unity_preflight)
                   == fuse::audio::HrtfAttenuationCouplingRejectReason::UnityAttenuation,
               "classify_hrtf_attenuation_coupling_reject maps unityAttenuation flag");

    const fuse::audio::HrtfAttenuationCouplingPreflight narrow_preflight =
        fuse::audio::preflight_hrtf_attenuation_coupling(fuse::audio::HrtfPanPath::IldItdStub,
                                                         0.2f, 0.3f);
    expectTrue(narrow_preflight.reason == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "reduced attenuation coupling preflight has no reject reason");
    expectTrue(narrow_preflight.can_narrow(), "reduced attenuation preflight can narrow");

    expectTrue(std::strcmp(fuse::audio::hrtf_attenuation_coupling_reject_reason_name(
                               fuse::audio::HrtfAttenuationCouplingRejectReason::UnityAttenuation),
                           "UnityAttenuation")
                   == 0,
               "hrtf_attenuation_coupling_reject_reason_name returns UnityAttenuation label");
}
...
void testHrtfBinauralRejectReasons() {
    ...
    const fuse::audio::HrtfBinauralPreflight stub_preflight =
        fuse::audio::preflight_hrtf_binaural(true, empty, offset, 0.2f, 0.3f);
    expectTrue(stub_preflight.reason == fuse::audio::HrtfBinauralRejectReason::None,
               "composite stub path has no reject reason");
    expectTrue(fuse::audio::classify_hrtf_binaural_reject(stub_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::None,
               "classify_hrtf_binaural_reject maps None on stub path");
    expectTrue(stub_preflight.can_convolve()
                   == stub_preflight.ir.can_convolve(),
               "composite can_convolve mirrors IR preflight on stub path");

    const fuse::audio::HrtfBinauralPreflight conv_preflight =
        fuse::audio::preflight_hrtf_binaural(true, valid, offset, 1.f, 1.f);
    expectTrue(conv_preflight.reason == fuse::audio::HrtfBinauralRejectReason::None,
               "valid IR composite path has no reject reason");
    expectTrue(conv_preflight.can_convolve(), "valid IR composite preflight can convolve");
    ...
    expectTrue(disabled_preflight.reason == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "disabled HRTF composite rejects with HrtfDisabled");
    expectTrue(fuse::audio::classify_hrtf_binaural_reject(disabled_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "classify_hrtf_binaural_reject maps HrtfDisabled");
    ...
    expectTrue(fuse::audio::classify_hrtf_binaural_reject(co_located_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::CoLocated,
               "classify_hrtf_binaural_reject maps CoLocated");

    expectTrue(std::strcmp(fuse::audio::hrtf_binaural_reject_reason_name(
                               fuse::audio::HrtfBinauralRejectReason::HrtfDisabled),
                           "HrtfDisabled")
                   == 0,
               "hrtf_binaural_reject_reason_name returns HrtfDisabled label");
}
...
void testHrtfIrRejectReasons() {
    ...
    expectTrue(fuse::audio::hrtf_ir_rejects_for_reason(empty, fuse::audio::HrtfIrRejectReason::NullSamples),
               "canonical empty IR rejects with NullSamples");
    expectTrue(
        std::strcmp(fuse::audio::hrtf_ir_reject_reason_name(fuse::audio::HrtfIrRejectReason::NullSamples),
                    "NullSamples") == 0,
        "empty-IR reject reason name is readable");
    ...
    expectTrue(fuse::audio::hrtf_ir_rejects_for_reason(malformed,
                                                       fuse::audio::HrtfIrRejectReason::MalformedIr),
               "non-null zero-length IR rejects with MalformedIr");
...
void testHrtfPanPathRejectReasons() {
    ...
    const fuse::audio::HrtfPanPathPreflight preflight =
        fuse::audio::preflight_hrtf_pan_path(true, offset);
    const fuse::audio::HrtfPanPath resolved =
        fuse::audio::resolve_hrtf_pan_path(true, offset);
    expectTrue(fuse::audio::hrtf_pan_path_matches_preflight(resolved, preflight),
               "resolve_hrtf_pan_path matches pan-path preflight");
    expectTrue(preflight.reason == fuse::audio::HrtfPanPathRejectReason::None,
               "spatial stub preflight carries None reject reason");
}
...
void testHrtfBinauralRejectReasons() {
    ...
    expectTrue(fuse::audio::hrtf_binaural_skip_rejects_for_reason(
                   false, offset, fuse::audio::HrtfBinauralRejectReason::HrtfDisabled),
               "composite skip reject marks disabled HRTF");
    expectTrue(fuse::audio::hrtf_binaural_skip_rejects_for_reason(
                   true, co_located, fuse::audio::HrtfBinauralRejectReason::CoLocated),
               "composite skip reject marks co-located source");
    expectTrue(fuse::audio::hrtf_binaural_convolution_rejects_for_reason(
                   empty, fuse::audio::HrtfBinauralRejectReason::NullSamples),
               "composite convolution reject maps empty IR to NullSamples");

    const fuse::audio::HrtfBinauralPreflight preflight =
        fuse::audio::preflight_hrtf_binaural(true, empty, offset, 0.2f, 0.3f);
    expectTrue(preflight.skipReason == fuse::audio::HrtfBinauralRejectReason::None,
               "spatial stub composite preflight has no skip reject reason");
    expectTrue(preflight.convolutionReason == fuse::audio::HrtfBinauralRejectReason::NullSamples,
               "empty IR composite preflight maps convolution reject");
    expectTrue(preflight.attenuationReason == fuse::audio::HrtfBinauralRejectReason::None,
               "reduced attenuation composite preflight has no attenuation reject reason");
    expectTrue(
        fuse::audio::hrtf_binaural_reject_reason_from_ir(fuse::audio::HrtfIrRejectReason::MalformedIr)
            == fuse::audio::HrtfBinauralRejectReason::MalformedIr,
        "IR reject reason maps into composite enum");
    expectTrue(
        std::strcmp(
            fuse::audio::hrtf_binaural_reject_reason_name(fuse::audio::HrtfBinauralRejectReason::BypassPath),
            "BypassPath") == 0,
        "composite reject reason name is readable");

    const fuse::audio::HrtfBinauralPreflight unity_preflight =
        fuse::audio::preflight_hrtf_binaural(true, empty, offset, 1.f, 1.f);
    expectTrue(unity_preflight.attenuationReason
                   == fuse::audio::HrtfBinauralRejectReason::UnityAttenuation,
               "unity attenuation composite preflight maps attenuation reject");
}
...
void testHrtfIrRejectReasonGuards() {
    ...
    expectTrue(empty_preflight.reason == fuse::audio::HrtfIrRejectReason::NullSamples,
               "empty IR reject reason is NullSamples");
    expectTrue(fuse::audio::hrtf_ir_rejects_for_reason(empty_preflight,
                                                       fuse::audio::HrtfIrRejectReason::NullSamples),
               "hrtf_ir_rejects_for_reason matches empty IR");
    expectTrue(std::strcmp(fuse::audio::hrtf_ir_reject_reason_label(
                               fuse::audio::HrtfIrRejectReason::NullSamples),
                           "NullSamples")
                   == 0,
               "IR reject reason label for NullSamples");
    ...
    expectTrue(fuse::audio::classify_hrtf_ir_reject(valid_preflight)
                   == fuse::audio::HrtfIrRejectReason::None,
               "classify_hrtf_ir_reject matches valid IR");
    ...
    expectTrue(!fuse::audio::try_preflight_hrtf_ir(empty, reason),
               "try_preflight_hrtf_ir false on empty IR");
    expectTrue(reason == fuse::audio::HrtfIrRejectReason::NullSamples,
               "try_preflight_hrtf_ir sets reject reason");
    expectTrue(fuse::audio::preflight_hrtf_ir_ready(valid) == valid_preflight.can_convolve(),
               "preflight_hrtf_ir_ready mirrors can_convolve on valid IR");
}
...
void testHrtfPanPathRejectReasonGuards() {
    ...
    const fuse::audio::HrtfPanPathPreflight stub_preflight =
        fuse::audio::preflight_hrtf_pan_path(true, empty, offset);
    expectTrue(stub_preflight.reason == fuse::audio::HrtfPanPathRejectReason::None,
               "ILD/ITD stub path spatial reject reason is None");
    ...
    expectTrue(fuse::audio::classify_hrtf_pan_path_convolution_reject(stub_preflight,
                                                                      empty_ir_preflight)
                   == fuse::audio::HrtfPanPathConvolutionRejectReason::EmptyIr,
               "classify convolution reject uses IR preflight");

    const fuse::audio::HrtfPanPathPreflight conv_preflight =
        fuse::audio::preflight_hrtf_pan_path(true, valid, offset);
    expectTrue(conv_preflight.convolutionReason == fuse::audio::HrtfPanPathConvolutionRejectReason::None,
               "valid IR convolution reject reason is None");

    const fuse::audio::HrtfPanPathPreflight disabled_preflight =
        fuse::audio::preflight_hrtf_pan_path(false, valid, offset);
    expectTrue(disabled_preflight.reason == fuse::audio::HrtfPanPathRejectReason::HrtfDisabled,
               "disabled HRTF pan reject reason is HrtfDisabled");
    expectTrue(disabled_preflight.convolutionReason
                   == fuse::audio::HrtfPanPathConvolutionRejectReason::HrtfDisabled,
               "disabled HRTF convolution reject reason is HrtfDisabled");

    const fuse::audio::HrtfPanPathPreflight co_located_preflight =
        fuse::audio::preflight_hrtf_pan_path(true, valid, co_located);
    expectTrue(co_located_preflight.reason == fuse::audio::HrtfPanPathRejectReason::CoLocated,
               "co-located pan reject reason is CoLocated");
    expectTrue(std::strcmp(fuse::audio::hrtf_pan_path_reject_reason_label(
                               fuse::audio::HrtfPanPathRejectReason::CoLocated),
                           "CoLocated")
                   == 0,
    ...
    fuse::audio::HrtfPanPathRejectReason reason = fuse::audio::HrtfPanPathRejectReason::None;
    expectTrue(fuse::audio::try_preflight_hrtf_pan_path(true, valid, offset, reason),
               "try_preflight_hrtf_pan_path true on valid spatial path");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::None,
               "try_preflight_hrtf_pan_path leaves None on valid path");
    expectTrue(fuse::audio::preflight_hrtf_pan_path_ready(false, valid, offset)
                   == disabled_preflight.can_spatial_pan(),
               "preflight_hrtf_pan_path_ready mirrors can_spatial_pan on bypass");
}
...
void testHrtfAttenuationCouplingRejectReasonGuards() {
    ...
    expectTrue(fuse::audio::hrtf_attenuation_coupling_rejects_for_reason(
               ...
               "hrtf_attenuation_coupling_rejects_for_reason matches unity attenuation");
    ...
    expectTrue(std::strcmp(fuse::audio::hrtf_attenuation_coupling_reject_reason_label(
                           ...
                           "BypassPath")
                   == 0,
               "attenuation coupling reject reason label for BypassPath");

    fuse::audio::HrtfAttenuationCouplingRejectReason reason =
        fuse::audio::HrtfAttenuationCouplingRejectReason::None;
    expectTrue(fuse::audio::try_preflight_hrtf_attenuation_coupling(
                   fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f, reason),
               "try_preflight_hrtf_attenuation_coupling true when narrowing applies");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "try_preflight_hrtf_attenuation_coupling leaves None when narrowing applies");
    expectTrue(fuse::audio::preflight_hrtf_attenuation_coupling_ready(
                   fuse::audio::HrtfPanPath::Bypass, 0.1f, 0.1f)
                   == bypass_preflight.can_narrow(),
               "preflight_hrtf_attenuation_coupling_ready mirrors can_narrow on bypass");
}
...
void testHrtfBinauralRejectReasonGuards() {
    ...
    const fuse::audio::HrtfBinauralPreflight stub_preflight =
        fuse::audio::preflight_hrtf_binaural(true, empty, offset, 0.2f, 0.3f);
    expectTrue(stub_preflight.reason == fuse::audio::HrtfBinauralRejectReason::EmptyIr,
               "composite stub path reject reason is EmptyIr");
    expectTrue(fuse::audio::classify_hrtf_binaural_reject(stub_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::EmptyIr,
               "classify_hrtf_binaural_reject matches stub path");
    ...
    expectTrue(narrow_preflight.reason == fuse::audio::HrtfBinauralRejectReason::None,
               "valid spatial path with reduced attenuation has None composite reject");
    ...
    expectTrue(fuse::audio::hrtf_binaural_rejects_for_reason(
                   bypass_preflight, fuse::audio::HrtfBinauralRejectReason::HrtfDisabled),
               "hrtf_binaural_rejects_for_reason matches bypass");

    fuse::audio::HrtfBinauralRejectReason reason = fuse::audio::HrtfBinauralRejectReason::None;
    expectTrue(fuse::audio::try_preflight_hrtf_binaural(true, valid, offset, 0.2f, 0.3f, reason),
               "try_preflight_hrtf_binaural true on valid spatial path");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::None,
               "try_preflight_hrtf_binaural leaves None on valid spatial path");
}
...
void testRejectReasonMirrorsExistingPreflights() {
    ...
    const fuse::audio::HrtfIrPreflight ir_preflight = fuse::audio::preflight_hrtf_ir(valid);
    expectTrue(fuse::audio::preflight_hrtf_ir_ready(valid) == ir_preflight.can_convolve(),
               "preflight_hrtf_ir_ready mirrors can_convolve");

    const fuse::audio::HrtfPanPathPreflight pan_preflight =
        fuse::audio::preflight_hrtf_pan_path(true, empty, offset);
    expectTrue(fuse::audio::preflight_hrtf_pan_path_ready(true, empty, offset)
                   == pan_preflight.can_spatial_pan(),
               "preflight_hrtf_pan_path_ready mirrors can_spatial_pan");
    ...
    expectTrue(fuse::audio::preflight_hrtf_attenuation_coupling_ready(
                   fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f)
                   == coupling_preflight.can_narrow(),
               "preflight_hrtf_attenuation_coupling_ready mirrors can_narrow");

    const fuse::audio::HrtfBinauralPreflight composite_preflight =
        fuse::audio::preflight_hrtf_binaural(true, empty, offset, 0.2f, 0.3f);
    expectTrue(fuse::audio::preflight_hrtf_binaural_ready(true, empty, offset, 0.2f, 0.3f)
                   == composite_preflight.can_spatial_pan(),
               "preflight_hrtf_binaural_ready mirrors can_spatial_pan");

    expectTrue(fuse::audio::classify_hrtf_ir_reject(ir_preflight) == ir_preflight.reason,
               "classify_hrtf_ir_reject mirrors stored IR reason");
    expectTrue(fuse::audio::classify_hrtf_pan_path_reject(pan_preflight) == pan_preflight.reason,
               "classify_hrtf_pan_path_reject mirrors stored pan-path reason");
    expectTrue(fuse::audio::classify_hrtf_attenuation_coupling_reject(coupling_preflight)
                   == coupling_preflight.reason,
               "classify_hrtf_attenuation_coupling_reject mirrors stored coupling reason");
    expectTrue(fuse::audio::classify_hrtf_binaural_reject(composite_preflight)
                   == composite_preflight.reason,
               "classify_hrtf_binaural_reject mirrors stored composite reason");
}
...
void testHrtfPanPathRejectReasonGuards() {
    ...
    expectTrue(fuse::audio::try_preflight_hrtf_pan_path(true, empty, offset, reason),
               "try_preflight_hrtf_pan_path accepts spatial stub path");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::None,
               "spatial stub path reject reason is None");
    ...
    expectTrue(fuse::audio::preflight_hrtf_pan_path_ready(true, offset, &reason),
               "no-IR overload accepts spatial stub path");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::None,
               "no-IR overload reject reason is None on valid path");
}
...
void testHrtfAttenuationCouplingRejectReasonGuards() {
    ...
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "reduced attenuation reject reason is None");
    ...
    expectTrue(std::strcmp(fuse::audio::hrtf_attenuation_coupling_reject_reason_label(
                   fuse::audio::HrtfAttenuationCouplingRejectReason::UnityAttenuation),
               "UnityAttenuation") == 0,
               "attenuation coupling reject reason label for UnityAttenuation");
}
...
void testHrtfBinauralRejectReasonGuards() {
    ...
    fuse::audio::HrtfBinauralRejectReason reason = fuse::audio::HrtfBinauralRejectReason::None;
    expectTrue(!fuse::audio::try_preflight_hrtf_binaural_pan(false, valid, offset, reason),
               "try_preflight_hrtf_binaural_pan rejects disabled HRTF");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "composite pan reject reason is HrtfDisabled");

    expectTrue(!fuse::audio::try_preflight_hrtf_binaural_convolution(empty, reason),
               "try_preflight_hrtf_binaural_convolution rejects empty IR");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::EmptyIr,
               "composite convolution reject reason is EmptyIr");

    const fuse::audio::HrtfIrStub malformed{samples, 0};
    expectTrue(!fuse::audio::preflight_hrtf_binaural_convolution_ready(malformed, &reason),
               "preflight_hrtf_binaural_convolution_ready rejects malformed IR");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::MalformedIr,
               "composite convolution reject reason is MalformedIr");

    expectTrue(fuse::audio::try_preflight_hrtf_binaural_convolution(valid, reason),
               "try_preflight_hrtf_binaural_convolution accepts valid IR");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::None,
               "valid IR composite convolution reject reason is None");

    expectTrue(!fuse::audio::try_preflight_hrtf_binaural_narrowing(
                   fuse::audio::HrtfPanPath::Bypass, 0.1f, 0.1f, reason),
               "try_preflight_hrtf_binaural_narrowing rejects bypass path");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::BypassPath,
               "composite narrowing reject reason is BypassPath");

    expectTrue(!fuse::audio::preflight_hrtf_binaural_narrowing_ready(
                   fuse::audio::HrtfPanPath::Convolution, 1.f, 1.f, {}, {}, &reason),
               "preflight_hrtf_binaural_narrowing_ready rejects unity attenuation");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::UnityAttenuation,
               "composite narrowing reject reason is UnityAttenuation");

    const fuse::audio::HrtfBinauralPreflight stub_preflight =
        fuse::audio::preflight_hrtf_binaural(true, empty, offset, 0.2f, 0.3f);
    expectTrue(fuse::audio::classify_hrtf_binaural_pan_reject(stub_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::None,
               "classify_hrtf_binaural_pan_reject is None on spatial stub path");
    expectTrue(fuse::audio::classify_hrtf_binaural_convolution_reject(stub_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::EmptyIr,
               "classify_hrtf_binaural_convolution_reject maps empty IR");
    expectTrue(fuse::audio::classify_hrtf_binaural_narrowing_reject(stub_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::None,
               "classify_hrtf_binaural_narrowing_reject is None on reduced attenuation");

    const fuse::audio::HrtfBinauralPreflight bypass_preflight =
        fuse::audio::preflight_hrtf_binaural(false, valid, co_located, 0.1f, 0.1f);
    expectTrue(fuse::audio::classify_hrtf_binaural_pan_reject(bypass_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "classify_hrtf_binaural_pan_reject maps disabled HRTF");
    expectTrue(std::strcmp(fuse::audio::hrtf_binaural_reject_reason_label(
                   fuse::audio::HrtfBinauralRejectReason::EmptyIr),
               "EmptyIr") == 0,
               "composite reject reason label for EmptyIr");

    expectTrue(fuse::audio::preflight_hrtf_binaural_pan_ready(true, offset, &reason),
               "no-IR composite pan preflight accepts spatial stub path");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::None,
               "no-IR composite pan reject reason is None");
}
...
void testHrtfIrRejectReasonGuards() {
    ...
    expectTrue(!fuse::audio::try_preflight_hrtf_ir(empty, reason),
               "empty IR rejects convolution preflight");
    expectTrue(reason == fuse::audio::HrtfIrRejectReason::NullSamples,
               "canonical empty IR classifies as NullSamples");
    expectTrue(std::strcmp(fuse::audio::hrtf_ir_reject_reason_label(reason), "NullSamples") == 0,
               "NullSamples label is stable");

    const float samples[] = {0.5f};
    const fuse::audio::HrtfIrStub valid{samples, 1};
    expectTrue(fuse::audio::try_preflight_hrtf_ir(valid, reason),
               "valid IR accepts convolution preflight");
    expectTrue(reason == fuse::audio::HrtfIrRejectReason::None,
               "valid IR reject reason is None");

    const fuse::audio::HrtfIrStub malformed{samples, 0};
    expectTrue(!fuse::audio::preflight_hrtf_ir_ready(malformed, &reason),
               "malformed IR rejects convolution preflight");
    expectTrue(reason == fuse::audio::HrtfIrRejectReason::MalformedIr,
               "malformed IR classifies as MalformedIr");
    expectTrue(fuse::audio::classify_hrtf_ir_reject(fuse::audio::preflight_hrtf_ir(malformed))
                   == fuse::audio::HrtfIrRejectReason::MalformedIr,
               "classify_hrtf_ir_reject mirrors malformed preflight");

    const fuse::audio::HrtfIrStub zero_length =
        fuse::audio::make_hrtf_ir_stub(samples, 0);
    expectTrue(fuse::audio::classify_hrtf_ir_reject(fuse::audio::preflight_hrtf_ir(zero_length))
                   == fuse::audio::HrtfIrRejectReason::NullSamples,
               "factory zero-length normalizes to canonical empty IR");
}
...
void testHrtfPanPathRejectReasonGuards() {
    ...
    fuse::audio::HrtfPanPathRejectReason reason = fuse::audio::HrtfPanPathRejectReason::None;
    expectTrue(fuse::audio::try_preflight_hrtf_pan_path(true, empty, offset, reason),
               "enabled offset source accepts spatial pan preflight");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::None,
               "spatial pan reject reason is None on valid path");

    expectTrue(!fuse::audio::try_preflight_hrtf_pan_path(false, valid, offset, reason),
               "disabled HRTF rejects spatial pan preflight");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::HrtfDisabled,
               "disabled HRTF classifies as HrtfDisabled");
    expectTrue(std::strcmp(fuse::audio::hrtf_pan_path_reject_reason_label(reason), "HrtfDisabled")
                   == 0,
               "HrtfDisabled label is stable");

    expectTrue(!fuse::audio::try_preflight_hrtf_pan_path(true, valid, co_located, reason),
               "co-located source rejects spatial pan preflight");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::CoLocated,
               "co-located source classifies as CoLocated");

    expectTrue(fuse::audio::preflight_hrtf_pan_path_ready(true, offset, &reason),
               "no-IR overload accepts spatial pan preflight");
               "no-IR overload reject reason is None on valid path");

void testHrtfConvolutionRejectReasonGuards() {

    fuse::audio::HrtfConvolutionRejectReason reason = fuse::audio::HrtfConvolutionRejectReason::None;
    expectTrue(!fuse::audio::try_preflight_hrtf_convolution(true, empty, offset, reason),
               "empty IR rejects convolution dispatch preflight");
    expectTrue(reason == fuse::audio::HrtfConvolutionRejectReason::EmptyIr,
               "empty IR classifies as EmptyIr for convolution");
    expectTrue(std::strcmp(fuse::audio::hrtf_convolution_reject_reason_label(reason), "EmptyIr")
               "EmptyIr convolution label is stable");

    expectTrue(fuse::audio::try_preflight_hrtf_convolution(true, valid, offset, reason),
               "valid IR accepts convolution dispatch preflight");
    expectTrue(reason == fuse::audio::HrtfConvolutionRejectReason::None,
               "valid IR convolution reject reason is None");

    expectTrue(!fuse::audio::try_preflight_hrtf_convolution(false, valid, offset, reason),
               "disabled HRTF rejects convolution dispatch");
    expectTrue(reason == fuse::audio::HrtfConvolutionRejectReason::HrtfDisabled,
               "disabled HRTF convolution classifies as HrtfDisabled");

    expectTrue(!fuse::audio::try_preflight_hrtf_convolution(true, valid, co_located, reason),
               "co-located source rejects convolution dispatch");
    expectTrue(reason == fuse::audio::HrtfConvolutionRejectReason::CoLocated,
               "co-located convolution classifies as CoLocated");

    expectTrue(fuse::audio::classify_hrtf_convolution_reject(
                   fuse::audio::preflight_hrtf_ir(malformed),
                   fuse::audio::preflight_hrtf_pan_path(true, malformed, offset))
                   == fuse::audio::HrtfConvolutionRejectReason::MalformedIr,
               "malformed IR classifies as MalformedIr for convolution");
...
void testHrtfAttenuationCouplingRejectReasonGuards() {
    ...
    expectTrue(!fuse::audio::try_preflight_hrtf_attenuation_coupling(
                   fuse::audio::HrtfPanPath::Bypass, 0.1f, 0.1f, reason),
               "bypass path rejects attenuation coupling preflight");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::BypassPath,
               "bypass path classifies as BypassPath");
    expectTrue(std::strcmp(fuse::audio::hrtf_attenuation_coupling_reject_reason_label(reason),
                           "BypassPath")
               "BypassPath coupling label is stable");

                   fuse::audio::HrtfPanPath::Convolution, 1.f, 1.f, reason),
               "unity attenuation rejects coupling preflight");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::UnityAttenuation,
               "unity attenuation classifies as UnityAttenuation");

    expectTrue(fuse::audio::try_preflight_hrtf_attenuation_coupling(
                   fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f, reason),
               "reduced attenuation accepts coupling preflight");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "narrowing coupling reject reason is None on valid path");
    expectTrue(fuse::audio::classify_hrtf_attenuation_coupling_reject(
                   fuse::audio::preflight_hrtf_attenuation_coupling(
                       fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f))
                   == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "classify coupling reject mirrors valid preflight");

void testHrtfBinauralRejectReasonGuards() {

    fuse::audio::HrtfBinauralRejectReason reason = fuse::audio::HrtfBinauralRejectReason::None;

    expectTrue(fuse::audio::try_preflight_hrtf_binaural_spatial(true, empty, offset, reason),
               "composite spatial preflight accepts stub path");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::None,
               "composite spatial reject reason is None on valid path");

    expectTrue(!fuse::audio::try_preflight_hrtf_binaural_spatial(false, valid, offset, reason),
               "composite spatial preflight rejects disabled HRTF");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "composite spatial classifies as HrtfDisabled");
    expectTrue(std::strcmp(fuse::audio::hrtf_binaural_reject_reason_label(reason), "HrtfDisabled")
               "composite HrtfDisabled label is stable");

    expectTrue(!fuse::audio::try_preflight_hrtf_binaural_spatial(true, valid, co_located, reason),
               "composite spatial preflight rejects co-located source");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::CoLocated,
               "composite spatial classifies as CoLocated");

    expectTrue(!fuse::audio::try_preflight_hrtf_binaural_convolution(true, empty, offset, reason),
               "composite convolution preflight rejects empty IR");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::EmptyIr,
               "composite convolution classifies as EmptyIr");

    expectTrue(fuse::audio::try_preflight_hrtf_binaural_convolution(true, valid, offset, reason),
               "composite convolution preflight accepts valid IR");
               "composite convolution reject reason is None on valid path");

    expectTrue(!fuse::audio::try_preflight_hrtf_binaural_coupling(
                   false, valid, offset, 0.1f, 0.1f, reason),
               "composite coupling preflight rejects bypass path");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::BypassPath,
               "composite coupling classifies as BypassPath");

    expectTrue(!fuse::audio::try_preflight_hrtf_binaural_coupling(true, empty, offset, 1.f, 1.f,
                                                                  reason),
               "composite coupling preflight rejects unity attenuation");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::UnityAttenuation,
               "composite coupling classifies as UnityAttenuation");

    expectTrue(fuse::audio::try_preflight_hrtf_binaural_coupling(true, empty, offset, 0.2f, 0.3f,
               "composite coupling preflight accepts narrowing path");
               "composite coupling reject reason is None on valid path");

    const fuse::audio::HrtfBinauralPreflight stub_preflight =
        fuse::audio::preflight_hrtf_binaural(true, empty, offset, 0.2f, 0.3f);
    expectTrue(fuse::audio::classify_hrtf_binaural_spatial_reject(stub_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::None,
               "classify composite spatial reject mirrors valid stub path");
    expectTrue(fuse::audio::classify_hrtf_binaural_convolution_reject(stub_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::EmptyIr,
               "classify composite convolution reject mirrors empty IR");
    expectTrue(fuse::audio::classify_hrtf_binaural_coupling_reject(stub_preflight)
               "classify composite coupling reject mirrors narrowing path");
...
void testRejectReasonMirrorsExistingPreflights() {
    ...
    expectTrue(fuse::audio::preflight_hrtf_pan_path_ready(true, empty, offset)
                   == fuse::audio::can_apply_spatial_hrtf_pan(
                          fuse::audio::preflight_hrtf_pan_path(true, empty, offset)),
               "preflight_hrtf_pan_path_ready mirrors can_apply_spatial_hrtf_pan");
    expectTrue(fuse::audio::preflight_hrtf_convolution_ready(true, valid, offset)
                   == fuse::audio::can_convolve_hrtf_pan_path(
                          fuse::audio::preflight_hrtf_pan_path(true, valid, offset)),
               "preflight_hrtf_convolution_ready mirrors can_convolve_hrtf_pan_path");
    expectTrue(fuse::audio::preflight_hrtf_attenuation_coupling_ready(
                   fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f)
                   == fuse::audio::can_narrow_hrtf_spatial_image(
                              fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f)),
               "preflight_hrtf_attenuation_coupling_ready mirrors can_narrow_hrtf_spatial_image");
    expectTrue(fuse::audio::preflight_hrtf_binaural_spatial_ready(true, empty, offset)
                   == fuse::audio::can_apply_hrtf_binaural_pan(
                          fuse::audio::preflight_hrtf_binaural(true, empty, offset, 1.f, 1.f)),
               "preflight_hrtf_binaural_spatial_ready mirrors can_apply_hrtf_binaural_pan");
    expectTrue(fuse::audio::preflight_hrtf_binaural_convolution_ready(true, valid, offset)
                   == fuse::audio::can_convolve_hrtf_binaural(
                          fuse::audio::preflight_hrtf_binaural(true, valid, offset, 1.f, 1.f)),
               "preflight_hrtf_binaural_convolution_ready mirrors can_convolve_hrtf_binaural");
    expectTrue(fuse::audio::preflight_hrtf_binaural_coupling_ready(true, empty, offset, 0.2f, 0.3f)
                   == fuse::audio::can_narrow_hrtf_binaural_spatial_image(
                          fuse::audio::preflight_hrtf_binaural(true, empty, offset, 0.2f, 0.3f)),
               "preflight_hrtf_binaural_coupling_ready mirrors can_narrow_hrtf_binaural_spatial_image");


    expectTrue(!fuse::audio::preflight_hrtf_ir_ready(empty, &reason),
               "empty IR preflight is not ready for convolution");
               "canonical empty IR reject reason is NullSamples");
               "try_preflight_hrtf_ir rejects empty IR");

    reason = fuse::audio::HrtfIrRejectReason::None;
    expectTrue(fuse::audio::preflight_hrtf_ir_ready(valid, &reason),
               "valid IR preflight is ready for convolution");
               "try_preflight_hrtf_ir accepts valid IR");

               "malformed IR preflight is not ready for convolution");
               "malformed IR reject reason is MalformedIr");
               "classify_hrtf_ir_reject marks malformed IR");

    const fuse::audio::HrtfIrStub null_samples_nonzero_length{nullptr, 4};
    expectTrue(!fuse::audio::preflight_hrtf_ir_ready(null_samples_nonzero_length, &reason),
               "null samples with non-zero length is not ready for convolution");
               "null samples reject reason is NullSamples");
    ...
    expectTrue(fuse::audio::preflight_hrtf_pan_path_ready(true, empty, offset, &reason),
               "enabled offset source is ready for spatial pan with empty IR");
               "ILD/ITD stub path reject reason is None");
    expectTrue(fuse::audio::try_preflight_hrtf_pan_path(true, valid, offset, reason),
               "try_preflight_hrtf_pan_path accepts valid convolution path");

    reason = fuse::audio::HrtfPanPathRejectReason::None;
    expectTrue(!fuse::audio::preflight_hrtf_pan_path_ready(false, valid, offset, &reason),
               "disabled HRTF pan-path preflight is not ready");
               "disabled HRTF reject reason is HrtfDisabled");

    expectTrue(!fuse::audio::preflight_hrtf_pan_path_ready(true, valid, co_located, &reason),
               "co-located source pan-path preflight is not ready");
               "co-located reject reason is CoLocated");
    expectTrue(!fuse::audio::try_preflight_hrtf_pan_path(true, co_located, reason),
               "no-IR try_preflight rejects co-located source");

    expectTrue(fuse::audio::preflight_hrtf_pan_path_ready(true, offset)
                   == fuse::audio::preflight_hrtf_pan_path(true, offset).can_spatial_pan(),
               "no-IR preflight_hrtf_pan_path_ready mirrors can_spatial_pan");
    expectTrue(std::strcmp(fuse::audio::hrtf_pan_path_reject_reason_label(
                   fuse::audio::HrtfPanPathRejectReason::CoLocated),
               "CoLocated") == 0,
               "pan-path reject reason label for CoLocated");
    ...
    expectTrue(!fuse::audio::preflight_hrtf_attenuation_coupling_ready(
                   fuse::audio::HrtfPanPath::Bypass, 0.1f, 0.1f, &reason),
               "bypass path attenuation-coupling preflight is not ready");
               "bypass path coupling reject reason is BypassPath");

    reason = fuse::audio::HrtfAttenuationCouplingRejectReason::None;
                   fuse::audio::HrtfPanPath::Convolution, 1.f, 1.f, &reason),
               "unity attenuation coupling preflight is not ready");
               "unity attenuation coupling reject reason is UnityAttenuation");

                   fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f, &reason),
               "reduced attenuation coupling preflight is ready");
               "narrowable coupling reject reason is None");
               "try_preflight_hrtf_attenuation_coupling accepts narrowable path");

                   == fuse::audio::should_narrow_hrtf_spatial_image(
                          fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f),
               "preflight_hrtf_attenuation_coupling_ready mirrors should_narrow_hrtf_spatial_image");
    expectTrue(std::strcmp(fuse::audio::hrtf_attenuation_coupling_reject_reason_label(
                   fuse::audio::HrtfAttenuationCouplingRejectReason::BypassPath),
               "BypassPath") == 0,
               "coupling reject reason label for BypassPath");
    ...
    expectTrue(fuse::audio::preflight_hrtf_binaural_ready(true, empty, offset, 0.2f, 0.3f, &reason),
               "composite stub-path preflight is ready for spatial pan");
               "composite stub-path reject reason is None");
    expectTrue(fuse::audio::try_preflight_hrtf_binaural(true, valid, offset, 1.f, 1.f, reason),
               "try_preflight_hrtf_binaural accepts valid convolution path");

    reason = fuse::audio::HrtfBinauralRejectReason::None;
    expectTrue(!fuse::audio::preflight_hrtf_binaural_ready(false, valid, offset, 0.1f, 0.1f,
                                                            &reason),
               "disabled HRTF composite preflight is not ready");
               "composite disabled reject reason is HrtfDisabled");

    expectTrue(!fuse::audio::preflight_hrtf_binaural_ready(true, valid, co_located, 0.1f, 0.1f,
               "co-located composite preflight is not ready");
               "composite co-located reject reason is CoLocated");

                   == fuse::audio::HrtfBinauralRejectReason::NullSamples,
               "composite convolution reject classifies empty IR as NullSamples");
               "composite coupling reject is None when narrowing applies");

    const fuse::audio::HrtfBinauralPreflight unity_preflight =
        fuse::audio::preflight_hrtf_binaural(true, valid, offset, 1.f, 1.f);
    expectTrue(fuse::audio::classify_hrtf_binaural_coupling_reject(unity_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::UnityAttenuation,
               "composite coupling reject classifies unity attenuation");

    const fuse::audio::HrtfBinauralPreflight bypass_preflight =
        fuse::audio::preflight_hrtf_binaural(false, valid, offset, 0.1f, 0.1f);
    expectTrue(fuse::audio::classify_hrtf_binaural_pan_reject(bypass_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "composite pan reject classifies disabled HRTF");
    expectTrue(fuse::audio::classify_hrtf_binaural_coupling_reject(bypass_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::BypassPath,
               "composite coupling reject classifies bypass path");

    expectTrue(fuse::audio::preflight_hrtf_binaural_ready(true, offset, 0.3f, 0.4f)
                   == fuse::audio::preflight_hrtf_binaural(true, offset, 0.3f, 0.4f)
                          .can_spatial_pan(),
               "no-IR preflight_hrtf_binaural_ready mirrors can_spatial_pan");
    expectTrue(std::strcmp(fuse::audio::hrtf_binaural_reject_reason_label(
                   fuse::audio::HrtfBinauralRejectReason::UnityAttenuation),
               "UnityAttenuation") == 0,
               "composite reject reason label for UnityAttenuation");
    ...
    expectTrue(reason == fuse::audio::HrtfIrRejectReason::EmptyIr,
               "empty IR reject reason is empty_ir");

               "valid IR try_preflight succeeds");
               "valid IR reject reason is none");

    expectTrue(!fuse::audio::try_preflight_hrtf_ir(malformed, reason),
               "malformed IR try_preflight fails");
               "malformed IR reject reason is malformed_ir");
    ...
    expectTrue(fuse::audio::classify_hrtf_ir_reject(malformed_preflight)
    expectTrue(malformed_preflight.reason == fuse::audio::HrtfIrRejectReason::MalformedIr,
               "preflight carries malformed IR reject reason");

                   fuse::audio::HrtfIrRejectReason::EmptyIr),
               "empty_ir") == 0,
               "empty_ir label is stable");


    expectTrue(fuse::audio::preflight_hrtf_pan_path_ready(true, valid, offset, &reason),
               "enabled offset source pan-path preflight is ready");
               "spatial pan reject reason is none");

               "disabled HRTF try_preflight pan path fails");
               "disabled HRTF reject reason is hrtf_disabled");

               "co-located try_preflight pan path fails");
               "co-located reject reason is co_located");
    ...
    expectTrue(fuse::audio::classify_hrtf_pan_path_reject(disabled_preflight)
               ...
               "classify_hrtf_pan_path_reject marks disabled HRTF");
               "co_located") == 0,
               "co_located label is stable");


               "bypass path coupling preflight is not ready");
               "bypass path coupling reject reason is bypass_path");

               "unity attenuation try_preflight coupling fails");
               "unity attenuation reject reason is unity_attenuation");

               "reduced attenuation try_preflight coupling succeeds");
    ...
    expectTrue(fuse::audio::classify_hrtf_attenuation_coupling_reject(unity_preflight)
                   == fuse::audio::HrtfAttenuationCouplingRejectReason::UnityAttenuation,
               "classify coupling reject marks unity attenuation");
    expectTrue(unity_preflight.reason
               "coupling preflight carries unity attenuation reason");


    expectTrue(fuse::audio::preflight_hrtf_binaural_ready(true, valid, offset, 0.2f, 0.3f, &reason),
               "valid composite binaural preflight is ready");
               "valid composite reject reason is none");

    expectTrue(!fuse::audio::try_preflight_hrtf_binaural(false, valid, offset, 0.1f, 0.1f, reason),
               "disabled composite try_preflight fails");
               "disabled composite reject reason is hrtf_disabled");

    expectTrue(!fuse::audio::try_preflight_hrtf_binaural_convolution(stub_preflight, reason),
               "empty IR composite convolution try_preflight fails");
               "empty IR convolution reject reason is empty_ir");
    expectTrue(fuse::audio::try_preflight_hrtf_binaural_coupling(stub_preflight, reason),
               "stub path composite coupling try_preflight succeeds");
               "narrowing-ready composite coupling reject reason is none");

    const fuse::audio::HrtfBinauralPreflight conv_preflight =
    expectTrue(fuse::audio::try_preflight_hrtf_binaural_convolution(conv_preflight, reason),
               "valid IR composite convolution try_preflight succeeds");
    expectTrue(!fuse::audio::try_preflight_hrtf_binaural_coupling(conv_preflight, reason),
               "unity attenuation composite coupling try_preflight fails");
               "unity attenuation composite coupling reject reason is unity_attenuation");

        fuse::audio::preflight_hrtf_binaural(true, valid, co_located, 0.1f, 0.1f);
    expectTrue(fuse::audio::classify_hrtf_binaural_reject(bypass_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::CoLocated,
               "classify composite reject marks co-located source");
    expectTrue(bypass_preflight.reason == fuse::audio::HrtfBinauralRejectReason::CoLocated,
               "composite preflight carries co-located reject reason");
                   fuse::audio::HrtfBinauralRejectReason::MalformedIr),
               "malformed_ir") == 0,
               "malformed_ir label is stable");
...
void testRejectReasonMirrorsExistingPreflights() {
    ...
    expectTrue(fuse::audio::preflight_hrtf_ir_ready(valid) == ir_preflight.can_convolve(),
               "preflight_hrtf_ir_ready mirrors can_convolve on valid IR");
    expectTrue(fuse::audio::preflight_hrtf_ir_ready(valid)
                   == fuse::audio::can_convolve_hrtf_ir(ir_preflight),
    ...
    expectTrue(fuse::audio::preflight_hrtf_pan_path_ready(true, empty, offset)
                   == pan_preflight.can_spatial_pan(),
               "preflight_hrtf_pan_path_ready mirrors can_spatial_pan on stub path");
    expectTrue(fuse::audio::preflight_hrtf_pan_path_ready(true, offset)
                   == fuse::audio::preflight_hrtf_pan_path(true, offset).can_spatial_pan(),
               "no-IR pan-path ready mirrors no-IR preflight");
    ...
    const fuse::audio::HrtfBinauralPreflight composite_preflight =
        fuse::audio::preflight_hrtf_binaural(true, valid, offset, 0.25f, 0.35f);
    expectTrue(fuse::audio::preflight_hrtf_binaural_ready(true, valid, offset, 0.25f, 0.35f)
                   == composite_preflight.can_spatial_pan(),
               "preflight_hrtf_binaural_ready mirrors can_spatial_pan");
    expectTrue(fuse::audio::preflight_hrtf_binaural_convolution_ready(composite_preflight)
                   == composite_preflight.can_convolve(),
               "preflight_hrtf_binaural_convolution_ready mirrors can_convolve");
    expectTrue(fuse::audio::preflight_hrtf_binaural_coupling_ready(composite_preflight)
                   == composite_preflight.can_narrow_spatial_image(),
               "preflight_hrtf_binaural_coupling_ready mirrors can_narrow_spatial_image");
    expectTrue(fuse::audio::classify_hrtf_ir_reject(ir_preflight) == ir_preflight.reason,
               "classify_hrtf_ir_reject matches stored IR reason");
    expectTrue(fuse::audio::classify_hrtf_pan_path_reject(pan_preflight) == pan_preflight.reason,
               "classify_hrtf_pan_path_reject matches stored pan-path reason");
    expectTrue(fuse::audio::classify_hrtf_attenuation_coupling_reject(coupling_preflight)
                   == coupling_preflight.reason,
               "classify coupling reject matches stored coupling reason");
    expectTrue(fuse::audio::classify_hrtf_binaural_reject(composite_preflight)
                   == composite_preflight.reason,
               "classify composite reject matches stored composite reason");
}
...
void testHrtfIrRejectReasonGuards() {
    ...
    expectTrue(!fuse::audio::tryPreflightHrtfIr(empty, reason),
               "tryPreflightHrtfIr rejects empty IR");
    expectTrue(reason == fuse::audio::HrtfIrRejectReason::EmptyIr,
               "empty IR reject reason is EmptyIr");
    expectTrue(fuse::audio::hrtfIrRejectsForReason(empty, fuse::audio::HrtfIrRejectReason::EmptyIr),
               "hrtfIrRejectsForReason matches empty IR");

    const float samples[] = {0.5f};
    const fuse::audio::HrtfIrStub valid{samples, 1};
    expectTrue(fuse::audio::preflightHrtfIrReady(valid, &reason),
               "preflightHrtfIrReady accepts valid IR");
    expectTrue(reason == fuse::audio::HrtfIrRejectReason::None,
               "valid IR reject reason is None");

    const fuse::audio::HrtfIrStub malformed{samples, 0};
    expectTrue(!fuse::audio::tryPreflightHrtfIr(malformed, reason),
               "tryPreflightHrtfIr rejects malformed IR");
    expectTrue(reason == fuse::audio::HrtfIrRejectReason::MalformedIr,
               "malformed IR reject reason is MalformedIr");

    const fuse::audio::HrtfIrStub null_samples{nullptr, 4};
    expectTrue(fuse::audio::hrtfIrRejectReason(null_samples)
                   == fuse::audio::HrtfIrRejectReason::NullSamples,
               "null samples with length rejects as NullSamples");
    expectTrue(std::strcmp(fuse::audio::hrtfIrRejectReasonName(
                               fuse::audio::HrtfIrRejectReason::MalformedIr),
                           "MalformedIr")
                   == 0,
               "hrtfIrRejectReasonName labels malformed IR");

    const fuse::audio::HrtfIrPreflight empty_preflight = fuse::audio::preflight_hrtf_ir(empty);
    expectTrue(fuse::audio::classifyHrtfIrReject(empty_preflight)
                   == fuse::audio::HrtfIrRejectReason::EmptyIr,
               "classifyHrtfIrReject mirrors preflight reason");
    expectTrue(empty_preflight.reason == fuse::audio::HrtfIrRejectReason::EmptyIr,
               "preflight carries empty-IR reject reason");
...
void testHrtfPanPathRejectReasonGuards() {
    ...
    fuse::audio::HrtfPanPathRejectReason reason = fuse::audio::HrtfPanPathRejectReason::None;
    expectTrue(fuse::audio::tryPreflightHrtfPanPath(true, valid, offset, reason),
               "tryPreflightHrtfPanPath accepts spatial offset");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::None,
               "spatial path reject reason is None");

    expectTrue(!fuse::audio::preflightHrtfPanPathReady(false, valid, offset, &reason),
               "preflightHrtfPanPathReady rejects disabled HRTF");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::HrtfDisabled,
               "disabled HRTF reject reason is HrtfDisabled");

    expectTrue(!fuse::audio::tryPreflightHrtfPanPath(true, valid, co_located, reason),
               "tryPreflightHrtfPanPath rejects co-located source");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::CoLocated,
               "co-located reject reason is CoLocated");
    expectTrue(fuse::audio::hrtfPanPathRejectsForReason(true, co_located,
                                                        fuse::audio::HrtfPanPathRejectReason::CoLocated),
               "hrtfPanPathRejectsForReason matches co-located source");

    const fuse::audio::HrtfPanPathPreflight stub_preflight =
        fuse::audio::preflight_hrtf_pan_path(true, empty, offset);
    expectTrue(fuse::audio::classifyHrtfPanPathReject(stub_preflight)
                   == fuse::audio::HrtfPanPathRejectReason::None,
               "ILD/ITD stub path is not a pan-path reject");
    expectTrue(stub_preflight.reason == fuse::audio::HrtfPanPathRejectReason::None,
               "stub preflight carries None reject reason");
    expectTrue(std::strcmp(fuse::audio::hrtfPanPathRejectReasonName(
                               fuse::audio::HrtfPanPathRejectReason::HrtfDisabled),
                           "HrtfDisabled")
               "hrtfPanPathRejectReasonName labels disabled HRTF");
...
void testHrtfAttenuationCouplingRejectReasonGuards() {
    ...
    expectTrue(!fuse::audio::tryPreflightHrtfAttenuationCoupling(
                   fuse::audio::HrtfPanPath::Bypass, 0.1f, 0.1f, reason),
               "tryPreflightHrtfAttenuationCoupling rejects bypass path");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::BypassPath,
               "bypass path coupling reject reason is BypassPath");

    expectTrue(!fuse::audio::preflightHrtfAttenuationCouplingReady(
                   fuse::audio::HrtfPanPath::Convolution, 1.f, 1.f, &reason),
               "preflightHrtfAttenuationCouplingReady rejects unity attenuation");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::UnityAttenuation,
               "unity attenuation reject reason is UnityAttenuation");

    expectTrue(fuse::audio::tryPreflightHrtfAttenuationCoupling(
                   fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f, reason),
               "tryPreflightHrtfAttenuationCoupling accepts narrowable path");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "narrowable path coupling reject reason is None");
    expectTrue(fuse::audio::hrtfAttenuationCouplingRejectsForReason(
                   fuse::audio::HrtfPanPath::Bypass, 0.2f, 0.3f,
                   fuse::audio::HrtfAttenuationCouplingRejectReason::BypassPath),
               "hrtfAttenuationCouplingRejectsForReason matches bypass path");
    ...
    const fuse::audio::HrtfAttenuationCouplingPreflight narrow_preflight =
        fuse::audio::preflight_hrtf_attenuation_coupling(fuse::audio::HrtfPanPath::Convolution,
    expectTrue(fuse::audio::classifyHrtfAttenuationCouplingReject(narrow_preflight)
                   == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "classifyHrtfAttenuationCouplingReject mirrors preflight reason");
    expectTrue(narrow_preflight.reason == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "narrowable preflight carries None reject reason");
    expectTrue(std::strcmp(fuse::audio::hrtfAttenuationCouplingRejectReasonName(
                               fuse::audio::HrtfAttenuationCouplingRejectReason::UnityAttenuation),
                           "UnityAttenuation")
               "hrtfAttenuationCouplingRejectReasonName labels unity attenuation");

void testHrtfBinauralRejectReasonGuards() {

    fuse::audio::HrtfBinauralRejectReason reason = fuse::audio::HrtfBinauralRejectReason::None;
    expectTrue(fuse::audio::tryPreflightHrtfBinaural(true, empty, offset, 0.2f, 0.3f, reason),
               "tryPreflightHrtfBinaural accepts spatial stub path");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::None,
               "spatial stub composite reject reason is None");
    ...
    expectTrue(!fuse::audio::tryPreflightHrtfBinaural(true, valid, co_located, 0.1f, 0.1f, reason),
               "tryPreflightHrtfBinaural rejects co-located source");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::CoLocated,
               "co-located composite reject reason is CoLocated");
    ...
    expectTrue(fuse::audio::classifyHrtfBinauralReject(bypass_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "classifyHrtfBinauralReject mirrors composite preflight reason");
    expectTrue(bypass_preflight.reason == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "bypass composite preflight carries reject reason");
    expectTrue(std::strcmp(fuse::audio::hrtfBinauralRejectReasonName(
                               fuse::audio::HrtfBinauralRejectReason::CoLocated),
                           "CoLocated")
               "hrtfBinauralRejectReasonName labels co-located source");
...
void testRejectReasonMirrorsExistingPreflights() {
    ...
    expectTrue(fuse::audio::preflightHrtfAttenuationCouplingReady(
                   == fuse::audio::can_narrow_hrtf_spatial_image(
                          fuse::audio::preflight_hrtf_attenuation_coupling(
                              fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f)),
               "preflightHrtfAttenuationCouplingReady mirrors can_narrow on narrowable path");
    expectTrue(fuse::audio::preflightHrtfBinauralReady(true, empty, offset, 0.2f, 0.3f)
                   == fuse::audio::can_apply_hrtf_binaural_pan(
                          fuse::audio::preflight_hrtf_binaural(true, empty, offset, 0.2f, 0.3f)),
               "preflightHrtfBinauralReady mirrors can_apply_hrtf_binaural_pan on stub path");
    ...
    expectTrue(fuse::audio::classify_hrtf_ir_reject(malformed_preflight)
               ...
               "malformed IR classifies as malformed_ir");
    expectTrue(malformed_preflight.reason == fuse::audio::HrtfIrRejectReason::MalformedIr,
               "malformed preflight carries reject reason");


    expectTrue(fuse::audio::try_preflight_hrtf_pan_path(true, valid, offset, reason),
               "try_preflight_hrtf_pan_path accepts spatial offset");
               "spatial pan path reason is none");

    expectTrue(!fuse::audio::preflight_hrtf_pan_path_ready(false, valid, offset, &reason),
               "preflight_hrtf_pan_path_ready rejects disabled HRTF");
               "disabled HRTF pan-path reject reason");

    expectTrue(!fuse::audio::try_preflight_hrtf_pan_path(true, valid, co_located, reason),
               "try_preflight_hrtf_pan_path rejects co-located source");
               "co-located pan-path reject reason");
    expectTrue(std::strcmp(fuse::audio::hrtf_pan_path_reject_reason_label(reason), "co_located")
               "co_located pan-path label is exposed");

        fuse::audio::preflight_hrtf_pan_path(true, fuse::audio::make_empty_hrtf_ir(), offset);
    expectTrue(fuse::audio::classify_hrtf_pan_path_reject(stub_preflight)
               "empty IR stub path is not a pan-path reject");
               "ILD/ITD stub preflight reason is none");
    ...
    expectTrue(std::strcmp(fuse::audio::hrtf_attenuation_coupling_reject_reason_label(reason),
                           "bypass_path")
               "bypass_path coupling label is exposed");
    ...
    expectTrue(fuse::audio::classify_hrtf_attenuation_coupling_reject(narrow_preflight)
               "reduced attenuation coupling is not rejected");
               "narrowing coupling preflight reason is none");


    expectTrue(fuse::audio::try_preflight_hrtf_binaural(true, empty, offset, 0.2f, 0.3f, reason),
               "try_preflight_hrtf_binaural accepts spatial stub path");
               "spatial stub composite reason is none");

    expectTrue(!fuse::audio::preflight_hrtf_binaural_ready(false, valid, offset, 0.1f, 0.1f, {}, {},
                                                           &reason),
               "preflight_hrtf_binaural_ready rejects disabled HRTF");
               "disabled HRTF composite reject reason");
    expectTrue(std::strcmp(fuse::audio::hrtf_binaural_reject_reason_label(reason), "hrtf_disabled")
               "hrtf_disabled composite label is exposed");

    expectTrue(!fuse::audio::try_preflight_hrtf_binaural(true, valid, co_located, 0.1f, 0.1f,
                                                         reason),
               "try_preflight_hrtf_binaural rejects co-located source");
               "co-located composite reject reason");

    fuse::audio::HrtfIrRejectReason conv_reason = fuse::audio::HrtfIrRejectReason::None;
    expectTrue(fuse::audio::preflight_hrtf_binaural_convolution_ready(true, valid, offset,
                                                                      &conv_reason),
               "composite convolution ready accepts valid IR");
    expectTrue(conv_reason == fuse::audio::HrtfIrRejectReason::None,
               "valid IR composite convolution reason is none");

    expectTrue(!fuse::audio::try_preflight_hrtf_binaural_convolution(true, empty, offset,
                                                                     conv_reason),
               "composite convolution rejects empty IR");
    expectTrue(conv_reason == fuse::audio::HrtfIrRejectReason::NullSamples,
               "empty IR composite convolution reject reason");


    const fuse::audio::HrtfIrPreflight empty_ir_preflight = fuse::audio::preflight_hrtf_ir(empty);
    expectTrue(empty_ir_preflight.reason
                   == fuse::audio::classify_hrtf_ir_reject(empty_ir_preflight),
               "IR preflight reason matches classify_hrtf_ir_reject");

    const fuse::audio::HrtfPanPathPreflight disabled_preflight =
        fuse::audio::preflight_hrtf_pan_path(false, valid, offset);
    expectTrue(disabled_preflight.reason
                   == fuse::audio::classify_hrtf_pan_path_reject(disabled_preflight),
               "pan-path preflight reason matches classify_hrtf_pan_path_reject");

    const fuse::audio::HrtfAttenuationCouplingPreflight unity_preflight =
                                                         1.f, 1.f);
    expectTrue(unity_preflight.reason
                   == fuse::audio::classify_hrtf_attenuation_coupling_reject(unity_preflight),
               "coupling preflight reason matches classify_hrtf_attenuation_coupling_reject");

        fuse::audio::preflight_hrtf_binaural(false, valid, co_located, 0.1f, 0.1f);
    expectTrue(bypass_preflight.reason
                   == fuse::audio::classify_hrtf_binaural_reject(bypass_preflight),
               "composite preflight reason matches classify_hrtf_binaural_reject");
    expectTrue(static_cast<int>(bypass_preflight.panPath.reason)
                   == static_cast<int>(bypass_preflight.reason),
               "composite reason mirrors pan-path reject reason on bypass");

    const fuse::audio::HrtfBinauralPreflight stub_preflight =
        fuse::audio::preflight_hrtf_binaural(true, empty, offset, 0.2f, 0.3f);
    expectTrue(stub_preflight.ir.reason == fuse::audio::HrtfIrRejectReason::NullSamples,
               "composite IR sub-preflight carries reject reason");
    expectTrue(stub_preflight.reason == fuse::audio::HrtfBinauralRejectReason::None,
               "ILD/ITD stub composite bypass reason remains none");
...
void testHrtfPreflightReadyMirrorsExistingPredicates() {
    ...
    expectTrue(!fuse::audio::try_preflight_hrtf_ir_convolution(empty, reason),
               "try_preflight_hrtf_ir_convolution rejects empty IR");
    expectTrue(std::strcmp(fuse::audio::hrtf_ir_reject_reason_label(reason), "NullSamples") == 0,
               "IR reject reason label for NullSamples");

    expectTrue(fuse::audio::try_preflight_hrtf_ir_convolution(valid, reason),
               "try_preflight_hrtf_ir_convolution accepts valid IR");

    expectTrue(!fuse::audio::preflight_hrtf_ir_convolution_ready(malformed, &reason),
               "preflight_hrtf_ir_convolution_ready rejects malformed IR");
    expectTrue(fuse::audio::classify_hrtf_ir_reject(fuse::audio::preflight_hrtf_ir(malformed))
               "classify_hrtf_ir_reject maps malformed flag");


    fuse::audio::HrtfPanPathRejectReason spatialReason = fuse::audio::HrtfPanPathRejectReason::None;
    expectTrue(fuse::audio::try_preflight_hrtf_pan_spatial(true, empty, offset, spatialReason),
               "spatial preflight accepts enabled offset source");
    expectTrue(spatialReason == fuse::audio::HrtfPanPathRejectReason::None,
               "spatial preflight None on valid path");

    expectTrue(!fuse::audio::try_preflight_hrtf_pan_spatial(false, valid, offset, spatialReason),
               "spatial preflight rejects disabled HRTF");
    expectTrue(spatialReason == fuse::audio::HrtfPanPathRejectReason::HrtfDisabled,
               "disabled HRTF spatial reject reason");
    expectTrue(std::strcmp(fuse::audio::hrtf_pan_path_reject_reason_label(spatialReason),
                          "HrtfDisabled") == 0,
               "pan-path reject reason label for HrtfDisabled");

    expectTrue(!fuse::audio::try_preflight_hrtf_pan_spatial(true, valid, co_located, spatialReason),
               "spatial preflight rejects co-located source");
    expectTrue(spatialReason == fuse::audio::HrtfPanPathRejectReason::CoLocated,
               "co-located spatial reject reason");

    expectTrue(fuse::audio::preflight_hrtf_pan_spatial_ready(true, offset),
               "no-IR spatial preflight accepts enabled offset source");

    fuse::audio::HrtfPanConvolutionRejectReason convReason =
        fuse::audio::HrtfPanConvolutionRejectReason::None;
    expectTrue(!fuse::audio::try_preflight_hrtf_pan_convolution(true, empty, offset, convReason),
               "pan convolution preflight rejects empty IR");
    expectTrue(convReason == fuse::audio::HrtfPanConvolutionRejectReason::EmptyIr,
               "empty IR pan convolution reject reason");
    expectTrue(std::strcmp(fuse::audio::hrtf_pan_convolution_reject_reason_label(convReason),
                          "EmptyIr") == 0,
               "pan convolution reject reason label for EmptyIr");

    expectTrue(fuse::audio::try_preflight_hrtf_pan_convolution(true, valid, offset, convReason),
               "pan convolution preflight accepts valid IR");
    expectTrue(convReason == fuse::audio::HrtfPanConvolutionRejectReason::None,
               "valid IR pan convolution reject reason is None");

    const fuse::audio::HrtfPanPathPreflight pan_preflight =
    expectTrue(fuse::audio::classify_hrtf_pan_convolution_reject(
                   pan_preflight, fuse::audio::preflight_hrtf_ir(empty))
                   == fuse::audio::HrtfPanConvolutionRejectReason::EmptyIr,
               "classify_hrtf_pan_convolution_reject maps empty IR");


               "coupling preflight rejects bypass path");
                          "BypassPath") == 0,
               "attenuation coupling reject reason label for BypassPath");

               "coupling preflight rejects unity attenuation");

               "coupling preflight accepts reduced attenuation on spatial path");
    ...
    expectTrue(fuse::audio::try_preflight_hrtf_binaural_spatial(true, empty, offset, reason),
               "composite spatial preflight accepts enabled offset source");
               "composite spatial reject reason is None on valid path");

    expectTrue(!fuse::audio::try_preflight_hrtf_binaural_spatial(false, valid, offset, reason),
               "composite spatial preflight rejects disabled HRTF");
               "composite spatial disabled reject reason");
    expectTrue(std::strcmp(fuse::audio::hrtf_binaural_reject_reason_label(reason), "HrtfDisabled")
               "binaural reject reason label for HrtfDisabled");

    expectTrue(!fuse::audio::try_preflight_hrtf_binaural_spatial(true, valid, co_located, reason),
               "composite spatial preflight rejects co-located source");
               "composite spatial co-located reject reason");

    expectTrue(!fuse::audio::try_preflight_hrtf_binaural_convolution(true, empty, offset, reason),
               "composite convolution preflight rejects empty IR");
               "composite convolution empty IR reject reason");

    expectTrue(fuse::audio::try_preflight_hrtf_binaural_convolution(true, valid, offset, reason),
               "composite convolution reject reason is None on valid path");

    expectTrue(!fuse::audio::try_preflight_hrtf_binaural_narrowing(true, empty, offset, 1.f, 1.f,
               "composite narrowing preflight rejects unity attenuation");
               "composite narrowing unity attenuation reject reason");

               "composite narrowing preflight accepts reduced attenuation");
               "composite narrowing reject reason is None when narrowing applies");

    expectTrue(fuse::audio::preflight_hrtf_binaural_spatial_ready(true, offset),
               "no-IR composite spatial preflight accepts enabled offset source");

    expectTrue(fuse::audio::classify_hrtf_binaural_spatial_reject(stub_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::None,
               "classify_hrtf_binaural_spatial_reject None on stub path");
    expectTrue(fuse::audio::classify_hrtf_binaural_convolution_reject(stub_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::EmptyIr,
               "classify_hrtf_binaural_convolution_reject maps empty IR");
    expectTrue(fuse::audio::classify_hrtf_binaural_narrowing_reject(stub_preflight)
               "classify_hrtf_binaural_narrowing_reject None when narrowing applies");



    expectTrue(fuse::audio::preflight_hrtf_ir_convolution_ready(valid, &reason),
               "preflight_hrtf_ir_convolution_ready accepts valid IR");
               "valid IR convolution reject reason is None");

               "classify_hrtf_ir_reject maps malformedIr flag");

    const fuse::audio::HrtfIrStub factory_zero_length =
        fuse::audio::make_hrtf_ir_stub(samples, 0);
    expectTrue(!fuse::audio::preflight_hrtf_ir_convolution_ready(factory_zero_length, &reason),
               "preflight_hrtf_ir_convolution_ready rejects factory zero-length IR");
               "factory zero-length IR reject reason is NullSamples");

    const fuse::audio::HrtfIrStub malformed_direct{samples, 0};
    expectTrue(!fuse::audio::try_preflight_hrtf_ir_convolution(malformed_direct, reason),
               "try_preflight_hrtf_ir_convolution rejects malformed direct stub");
               "non-null zero-length direct stub reject reason is MalformedIr");


    expectTrue(fuse::audio::preflight_hrtf_spatial_pan_ready(true, empty, offset, &reason),
               "preflight_hrtf_spatial_pan_ready accepts ILD/ITD stub path");
               "ILD/ITD stub spatial pan reject reason is None");

    expectTrue(!fuse::audio::try_preflight_hrtf_spatial_pan(false, valid, offset, reason),
               "try_preflight_hrtf_spatial_pan rejects disabled HRTF");
               "disabled HRTF spatial pan reject reason is HrtfDisabled");

    expectTrue(!fuse::audio::preflight_hrtf_spatial_pan_ready(true, valid, co_located, &reason),
               "preflight_hrtf_spatial_pan_ready rejects co-located source");
               "co-located spatial pan reject reason is CoLocated");
    ...
    expectTrue(fuse::audio::preflight_hrtf_pan_path_convolution_ready(true, valid, offset,
               "pan-path convolution ready with valid IR");
               "valid IR pan-path convolution reject reason is None");

    expectTrue(!fuse::audio::try_preflight_hrtf_pan_path_convolution(true, empty, offset,
               "pan-path convolution rejects empty IR");
               "empty IR pan-path convolution reject reason is NullSamples");

    expectTrue(fuse::audio::preflight_hrtf_spatial_pan_ready(true, offset) ==
                       fuse::audio::preflight_hrtf_pan_path(true, offset)),
               "preflight_hrtf_spatial_pan_ready mirrors can_spatial_pan on no-IR overload");




                   fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f, &reason),
               "preflight_hrtf_attenuation_coupling_ready accepts reduced attenuation");

    expectTrue(fuse::audio::classify_hrtf_attenuation_coupling_reject(narrow_preflight) ==
                   fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "classify_hrtf_attenuation_coupling_reject returns None when narrowing applies");
    expectTrue(std::strcmp(fuse::audio::hrtf_attenuation_coupling_reject_reason_label(


    expectTrue(fuse::audio::classify_hrtf_binaural_spatial_reject(stub_preflight) ==
                   fuse::audio::HrtfPanPathRejectReason::None,
               "composite spatial reject is None on stub path");
    expectTrue(fuse::audio::classify_hrtf_binaural_convolution_reject(stub_preflight) ==
                   fuse::audio::HrtfIrRejectReason::NullSamples,
               "composite convolution reject maps empty IR");
    expectTrue(fuse::audio::classify_hrtf_binaural_narrowing_reject(stub_preflight) ==
               "composite narrowing reject is None when attenuation reduced");

    fuse::audio::HrtfPanPathRejectReason spatial_reason =
        fuse::audio::HrtfPanPathRejectReason::None;
    expectTrue(fuse::audio::try_preflight_hrtf_binaural_spatial(true, empty, offset,
                                                              spatial_reason),
               "try_preflight_hrtf_binaural_spatial accepts stub path");
    expectTrue(spatial_reason == fuse::audio::HrtfPanPathRejectReason::None,
               "binaural spatial try returns None on stub path");

    expectTrue(!fuse::audio::preflight_hrtf_binaural_spatial_ready(false, valid, offset,
                                                                  &spatial_reason),
               "preflight_hrtf_binaural_spatial_ready rejects disabled HRTF");
    expectTrue(spatial_reason == fuse::audio::HrtfPanPathRejectReason::HrtfDisabled,
               "binaural spatial reject is HrtfDisabled when disabled");

               "preflight_hrtf_binaural_convolution_ready accepts valid IR");
               "binaural convolution ready returns None for valid IR");

    fuse::audio::HrtfAttenuationCouplingRejectReason narrow_reason =
    expectTrue(!fuse::audio::preflight_hrtf_binaural_narrowing_ready(true, offset, 1.f, 1.f,
                                                                     &narrow_reason),
               "preflight_hrtf_binaural_narrowing_ready rejects unity attenuation on no-IR overload");
    expectTrue(narrow_reason == fuse::audio::HrtfAttenuationCouplingRejectReason::UnityAttenuation,
               "binaural narrowing reject is UnityAttenuation at full gain");
                                                                narrow_reason),
    expectTrue(narrow_reason == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "binaural narrowing try returns None when narrowing applies");

    expectTrue(fuse::audio::preflight_hrtf_binaural_spatial_ready(true, offset) ==
                   stub_preflight.can_spatial_pan(),
               "binaural spatial ready mirrors composite can_spatial_pan");
    expectTrue(fuse::audio::preflight_hrtf_binaural_convolution_ready(true, valid, offset) ==
                   fuse::audio::preflight_hrtf_binaural(true, valid, offset, 1.f, 1.f).can_convolve(),
               "binaural convolution ready mirrors composite can_convolve");
                   == fuse::audio::can_apply_spatial_hrtf_pan(pan_preflight),
               "preflight_hrtf_pan_path_ready mirrors can_apply_spatial_hrtf_pan on stub path");

        fuse::audio::preflight_hrtf_attenuation_coupling(fuse::audio::HrtfPanPath::IldItdStub, 0.2f,
                                                          0.3f);
                   == fuse::audio::can_narrow_hrtf_spatial_image(coupling_preflight),
               "preflight_hrtf_attenuation_coupling_ready mirrors can_narrow on reduced attenuation");

    const fuse::audio::HrtfBinauralPreflight binaural_preflight =
    expectTrue(fuse::audio::preflight_hrtf_binaural_pan_ready(true, empty, offset)
                   == fuse::audio::can_apply_hrtf_binaural_pan(binaural_preflight),
               "preflight_hrtf_binaural_pan_ready mirrors can_apply_hrtf_binaural_pan on stub path");
    expectTrue(fuse::audio::preflight_hrtf_binaural_convolution_ready(valid)
                   == fuse::audio::can_convolve_hrtf_binaural(
                          fuse::audio::preflight_hrtf_binaural(true, valid, offset, 1.f, 1.f)),
               "preflight_hrtf_binaural_convolution_ready mirrors can_convolve on valid IR");
}
...
void testHrtfIrRejectReasonGuards() {
    ...
    expectTrue(fuse::audio::classify_hrtf_ir_reject(empty_preflight)
                   == fuse::audio::HrtfIrRejectReason::NullSamples,
               "classify_hrtf_ir_reject on canonical empty IR");
    expectTrue(std::strcmp(fuse::audio::hrtf_ir_reject_reason_label(
                               fuse::audio::HrtfIrRejectReason::MalformedIr),
                           "MalformedIr")
                   == 0,
...
void testHrtfPanPathRejectReasonGuards() {
    ...
    expectTrue(fuse::audio::preflight_hrtf_spatial_pan_ready(true, empty, offset, &pan_reason),
               "enabled offset source passes spatial-pan preflight");
    expectTrue(pan_reason == fuse::audio::HrtfPanPathRejectReason::None,
               "spatial pan reject reason is None on valid path");

    pan_reason = fuse::audio::HrtfPanPathRejectReason::None;
    expectTrue(!fuse::audio::try_preflight_hrtf_spatial_pan(false, valid, offset, pan_reason),
               "disabled HRTF fails spatial-pan try_preflight");
    expectTrue(pan_reason == fuse::audio::HrtfPanPathRejectReason::HrtfDisabled,
               "disabled HRTF spatial pan reject reason is HrtfDisabled");

    pan_reason = fuse::audio::HrtfPanPathRejectReason::None;
    expectTrue(!fuse::audio::preflight_hrtf_spatial_pan_ready(true, valid, co_located, &pan_reason),
               "co-located source fails spatial-pan preflight");
    ...
    fuse::audio::HrtfConvolutionRejectReason conv_reason =
        fuse::audio::HrtfConvolutionRejectReason::None;
    expectTrue(fuse::audio::preflight_hrtf_convolution_ready(true, valid, offset, &conv_reason),
               "valid IR passes convolution preflight");
    expectTrue(conv_reason == fuse::audio::HrtfConvolutionRejectReason::None,
               "convolution reject reason is None on valid path");

    conv_reason = fuse::audio::HrtfConvolutionRejectReason::None;
    expectTrue(!fuse::audio::try_preflight_hrtf_convolution(true, empty, offset, conv_reason),
               "empty IR fails convolution try_preflight");
    expectTrue(conv_reason == fuse::audio::HrtfConvolutionRejectReason::EmptyIr,
               "empty IR convolution reject reason is EmptyIr");

    conv_reason = fuse::audio::HrtfConvolutionRejectReason::None;
    expectTrue(!fuse::audio::preflight_hrtf_convolution_ready(true, valid, co_located, &conv_reason),
               "co-located source fails convolution preflight");
    expectTrue(conv_reason == fuse::audio::HrtfConvolutionRejectReason::CoLocated,
               "co-located convolution reject reason is CoLocated");
    ...
    const fuse::audio::HrtfPanPathPreflight pan_preflight =
        fuse::audio::preflight_hrtf_pan_path(false, valid, offset);
    expectTrue(fuse::audio::classify_hrtf_pan_path_reject(pan_preflight)
                   == fuse::audio::HrtfPanPathRejectReason::HrtfDisabled,
               "classify_hrtf_pan_path_reject on disabled HRTF");
    expectTrue(std::strcmp(fuse::audio::hrtf_convolution_reject_reason_label(
                               fuse::audio::HrtfConvolutionRejectReason::EmptyIr),
                           "EmptyIr")
                   == 0,
               "convolution reject reason label for EmptyIr");
}
...
void testHrtfAttenuationCouplingRejectReasonGuards() {
    ...
    expectTrue(!fuse::audio::preflight_hrtf_narrowing_ready(fuse::audio::HrtfPanPath::Bypass, 0.1f,
                                                            0.1f, &reason),
               "bypass path fails narrowing preflight");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::BypassPath,
               "bypass narrowing reject reason is BypassPath");

    reason = fuse::audio::HrtfAttenuationCouplingRejectReason::None;
    expectTrue(!fuse::audio::try_preflight_hrtf_narrowing(fuse::audio::HrtfPanPath::Convolution,
                                                          1.f, 1.f, reason),
               "unity attenuation fails narrowing try_preflight");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::UnityAttenuation,
               "unity attenuation narrowing reject reason is UnityAttenuation");

    reason = fuse::audio::HrtfAttenuationCouplingRejectReason::None;
    expectTrue(fuse::audio::preflight_hrtf_narrowing_ready(fuse::audio::HrtfPanPath::IldItdStub,
                                                          0.2f, 0.3f, &reason),
               "reduced attenuation passes narrowing preflight");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "narrowing reject reason is None on valid path");
    ...
    expectTrue(fuse::audio::classify_hrtf_attenuation_coupling_reject(unity_preflight)
                   == fuse::audio::HrtfAttenuationCouplingRejectReason::UnityAttenuation,
               "classify attenuation coupling reject on unity attenuation");
    expectTrue(std::strcmp(fuse::audio::hrtf_attenuation_coupling_reject_reason_label(
                               fuse::audio::HrtfAttenuationCouplingRejectReason::BypassPath),
                           "BypassPath")
                   == 0,
...
void testHrtfBinauralRejectReasonGuards() {
    ...
    fuse::audio::HrtfBinauralRejectReason reason = fuse::audio::HrtfBinauralRejectReason::None;
    expectTrue(fuse::audio::preflight_hrtf_binaural_ready(true, empty, offset, 0.2f, 0.3f, &reason),
               "composite binaural preflight ready on stub path");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::None,
               "composite binaural reject reason is None on valid path");
    ...
    expectTrue(!fuse::audio::preflight_hrtf_binaural_convolution_ready(stub_preflight, &conv_reason),
               "empty IR composite convolution preflight is not ready");
    ...
    expectTrue(fuse::audio::try_preflight_hrtf_binaural_convolution(conv_preflight, conv_reason),
               "valid IR composite convolution try_preflight succeeds");
    ...
    fuse::audio::HrtfBinauralNarrowingRejectReason narrow_reason =
        fuse::audio::HrtfBinauralNarrowingRejectReason::None;
    expectTrue(fuse::audio::preflight_hrtf_binaural_narrowing_ready(stub_preflight, &narrow_reason),
               "reduced attenuation composite narrowing preflight is ready");
    expectTrue(narrow_reason == fuse::audio::HrtfBinauralNarrowingRejectReason::None,
               "composite narrowing reject reason is None on valid path");

    narrow_reason = fuse::audio::HrtfBinauralNarrowingRejectReason::None;
    expectTrue(!fuse::audio::try_preflight_hrtf_binaural_narrowing(conv_preflight, narrow_reason),
               "unity attenuation composite narrowing try_preflight fails");
    ...
    expectTrue(std::strcmp(fuse::audio::hrtf_binaural_reject_reason_label(
                           ...
                           "CoLocated")
                   == 0,
               "binaural reject reason label for CoLocated");
}
...
void testRejectReasonMirrorsExistingPreflights() {
    ...
    expectTrue(fuse::audio::preflight_hrtf_ir_ready(valid)
                   == fuse::audio::can_convolve_hrtf_ir(fuse::audio::preflight_hrtf_ir(valid)),
               "preflight_hrtf_ir_ready mirrors can_convolve_hrtf_ir on valid IR");
    expectTrue(fuse::audio::preflight_hrtf_spatial_pan_ready(true, empty, offset)
                   == fuse::audio::can_apply_spatial_hrtf_pan(
                          fuse::audio::preflight_hrtf_pan_path(true, empty, offset)),
               "preflight_hrtf_spatial_pan_ready mirrors can_apply_spatial_hrtf_pan");
    expectTrue(fuse::audio::preflight_hrtf_convolution_ready(true, valid, offset)
                   == fuse::audio::can_convolve_hrtf_pan_path(
                          fuse::audio::preflight_hrtf_pan_path(true, valid, offset)),
               "preflight_hrtf_convolution_ready mirrors can_convolve_hrtf_pan_path");
    expectTrue(fuse::audio::preflight_hrtf_narrowing_ready(fuse::audio::HrtfPanPath::IldItdStub,
                   ...
                   == fuse::audio::can_narrow_hrtf_spatial_image(
                          fuse::audio::preflight_hrtf_attenuation_coupling(
                              fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f)),
               "preflight_hrtf_narrowing_ready mirrors can_narrow_hrtf_spatial_image");
    expectTrue(fuse::audio::preflight_hrtf_binaural_ready(true, empty, offset, 0.2f, 0.3f)
                   == fuse::audio::can_apply_hrtf_binaural_pan(
                          fuse::audio::preflight_hrtf_binaural(true, empty, offset, 0.2f, 0.3f)),
               "preflight_hrtf_binaural_ready mirrors can_apply_hrtf_binaural_pan");
...
void testHrtfPanPathRejectReasonGuards() {
    ...
    expectTrue(fuse::audio::try_preflight_hrtf_pan_path(true, empty, offset, reason),
               "try_preflight_hrtf_pan_path accepts empty IR stub path");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::None,
               "empty IR spatial pan reject reason is None");
    ...
    expectTrue(!fuse::audio::try_preflight_hrtf_pan_convolution(true, empty, offset, reason),
               "try_preflight_hrtf_pan_convolution rejects empty IR");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::EmptyIr,
               "empty IR convolution reject reason is EmptyIr");
    ...
    expectTrue(fuse::audio::try_preflight_hrtf_pan_convolution(true, valid, offset, reason),
               "try_preflight_hrtf_pan_convolution accepts valid IR");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::None,
               "valid IR convolution reject reason is None");
    ...
    expectTrue(!fuse::audio::try_preflight_hrtf_pan_path(false, valid, offset, reason),
               "try_preflight_hrtf_pan_path rejects disabled HRTF");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::HrtfDisabled,
               "disabled HRTF spatial reject reason is HrtfDisabled");
    expectTrue(fuse::audio::should_skip_hrtf_pan_path_preflight(false, valid, offset),
               "should_skip_hrtf_pan_path_preflight true when disabled");
    ...
    expectTrue(!fuse::audio::preflight_hrtf_pan_path_ready(true, valid, co_located, &reason),
               "preflight_hrtf_pan_path_ready rejects co-located source");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::CoLocated,
               "co-located spatial reject reason is CoLocated");

    const fuse::audio::HrtfPanPathPreflight stub_preflight =
        fuse::audio::preflight_hrtf_pan_path(true, empty, offset);
    expectTrue(fuse::audio::classify_hrtf_pan_path_convolution_reject(stub_preflight)
                   == fuse::audio::HrtfPanPathRejectReason::EmptyIr,
               "classify_hrtf_pan_path_convolution_reject maps emptyIr flag");
    expectTrue(std::strcmp(fuse::audio::hrtf_pan_path_reject_reason_label(
                   fuse::audio::HrtfPanPathRejectReason::CoLocated),
               "CoLocated") == 0,
               "pan-path reject reason label for CoLocated");
...
void testHrtfBinauralRejectReasonGuards() {
    ...
    expectTrue(fuse::audio::try_preflight_hrtf_binaural_spatial(true, empty, offset, reason),
               "try_preflight_hrtf_binaural_spatial accepts empty IR stub path");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::None,
               "empty IR composite spatial reject reason is None");

    expectTrue(!fuse::audio::try_preflight_hrtf_binaural_convolution(true, empty, offset, reason),
               "try_preflight_hrtf_binaural_convolution rejects empty IR");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::NullSamples,
               "empty IR composite convolution reject reason is NullSamples");
    ...
    expectTrue(!fuse::audio::preflight_hrtf_binaural_spatial_ready(false, valid, offset, &reason),
               "preflight_hrtf_binaural_spatial_ready rejects disabled HRTF");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "disabled HRTF composite spatial reject reason is HrtfDisabled");

    expectTrue(!fuse::audio::try_preflight_hrtf_binaural_spatial(true, valid, co_located, reason),
               "try_preflight_hrtf_binaural_spatial rejects co-located source");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::CoLocated,
               "co-located composite spatial reject reason is CoLocated");
    ...
    const fuse::audio::HrtfBinauralPreflight bypass_preflight =
        fuse::audio::preflight_hrtf_binaural(false, valid, offset, 0.1f, 0.1f);
    expectTrue(fuse::audio::classify_hrtf_binaural_spatial_reject(bypass_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "classify_hrtf_binaural_spatial_reject maps hrtfDisabled flag");
    expectTrue(std::strcmp(fuse::audio::hrtf_binaural_reject_reason_label(
                   fuse::audio::HrtfBinauralRejectReason::EmptyIr),
               "EmptyIr") == 0,
               "binaural reject reason label for EmptyIr");
}
...
void testRejectReasonMirrorsExistingPreflights() {
    ...
    expectTrue(fuse::audio::preflight_hrtf_pan_path_ready(true, empty, offset)
                   == fuse::audio::can_apply_spatial_hrtf_pan(
                          fuse::audio::preflight_hrtf_pan_path(true, empty, offset)),
               "preflight_hrtf_pan_path_ready mirrors can_apply_spatial_hrtf_pan on stub path");
    expectTrue(fuse::audio::preflight_hrtf_pan_convolution_ready(true, valid, offset)
                   == fuse::audio::can_convolve_hrtf_pan_path(
                          fuse::audio::preflight_hrtf_pan_path(true, valid, offset)),
               "preflight_hrtf_pan_convolution_ready mirrors can_convolve_hrtf_pan_path");
    expectTrue(fuse::audio::preflight_hrtf_attenuation_coupling_ready(
                   fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f)
                   == fuse::audio::can_narrow_hrtf_spatial_image(
                          fuse::audio::preflight_hrtf_attenuation_coupling(
    ...
    expectTrue(fuse::audio::preflight_hrtf_binaural_convolution_ready(true, valid, offset)
                   == fuse::audio::can_convolve_hrtf_binaural(
                          fuse::audio::preflight_hrtf_binaural(true, valid, offset, 1.f, 1.f)),
               "preflight_hrtf_binaural_convolution_ready mirrors can_convolve_hrtf_binaural");
    expectTrue(fuse::audio::preflight_hrtf_binaural_narrowing_ready(true, empty, offset, 0.2f, 0.3f)
                   == fuse::audio::can_narrow_hrtf_binaural_spatial_image(
                          fuse::audio::preflight_hrtf_binaural(true, empty, offset, 0.2f, 0.3f)),
               "preflight_hrtf_binaural_narrowing_ready mirrors can_narrow_hrtf_binaural_spatial_image");
}
...
void testHrtfPanPathRejectReasonGuards() {
    ...
    expectTrue(fuse::audio::try_preflight_hrtf_pan_path(true, valid, offset, reason),
               "try_preflight_hrtf_pan_path accepts valid spatial path");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::None,
               "valid spatial pan reject reason is None");
    ...
    const fuse::audio::HrtfPanPathPreflight stub_preflight =
        fuse::audio::preflight_hrtf_pan_path(true, empty, offset);
    expectTrue(fuse::audio::classify_hrtf_pan_path_spatial_reject(stub_preflight)
                   == fuse::audio::HrtfPanPathRejectReason::None,
               "ILD/ITD stub path spatial reject reason is None");
    expectTrue(fuse::audio::classify_hrtf_pan_path_convolution_reject(stub_preflight)
                   == fuse::audio::HrtfPanPathRejectReason::EmptyIr,
               "ILD/ITD stub path convolution reject reason is EmptyIr");
    expectTrue(std::strcmp(fuse::audio::hrtf_pan_path_reject_reason_label(
                   fuse::audio::HrtfPanPathRejectReason::EmptyIr),
               "EmptyIr") == 0,
               "pan-path reject reason label for EmptyIr");

    expectTrue(fuse::audio::preflight_hrtf_pan_path_ready(true, offset, &reason),
               "no-IR overload accepts spatial pan");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::None,
               "no-IR overload spatial reject reason is None");
}
...
void testHrtfBinauralRejectReasonGuards() {
    ...
    expectTrue(fuse::audio::try_preflight_hrtf_binaural_spatial(true, empty, offset, 0.2f, 0.3f,
                                                                reason),
               "try_preflight_hrtf_binaural_spatial accepts stub path");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::None,
               "stub path spatial reject reason is None");

    expectTrue(!fuse::audio::try_preflight_hrtf_binaural_spatial(false, valid, offset, 0.1f, 0.1f,
                                                                 reason),
               "try_preflight_hrtf_binaural_spatial rejects disabled HRTF");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "composite disabled spatial reject reason is HrtfDisabled");
    ...
    expectTrue(!fuse::audio::try_preflight_hrtf_binaural_spatial(true, valid, co_located, 0.1f,
                                                                 0.1f, reason),
               "try_preflight_hrtf_binaural_spatial rejects co-located source");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::CoLocated,
               "composite co-located spatial reject reason is CoLocated");

    expectTrue(!fuse::audio::try_preflight_hrtf_binaural_convolution(true, empty, offset, 0.2f, 0.3f,
                                                                   reason),
               "try_preflight_hrtf_binaural_convolution rejects empty IR");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::EmptyIr,
               "composite empty IR convolution reject reason is EmptyIr");
    ...
    expectTrue(fuse::audio::try_preflight_hrtf_binaural_convolution(true, valid, offset, 1.f, 1.f,
                                                                    reason),
               "try_preflight_hrtf_binaural_convolution accepts valid IR");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::None,
               "valid IR composite convolution reject reason is None");

    expectTrue(!fuse::audio::try_preflight_hrtf_binaural_coupling(false, valid, offset, 0.1f, 0.1f,
                                                                  reason),
               "try_preflight_hrtf_binaural_coupling rejects bypass path");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::BypassPath,
               "composite bypass coupling reject reason is BypassPath");

    expectTrue(!fuse::audio::try_preflight_hrtf_binaural_coupling(true, valid, offset, 1.f, 1.f,
                                                                  reason),
               "try_preflight_hrtf_binaural_coupling rejects unity attenuation");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::UnityAttenuation,
               "composite unity coupling reject reason is UnityAttenuation");

    expectTrue(fuse::audio::try_preflight_hrtf_binaural_coupling(true, empty, offset, 0.2f, 0.3f,
                                                                 reason),
               "try_preflight_hrtf_binaural_coupling accepts reduced attenuation on stub path");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::None,
               "reduced attenuation composite coupling reject reason is None");
    ...
    const fuse::audio::HrtfBinauralPreflight malformed_preflight =
        fuse::audio::preflight_hrtf_binaural(true, malformed, offset, 1.f, 1.f);
    expectTrue(fuse::audio::classify_hrtf_binaural_convolution_reject(malformed_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::MalformedIr,
               "classify_hrtf_binaural_convolution_reject maps malformedIr flag");
    expectTrue(std::strcmp(fuse::audio::hrtf_binaural_reject_reason_label(
                   fuse::audio::HrtfBinauralRejectReason::MalformedIr),
               "MalformedIr") == 0,
               "binaural reject reason label for MalformedIr");

    expectTrue(fuse::audio::preflight_hrtf_binaural_spatial_ready(true, offset, 0.25f, 0.35f,
                                                                  &reason),
               "Vec3 overload accepts spatial pan");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::None,
               "Vec3 overload spatial reject reason is None");
}
    ...
    expectTrue(fuse::audio::preflight_hrtf_attenuation_coupling_ready(
                   ...
                   == fuse::audio::can_narrow_hrtf_spatial_image(
                          fuse::audio::preflight_hrtf_attenuation_coupling(
                              fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f)),
               "preflight_hrtf_attenuation_coupling_ready mirrors can_narrow_hrtf_spatial_image");
    expectTrue(fuse::audio::preflight_hrtf_binaural_spatial_ready(true, empty, offset, 0.2f, 0.3f)
                   == fuse::audio::can_apply_hrtf_binaural_pan(
                          fuse::audio::preflight_hrtf_binaural(true, empty, offset, 0.2f, 0.3f)),
               "preflight_hrtf_binaural_spatial_ready mirrors can_apply_hrtf_binaural_pan");
    expectTrue(fuse::audio::preflight_hrtf_binaural_convolution_ready(true, valid, offset, 1.f,
...
void testHrtfIrRejectReasonEnumsAndPreflightWrappers() {
    const fuse::audio::HrtfIrStub empty = fuse::audio::make_empty_hrtf_ir();
    expectTrue(fuse::audio::classify_hrtf_ir_reject_reason(empty)
                   == fuse::audio::HrtfIrRejectReason::NullSamples,
               "empty IR classifies as NullSamples");

    const float samples[] = {0.5f};
    const fuse::audio::HrtfIrStub malformed{samples, 0};
    expectTrue(fuse::audio::classify_hrtf_ir_reject_reason(malformed)
                   == fuse::audio::HrtfIrRejectReason::MalformedIr,
               "malformed IR classifies as MalformedIr");

    const fuse::audio::HrtfIrStub valid{samples, 1};
    expectTrue(fuse::audio::classify_hrtf_ir_reject_reason(valid)
                   == fuse::audio::HrtfIrRejectReason::None,
               "valid IR classifies as None");

    fuse::audio::HrtfIrRejectReason rejectReason = fuse::audio::HrtfIrRejectReason::None;
    expectTrue(fuse::audio::try_preflight_hrtf_ir_convolution(valid, rejectReason),
               "try_preflight_hrtf_ir_convolution passes for valid IR");
    expectTrue(rejectReason == fuse::audio::HrtfIrRejectReason::None,
               "valid IR reject reason stays None");
    expectTrue(!fuse::audio::preflight_hrtf_ir_convolution(empty, &rejectReason),
               "preflight_hrtf_ir_convolution rejects empty IR");
    expectTrue(rejectReason == fuse::audio::HrtfIrRejectReason::NullSamples,
               "empty IR reject reason is NullSamples");

    const fuse::audio::HrtfIrPreflight empty_preflight = fuse::audio::preflight_hrtf_ir(empty);
    expectTrue(empty_preflight.rejectReason == fuse::audio::HrtfIrRejectReason::NullSamples,
               "IR preflight carries NullSamples reject reason");
    expectTrue(std::strcmp(fuse::audio::hrtf_ir_reject_reason_name(empty_preflight.rejectReason),
                           "NullSamples")
                   == 0,
               "IR reject reason name matches NullSamples");
}
...
void testHrtfPanPathRejectReasonEnumsAndPreflightWrappers() {
    ...
    expectTrue(fuse::audio::classify_hrtf_pan_path_reject_reason(true, valid, offset)
                   == fuse::audio::HrtfPanPathRejectReason::None,
               "enabled offset source has no pan-path reject");
    expectTrue(fuse::audio::classify_hrtf_pan_path_reject_reason(false, valid, offset)
                   == fuse::audio::HrtfPanPathRejectReason::HrtfDisabled,
               "disabled HRTF classifies as HrtfDisabled");
    expectTrue(fuse::audio::classify_hrtf_pan_path_reject_reason(true, valid, co_located)
                   == fuse::audio::HrtfPanPathRejectReason::CoLocated,
               "co-located source classifies as CoLocated");
    expectTrue(fuse::audio::classify_hrtf_pan_path_reject_reason(true, offset)
                   == fuse::audio::HrtfPanPathRejectReason::None,
               "no-IR classify matches IR-aware classify on valid path");

    fuse::audio::HrtfPanPathRejectReason rejectReason = fuse::audio::HrtfPanPathRejectReason::None;
    expectTrue(fuse::audio::try_preflight_hrtf_spatial_pan(true, valid, offset, rejectReason),
               "try_preflight_hrtf_spatial_pan passes for enabled offset source");
    expectTrue(!fuse::audio::preflight_hrtf_spatial_pan(false, valid, offset, &rejectReason),
               "preflight_hrtf_spatial_pan rejects disabled HRTF");
    expectTrue(rejectReason == fuse::audio::HrtfPanPathRejectReason::HrtfDisabled,
               "disabled HRTF reject reason is HrtfDisabled");
    expectTrue(!fuse::audio::preflight_hrtf_spatial_pan(true, co_located, &rejectReason),
               "no-IR preflight rejects co-located source");
    ...
    expectTrue(stub_preflight.rejectReason == fuse::audio::HrtfPanPathRejectReason::None,
               "ILD/ITD stub path carries no pan-path reject reason");
    expectTrue(std::strcmp(fuse::audio::hrtf_pan_path_reject_reason_name(
                               fuse::audio::HrtfPanPathRejectReason::CoLocated),
                           "CoLocated")
                   == 0,
               "pan-path reject reason name matches CoLocated");
}
...
void testHrtfAttenuationCouplingRejectReasonEnumsAndPreflightWrappers() {
    ...
    expectTrue(fuse::audio::classify_hrtf_attenuation_coupling_reject_reason(
                   fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f)
                   == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "reduced attenuation has no coupling reject");
    expectTrue(fuse::audio::classify_hrtf_attenuation_coupling_reject_reason(
    ...
    expectTrue(fuse::audio::try_preflight_hrtf_attenuation_narrowing(
                   fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f, rejectReason),
               "try_preflight_hrtf_attenuation_narrowing passes for reduced attenuation");
    expectTrue(rejectReason == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "narrowing reject reason stays None on valid path");
    expectTrue(!fuse::audio::preflight_hrtf_attenuation_narrowing(
                   fuse::audio::HrtfPanPath::Bypass, 0.1f, 0.1f, &rejectReason),
               "preflight_hrtf_attenuation_narrowing rejects bypass path");
    expectTrue(rejectReason == fuse::audio::HrtfAttenuationCouplingRejectReason::BypassPath,
               "bypass narrowing reject reason is BypassPath");
    ...
    expectTrue(narrow_preflight.rejectReason
                   == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "narrowing preflight carries no reject reason on valid path");
    expectTrue(std::strcmp(fuse::audio::hrtf_attenuation_coupling_reject_reason_name(
                               fuse::audio::HrtfAttenuationCouplingRejectReason::UnityAttenuation),
                           "UnityAttenuation")
                   == 0,
               "coupling reject reason name matches UnityAttenuation");
}
...
void testHrtfBinauralRejectReasonEnumsAndPreflightWrappers() {
    ...
    expectTrue(fuse::audio::classify_hrtf_binaural_reject_reason(true, valid, offset)
                   == fuse::audio::HrtfBinauralRejectReason::None,
               "enabled offset composite classify is None");
    expectTrue(fuse::audio::classify_hrtf_binaural_reject_reason(false, valid, offset)
                   == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "disabled composite classify is HrtfDisabled");
    expectTrue(fuse::audio::classify_hrtf_binaural_reject_reason(true, offset)
                   == fuse::audio::HrtfBinauralRejectReason::None,
               "no-IR composite classify matches IR-aware classify");

    fuse::audio::HrtfBinauralRejectReason rejectReason = fuse::audio::HrtfBinauralRejectReason::None;
    expectTrue(fuse::audio::try_preflight_hrtf_binaural_spatial(true, empty, offset, rejectReason),
               "try_preflight_hrtf_binaural_spatial passes for stub path");
    expectTrue(!fuse::audio::preflight_hrtf_binaural_spatial(true, valid, co_located, &rejectReason),
               "preflight_hrtf_binaural_spatial rejects co-located source");
    expectTrue(rejectReason == fuse::audio::HrtfBinauralRejectReason::CoLocated,
               "composite reject reason is CoLocated");

    const fuse::audio::HrtfBinauralPreflight stub_preflight =
        fuse::audio::preflight_hrtf_binaural(true, empty, offset, 0.2f, 0.3f);
    expectTrue(stub_preflight.rejectReason == fuse::audio::HrtfBinauralRejectReason::None,
               "composite preflight carries no reject reason on stub path");
    expectTrue(std::strcmp(fuse::audio::hrtf_binaural_reject_reason_name(
                               fuse::audio::HrtfBinauralRejectReason::HrtfDisabled),
                           "HrtfDisabled")
                   == 0,
               "binaural reject reason name matches HrtfDisabled");
}
...
void testHrtfIrRejectReasons() {
    ...
    expectTrue(std::strcmp(fuse::audio::hrtfIrRejectReasonLabel(reason), "null_samples") == 0,
               "null_samples IR reject reason label");
    expectTrue(fuse::audio::hrtfIrRejectReasonIsBlocking(reason),
               "null_samples IR reject reason is blocking");
    ...
    const fuse::audio::HrtfIrPreflight preflight = fuse::audio::preflight_hrtf_ir(empty);
    expectTrue(preflight.rejectReason == fuse::audio::HrtfIrRejectReason::NullSamples,
               "preflight_hrtf_ir carries rejectReason for empty IR");
...
void testHrtfPanPathRejectReasons() {
    ...
    expectTrue(preflight.rejectReason == fuse::audio::HrtfPanPathRejectReason::HrtfDisabled,
               "preflight_hrtf_pan_path carries rejectReason for disabled HRTF");
}
...
void testHrtfAttenuationCouplingRejectReasons() {
    ...
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "narrowing reports none reject reason");
    expectTrue(std::strcmp(fuse::audio::hrtfAttenuationCouplingRejectReasonLabel(reason), "none") == 0,
               "none attenuation-coupling reject reason label");

    expectTrue(!fuse::audio::tryPreflightHrtfAttenuationCoupling(
                   fuse::audio::HrtfPanPath::Bypass, 0.1f, 0.1f, reason),
               "bypass path fails coupling tryPreflight");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::BypassPath,
               "bypass path reports bypass_path reject reason");
    expectTrue(std::strcmp(fuse::audio::hrtfAttenuationCouplingRejectReasonLabel(reason),
                           "bypass_path")
                   == 0,
               "bypass_path attenuation-coupling reject reason label");

    expectTrue(!fuse::audio::tryPreflightHrtfAttenuationCoupling(
                   fuse::audio::HrtfPanPath::Convolution, 1.f, 1.f, reason),
               "unity attenuation fails coupling tryPreflight");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::UnityAttenuation,
               "unity attenuation reports unity_attenuation reject reason");
    expectTrue(std::strcmp(fuse::audio::hrtfAttenuationCouplingRejectReasonLabel(reason),
                           "unity_attenuation")
                   == 0,
               "unity_attenuation reject reason label");
    expectTrue(fuse::audio::hrtfAttenuationCouplingRejectReasonIsBlocking(reason),
               "unity_attenuation reject reason is blocking");

    const fuse::audio::HrtfAttenuationCouplingPreflight preflight =
        fuse::audio::preflight_hrtf_attenuation_coupling(fuse::audio::HrtfPanPath::IldItdStub,
                                                         0.2f, 0.3f);
    expectTrue(preflight.rejectReason == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "preflight_hrtf_attenuation_coupling carries none rejectReason when narrowing");
    expectTrue(preflight.rejectReason
                   == fuse::audio::classifyHrtfAttenuationCouplingReject(
                          fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f),
...
void testHrtfBinauralRejectReasons() {
    ...
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "disabled HRTF reports hrtf_disabled composite reject reason");
    expectTrue(fuse::audio::hrtfBinauralRejectReasonIsBypass(reason),
               "hrtf_disabled composite reject reason is bypass");
    expectTrue(fuse::audio::hrtfBinauralRejectReasonBlocksConvolution(reason),
               "hrtf_disabled blocks convolution");
    ...
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::EmptyIr,
               "empty IR reports empty_ir composite reject reason");
    expectTrue(std::strcmp(fuse::audio::hrtfBinauralRejectReasonLabel(reason), "empty_ir") == 0,
               "empty_ir composite reject reason label");
    expectTrue(fuse::audio::hrtfBinauralRejectReasonBlocksConvolution(reason),
               "empty_ir blocks convolution");
    expectTrue(!fuse::audio::hrtfBinauralRejectReasonIsBypass(reason),
               "empty_ir is not a bypass reject reason");
    ...
    expectTrue(preflight.rejectReason == fuse::audio::HrtfBinauralRejectReason::EmptyIr,
               "preflight_hrtf_binaural carries rejectReason for empty IR");
    expectTrue(preflight.ir.rejectReason == fuse::audio::HrtfIrRejectReason::NullSamples,
               "composite preflight carries nested IR rejectReason");
    expectTrue(preflight.panPath.rejectReason == fuse::audio::HrtfPanPathRejectReason::None,
               "composite preflight carries nested pan-path rejectReason");
    expectTrue(preflight.attenuationCoupling.rejectReason
                   == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "composite preflight carries nested attenuation rejectReason");
}
...
void testHrtfIrRejectReasonGuards() {
    ...
    const fuse::audio::HrtfIrStub malformed{samples, 0};
    expectTrue(!fuse::audio::try_preflight_hrtf_ir_convolution(malformed, reason),
               "try_preflight_hrtf_ir_convolution rejects malformed IR");
    expectTrue(reason == fuse::audio::HrtfIrRejectReason::MalformedIr,
               "malformed IR reject reason is MalformedIr");
    ...
    expectTrue(fuse::audio::preflight_hrtf_ir_convolution_ready(valid)
                   == fuse::audio::can_convolve_hrtf_ir(fuse::audio::preflight_hrtf_ir(valid)),
               "preflight_hrtf_ir_convolution_ready mirrors can_convolve_hrtf_ir");
}
...
void testHrtfPanPathRejectReasonGuards() {
    ...
    fuse::audio::HrtfPanPathRejectReason reason = fuse::audio::HrtfPanPathRejectReason::None;
    expectTrue(fuse::audio::try_preflight_hrtf_pan_spatial(true, empty, offset, reason),
               "try_preflight_hrtf_pan_spatial accepts enabled offset source");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::None,
               "enabled offset spatial reject reason is None");
    ...
    expectTrue(!fuse::audio::try_preflight_hrtf_pan_spatial(false, valid, offset, reason),
               "try_preflight_hrtf_pan_spatial rejects disabled HRTF");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::HrtfDisabled,
               "disabled HRTF spatial reject reason is HrtfDisabled");
    expectTrue(fuse::audio::should_skip_hrtf_pan_spatial(false, offset),
               "should_skip_hrtf_pan_spatial true when disabled");

    expectTrue(!fuse::audio::try_preflight_hrtf_pan_spatial(true, valid, co_located, reason),
               "try_preflight_hrtf_pan_spatial rejects co-located source");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::CoLocated,
               "co-located spatial reject reason is CoLocated");
    ...
    expectTrue(fuse::audio::preflight_hrtf_pan_spatial_ready(true, offset)
                   == fuse::audio::can_apply_spatial_hrtf_pan(
                          fuse::audio::preflight_hrtf_pan_path(true, offset)),
               "preflight_hrtf_pan_spatial_ready mirrors can_apply_spatial_hrtf_pan");
}
...
void testHrtfAttenuationCouplingRejectReasonGuards() {
    ...
    expectTrue(!fuse::audio::try_preflight_hrtf_attenuation_narrowing(
                   fuse::audio::HrtfPanPath::Bypass, 0.1f, 0.1f, reason),
               "try_preflight_hrtf_attenuation_narrowing rejects bypass path");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::BypassPath,
               "bypass narrowing reject reason is BypassPath");
    ...
    expectTrue(!fuse::audio::try_preflight_hrtf_attenuation_narrowing(
                   fuse::audio::HrtfPanPath::Convolution, 1.f, 1.f, reason),
               "try_preflight_hrtf_attenuation_narrowing rejects unity attenuation");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::UnityAttenuation,
               "unity attenuation narrowing reject reason is UnityAttenuation");

    expectTrue(fuse::audio::try_preflight_hrtf_attenuation_narrowing(
                   fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f, reason),
               "try_preflight_hrtf_attenuation_narrowing accepts reduced attenuation");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "reduced attenuation narrowing reject reason is None");
    ...
    expectTrue(fuse::audio::preflight_hrtf_attenuation_narrowing_ready(
                   fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f)
                   == fuse::audio::can_narrow_hrtf_spatial_image(
                          fuse::audio::preflight_hrtf_attenuation_coupling(
                              fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f)),
               "preflight_hrtf_attenuation_narrowing_ready mirrors can_narrow_hrtf_spatial_image");
}
...
void testHrtfBinauralRejectReasonGuards() {
    ...
    expectTrue(fuse::audio::try_preflight_hrtf_binaural_spatial(true, offset, reason),
               "try_preflight_hrtf_binaural_spatial accepts enabled offset source");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::None,
               "enabled offset binaural spatial reject reason is None");

    expectTrue(!fuse::audio::try_preflight_hrtf_binaural_spatial(false, valid, offset, reason),
               "try_preflight_hrtf_binaural_spatial rejects disabled HRTF");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "disabled HRTF binaural spatial reject reason is HrtfDisabled");

    expectTrue(!fuse::audio::try_preflight_hrtf_binaural_convolution(true, empty, offset, reason),
               "try_preflight_hrtf_binaural_convolution rejects empty IR");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::EmptyIr,
               "empty IR binaural convolution reject reason is EmptyIr");

    expectTrue(fuse::audio::try_preflight_hrtf_binaural_convolution(true, valid, offset, reason),
               "try_preflight_hrtf_binaural_convolution accepts valid IR");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::None,
               "valid IR binaural convolution reject reason is None");

    expectTrue(!fuse::audio::try_preflight_hrtf_binaural_narrowing(true, offset, 1.f, 1.f, reason),
               "try_preflight_hrtf_binaural_narrowing rejects unity attenuation");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::UnityAttenuation,
               "unity attenuation binaural narrowing reject reason is UnityAttenuation");

    expectTrue(fuse::audio::try_preflight_hrtf_binaural_narrowing(true, empty, offset, 0.2f, 0.3f,
                                                                  reason),
               "try_preflight_hrtf_binaural_narrowing accepts reduced attenuation");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::None,
               "reduced attenuation binaural narrowing reject reason is None");
    ...
    expectTrue(fuse::audio::classify_hrtf_binaural_convolution_reject(bypass_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "classify_hrtf_binaural_convolution_reject maps hrtfDisabled flag");
    expectTrue(fuse::audio::classify_hrtf_binaural_narrowing_reject(bypass_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::BypassPath,
               "classify_hrtf_binaural_narrowing_reject maps bypassPath flag");

    const fuse::audio::HrtfBinauralPreflight co_located_preflight =
        fuse::audio::preflight_hrtf_binaural(true, valid, co_located, 0.1f, 0.1f);
    expectTrue(fuse::audio::classify_hrtf_binaural_spatial_reject(co_located_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::CoLocated,
               "classify_hrtf_binaural_spatial_reject maps coLocated flag");
    ...
    expectTrue(fuse::audio::preflight_hrtf_binaural_spatial_ready(true, offset)
                   == fuse::audio::can_apply_hrtf_binaural_pan(
                          fuse::audio::preflight_hrtf_binaural(true, offset, 1.f, 1.f)),
               "preflight_hrtf_binaural_spatial_ready mirrors can_apply_hrtf_binaural_pan");
    expectTrue(fuse::audio::preflight_hrtf_binaural_convolution_ready(true, valid, offset)
                   == fuse::audio::can_convolve_hrtf_binaural(
                          fuse::audio::preflight_hrtf_binaural(true, valid, offset, 1.f, 1.f)),
               "preflight_hrtf_binaural_convolution_ready mirrors can_convolve_hrtf_binaural");
    expectTrue(fuse::audio::preflight_hrtf_binaural_narrowing_ready(true, offset, 0.2f, 0.3f)
                   == fuse::audio::can_narrow_hrtf_binaural_spatial_image(
                          fuse::audio::preflight_hrtf_binaural(true, offset, 0.2f, 0.3f)),
               "preflight_hrtf_binaural_narrowing_ready mirrors can_narrow_hrtf_binaural_spatial_image");
}
...
void testHrtfIrRejectReasonGuards() {
    ...
    const fuse::audio::HrtfIrStub malformed{samples, 0};
    expectTrue(!fuse::audio::try_preflight_hrtf_ir(malformed, reason),
               "try_preflight_hrtf_ir rejects malformed IR");
    expectTrue(reason == fuse::audio::HrtfIrRejectReason::MalformedIr,
               "malformed IR reject reason is MalformedIr");
...
void testHrtfPanPathRejectReasonGuards() {
    ...
    expectTrue(fuse::audio::preflight_hrtf_pan_convolution_ready(true, valid, offset, &reason),
               "preflight_hrtf_pan_convolution_ready accepts valid IR");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::None,
               "valid IR convolution reject reason is None");
    ...
    expectTrue(fuse::audio::classify_hrtf_pan_path_reject(stub_preflight)
                   == fuse::audio::HrtfPanPathRejectReason::None,
               "classify_hrtf_pan_path_reject is None on stub path");
    expectTrue(fuse::audio::classify_hrtf_pan_path_convolution_reject(stub_preflight)
                   == fuse::audio::HrtfPanPathRejectReason::EmptyIr,
               "classify_hrtf_pan_path_convolution_reject maps emptyIr flag");
    expectTrue(std::strcmp(fuse::audio::hrtf_pan_path_reject_reason_label(
...
void testHrtfBinauralRejectReasonGuards() {
    ...
    expectTrue(fuse::audio::try_preflight_hrtf_binaural(true, empty, offset, 0.2f, 0.3f, reason),
               "try_preflight_hrtf_binaural accepts stub path");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::None,
               "stub path composite reject reason is None");
    expectTrue(!fuse::audio::should_skip_hrtf_binaural_preflight(true, empty, offset, 0.2f, 0.3f),
               "should_skip_hrtf_binaural_preflight false on stub path");
    ...
    expectTrue(fuse::audio::classify_hrtf_binaural_reject(bypass_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "classify_hrtf_binaural_reject maps hrtfDisabled flag");
    expectTrue(std::strcmp(fuse::audio::hrtf_binaural_reject_reason_label(
                   fuse::audio::HrtfBinauralRejectReason::EmptyIr),
               "EmptyIr") == 0,
               "composite reject reason label for EmptyIr");
...
void testHrtfPanPathRejectReasonGuards() {
    ...
    fuse::audio::HrtfPanPathRejectReason reason = fuse::audio::HrtfPanPathRejectReason::None;
    expectTrue(fuse::audio::try_preflight_hrtf_pan_path_spatial(true, empty, offset, reason),
               "try_preflight_hrtf_pan_path_spatial accepts empty IR stub path");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::None,
               "ILD/ITD stub spatial path reject reason is None");

    expectTrue(!fuse::audio::try_preflight_hrtf_pan_path_spatial(false, valid, offset, reason),
               "try_preflight_hrtf_pan_path_spatial rejects disabled HRTF");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::HrtfDisabled,
               "disabled HRTF spatial reject reason is HrtfDisabled");
    expectTrue(fuse::audio::should_skip_hrtf_pan_path_spatial_input(false, valid, offset),
               "should_skip_hrtf_pan_path_spatial_input true when disabled");

    expectTrue(!fuse::audio::try_preflight_hrtf_pan_path_spatial(true, valid, co_located, reason),
               "try_preflight_hrtf_pan_path_spatial rejects co-located source");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::CoLocated,
               "co-located spatial reject reason is CoLocated");

    expectTrue(!fuse::audio::try_preflight_hrtf_pan_path_convolution(true, empty, offset, reason),
               "try_preflight_hrtf_pan_path_convolution rejects empty IR");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::EmptyIr,
               "empty IR convolution reject reason is EmptyIr");

    expectTrue(fuse::audio::try_preflight_hrtf_pan_path_convolution(true, valid, offset, reason),
               "try_preflight_hrtf_pan_path_convolution accepts valid IR");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::None,
               "valid IR convolution reject reason is None");

    const fuse::audio::HrtfIrStub malformed{samples, 0};
    expectTrue(!fuse::audio::preflight_hrtf_pan_path_convolution_ready(true, malformed, offset,
                                                                       &reason),
               "preflight_hrtf_pan_path_convolution_ready rejects malformed IR");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::MalformedIr,
               "malformed IR convolution reject reason is MalformedIr");

    const fuse::audio::HrtfPanPathPreflight disabled_preflight =
        fuse::audio::preflight_hrtf_pan_path(false, valid, offset);
    expectTrue(fuse::audio::classify_hrtf_pan_path_spatial_reject(disabled_preflight)
                   == fuse::audio::HrtfPanPathRejectReason::HrtfDisabled,
               "classify_hrtf_pan_path_spatial_reject maps hrtfDisabled flag");
    expectTrue(std::strcmp(fuse::audio::hrtfPanPathRejectReasonLabel(
                   fuse::audio::HrtfPanPathRejectReason::EmptyIr),
               "EmptyIr") == 0,
               "pan-path reject reason label for EmptyIr");

    expectTrue(fuse::audio::try_preflight_hrtf_pan_path_spatial(true, offset, reason),
               "no-IR spatial preflight wrapper accepts offset source");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::None,
               "no-IR spatial wrapper reject reason is None");
}
...
void testHrtfAttenuationCouplingRejectReasonGuards() {
    ...
    expectTrue(!fuse::audio::try_preflight_hrtf_attenuation_narrowing(
               ...
               "try_preflight_hrtf_attenuation_narrowing rejects bypass path");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::BypassPath,
               "bypass path narrowing reject reason is BypassPath");
    expectTrue(fuse::audio::should_skip_hrtf_attenuation_narrowing_input(
                   fuse::audio::HrtfPanPath::Bypass, 0.1f, 0.1f),
               "should_skip_hrtf_attenuation_narrowing_input true on bypass");
    ...
    expectTrue(!fuse::audio::should_skip_hrtf_attenuation_narrowing_input(
                   fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f),
               "should_skip_hrtf_attenuation_narrowing_input false when narrowing applies");
...
void testHrtfBinauralRejectReasonGuards() {
    ...
    expectTrue(fuse::audio::try_preflight_hrtf_binaural_spatial(true, empty, offset, 0.2f, 0.3f,
               ...
               "try_preflight_hrtf_binaural_spatial accepts stub path");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::None,
               "stub spatial binaural reject reason is None");
    ...
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "disabled HRTF binaural spatial reject reason is HrtfDisabled");
    expectTrue(fuse::audio::should_skip_hrtf_binaural_spatial_input(false, valid, offset, 0.1f,
                                                                    0.1f),
               "should_skip_hrtf_binaural_spatial_input true when disabled");

    expectTrue(!fuse::audio::try_preflight_hrtf_binaural_spatial(true, valid, co_located, 0.1f,
                                                                 0.1f, reason),
               "try_preflight_hrtf_binaural_spatial rejects co-located source");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::CoLocated,
               "co-located binaural spatial reject reason is CoLocated");
...
void testRejectReasonMirrorsExistingPreflights() {
    ...
    expectTrue(fuse::audio::preflight_hrtf_ir_ready(valid)
                   == fuse::audio::can_convolve_hrtf_ir(fuse::audio::preflight_hrtf_ir(valid)),
               "preflight_hrtf_ir_ready mirrors can_convolve_hrtf_ir on valid IR");
    expectTrue(fuse::audio::preflight_hrtf_pan_path_spatial_ready(true, empty, offset)
                   == fuse::audio::can_apply_spatial_hrtf_pan(
                          fuse::audio::preflight_hrtf_pan_path(true, empty, offset)),
               "preflight_hrtf_pan_path_spatial_ready mirrors can_apply_spatial_hrtf_pan");
    expectTrue(fuse::audio::preflight_hrtf_pan_path_convolution_ready(true, valid, offset)
                   == fuse::audio::can_convolve_hrtf_pan_path(
                          fuse::audio::preflight_hrtf_pan_path(true, valid, offset)),
               "preflight_hrtf_pan_path_convolution_ready mirrors can_convolve_hrtf_pan_path");
    expectTrue(fuse::audio::preflight_hrtf_attenuation_narrowing_ready(
                   fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f)
                   == fuse::audio::can_narrow_hrtf_spatial_image(
                          fuse::audio::preflight_hrtf_attenuation_coupling(
    ...
    expectTrue(fuse::audio::preflight_hrtf_binaural_spatial_ready(true, empty, offset, 0.2f, 0.3f)
                   == fuse::audio::can_apply_hrtf_binaural_pan(
                          fuse::audio::preflight_hrtf_binaural(true, empty, offset, 0.2f, 0.3f)),
               "preflight_hrtf_binaural_spatial_ready mirrors can_apply_hrtf_binaural_pan");
    expectTrue(fuse::audio::preflight_hrtf_binaural_convolution_ready(true, valid, offset, 1.f, 1.f)
                   == fuse::audio::can_convolve_hrtf_binaural(
                          fuse::audio::preflight_hrtf_binaural(true, valid, offset, 1.f, 1.f)),
               "preflight_hrtf_binaural_convolution_ready mirrors can_convolve_hrtf_binaural");
    expectTrue(fuse::audio::preflight_hrtf_binaural_narrowing_ready(true, empty, offset, 0.2f, 0.3f)
               ...
               "preflight_hrtf_binaural_narrowing_ready mirrors can_narrow_hrtf_binaural_spatial_image");
    expectTrue(fuse::audio::preflight_hrtf_binaural_spatial_ready(true, offset, 0.25f, 0.35f)
                   == fuse::audio::preflight_hrtf_binaural_spatial_ready(true, empty, offset, 0.25f,
                                                                         0.35f),
               "no-IR binaural spatial preflight wrapper matches IR-aware overload");
}
...
void testHrtfPanPathRejectReasonGuards() {
    ...
    fuse::audio::HrtfPanPathRejectReason reason = fuse::audio::HrtfPanPathRejectReason::None;
    expectTrue(fuse::audio::preflight_hrtf_pan_path_ready(true, empty, offset, &reason),
               "ILD/ITD stub path is spatial-pan ready");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::None,
    ...
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::HrtfDisabled,
               "disabled HRTF rejects with HrtfDisabled");
    expectTrue(fuse::audio::try_preflight_hrtf_pan_path(true, valid, offset, reason),
               "valid IR try_preflight accepts spatial pan path");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::None,
               "valid spatial pan path has no reject reason");
}
...
void testHrtfPanPathRejectReasonCoLocated() {
    ...
    expectTrue(!fuse::audio::preflight_hrtf_pan_path_ready(true, valid, co_located, &reason),
               "co-located pan-path preflight is not ready");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::CoLocated,
               "co-located source rejects with CoLocated");
    ...
    expectTrue(std::strcmp(fuse::audio::hrtf_pan_path_reject_reason_name(
                   fuse::audio::HrtfPanPathRejectReason::CoLocated),
               "CoLocated") == 0,
               "co-located pan-path reject label");
}
...
void testHrtfAttenuationCouplingRejectReasonGuards() {
    ...
    expectTrue(fuse::audio::try_preflight_hrtf_attenuation_coupling(
               ...
               "reduced attenuation coupling preflight is ready");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "reduced attenuation has no reject reason");
    expectTrue(!fuse::audio::should_skip_hrtf_attenuation_coupling_ready(
                   fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f),
               "reduced attenuation should not skip coupling ready");
}
...
void testHrtfBinauralRejectReasonGuards() {
    ...
    fuse::audio::HrtfBinauralRejectReason reason = fuse::audio::HrtfBinauralRejectReason::None;
    expectTrue(fuse::audio::preflight_hrtf_binaural_ready(true, empty, offset, 0.2f, 0.3f, &reason),
               "stub path composite preflight is ready");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::None,
               "spatial stub composite has no bypass reject");

    expectTrue(!fuse::audio::preflight_hrtf_binaural_ready(false, valid, offset, 0.1f, 0.1f,
                                                          &reason),
               "disabled composite preflight is not ready");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "disabled composite rejects with HrtfDisabled");
    ...
    expectTrue(fuse::audio::classify_hrtf_binaural_reject(co_located_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::CoLocated,
               "co-located composite classifies as CoLocated");
    expectTrue(std::strcmp(fuse::audio::hrtf_binaural_reject_reason_name(
                   fuse::audio::HrtfBinauralRejectReason::HrtfDisabled),
               "HrtfDisabled") == 0,
               "disabled composite reject label");
}
...
void testHrtfConvolutionRejectReasonGuards() {
    ...
    expectTrue(!fuse::audio::preflight_hrtf_convolution_ready(true, empty, offset, &reason),
               "empty IR convolution preflight is not ready");
    expectTrue(reason == fuse::audio::HrtfConvolutionRejectReason::EmptyIr,
               "empty IR rejects convolution with EmptyIr");

    const fuse::audio::HrtfIrStub malformed{samples, 0};
    expectTrue(!fuse::audio::preflight_hrtf_convolution_ready(true, malformed, offset, &reason),
               "malformed IR convolution preflight is not ready");
    expectTrue(reason == fuse::audio::HrtfConvolutionRejectReason::MalformedIr,
               "malformed IR rejects convolution with MalformedIr");

    expectTrue(!fuse::audio::preflight_hrtf_convolution_ready(false, valid, offset, &reason),
               "disabled HRTF convolution preflight is not ready");
    expectTrue(reason == fuse::audio::HrtfConvolutionRejectReason::HrtfDisabled,
               "disabled HRTF rejects convolution with HrtfDisabled");
    ...
    expectTrue(std::strcmp(fuse::audio::hrtf_convolution_reject_reason_name(
                   fuse::audio::HrtfConvolutionRejectReason::EmptyIr),
               "EmptyIr") == 0,
               "empty IR convolution reject label");
}
...
void testRejectReasonMirrorsExistingPreflights() {
    ...
    expectTrue(fuse::audio::preflight_hrtf_ir_ready(empty)
                   == fuse::audio::can_convolve_hrtf_ir(fuse::audio::preflight_hrtf_ir(empty)),
               "preflight_hrtf_ir_ready mirrors can_convolve_hrtf_ir");
    expectTrue(fuse::audio::preflight_hrtf_pan_path_ready(true, empty, offset)
                   == fuse::audio::can_apply_spatial_hrtf_pan(
                          fuse::audio::preflight_hrtf_pan_path(true, empty, offset)),
    ...
    const fuse::audio::Vec3 rel_listener =
        fuse::audio::to_listener_space(source_position - listener.position,
                                       fuse::audio::compute_listener_basis(listener));
    expectTrue(
        fuse::audio::preflight_hrtf_binaural_ready(true, listener, source_position, 0.25f, 0.35f)
            == fuse::audio::preflight_hrtf_binaural_ready(true, rel_listener, 0.25f, 0.35f),
        "listener-aware preflight_hrtf_binaural_ready mirrors offset overload on valid path");
}
...
void testHrtfPanPathRejectReasonNoIrOverload() {
    ...
    fuse::audio::HrtfPanPathRejectReason reason = fuse::audio::HrtfPanPathRejectReason::None;
    expectTrue(fuse::audio::preflight_hrtf_pan_path_ready(true, offset, &reason),
               "no-IR pan-path preflight is ready on offset source");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::None,
               "no-IR spatial path has no reject reason");
    expectTrue(fuse::audio::try_preflight_hrtf_pan_path(true, offset, reason),
               "no-IR try_preflight accepts spatial stub path");
}
...
void testHrtfIrRejectReasonGuards() {
    ...
    reason = fuse::audio::HrtfIrRejectReason::None;
    expectTrue(!fuse::audio::try_preflight_hrtf_ir(malformed, reason),
               "malformed IR try_preflight rejects");
    expectTrue(reason == fuse::audio::HrtfIrRejectReason::MalformedIr,
               "malformed IR rejects with malformed_ir reason");

    const fuse::audio::HrtfIrPreflight null_preflight =
        fuse::audio::preflight_hrtf_ir(fuse::audio::HrtfIrStub{nullptr, 4});
    expectTrue(fuse::audio::classify_hrtf_ir_reject(null_preflight)
                   == fuse::audio::HrtfIrRejectReason::NullSamples,
               "null samples classify as null_samples");
    expectTrue(std::strcmp(fuse::audio::hrtf_ir_reject_reason_label(
                   fuse::audio::HrtfIrRejectReason::EmptyIr),
               "empty_ir") == 0,
               "empty_ir label is stable");
...
void testHrtfPanPathRejectReasonGuards() {
    ...
    expectTrue(fuse::audio::preflight_hrtf_pan_path_ready(true, valid, offset, &reason),
               "enabled offset source is ready for spatial pan");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::None,
               "spatial pan reject reason is none on valid path");

    reason = fuse::audio::HrtfPanPathRejectReason::None;
    expectTrue(!fuse::audio::preflight_hrtf_pan_path_ready(false, valid, offset, &reason),
               "disabled HRTF is not ready for spatial pan");
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::HrtfDisabled,
               "disabled HRTF rejects with hrtf_disabled reason");
    ...
    const fuse::audio::HrtfPanPathPreflight no_ir_preflight =
        fuse::audio::preflight_hrtf_pan_path(true, offset);
    expectTrue(fuse::audio::preflight_hrtf_pan_path_ready(true, offset)
                   == no_ir_preflight.can_spatial_pan(),
               "no-IR preflight_hrtf_pan_path_ready mirrors can_spatial_pan");
    expectTrue(std::strcmp(fuse::audio::hrtf_pan_path_reject_reason_label(
                   fuse::audio::HrtfPanPathRejectReason::CoLocated),
               "co_located") == 0,
...
void testHrtfAttenuationCouplingRejectReasonGuards() {
    ...
    expectTrue(!fuse::audio::preflight_hrtf_attenuation_coupling_ready(
                   fuse::audio::HrtfPanPath::Bypass, 0.1f, 0.1f, &reason),
               "bypass path is not ready for attenuation narrowing");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::BypassPath,
               "bypass path rejects with bypass_path reason");

    reason = fuse::audio::HrtfAttenuationCouplingRejectReason::BypassPath;
    expectTrue(!fuse::audio::preflight_hrtf_attenuation_coupling_ready(
                   fuse::audio::HrtfPanPath::Convolution, 1.f, 1.f, &reason),
               "unity attenuation is not ready for narrowing");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::UnityAttenuation,
               "unity attenuation rejects with unity_attenuation reason");

    reason = fuse::audio::HrtfAttenuationCouplingRejectReason::None;
    expectTrue(fuse::audio::try_preflight_hrtf_attenuation_coupling(
                   fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f, reason),
               "reduced attenuation try_preflight is ready to narrow");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "narrowing reject reason is none when coupling applies");
    ...
    expectTrue(fuse::audio::preflight_hrtf_attenuation_coupling_ready(
                   fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f)
                   == narrow_preflight.can_narrow(),
               "preflight_hrtf_attenuation_coupling_ready mirrors can_narrow");
    expectTrue(fuse::audio::classify_hrtf_attenuation_coupling_reject(narrow_preflight)
                   == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "narrowing classify returns none on valid path");
}
...
void testHrtfBinauralRejectReasonGuards() {
    ...
    expectTrue(fuse::audio::preflight_hrtf_binaural_pan_ready(true, empty, offset, 0.2f, 0.3f,
                                                              &reason),
               "stub path composite preflight is ready for spatial pan");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::None,
               "stub path composite pan reject reason is none");

    const fuse::audio::HrtfBinauralPreflight stub_preflight =
        fuse::audio::preflight_hrtf_binaural(true, empty, offset, 0.2f, 0.3f);
    expectTrue(fuse::audio::classify_hrtf_binaural_convolution_reject(stub_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::EmptyIr,
               "empty IR composite convolution rejects with empty_ir");
    expectTrue(fuse::audio::classify_hrtf_binaural_narrowing_reject(stub_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::None,
               "reduced attenuation composite narrowing is allowed");

    reason = fuse::audio::HrtfBinauralRejectReason::None;
    expectTrue(!fuse::audio::try_preflight_hrtf_binaural_pan(false, valid, offset, 0.1f, 0.1f,
                                                             reason),
               "disabled composite try_preflight rejects spatial pan");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "disabled composite rejects with hrtf_disabled reason");
    ...
    expectTrue(fuse::audio::classify_hrtf_binaural_narrowing_reject(conv_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::UnityAttenuation,
               "unity attenuation composite narrowing rejects with unity_attenuation");

    const fuse::audio::HrtfBinauralPreflight co_located_preflight =
        fuse::audio::preflight_hrtf_binaural(true, valid, co_located, 0.1f, 0.1f);
    expectTrue(fuse::audio::classify_hrtf_binaural_pan_reject(co_located_preflight)
                   == fuse::audio::HrtfBinauralRejectReason::CoLocated,
               "co-located composite pan rejects with co_located reason");

    expectTrue(fuse::audio::preflight_hrtf_binaural_pan_ready(true, offset, 0.25f, 0.35f)
                   == fuse::audio::preflight_hrtf_binaural(true, offset, 0.25f, 0.35f)
                          .can_spatial_pan(),
               "no-IR composite preflight_hrtf_binaural_pan_ready mirrors can_spatial_pan");
    expectTrue(std::strcmp(fuse::audio::hrtf_binaural_reject_reason_label(
                   fuse::audio::HrtfBinauralRejectReason::MalformedIr),
               "malformed_ir") == 0,
               "malformed_ir label is stable");
...
void testRejectReasonMirrorsExistingPreflights() {
    ...
    expectTrue(fuse::audio::preflight_hrtf_binaural_pan_ready(true, empty, offset, 0.2f, 0.3f)
                   == fuse::audio::can_apply_hrtf_binaural_pan(
                          fuse::audio::preflight_hrtf_binaural(true, empty, offset, 0.2f, 0.3f)),
               "preflight_hrtf_binaural_pan_ready mirrors can_apply_hrtf_binaural_pan");
    expectTrue(fuse::audio::preflight_hrtf_ir_ready(empty)
                   == !fuse::audio::should_skip_hrtf_convolution(empty),
...
void testHrtfIrRejectReasonGuards() {
    ...
    expectTrue(!fuse::audio::preflightHrtfIrReady(empty, &reason),
               "empty IR fails IR preflight ready");
    expectTrue(reason == fuse::audio::HrtfIrRejectReason::NullSamples,
               "empty IR reject reason is NullSamples");
    ...
    const fuse::audio::HrtfIrStub valid{samples, 1};
    expectTrue(fuse::audio::preflightHrtfIrReady(valid, &reason),
               "valid IR passes IR preflight ready");
    expectTrue(reason == fuse::audio::HrtfIrRejectReason::None,
               "valid IR reject reason is None");
...
void testHrtfPanPathRejectReasonGuards() {
    ...
    expectTrue(fuse::audio::preflightHrtfPanPathReady(true, empty, offset, &panReason),
               "enabled offset source passes pan-path preflight ready");
    ...
    expectTrue(!fuse::audio::preflightHrtfPanPathReady(false, valid, offset, &panReason),
               "disabled HRTF fails pan-path preflight ready");
    ...
    expectTrue(fuse::audio::preflightHrtfPanConvolutionReady(true, valid, offset,
                                                             &convolutionReason),
               "valid IR passes pan convolution preflight ready");
    ...
    expectTrue(!fuse::audio::preflightHrtfPanConvolutionReady(true, empty, offset,
                                                              &convolutionReason),
               "empty IR fails pan convolution preflight ready");
...
void testHrtfPanPathResolutionDiagnostic() {
    const fuse::audio::Vec3 offset{5.f, 0.f, 0.f};
    const fuse::audio::HrtfIrStub empty{};
    const float samples[] = {1.f};
    const fuse::audio::HrtfIrStub valid{samples, 1};

    const fuse::audio::HrtfPanPathResolution stub_resolution =
        fuse::audio::resolve_hrtf_pan_path_diagnostic(true, empty, offset);
    expectTrue(stub_resolution.path == fuse::audio::HrtfPanPath::IldItdStub,
               "diagnostic resolution selects ILD/ITD stub for empty IR");
    expectTrue(stub_resolution.path == stub_resolution.preflight.path,
               "diagnostic resolution path matches embedded preflight");
    expectTrue(stub_resolution.can_spatial_pan(),
               "diagnostic stub resolution can spatial-pan");

    const fuse::audio::HrtfPanPathResolution conv_resolution =
        fuse::audio::resolve_hrtf_pan_path_diagnostic(true, valid, offset);
    expectTrue(conv_resolution.can_convolve(),
               "diagnostic convolution resolution can convolve");
    expectTrue(conv_resolution.path
                   == fuse::audio::resolve_hrtf_pan_path(true, valid, offset),
               "diagnostic resolution path matches resolve_hrtf_pan_path");

    const fuse::audio::HrtfPanPathResolution no_ir_resolution =
        fuse::audio::resolve_hrtf_pan_path_diagnostic(true, offset);
    expectTrue(no_ir_resolution.path == fuse::audio::HrtfPanPath::IldItdStub,
               "no-IR diagnostic resolution selects ILD/ITD stub");
}
...
void testHrtfAttenuationCouplingRejectReasonGuards() {
    ...
    expectTrue(fuse::audio::preflightHrtfAttenuationCouplingReady(
                   fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f, {}, {}, &reason),
               "reduced attenuation passes coupling preflight ready");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::None,
               "reduced attenuation coupling reject reason is None");

    expectTrue(!fuse::audio::preflightHrtfAttenuationCouplingReady(
                   fuse::audio::HrtfPanPath::Bypass, 0.1f, 0.1f, {}, {}, &reason),
               "bypass path fails coupling preflight ready");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::BypassPath,
               "bypass path coupling reject reason is BypassPath");
...
void testHrtfBinauralRejectReasonGuards() {
    ...
    expectTrue(fuse::audio::preflightHrtfBinauralPanReady(true, empty, offset, 0.2f, 0.3f, {}, {},
                                                          &reason),
               "enabled stub path passes composite pan preflight ready");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::None,
               "enabled stub path composite pan reject reason is None");

    expectTrue(!fuse::audio::preflightHrtfBinauralPanReady(false, valid, offset, 0.1f, 0.1f, {}, {},
                                                           &reason),
               "disabled HRTF fails composite pan preflight ready");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
               "disabled HRTF composite pan reject reason is HrtfDisabled");

    expectTrue(!fuse::audio::preflightHrtfBinauralConvolutionReady(true, empty, offset, 1.f, 1.f, {},
                                                                   {}, &reason),
               "empty IR fails composite convolution preflight ready");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::EmptyIr,
               "empty IR composite convolution reject reason is EmptyIr");

    expectTrue(!fuse::audio::tryPreflightHrtfBinauralNarrowing(true, valid, co_located, 0.1f, 0.1f,
                                                               {}, {}, reason),
               "co-located source fails composite narrowing preflight");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::BypassPath,
               "co-located composite narrowing reject reason is BypassPath");

    expectTrue(!fuse::audio::preflightHrtfBinauralNarrowingReady(true, valid, offset, 1.f, 1.f, {},
                                                                 {}, &reason),
               "unity attenuation fails composite narrowing preflight ready");
    expectTrue(reason == fuse::audio::HrtfBinauralRejectReason::UnityAttenuation,
               "unity attenuation composite narrowing reject reason is UnityAttenuation");
    ...
    expectTrue(std::strcmp(fuse::audio::hrtfBinauralRejectReasonLabel(
                   fuse::audio::HrtfBinauralRejectReason::EmptyIr),
               "empty_ir") == 0,
               "composite binaural reject reason label for EmptyIr");
}
...
int main() {
    ...
    testHrtfPanPathResolutionDiagnostic();
    fuse::core::shutdown();
