...
void testHrtfIrRejectReasonGuards() {
    ...
    expectTrue(!fuse::audio::preflight_hrtf_ir_ready(empty, &reason),
               "empty IR preflight is not ready");
    ...
    expectTrue(!fuse::audio::should_skip_hrtf_ir_ready(valid),
               "valid IR should not skip ready preflight");
...
void testHrtfPanPathRejectReasonGuards() {
    ...
    expectTrue(fuse::audio::preflight_hrtf_pan_path_ready(true, empty, offset, &reason),
               "enabled offset source passes pan-path ready");
    ...
    expectTrue(!fuse::audio::preflight_hrtf_pan_path_ready(false, valid, offset, &reason),
               "disabled HRTF fails pan-path ready");
    ...
    expectTrue(!fuse::audio::try_preflight_hrtf_pan_path(true, valid, co_located, reason),
               "co-located source fails pan-path try_preflight");
...
void testHrtfPanConvolveRejectReasonGuards() {
    ...
    expectTrue(fuse::audio::preflight_hrtf_pan_convolve_ready(true, valid, offset, &reason),
               "valid IR passes pan convolve ready");
    expectTrue(reason == fuse::audio::HrtfPanConvolveRejectReason::None,
               "convolve reject reason is none on valid IR");

    reason = fuse::audio::HrtfPanConvolveRejectReason::None;
    expectTrue(!fuse::audio::preflight_hrtf_pan_convolve_ready(true, empty, offset, &reason),
               "empty IR fails pan convolve ready");
    expectTrue(reason == fuse::audio::HrtfPanConvolveRejectReason::EmptyIr,
               "empty IR convolve reject reason is empty_ir");

    reason = fuse::audio::HrtfPanConvolveRejectReason::None;
    expectTrue(!fuse::audio::try_preflight_hrtf_pan_convolve(true, malformed, offset, reason),
               "malformed IR fails pan convolve try_preflight");
    expectTrue(reason == fuse::audio::HrtfPanConvolveRejectReason::MalformedIr,
               "malformed IR convolve reject reason is malformed_ir");
    expectTrue(fuse::audio::should_skip_hrtf_pan_convolve(true, empty, offset),
               "empty IR skips pan convolve");

    const fuse::audio::HrtfPanPathPreflight malformed_preflight =
        fuse::audio::preflight_hrtf_pan_path(true, malformed, offset);
    expectTrue(malformed_preflight.malformedIr, "pan-path preflight marks malformed IR");

    expectTrue(std::strcmp(fuse::audio::hrtf_pan_convolve_reject_reason_label(
                   fuse::audio::HrtfPanConvolveRejectReason::EmptyIr),
               "empty_ir") == 0,
               "empty_ir convolve label is stable");
}
...
void testHrtfAttenuationCouplingRejectReasonGuards() {
    ...
    expectTrue(!fuse::audio::preflight_hrtf_attenuation_coupling_ready(
               ...
               "bypass path fails attenuation coupling ready");
    ...
    expectTrue(!fuse::audio::preflight_hrtf_attenuation_coupling_ready(
               ...
               "unity attenuation fails coupling ready");
    expectTrue(reason == fuse::audio::HrtfAttenuationCouplingRejectReason::UnityAttenuation,
               "unity attenuation coupling reject reason is unity_attenuation");
    ...
    expectTrue(!fuse::audio::should_skip_hrtf_attenuation_coupling_ready(
                   fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f),
               "reduced attenuation does not skip coupling ready");
    ...
    expectTrue(std::strcmp(fuse::audio::hrtf_attenuation_coupling_reject_reason_label(
               ...
               "bypass_path") == 0,
               "bypass_path coupling label is stable");
}
...
void testHrtfBinauralRejectReasonGuards() {
    ...
    expectTrue(fuse::audio::preflight_hrtf_binaural_ready(true, empty, offset, 0.2f, 0.3f, &reason),
               "enabled offset source passes composite ready");
...
void testRejectReasonMirrorsExistingPreflights() {
    ...
    expectTrue(fuse::audio::preflight_hrtf_pan_convolve_ready(true, valid, offset)
                   == fuse::audio::can_convolve_hrtf_pan_path(
                          fuse::audio::preflight_hrtf_pan_path(true, valid, offset)),
               "preflight_hrtf_pan_convolve_ready mirrors can_convolve_hrtf_pan_path");
    expectTrue(fuse::audio::preflight_hrtf_attenuation_coupling_ready(
                   fuse::audio::HrtfPanPath::IldItdStub, 0.2f, 0.3f)
    ...
    expectTrue(fuse::audio::classify_hrtf_ir_reject(fuse::audio::preflight_hrtf_ir(empty))
                   == fuse::audio::HrtfIrRejectReason::EmptyIr,
               "classify_hrtf_ir_reject mirrors empty IR preflight");
    expectTrue(fuse::audio::classify_hrtf_pan_convolve_reject(
                   fuse::audio::preflight_hrtf_pan_path(true, empty, offset))
                   == fuse::audio::HrtfPanConvolveRejectReason::EmptyIr,
               "classify_hrtf_pan_convolve_reject mirrors empty IR pan path");
}
