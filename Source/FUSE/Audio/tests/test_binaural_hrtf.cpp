...
// --- deepen additive from deepen-b72-binaural-reject-reasons-564d ---
    expectTrue(!fuse::audio::tryCanConvolveHrtfIr(empty, reason),
               "empty IR tryCanConvolve rejects convolution");
    expectTrue(fuse::audio::tryCanConvolveHrtfIr(valid, reason),
               "valid IR tryCanConvolve accepts convolution");
    expectTrue(!fuse::audio::tryCanConvolveHrtfIr(malformed, reason),
               "malformed IR tryCanConvolve rejects convolution");
    const fuse::audio::HrtfIrPreflight emptyPreflight = fuse::audio::preflight_hrtf_ir(empty);
    expectTrue(fuse::audio::classifyHrtfIrReject(emptyPreflight)
               "classifyHrtfIrReject maps nullSamples flag");
    fuse::audio::HrtfIrRejectReason optionalReason = fuse::audio::HrtfIrRejectReason::None;
    const fuse::audio::HrtfIrPreflight withReason =
    expectTrue(optionalReason == fuse::audio::HrtfIrRejectReason::NullSamples,
    expectTrue(fuse::audio::tryCanApplySpatialHrtfPan(true, valid, offset, reason),
               "valid spatial pan tryPreflight succeeds");
    expectTrue(!fuse::audio::tryCanApplySpatialHrtfPan(false, valid, offset, reason),
               "disabled HRTF tryPreflight rejects spatial pan");
    expectTrue(!fuse::audio::tryCanApplySpatialHrtfPan(true, valid, co_located, reason),
               "co-located source tryPreflight rejects spatial pan");
    expectTrue(fuse::audio::tryCanApplySpatialHrtfPan(true, empty, offset, reason),
    expectTrue(fuse::audio::tryCanApplySpatialHrtfPan(true, malformed, offset, reason),
    expectTrue(reason == fuse::audio::HrtfPanPathRejectReason::MalformedIr,
    const fuse::audio::HrtfPanPathPreflight stubPreflight =
    expectTrue(fuse::audio::classifyHrtfPanPathReject(stubPreflight)
               "classifyHrtfPanPathReject maps emptyIr flag");
    fuse::audio::HrtfPanPathRejectReason optionalReason =
    const fuse::audio::HrtfPanPathPreflight withReason =
    expectTrue(optionalReason == fuse::audio::HrtfPanPathRejectReason::HrtfDisabled,
    expectTrue(!fuse::audio::tryCanNarrowHrtfSpatialImage(fuse::audio::HrtfPanPath::Bypass, 0.1f,
               "bypass path tryCanNarrow rejects narrowing");
    expectTrue(!fuse::audio::tryCanNarrowHrtfSpatialImage(fuse::audio::HrtfPanPath::Convolution,
               "unity attenuation tryCanNarrow rejects narrowing");
    expectTrue(fuse::audio::tryCanNarrowHrtfSpatialImage(fuse::audio::HrtfPanPath::IldItdStub, 0.2f,
               "reduced attenuation tryCanNarrow accepts narrowing");
    const fuse::audio::HrtfAttenuationCouplingPreflight unityPreflight =
    expectTrue(fuse::audio::classifyHrtfAttenuationCouplingReject(unityPreflight)
               "classifyHrtfAttenuationCouplingReject maps unityAttenuation flag");
    fuse::audio::HrtfAttenuationCouplingRejectReason optionalReason =
    const fuse::audio::HrtfAttenuationCouplingPreflight withReason =
    expectTrue(optionalReason == fuse::audio::HrtfAttenuationCouplingRejectReason::BypassPath,
               "valid composite tryPreflight succeeds");
               "disabled HRTF composite tryPreflight rejects spatial pan");
               "co-located composite tryPreflight rejects spatial pan");
    const fuse::audio::HrtfBinauralPreflight stubPreflight =
    expectTrue(fuse::audio::classifyHrtfBinauralConvolutionReject(stubPreflight)
               "classifyHrtfBinauralConvolutionReject maps empty IR");
    expectTrue(fuse::audio::classifyHrtfBinauralReject(stubPreflight)
               "classifyHrtfBinauralReject none on spatial stub path");
    const fuse::audio::HrtfBinauralPreflight unityPreflight =
    expectTrue(fuse::audio::classifyHrtfBinauralNarrowingReject(unityPreflight)
               "classifyHrtfBinauralNarrowingReject maps unity attenuation");
    const fuse::audio::HrtfBinauralPreflight malformedPreflight =
    expectTrue(fuse::audio::classifyHrtfBinauralConvolutionReject(malformedPreflight)
    fuse::audio::HrtfBinauralRejectReason optionalReason =
    const fuse::audio::HrtfBinauralPreflight withReason =
    expectTrue(withReason.should_skip(),
    expectTrue(optionalReason == fuse::audio::HrtfBinauralRejectReason::HrtfDisabled,
    expectTrue(fuse::audio::tryCanConvolveHrtfIr(valid, irReason)
               "tryCanConvolveHrtfIr mirrors can_convolve_hrtf_ir on valid IR");
    expectTrue(fuse::audio::tryCanConvolveHrtfIr(empty, irReason)
               "tryCanConvolveHrtfIr mirrors can_convolve_hrtf_ir on empty IR");
    expectTrue(fuse::audio::tryCanApplySpatialHrtfPan(true, empty, offset, panReason)
               "tryCanApplySpatialHrtfPan mirrors can_apply_spatial_hrtf_pan on stub path");
    fuse::audio::HrtfAttenuationCouplingRejectReason couplingReason =
               "tryCanNarrowHrtfSpatialImage mirrors can_narrow_hrtf_spatial_image");
    fuse::audio::HrtfBinauralRejectReason binauralReason = fuse::audio::HrtfBinauralRejectReason::None;
    expectTrue(fuse::audio::tryPreflightHrtfBinaural(true, empty, offset, 0.2f, 0.3f, binauralReason)
               "tryPreflightHrtfBinaural mirrors can_apply_hrtf_binaural_pan on stub path");

// --- deepen additive from b7-2-hrtf-reject-reason-wrappers-3c1c ---
    expectTrue(fuse::audio::classifyHrtfIrReject(empty_preflight)
               "classifyHrtfIrReject maps null samples");
    expectTrue(std::strcmp(fuse::audio::hrtfIrRejectReasonName(empty_preflight.reason),
               "hrtfIrRejectReasonName labels NullSamples");
    expectTrue(fuse::audio::hrtfIrRejectsForReason(valid, fuse::audio::HrtfIrRejectReason::None),
    expectTrue(fuse::audio::hrtfIrRejectsForReason(malformed,
               "tryPreflightHrtfIr rejects empty IR");
               "tryPreflightHrtfIr returns NullSamples on empty IR");
    expectTrue(fuse::audio::preflightHrtfIrReady(valid),
               "preflightHrtfIrReady accepts valid IR");
               "preflightHrtfIrReady rejects empty IR with reason output");
               "classifyHrtfPanPathReject is None on stub path");
                   fuse::audio::hrtfPanPathRejectReasonName(disabled_preflight.reason),
               "hrtfPanPathRejectReasonName labels HrtfDisabled");
    expectTrue(fuse::audio::hrtfPanPathRejectsForReason(true, valid, co_located,
               "tryPreflightHrtfPanPath accepts spatial stub path");
               "tryPreflightHrtfPanPath returns None on stub path");
    expectTrue(!fuse::audio::tryPreflightHrtfPanPath(false, valid, offset, reason),
               "tryPreflightHrtfPanPath rejects disabled HRTF");
               "tryPreflightHrtfPanPath returns HrtfDisabled when disabled");
    expectTrue(fuse::audio::classifyHrtfAttenuationCouplingReject(unity_preflight)
               "classifyHrtfAttenuationCouplingReject maps unity attenuation");
                   fuse::audio::hrtfAttenuationCouplingRejectReasonName(unity_preflight.reason),
               "hrtfAttenuationCouplingRejectReasonName labels UnityAttenuation");
               "preflightHrtfAttenuationCouplingReady accepts narrowing path");
               "tryPreflightHrtfAttenuationCoupling rejects bypass path");
               "tryPreflightHrtfAttenuationCoupling returns BypassPath");
    expectTrue(fuse::audio::classifyHrtfBinauralCouplingReject(stub_preflight)
    expectTrue(fuse::audio::classifyHrtfBinauralCouplingReject(bypass_preflight)
    expectTrue(std::strcmp(fuse::audio::hrtfBinauralRejectReasonName(bypass_preflight.reason),
               "hrtfBinauralRejectReasonName labels HrtfDisabled");
    expectTrue(fuse::audio::hrtfBinauralRejectsForReason(true, valid, co_located, 0.1f, 0.1f,
    expectTrue(fuse::audio::classifyHrtfBinauralCouplingReject(unity_preflight)
               "tryPreflightHrtfBinaural accepts spatial stub path");
               "tryPreflightHrtfBinaural returns None on stub path");
...
// --- deepen additive from b7-2-hrtf-reject-reasons-410f ---
    expectTrue(fuse::audio::classifyHrtfIrReject(empty_preflight) == empty_preflight.reason,
               "classifyHrtfIrReject matches preflight reason field");
    expectTrue(std::strcmp(fuse::audio::hrtfIrRejectReasonLabel(empty_preflight.reason),
               "hrtfIrRejectReasonLabel for NullSamples");
    expectTrue(std::strcmp(fuse::audio::hrtfIrRejectReasonLabel(valid_preflight.reason), "None")
               "hrtfIrRejectReasonLabel for None");
               "tryPreflightHrtfIr returns NullSamples for empty IR");
               "tryPreflightHrtfIr accepts valid IR");
               "tryPreflightHrtfIr returns None for valid IR");
    fuse::audio::HrtfIrRejectReason optional_reason = fuse::audio::HrtfIrRejectReason::MalformedIr;
    expectTrue(!fuse::audio::preflightHrtfIrReady(empty, &optional_reason),
               "preflightHrtfIrReady rejects empty IR");
               "preflightHrtfIrReady writes reject reason");
    expectTrue(fuse::audio::classifyHrtfPanPathReject(stub_preflight) == stub_preflight.reason,
               "classifyHrtfPanPathReject matches preflight reason field");
                   fuse::audio::hrtfPanPathRejectReasonLabel(disabled_preflight.reason),
               "hrtfPanPathRejectReasonLabel for HrtfDisabled");
               "tryPreflightHrtfPanPath returns HrtfDisabled");
    fuse::audio::HrtfPanPathRejectReason no_ir_reason = fuse::audio::HrtfPanPathRejectReason::None;
    expectTrue(fuse::audio::tryPreflightHrtfPanPath(true, offset, no_ir_reason),
               "no-IR tryPreflightHrtfPanPath accepts spatial stub path");
    expectTrue(no_ir_reason == fuse::audio::HrtfPanPathRejectReason::None,
               "no-IR tryPreflightHrtfPanPath returns None on stub path");
               "classifyHrtfAttenuationCouplingReject matches preflight reason field");
               "hrtfAttenuationCouplingRejectReasonLabel for UnityAttenuation");
               "tryPreflightHrtfAttenuationCoupling accepts narrowable path");
               "tryPreflightHrtfAttenuationCoupling returns None when narrowing applies");
    expectTrue(fuse::audio::classifyHrtfBinauralReject(stub_preflight) == stub_preflight.reason,
               "classifyHrtfBinauralReject matches preflight reason field");
    expectTrue(std::strcmp(fuse::audio::hrtfBinauralRejectReasonLabel(disabled_preflight.reason),
               "hrtfBinauralRejectReasonLabel for HrtfDisabled");
               "tryPreflightHrtfBinaural rejects disabled HRTF");
               "tryPreflightHrtfBinaural returns HrtfDisabled");
    fuse::audio::HrtfBinauralRejectReason vec3_reason = fuse::audio::HrtfBinauralRejectReason::None;
    expectTrue(fuse::audio::tryPreflightHrtfBinaural(true, offset, 0.3f, 0.4f, vec3_reason),
               "no-IR tryPreflightHrtfBinaural accepts spatial stub path");
    expectTrue(vec3_reason == fuse::audio::HrtfBinauralRejectReason::None,
               "no-IR tryPreflightHrtfBinaural returns None on stub path");
    fuse::audio::HrtfBinauralRejectReason optional_reason =
    ...
    expectTrue(fuse::audio::preflightHrtfBinauralReady(true, empty, offset, 0.2f, 0.3f,
               "preflightHrtfBinauralReady accepts spatial stub path");
    expectTrue(optional_reason == fuse::audio::HrtfBinauralRejectReason::None,
               "preflightHrtfBinauralReady writes None on stub path");
