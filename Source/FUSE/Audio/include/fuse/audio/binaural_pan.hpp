...
/// Why HRTF IR convolution would be skipped (B7.2 deepen pass).
    MalformedIr,
};
...
/// Why IR preflight rejects convolution dispatch (B7.2 deepen).

/// Why HRTF IR convolution preflight rejects dispatch (B7.2 deepen).

/// Human-readable label for HRTF IR reject reasons (B7.2 deepen).

HrtfIrRejectReason classify_hrtf_ir_reject_reason(const HrtfIrStub& ir);

/// True when IR convolution can proceed; optional reject-reason output (B7.2 deepen).

bool try_preflight_hrtf_ir_convolution(const HrtfIrStub& ir, HrtfIrRejectReason& reason);

/// Returns true when \c hrtf_ir_reject_reason matches \p expected (B7.2 deepen pass).
/// Why HRTF IR convolution is rejected (B7.2 deepen pass).


/// Returns the first reject reason for an IR stub, or \c None when convolution may run.

/// Diagnostic reason HRTF IR convolution is rejected (B7.2 deepen).




/// Returns true when `hrtf_ir_reject_reason` matches `expected`.


/// Why HRTF IR preflight rejected convolution dispatch (B7.2 deepen pass).
/// Why IR convolution preflight rejected the request (B7.2 deepen).
...
/// Why HRTF IR preflight rejects convolution (B7.2 deepen pass).





/// Human-readable label for HRTF IR reject reasons (B7.2 deepen pass).
...
/// Why HRTF IR preflight rejects convolution dispatch (B7.2 deepen pass).


/// Diagnose why an HRTF IR stub cannot convolve; vacuously succeeds on valid IR.

/// Returns true when `hrtfIrRejectReason` matches `expected` (B7.2 deepen pass).

/// Why IR convolution preflight rejected the stub (B7.2 deepen pass).
...
/// Why HRTF IR convolution preflight would reject (B7.2 deepen pass).
...
const char* hrtfIrRejectReasonLabel(HrtfIrRejectReason reason);
    ...
    [[nodiscard]] bool ok() const { return can_convolve(); }

const char* hrtf_ir_reject_reason_label(HrtfIrRejectReason reason);
HrtfIrRejectReason classify_hrtf_ir_reject(const HrtfIrPreflight& preflight);
HrtfIrRejectReason classifyHrtfIrReject(const HrtfIrPreflight& preflight);

/// IR convolution preflight with optional reject-reason output (B7.2 deepen pass).
...
/// Classify an HRTF IR preflight bundle into a reject reason.
...
/// IR preflight with mandatory reject-reason output (B7.2 deepen pass).

/// Map an IR preflight bundle to its primary reject reason.
HrtfIrRejectReason classify_hrtf_ir_reject(const HrtfIrPreflight& preflight);
...
/// IR convolution preflight with optional reject-reason output (B7.2 deepen pass).
/// Classify reject reason from an IR preflight bundle.
HrtfIrRejectReason classifyHrtfIrReject(const HrtfIrPreflight& preflight);

bool preflightHrtfIrReady(const HrtfIrStub& ir, HrtfIrRejectReason* reason = nullptr);
bool tryPreflightHrtfIr(const HrtfIrStub& ir, HrtfIrRejectReason& reason);
/// Map IR preflight diagnostics to a reject reason (B7.2 deepen pass).

/// Why IR preflight rejected convolution (B7.2 deepen pass).
enum class HrtfIrRejectReason : u8 {
    None = 0,
    NullSamples,
    ZeroLength,
...
/// Classify why HRTF IR preflight would reject convolution (B7.2 deepen pass).
...
/// Why empty-IR preflight rejected convolution (B7.2 deepen pass).
enum class HrtfIrRejectReason : u8 {
    None = 0,
    NullSamples,
    ZeroLength,
...
/// Classify empty-IR preflight into a reject reason (priority: malformed → null → zero-length).
HrtfIrRejectReason classify_hrtf_ir_reject(const HrtfIrPreflight& preflight);
...
/// IR convolution preflight with optional reject-reason output (B7.2 deepen).
bool preflight_hrtf_ir_convolution_ready(const HrtfIrStub& ir,
                                         HrtfIrRejectReason* reason = nullptr);
bool try_preflight_hrtf_ir_convolution(const HrtfIrStub& ir, HrtfIrRejectReason& reason);
...
/// Map empty-IR preflight flags to a single reject reason.
HrtfIrRejectReason classify_hrtf_ir_reject(const HrtfIrPreflight& preflight);
...
/// Human-readable label for empty-IR diagnostics and test assertions (B7.2 deepen pass).
const char* hrtf_ir_reject_reason_name(HrtfIrRejectReason reason);

/// Map empty-IR preflight flags to the first reject reason (B7.2 deepen pass).
HrtfIrRejectReason classify_hrtf_ir_reject(const HrtfIrPreflight& preflight);
...
/// Map empty-IR diagnostics to a single reject reason (first matching flag wins).
HrtfIrRejectReason classify_hrtf_ir_reject(const HrtfIrPreflight& preflight);

/// IR convolution preflight with optional reject-reason output (B7.2 deepen).
bool preflight_hrtf_ir_ready(const HrtfIrStub& ir, HrtfIrRejectReason* reason = nullptr);

/// IR convolution preflight with required reject-reason output (B7.2 deepen).
bool try_preflight_hrtf_ir(const HrtfIrStub& ir, HrtfIrRejectReason& reason);

/// Early-out inverse of \c preflight_hrtf_ir_ready.
bool should_skip_hrtf_ir_ready(const HrtfIrStub& ir);

/// Why IR convolution preflight rejected the request (B7.2 deepen pass).
enum class HrtfIrRejectReason : u8 {
    None = 0,
    EmptyIr,
    NullSamples,
...
/// Why empty-IR / IR preflight rejected convolution (B7.2 deepen pass).
enum class HrtfIrRejectReason : u8 {
    None = 0,
    EmptyIr,
    NullSamples,
...
/// Why HRTF IR preflight rejects convolution dispatch (B7.2 deepen).
enum class HrtfIrRejectReason : u8 {
    None = 0,
    NullSamples,
    ZeroLength,
...
/// Human-readable label for empty-IR reject diagnostics.
const char* hrtf_ir_reject_reason_label(HrtfIrRejectReason reason);

/// Classify the first empty-IR reject reason from an IR preflight bundle.
HrtfIrRejectReason classify_hrtf_ir_reject(const HrtfIrPreflight& preflight);

/// Returns true when \c classify_hrtf_ir_reject matches \p expected.
bool hrtf_ir_rejects_for_reason(const HrtfIrPreflight& preflight, HrtfIrRejectReason expected);

/// IR preflight with optional reject-reason output — returns \c can_convolve.
bool preflight_hrtf_ir_ready(const HrtfIrStub& ir, HrtfIrRejectReason* reason = nullptr);

/// IR preflight with required reject-reason output — returns \c can_convolve.
bool try_preflight_hrtf_ir(const HrtfIrStub& ir, HrtfIrRejectReason& reason);
...
/// Map empty-IR preflight diagnostics to a reject reason (read-only guard).
HrtfIrRejectReason classifyHrtfIrReject(const HrtfIrPreflight& preflight);
...
/// Why IR convolution preflight rejected (B7.2 deepen).
enum class HrtfIrRejectReason : u8 {
    None = 0,
    NullSamples,
    ZeroLength,
...
/// Map empty-IR preflight diagnostics to a primary reject reason.
HrtfIrRejectReason classify_hrtf_ir_reject(const HrtfIrPreflight& preflight);

/// True when IR convolution preflight can proceed (optional reject-reason output).
bool preflight_hrtf_ir_ready(const HrtfIrStub& ir, HrtfIrRejectReason* reason = nullptr);

/// Out-param variant of \c preflight_hrtf_ir_ready.
bool try_preflight_hrtf_ir(const HrtfIrStub& ir, HrtfIrRejectReason& reason);
...
/// True when \c classify_hrtf_ir_reject matches \p expected (B7.2 deepen pass).
bool hrtf_ir_rejects_for_reason(const HrtfIrPreflight& preflight, HrtfIrRejectReason expected);
...
/// Diagnose empty-IR reject reason from a preflight bundle.
HrtfIrRejectReason hrtf_ir_reject_reason(const HrtfIrPreflight& preflight);

/// Returns true when IR preflight reject reason matches \p expected (B7.2 deepen pass).
bool hrtf_ir_rejects_for_reason(const HrtfIrPreflight& preflight, HrtfIrRejectReason expected);
...
/// Empty-IR preflight with optional reject-reason output (B7.2 deepen).
bool preflight_hrtf_ir_ready(const HrtfIrStub& ir, HrtfIrRejectReason* reason = nullptr);
bool try_preflight_hrtf_ir(const HrtfIrStub& ir, HrtfIrRejectReason& reason);
...
/// Map empty-IR preflight diagnostics to a reject reason.
HrtfIrRejectReason classify_hrtf_ir_reject(const HrtfIrPreflight& preflight);
...
/// Why empty-IR preflight rejected convolution dispatch (B7.2 deepen pass).
enum class HrtfIrRejectReason : u8 {
    None = 0,
    MalformedIr,
    NullSamples,
...
/// Empty-IR preflight with optional reject-reason output (B7.2 deepen pass).
bool preflight_hrtf_ir_ready(const HrtfIrStub& ir, HrtfIrRejectReason* reason = nullptr);
...
/// Classify why IR preflight would reject convolution.
HrtfIrRejectReason classify_hrtf_ir_reject(const HrtfIrPreflight& preflight);
...
/// Classify the primary empty-IR reject reason from preflight diagnostics.
HrtfIrRejectReason classify_hrtf_ir_reject(const HrtfIrPreflight& preflight);
...
/// Returns the first reject reason for an IR preflight bundle, or \c None when convolution is allowed.
HrtfIrRejectReason classify_hrtf_ir_reject(const HrtfIrPreflight& preflight);

/// Returns true when \c classify_hrtf_ir_reject matches \p expected (B7.2 deepen).
bool hrtf_ir_rejects_for_reason(const HrtfIrStub& ir, HrtfIrRejectReason expected);
...
/// Map empty-IR preflight flags to a reject reason (None when convolution is allowed).
HrtfIrRejectReason classify_hrtf_ir_reject(const HrtfIrPreflight& preflight);
...
/// Map empty-IR preflight flags to a primary reject reason.
HrtfIrRejectReason classifyHrtfIrReject(const HrtfIrPreflight& preflight);

/// Diagnose why IR convolution preflight would reject; succeeds when \c can_convolve().
...
/// Classify why HRTF IR preflight rejected convolution.
HrtfIrRejectReason classify_hrtf_ir_reject(const HrtfIrPreflight& preflight);
...
/// Classify an HRTF IR preflight into a reject reason (B7.2 deepen pass).
HrtfIrRejectReason classify_hrtf_ir_reject(const HrtfIrPreflight& preflight);
...
/// Why HRTF pan-path preflight bypasses spatial pan (B7.2 deepen pass).
/// Why pan-path preflight selects bypass (B7.2 deepen pass).
/// Why HRTF spatial pan preflight rejects centre bypass (B7.2 deepen).
/// Why HRTF pan routing is rejected (B7.2 deepen pass).
/// Diagnostic reason HRTF pan routing is rejected (B7.2 deepen).
/// Why pan-path preflight rejected spatial pan (B7.2 deepen pass).
/// Why HRTF pan-path preflight would reject spatial pan (B7.2 deepen pass).
/// Why pan-path preflight rejects spatial pan (B7.2 deepen pass).
    HrtfDisabled,
    CoLocated,
};
...
/// Why pan-path preflight skips spatial pan (B7.2 deepen).
/// Why pan-path preflight rejected spatial routing (B7.2 deepen pass).
...
/// Diagnose why HRTF pan routing would bypass; vacuously succeeds on spatial paths.

/// Returns true when `hrtfPanPathRejectReason` matches `expected` (B7.2 deepen pass).
/// Why pan-path preflight rejected IR convolution (B7.2 deepen pass).
enum class HrtfPanConvolutionRejectReason : u8 {
/// Human-readable label for HRTF pan-path reject reasons (B7.2 deepen).

/// Classify why spatial pan would be bypassed (B7.2 deepen).
HrtfPanPathRejectReason classify_hrtf_pan_path_reject_reason(bool hrtf_enabled,

/// Classify pan-path bypass when no IR is wired (B7.2 deepen).

/// True when spatial pan can proceed; optional reject-reason output (B7.2 deepen).
bool preflight_hrtf_spatial_pan(bool hrtf_enabled, const HrtfIrStub& ir, const Vec3& rel_listener,

/// Spatial pan preflight with mandatory reject-reason output (B7.2 deepen).
bool try_preflight_hrtf_spatial_pan(bool hrtf_enabled, const HrtfIrStub& ir, const Vec3& rel_listener,

/// True when spatial pan can proceed without IR wired (B7.2 deepen).
bool preflight_hrtf_spatial_pan(bool hrtf_enabled, const Vec3& rel_listener,

/// No-IR spatial pan preflight with mandatory reject-reason output (B7.2 deepen).
bool try_preflight_hrtf_spatial_pan(bool hrtf_enabled, const Vec3& rel_listener,



/// Diagnose why pan routing would bypass; vacuously succeeds on enabled, separated sources.

/// Returns true when \c hrtf_pan_path_reject_reason matches \p expected (B7.2 deepen pass).
/// Why HRTF convolution pan-path dispatch is rejected (B7.2 deepen pass).
enum class HrtfPanPathConvolveRejectReason : u8 {


/// Human-readable label for convolution pan-path reject reasons (logging / tests).
const char* hrtf_pan_path_convolve_reject_reason_label(HrtfPanPathConvolveRejectReason reason);

/// Returns the first pan-path bypass reject reason, or \c None when spatial pan may run.

/// Returns the first convolution pan-path reject reason, or \c None when convolution may run.
HrtfPanPathConvolveRejectReason hrtf_pan_path_convolve_reject_reason(bool hrtf_enabled,


/// Returns true when \c hrtf_pan_path_convolve_reject_reason matches \p expected.
...
/// Returns true when `hrtf_pan_path_reject_reason` matches `expected`.

/// Why HRTF pan-path preflight rejected spatial pan or convolution (B7.2 deepen pass).

struct HrtfPanPathPreflight;

/// Returns the first reject reason for a pan-path preflight bundle, or \c None when spatial pan may run.
...
/// Human-readable label for HRTF pan-path reject reasons (B7.2 deepen pass).
...
/// True when a pan-path reject reason blocks spatial pan (B7.2 deepen).
    ...
    HrtfPanConvolutionRejectReason convolutionRejectReason = HrtfPanConvolutionRejectReason::None;
...
bool hrtf_pan_path_convolution_rejects_for_reason(
    ...
    [[nodiscard]] bool ok() const { return can_spatial_pan(); }


/// Resolved pan path plus read-only preflight diagnostics (B7.2 deepen pass).
struct HrtfPanPathResolution {
    HrtfPanPath path = HrtfPanPath::Bypass;
    HrtfPanPathPreflight preflight{};

    bool can_spatial_pan() const { return preflight.can_spatial_pan(); }
    bool can_convolve() const { return preflight.can_convolve(); }
};

/// Resolve pan path and attach pan-path preflight diagnostics.
HrtfPanPathResolution resolve_hrtf_pan_path_diagnostic(bool hrtf_enabled, const HrtfIrStub& ir,
                                                      const Vec3& rel_listener);

/// Resolve pan path when no IR is wired, with preflight diagnostics attached.
...
/// Spatial pan preflight with optional reject-reason output (B7.2 deepen pass).
...
/// Why pan-path preflight rejected IR convolution (B7.2 deepen).
...
/// Classify an HRTF pan-path preflight bundle into a reject reason.
...
/// Pan-path preflight with optional bypass reject-reason output (B7.2 deepen pass).
bool preflight_hrtf_pan_path_ready(bool hrtf_enabled, const HrtfIrStub& ir,
                                   const Vec3& rel_listener,
                                   HrtfPanPathRejectReason* reason = nullptr);

/// Pan-path preflight with mandatory bypass reject-reason output (B7.2 deepen pass).
bool try_preflight_hrtf_pan_path(bool hrtf_enabled, const HrtfIrStub& ir,
                                 const Vec3& rel_listener, HrtfPanPathRejectReason& reason);

/// Convolution pan-path preflight with optional reject-reason output (B7.2 deepen pass).
...
/// Convolution pan-path preflight with mandatory reject-reason output (B7.2 deepen pass).
bool try_preflight_hrtf_pan_path_convolve(bool hrtf_enabled, const HrtfIrStub& ir,
                                          const Vec3& rel_listener,
                                          HrtfPanPathConvolveRejectReason& reason);

/// Map a pan-path preflight bundle to its primary bypass reject reason.
HrtfPanPathRejectReason classify_hrtf_pan_path_reject(const HrtfPanPathPreflight& preflight);

/// Map a pan-path preflight bundle to its convolution reject reason.
...
/// Classify reject reason from a pan-path preflight bundle.
HrtfPanPathRejectReason classifyHrtfPanPathReject(const HrtfPanPathPreflight& preflight);
...
bool tryPreflightHrtfPanPath(bool hrtf_enabled, const HrtfIrStub& ir, const Vec3& rel_listener,
bool tryPreflightHrtfPanPath(bool hrtf_enabled, const Vec3& rel_listener,
/// Map pan-path preflight diagnostics to a reject reason (B7.2 deepen pass).
/// Why pan-path preflight rejected spatial pan (B7.2 deepen pass).
enum class HrtfPanPathRejectReason : u8 {
    None = 0,
    HrtfDisabled,
...
/// Why convolution preflight rejected IR dispatch (B7.2 deepen pass).
enum class HrtfConvolutionRejectReason : u8 {
    NullSamples,
    ZeroLength,
    MalformedIr,
...
/// Convolution preflight with optional reject-reason output (B7.2 deepen pass).
...
/// Classify why HRTF pan-path preflight would reject spatial pan or convolution (B7.2 deepen pass).


/// Pan-path preflight when no IR is wired, with optional reject-reason output (B7.2 deepen pass).

/// Pan-path preflight with mandatory reject-reason output (B7.2 deepen pass).

/// Pan-path preflight when no IR is wired, with mandatory reject-reason output (B7.2 deepen pass).
...
/// Why HRTF pan-path spatial preflight rejected the request (B7.2 deepen).
/// Why HRTF pan-path preflight bypasses spatial pan (B7.2 deepen pass).
/// Why spatial-pan preflight rejected the request (B7.2 deepen).
/// Why pan-path preflight rejected spatial pan or convolution (B7.2 deepen pass).
/// Why pan-path spatial preflight rejected the request (B7.2 deepen).
/// Why pan-path preflight rejects spatial pan (B7.2 deepen).
/// Why pan-path preflight bypassed spatial pan (B7.2 deepen pass).
/// Why pan-path preflight bypasses or falls back from convolution (B7.2 deepen).
/// Why HRTF pan-path preflight rejected spatial pan (B7.2 deepen pass).
/// Why HRTF pan-path spatial routing would be skipped (B7.2 deepen pass).
/// Why pan-path preflight rejects spatial pan or convolution (B7.2 deepen).
/// Why pan-path preflight bypassed or deferred convolution (B7.2 deepen).
enum class HrtfPanPathRejectReason : u8 {
    None = 0,
    HrtfDisabled,
    CoLocated,
...
/// Why convolution dispatch was rejected on a resolved pan path (B7.2 deepen pass).
enum class HrtfConvolutionRejectReason : u8 {
    MalformedIr,

const char* hrtf_convolution_reject_reason_label(HrtfConvolutionRejectReason reason);

/// Classify pan-path preflight into a spatial-pan reject reason.

/// Classify IR + pan-path preflight into a convolution reject reason.
HrtfConvolutionRejectReason classify_hrtf_convolution_reject(const HrtfIrPreflight& ir,
                                                             const HrtfPanPathPreflight& panPath);

/// Pan-path spatial preflight with optional reject-reason output (B7.2 deepen pass).

/// Convolution dispatch preflight with optional reject-reason output (B7.2 deepen pass).
bool preflight_hrtf_convolution_ready(bool hrtf_enabled, const HrtfIrStub& ir,
                                      HrtfConvolutionRejectReason* reason = nullptr);
bool try_preflight_hrtf_convolution(bool hrtf_enabled, const HrtfIrStub& ir,
                                    const Vec3& rel_listener, HrtfConvolutionRejectReason& reason);
...
/// Why HRTF pan-path convolution preflight rejected the request (B7.2 deepen).
enum class HrtfPanConvolutionRejectReason : u8 {
...
/// Map pan-path preflight flags to a single spatial-pan reject reason.
...
/// Pan-path convolution preflight with optional IR reject-reason output (B7.2 deepen pass).
...
/// Map pan-path preflight flags to the first reject reason (B7.2 deepen pass).

bool should_skip_hrtf_pan_path_ready(bool hrtf_enabled, const HrtfIrStub& ir,
bool should_skip_hrtf_pan_path_ready(bool hrtf_enabled, const Vec3& rel_listener);

/// Why convolution-path preflight rejected the request (B7.2 deepen).
...
/// Convolution-path preflight with optional reject-reason output (B7.2 deepen).
...
/// Pan-path convolution preflight with optional reject-reason output (B7.2 deepen pass).




/// Map pan-path preflight flags to a single spatial reject reason.
...
/// Map pan-path diagnostics to a single reject reason (first matching flag wins).


/// Pan-path spatial preflight when no IR is wired (B7.2 deepen).

/// Pan-path spatial preflight with required reject-reason output (B7.2 deepen).


/// Early-out inverse of \c preflight_hrtf_pan_path_ready.

/// Early-out inverse of no-IR \c preflight_hrtf_pan_path_ready.
...
/// Human-readable label for pan-path reject diagnostics.

/// Classify the first pan-path reject reason from a pan-path preflight bundle.

/// Returns true when \c classify_hrtf_pan_path_reject matches \p expected.

/// Pan-path preflight with optional reject-reason output — returns \c can_spatial_pan.

/// Pan-path preflight (no IR wired) with optional reject-reason output.

/// Pan-path preflight with required reject-reason output — returns \c can_spatial_pan.

/// Pan-path preflight (no IR wired) with required reject-reason output.

const char* hrtfPanPathRejectReasonLabel(HrtfPanPathRejectReason reason);

/// Map pan-path preflight diagnostics to a reject reason (read-only guard).
HrtfPanPathRejectReason classifyHrtfPanPathReject(const HrtfPanPathPreflight& preflight);



/// Map pan-path preflight flags to a single reject reason.
...
/// Map pan-path preflight diagnostics to a primary reject reason.

/// True when spatial pan preflight can proceed (optional reject-reason output).

/// True when spatial pan preflight can proceed without IR wired (optional reject-reason output).

/// Out-param variant of \c preflight_hrtf_pan_path_ready (IR-aware).

/// Out-param variant of \c preflight_hrtf_pan_path_ready (no IR wired).

/// Resolve pan path with optional reject-reason output (B7.2 deepen).
HrtfPanPath resolve_hrtf_pan_path(bool hrtf_enabled, const HrtfIrStub& ir, const Vec3& rel_listener,
                                  HrtfPanPathRejectReason* reason);

/// Resolve pan path when no IR is wired, with optional reject-reason output.
HrtfPanPath resolve_hrtf_pan_path(bool hrtf_enabled, const Vec3& rel_listener,

/// True when pan-path resolution yields a spatial path (optional path + reject-reason output).
...
/// True when no-IR pan-path resolution yields a spatial path (optional path + reject-reason output).
bool resolve_hrtf_pan_path_ready(bool hrtf_enabled, const Vec3& rel_listener,



/// True when \c classify_hrtf_pan_path_reject matches \p expected (B7.2 deepen pass).


/// Diagnose pan-path reject reason from a preflight bundle.
HrtfPanPathRejectReason hrtf_pan_path_reject_reason(const HrtfPanPathPreflight& preflight);

/// Returns true when pan-path preflight reject reason matches \p expected (B7.2 deepen pass).





/// Map pan-path preflight flags to a spatial-pan reject reason.

/// Map pan-path preflight flags to a convolution reject reason.
...
/// Map pan-path preflight diagnostics to a reject reason.
...
/// Classify why pan-path preflight would reject spatial pan.


/// Pan-path preflight when no IR is wired, with optional reject-reason output (B7.2 deepen).

/// Pan-path preflight with mandatory reject-reason output (B7.2 deepen).

/// Pan-path preflight when no IR is wired, with mandatory reject-reason output (B7.2 deepen).


/// Classify why spatial pan preflight was skipped (disabled or co-located).

/// Classify why convolution pan-path preflight was rejected.
...
/// Pan-path spatial preflight with optional reject-reason output (IR-aware, B7.2 deepen).

/// Pan-path spatial preflight with mandatory reject-reason output (B7.2 deepen).
...
/// Map pan-path preflight flags to a primary reject reason.

/// Diagnose why spatial pan preflight would reject; succeeds when \c can_spatial_pan().
bool tryCanApplySpatialHrtfPan(bool hrtf_enabled, const HrtfIrStub& ir, const Vec3& rel_listener,

/// Diagnose why spatial pan preflight would reject when no IR is wired.
...
/// Classify why HRTF pan-path preflight rejected spatial pan.

/// Pan-path preflight with optional reject-reason output (IR-aware).

/// Pan-path preflight with optional reject-reason output (no IR wired).

/// Pan-path preflight with mandatory reject-reason output (IR-aware).

/// Pan-path preflight with mandatory reject-reason output (no IR wired).

/// Early-out when pan-path preflight would reject spatial pan (IR-aware).

/// Early-out when pan-path preflight would reject spatial pan (no IR wired).
/// Human-readable label for HRTF pan-path reject reasons (B7.2 deepen pass).

/// Classify an HRTF pan-path preflight into a reject reason (B7.2 deepen pass).
...
/// Why attenuation-coupling preflight rejected spatial narrowing (B7.2 deepen pass).
/// Why attenuation-coupling preflight skips spatial image narrowing (B7.2 deepen pass).
...
/// Returns true when `hrtfAttenuationCouplingRejectReason` matches `expected` (B7.2 deepen pass).
bool hrtfAttenuationCouplingRejectsForReason(HrtfPanPath path, float distance_attenuation,

/// Why attenuation-coupling preflight skips spatial image narrowing (B7.2 deepen).
...
/// True when attenuation coupling can narrow the image; optional reject-reason output (B7.2 deepen).
bool preflight_hrtf_attenuation_narrowing(
    HrtfAttenuationCouplingRejectReason* reason = nullptr,

/// Attenuation narrowing preflight with mandatory reject-reason output (B7.2 deepen).
bool try_preflight_hrtf_attenuation_narrowing(




/// Why distance/occlusion attenuation coupling would skip (B7.2 deepen reject-reason pass).


/// Diagnose why attenuation coupling would skip; vacuously succeeds on spatial paths with reduced gain.

/// Returns true when `hrtf_attenuation_coupling_reject_reason` matches `expected` (B7.2 deepen reject-reason pass).


/// Diagnose why attenuation coupling would skip; vacuously succeeds when narrowing applies.

/// Returns true when \c hrtf_attenuation_coupling_reject_reason matches \p expected (B7.2 deepen pass).
/// Why attenuation coupling narrows the spatial image (B7.2 deepen pass).


/// Returns the first attenuation-coupling reject reason, or \c None when narrowing may run.





/// Diagnostic reason attenuation coupling is rejected (B7.2 deepen).
...
/// Why distance/occlusion attenuation coupling would skip spatial narrowing (B7.2 deepen pass).
...
/// Returns the first reject reason for an attenuation-coupling preflight bundle, or \c None when
/// narrowing may run.
...
/// Why attenuation-coupling preflight rejects spatial narrowing (B7.2 deepen pass).
...
/// Why attenuation-coupling preflight skips narrowing (B7.2 deepen pass).
...
/// Why attenuation-coupling preflight would reject spatial narrowing (B7.2 deepen pass).
...
const char* hrtfAttenuationCouplingRejectReasonLabel(HrtfAttenuationCouplingRejectReason reason);
    ...
    [[nodiscard]] bool ok() const { return can_narrow(); }
...
/// Classify an attenuation-coupling preflight bundle into a reject reason.
...
/// Attenuation-coupling preflight with optional reject-reason output (B7.2 deepen pass).
/// Classify reject reason from an attenuation-coupling preflight bundle.
HrtfAttenuationCouplingRejectReason classifyHrtfAttenuationCouplingReject(

bool preflightHrtfAttenuationCouplingReady(HrtfPanPath path, float distance_attenuation,
bool tryPreflightHrtfAttenuationCoupling(HrtfPanPath path, float distance_attenuation,
/// Map attenuation-coupling preflight diagnostics to a reject reason (B7.2 deepen pass).
...
/// Map an attenuation-coupling preflight bundle to its primary reject reason.
...
/// Classify why attenuation-coupling preflight would reject narrowing (B7.2 deepen pass).
...
/// Why attenuation-coupling preflight rejected spatial narrowing (B7.2 deepen pass).
/// Why HRTF attenuation-coupling preflight rejected narrowing (B7.2 deepen).
/// Why attenuation-coupling preflight skips spatial image narrowing (B7.2 deepen pass).
/// Why attenuation-coupling preflight rejected narrowing (B7.2 deepen).
/// Why attenuation-coupling preflight rejects spatial image narrowing (B7.2 deepen).
/// Why attenuation-coupling preflight skipped narrowing (B7.2 deepen pass).
/// Why attenuation-coupling preflight skips spatial image narrowing (B7.2 deepen).
/// Why attenuation-coupling preflight rejected spatial narrowing (B7.2 deepen).
/// Why distance/occlusion attenuation coupling would be skipped (B7.2 deepen pass).
...
/// Classify attenuation-coupling preflight into a reject reason.
...
/// Map attenuation-coupling preflight flags to a single reject reason.
/// Map attenuation-coupling diagnostics to a single reject reason (first matching flag wins).
...
/// Map attenuation-coupling preflight flags to the first reject reason (B7.2 deepen pass).
...
bool should_skip_hrtf_attenuation_coupling_narrowing_preflight(HrtfPanPath path,
                                                               float distance_attenuation,

/// Attenuation-coupling preflight with required reject-reason output (B7.2 deepen).

/// Early-out inverse of \c preflight_hrtf_attenuation_coupling_ready.
...
/// Human-readable label for attenuation-coupling reject diagnostics.


/// Returns true when \c classify_hrtf_attenuation_coupling_reject matches \p expected.
bool hrtf_attenuation_coupling_rejects_for_reason(const HrtfAttenuationCouplingPreflight& preflight,

/// Attenuation-coupling preflight with optional reject-reason output — returns \c can_narrow.

/// Attenuation-coupling preflight with required reject-reason output — returns \c can_narrow.


const char* hrtfAttenuationCouplingRejectReasonLabel(HrtfAttenuationCouplingRejectReason reason);

/// Map attenuation-coupling preflight diagnostics to a reject reason (read-only guard).
HrtfAttenuationCouplingRejectReason classifyHrtfAttenuationCouplingReject(




/// Map attenuation-coupling preflight diagnostics to a primary reject reason.

/// True when distance/occlusion coupling preflight can narrow (optional reject-reason output).

/// Out-param variant of \c preflight_hrtf_attenuation_coupling_ready.

/// True when \c classify_hrtf_attenuation_coupling_reject matches \p expected (B7.2 deepen pass).


/// Diagnose attenuation-coupling reject reason from a preflight bundle.
HrtfAttenuationCouplingRejectReason hrtf_attenuation_coupling_reject_reason(

/// Returns true when attenuation-coupling preflight reject reason matches \p expected (B7.2 deepen pass).
...
/// Map attenuation-coupling preflight flags to a reject reason.
...
/// Attenuation-coupling narrowing preflight with mandatory reject-reason output (B7.2 deepen).
bool try_preflight_hrtf_spatial_narrowing(HrtfPanPath path, float distance_attenuation,
                                          const HrtfAttenuationCoupling& coupling,
                                          const BinauralPanParams& params,


/// Map attenuation-coupling preflight diagnostics to a reject reason.
...
/// Classify why attenuation-coupling preflight would reject narrowing.




/// Classify why attenuation-coupling preflight skips narrowing.
...
/// Map attenuation-coupling preflight flags to a primary reject reason.

/// Diagnose why spatial-image narrowing preflight would reject; succeeds when \c can_narrow().
...
/// Classify why attenuation-coupling preflight rejected narrowing.

/// Attenuation-coupling preflight with optional reject-reason output.


/// Early-out when attenuation-coupling preflight would reject narrowing.
/// Human-readable label for attenuation-coupling reject reasons (B7.2 deepen pass).

/// Classify an attenuation-coupling preflight into a reject reason (B7.2 deepen pass).
...
/// Why composite binaural/HRTF preflight rejected a guarded path (B7.2 deepen pass).
    HrtfDisabled,
    CoLocated,
    EmptyIr,
    MalformedIr,
...
/// Why composite binaural/HRTF preflight rejected the request (B7.2 deepen pass).
/// Why composite binaural/HRTF preflight bypasses spatial pan (B7.2 deepen pass).
/// Why composite binaural preflight bypasses spatial pan (B7.2 deepen pass).
/// Why composite binaural/HRTF preflight skips pan or convolution (B7.2 deepen).
/// Why composite binaural/HRTF preflight rejected a guarded operation (B7.2 deepen pass).
/// Why composite binaural preflight rejects spatial pan (B7.2 deepen).
    NullSamples,
    ZeroLength,
    UnityAttenuation,
...
/// Classify why composite spatial pan would be bypassed (B7.2 deepen).
HrtfBinauralRejectReason classify_hrtf_binaural_reject_reason(bool hrtf_enabled, const HrtfIrStub& ir,

/// Classify composite bypass when no IR is wired (B7.2 deepen).
HrtfBinauralRejectReason classify_hrtf_binaural_reject_reason(bool hrtf_enabled,

/// True when composite spatial pan can proceed; optional reject-reason output (B7.2 deepen).
bool preflight_hrtf_binaural_spatial(bool hrtf_enabled, const HrtfIrStub& ir, const Vec3& rel_listener,

/// Composite spatial pan preflight with mandatory reject-reason output (B7.2 deepen).
bool try_preflight_hrtf_binaural_spatial(bool hrtf_enabled, const HrtfIrStub& ir, const Vec3& rel_listener,
                                         HrtfBinauralRejectReason& reason);

/// True when composite spatial pan can proceed without IR wired (B7.2 deepen).

/// No-IR composite spatial pan preflight with mandatory reject-reason output (B7.2 deepen).
bool try_preflight_hrtf_binaural_spatial(bool hrtf_enabled, const Vec3& rel_listener,
/// Unified reject reason across composite binaural/HRTF guards (B7.2 deepen pass).




/// Composite binaural/HRTF reject reasons — wraps layer-specific diagnostics (B7.2 deepen pass).


/// Composite spatial-pan reject reason from a preflight bundle.
HrtfBinauralRejectReason hrtf_binaural_spatial_reject_reason(const HrtfBinauralPreflight& preflight);

/// Composite convolution reject reason from a preflight bundle.
HrtfBinauralRejectReason hrtf_binaural_convolution_reject_reason(const HrtfBinauralPreflight& preflight);

/// Composite attenuation-coupling reject reason from a preflight bundle.
HrtfBinauralRejectReason hrtf_binaural_attenuation_coupling_reject_reason(

/// Returns true when `hrtf_binaural_spatial_reject_reason` matches `expected`.
bool hrtf_binaural_rejects_for_spatial_reason(const HrtfBinauralPreflight& preflight,

/// Returns true when `hrtf_binaural_convolution_reject_reason` matches `expected`.
bool hrtf_binaural_rejects_for_convolution_reason(const HrtfBinauralPreflight& preflight,

/// Returns true when `hrtf_binaural_attenuation_coupling_reject_reason` matches `expected`.
bool hrtf_binaural_rejects_for_attenuation_coupling_reason(const HrtfBinauralPreflight& preflight,


/// Why composite binaural/HRTF preflight rejected spatial pan or convolution (B7.2 deepen pass).
/// Why composite binaural/HRTF preflight rejected spatial pan (B7.2 deepen).



/// Returns the first reject reason for a composite binaural preflight bundle, or \c None when spatial
/// pan may run.
...
/// Why composite binaural/HRTF preflight rejects spatial processing (B7.2 deepen pass).

/// Why composite binaural/HRTF preflight rejected spatial pan, convolution, or narrowing (B7.2 deepen pass).

/// Why composite binaural/HRTF preflight rejected spatial processing (B7.2 deepen pass).
...
/// Why composite binaural/HRTF pan would bypass (B7.2 deepen reject-reason pass).


/// Diagnose why composite binaural pan would bypass; vacuously succeeds on enabled, separated sources.

/// Returns true when `hrtf_binaural_reject_reason` matches `expected` (B7.2 deepen reject-reason pass).

/// Diagnostic reason composite binaural/HRTF processing is rejected (B7.2 deepen).
...
/// Human-readable label for composite binaural/HRTF reject reasons (logging / tests).

/// Why composite binaural/HRTF preflight would reject spatial pan (B7.2 deepen pass).
...
/// Composite spatial-pan preflight with optional reject-reason output (B7.2 deepen pass).
bool preflightHrtfBinauralPanReady(bool hrtf_enabled, const HrtfIrStub& ir, const Vec3& rel_listener,
                                   const BinauralPanParams& params = {},
                                   HrtfBinauralRejectReason* reason = nullptr);
bool preflightHrtfBinauralPanReady(bool hrtf_enabled, const Vec3& rel_listener,
bool tryPreflightHrtfBinauralPan(bool hrtf_enabled, const HrtfIrStub& ir, const Vec3& rel_listener,
                                 const HrtfAttenuationCoupling& coupling,
                                 const BinauralPanParams& params,
                                 HrtfBinauralRejectReason& reason);
bool tryPreflightHrtfBinauralPan(bool hrtf_enabled, const Vec3& rel_listener,

/// Composite IR convolution preflight with optional reject-reason output (B7.2 deepen pass).
...
/// Composite attenuation-coupling preflight with optional reject-reason output (B7.2 deepen pass).
...
/// Composite reject-reason bundle for binaural/HRTF dispatch (B7.2 deepen).
struct HrtfBinauralRejectBundle {
    ...
    bool convolution_blocked() const { return hrtf_ir_reject_reason_is_blocking(convolution); }
    bool spatial_pan_blocked() const { return hrtf_pan_path_reject_reason_is_blocking(spatialPan); }
    bool narrowing_blocked() const {
        return hrtf_attenuation_coupling_reject_reason_is_blocking(narrowing);
    }

    /// Composite spatial-pan reject reason (pan-path layer).
    HrtfBinauralRejectReason spatial_reject_reason() const;

    /// Composite convolution reject reason (IR + pan-path layers).
    HrtfBinauralRejectReason convolution_reject_reason() const;

    /// Composite attenuation-coupling reject reason (coupling layer).
...
/// Map composite preflight flags to a primary pan-path reject reason.

/// Map composite preflight flags to a convolution reject reason.

/// Map composite preflight flags to a spatial-image narrowing reject reason.

/// Diagnose why composite binaural preflight would reject spatial pan; succeeds when \c can_spatial_pan().
bool tryPreflightHrtfBinaural(bool hrtf_enabled, const HrtfIrStub& ir, const Vec3& rel_listener,

/// Diagnose why composite binaural preflight would reject spatial pan when no IR is wired.
bool tryPreflightHrtfBinaural(bool hrtf_enabled, const Vec3& rel_listener,



/// Classify a composite binaural/HRTF preflight bundle into a reject reason.

/// Classify why composite preflight would skip IR convolution.
...
HrtfBinauralPreflight preflight_hrtf_binaural(bool hrtf_enabled, const HrtfIrStub& ir,
                                              ...
                                              const HrtfAttenuationCoupling& coupling = {},
                                              const BinauralPanParams& params = {},
                                              HrtfBinauralRejectBundle* rejects = nullptr);
...
bool try_preflight_hrtf_binaural(bool hrtf_enabled, const HrtfIrStub& ir,
                                 const Vec3& rel_listener, float distance_attenuation,
                                 float occlusion_gain, HrtfBinauralRejectReason& reason,
/// Classify bypass reject reason from a composite binaural preflight bundle.
HrtfBinauralRejectReason classifyHrtfBinauralReject(const HrtfBinauralPreflight& preflight);
...
/// Map composite binaural preflight diagnostics to a reject reason (B7.2 deepen pass).
HrtfBinauralRejectReason classify_hrtf_binaural_reject(const HrtfBinauralPreflight& preflight);

/// Why composite binaural preflight rejected spatial pan (B7.2 deepen pass).
enum class HrtfBinauralRejectReason : u8 {
    None = 0,
    HrtfDisabled,
    CoLocated,
...
/// Composite binaural preflight with optional pan reject-reason output (B7.2 deepen pass).

/// Composite binaural preflight with mandatory pan reject-reason output (B7.2 deepen pass).

/// Composite convolution preflight with optional reject-reason output (B7.2 deepen pass).
bool preflight_hrtf_binaural_convolve_ready(bool hrtf_enabled, const HrtfIrStub& ir,

/// Composite convolution preflight with mandatory reject-reason output (B7.2 deepen pass).
bool try_preflight_hrtf_binaural_convolve(bool hrtf_enabled, const HrtfIrStub& ir,

/// Map a composite preflight bundle to its primary pan bypass reject reason.
HrtfBinauralRejectReason classify_hrtf_binaural_pan_reject(const HrtfBinauralPreflight& preflight);

/// Map a composite preflight bundle to its convolution reject reason.
HrtfBinauralRejectReason classify_hrtf_binaural_convolve_reject(const HrtfBinauralPreflight& preflight);

/// Map a composite preflight bundle to its attenuation-coupling reject reason.
HrtfBinauralRejectReason classify_hrtf_binaural_narrow_reject(const HrtfBinauralPreflight& preflight);

/// Map a composite preflight bundle to its primary reject reason (pan bypass first).
...
/// Preflight all binaural/HRTF guards with optional reject-reason output (B7.2 deepen).
HrtfBinauralPreflight preflight_hrtf_binaural(bool hrtf_enabled, const HrtfIrStub& ir,
                                              const Vec3& rel_listener, float distance_attenuation,
                                              float occlusion_gain,
                                              const HrtfAttenuationCoupling& coupling,
...
/// Preflight all binaural/HRTF guards when no IR is wired, with optional reject-reason output.
HrtfBinauralPreflight preflight_hrtf_binaural(bool hrtf_enabled, const Vec3& rel_listener,
                                              float distance_attenuation, float occlusion_gain,
                                              const HrtfAttenuationCoupling& coupling,
                                              const BinauralPanParams& params,
...
/// Composite narrowing preflight with optional reject-reason output (B7.2 deepen pass).
...
/// Classify why composite binaural/HRTF preflight would reject (B7.2 deepen pass).
HrtfBinauralRejectReason classify_hrtf_binaural_reject(const HrtfBinauralPreflight& preflight);
...
/// Composite binaural preflight with mandatory reject-reason output (B7.2 deepen pass).
bool try_preflight_hrtf_binaural(bool hrtf_enabled, const HrtfIrStub& ir, const Vec3& rel_listener,
                                 float distance_attenuation, float occlusion_gain,
                                 HrtfBinauralRejectReason& reason,
                                 const HrtfAttenuationCoupling& coupling = {},
...
/// Composite binaural preflight when no IR is wired, with mandatory reject-reason output (B7.2 deepen pass).
bool try_preflight_hrtf_binaural(bool hrtf_enabled, const Vec3& rel_listener,
                                 float distance_attenuation, float occlusion_gain,
                                 HrtfBinauralRejectReason& reason,
                                 const HrtfAttenuationCoupling& coupling = {},
...
/// Composite spatial-pan predicate with optional reject-reason output (B7.2 deepen pass).
bool can_apply_hrtf_binaural_pan(const HrtfBinauralPreflight& preflight,
                                 HrtfBinauralRejectReason* reason);
...
/// Composite convolution predicate with optional reject-reason output (B7.2 deepen pass).
bool can_convolve_hrtf_binaural(const HrtfBinauralPreflight& preflight,
                                HrtfBinauralRejectReason* reason);
...
/// Composite narrowing predicate with optional reject-reason output (B7.2 deepen pass).
bool can_narrow_hrtf_binaural_spatial_image(const HrtfBinauralPreflight& preflight,
                                            HrtfBinauralRejectReason* reason);
...
/// Why composite binaural preflight rejected spatial pan or convolution (B7.2 deepen pass).
/// Why composite binaural/HRTF preflight rejected spatial pan or convolution (B7.2 deepen pass).
/// Why HRTF binaural composite preflight rejected an operation (B7.2 deepen).
/// Why composite binaural spatial-pan preflight rejected the request (B7.2 deepen).
/// Why composite binaural/HRTF preflight rejected a dispatch path (B7.2 deepen).
/// Why composite binaural/HRTF preflight rejected a stage (B7.2 deepen pass).
/// Why composite binaural/HRTF preflight rejected a request (B7.2 deepen).
/// Why composite binaural preflight rejects a dispatch path (B7.2 deepen).
/// Why composite binaural preflight bypassed spatial pan (B7.2 deepen pass).
/// Why composite binaural preflight rejected centre bypass (B7.2 deepen pass).
/// Why composite binaural preflight bypasses or limits HRTF processing (B7.2 deepen).
/// Why composite binaural preflight rejected a sub-operation (B7.2 deepen pass).
/// Why composite binaural preflight rejected the request (B7.2 deepen pass).
/// Why composite binaural/HRTF preflight rejected a sub-operation (B7.2 deepen).
/// Why composite binaural/HRTF spatial routing would be skipped (B7.2 deepen pass).
/// Why composite binaural/HRTF preflight rejected spatial pan or convolution (B7.2 deepen).
/// Why composite binaural/HRTF preflight rejected spatial pan, convolution, or narrowing (B7.2 deepen).
/// Why composite binaural preflight rejected an operation (B7.2 deepen).
/// Why composite binaural preflight rejected spatial pan (B7.2 deepen pass).
/// Why composite binaural/HRTF preflight rejects spatial pan, convolution, or narrowing (B7.2 deepen).
/// Why composite binaural/HRTF preflight rejected a guarded path (B7.2 deepen pass).
enum class HrtfBinauralRejectReason : u8 {
/// Why composite binaural preflight rejected centre bypass (B7.2 deepen).
...
/// Classify composite binaural preflight into a spatial-pan reject reason.
HrtfBinauralRejectReason classify_hrtf_binaural_spatial_reject(

/// Classify composite binaural preflight into a convolution reject reason.

/// Classify composite binaural preflight into an attenuation-coupling reject reason.
HrtfBinauralRejectReason classify_hrtf_binaural_coupling_reject(
...
/// Classify spatial-pan reject from composite preflight (disabled/co-located only).

/// Classify convolution reject from composite preflight (empty/malformed IR).

/// Classify attenuation-coupling reject from composite preflight.
...
/// Composite convolution preflight with optional IR reject-reason output (B7.2 deepen pass).
                                               HrtfIrRejectReason* reason = nullptr);
                                            const Vec3& rel_listener, HrtfIrRejectReason& reason);
...
/// Composite spatial-pan reject reason — delegates to pan-path classifier (B7.2 deepen pass).
HrtfPanPathRejectReason classify_hrtf_binaural_spatial_reject(const HrtfBinauralPreflight& preflight);

/// Composite convolution reject reason — delegates to IR classifier (B7.2 deepen pass).

/// Composite narrowing reject reason — delegates to attenuation-coupling classifier (B7.2 deepen pass).

/// Composite binaural spatial-pan preflight with optional reject-reason output (B7.2 deepen pass).
                                           HrtfPanPathRejectReason* reason = nullptr);
                                         const Vec3& rel_listener, HrtfPanPathRejectReason& reason);
                                         HrtfPanPathRejectReason& reason);

/// Composite binaural convolution preflight with optional reject-reason output (B7.2 deepen pass).

/// Composite binaural narrowing preflight with optional reject-reason output (B7.2 deepen pass).
                                             HrtfAttenuationCouplingRejectReason* reason = nullptr,
                                           HrtfAttenuationCouplingRejectReason& reason,
/// Why composite binaural convolution preflight rejected the request (B7.2 deepen).
/// Why composite binaural/HRTF convolution would be skipped (B7.2 deepen pass).
enum class HrtfBinauralConvolutionRejectReason : u8 {

/// Why composite binaural narrowing preflight rejected the request (B7.2 deepen).
enum class HrtfBinauralNarrowingRejectReason : u8 {

/// Why composite binaural/HRTF spatial-image narrowing would be skipped (B7.2 deepen pass).




/// Composite convolution/narrowing preflight with optional reject-reason output (B7.2 deepen).
...
/// Composite spatial-pan preflight with optional reject-reason output (B7.2 deepen).
...
/// Map composite preflight to spatial-pan, convolution, or narrowing reject reasons.
HrtfBinauralRejectReason classify_hrtf_binaural_convolution_reject(const HrtfBinauralPreflight& preflight);
...
/// Reject reason when spatial pan cannot run.

/// Reject reason when IR convolution cannot run.

/// Reject reason when attenuation coupling cannot narrow the spatial image.
HrtfBinauralRejectReason classify_hrtf_binaural_narrow_reject(const HrtfBinauralPreflight& preflight);


/// Composite spatial-pan preflight when no IR is wired (B7.2 deepen).


bool preflight_hrtf_binaural_narrow_ready(bool hrtf_enabled, const HrtfIrStub& ir,

/// Composite narrowing preflight when no IR is wired (B7.2 deepen).
bool preflight_hrtf_binaural_narrow_ready(bool hrtf_enabled, const Vec3& rel_listener,

/// Composite spatial-pan preflight with required reject-reason output (B7.2 deepen).

/// Composite convolution preflight with required reject-reason output (B7.2 deepen).

/// Composite narrowing preflight with required reject-reason output (B7.2 deepen).
bool try_preflight_hrtf_binaural_narrow(bool hrtf_enabled, const HrtfIrStub& ir,
/// Why composite binaural preflight rejected convolution dispatch (B7.2 deepen pass).
enum class HrtfConvolutionRejectReason : u8 {
...
/// Human-readable label for composite binaural reject diagnostics.

/// Classify the primary composite binaural reject reason from a preflight bundle.

/// Returns true when \c classify_hrtf_binaural_reject matches \p expected.

/// Composite binaural preflight with optional reject-reason output — returns \c can_spatial_pan.

/// Composite binaural preflight (no IR wired) with optional reject-reason output.

/// Composite binaural preflight with required reject-reason output — returns \c can_spatial_pan.

/// Composite binaural preflight (no IR wired) with required reject-reason output.

const char* hrtfBinauralRejectReasonLabel(HrtfBinauralRejectReason reason);

/// Map composite preflight to a spatial-pan reject reason (read-only guard).
HrtfBinauralRejectReason classifyHrtfBinauralPanReject(const HrtfBinauralPreflight& preflight);

/// Map composite preflight to a convolution reject reason (read-only guard).
HrtfBinauralRejectReason classifyHrtfBinauralConvolutionReject(const HrtfBinauralPreflight& preflight);

/// Map composite preflight to a spatial-narrowing reject reason (read-only guard).
HrtfBinauralRejectReason classifyHrtfBinauralNarrowingReject(const HrtfBinauralPreflight& preflight);
...
/// Composite binaural pan preflight from source parameters (IR-aware).


/// Map composite preflight bypass flags to a single reject reason.

/// Map composite preflight IR flags to a convolution reject reason.
...
/// Map composite preflight to a spatial-pan reject reason.
HrtfBinauralRejectReason classify_hrtf_binaural_spatial_pan_reject(

/// Map composite preflight to a convolution reject reason.

/// Map composite preflight to a spatial-narrowing reject reason.

/// Readable alias — spatial-pan reject classification.

/// Returns true when \p preflight rejects for \p expected (B7.2 deepen).

/// True when composite spatial-pan preflight can proceed (optional reject-reason output).

/// True when composite spatial-pan preflight can proceed without IR wired.

/// True when composite convolution preflight can proceed (optional reject-reason output).

/// True when composite spatial-narrowing preflight can proceed (optional reject-reason output).

/// Out-param variant of \c preflight_hrtf_binaural_ready (IR-aware).

/// Out-param variant of \c preflight_hrtf_binaural_ready (no IR wired).

/// True when composite reject classifiers match \p expected (B7.2 deepen pass).
bool hrtf_binaural_convolution_rejects_for_reason(const HrtfBinauralPreflight& preflight,
...
bool hrtf_binaural_narrowing_rejects_for_reason(const HrtfBinauralPreflight& preflight,
                                             ...
                                             HrtfBinauralNarrowingRejectReason* reason = nullptr,
                                           HrtfBinauralNarrowingRejectReason& reason,
/// Composite reject-reason wrappers — delegate to bundled subsystem preflights (B7.2 deepen reject-reason pass).
...
/// Composite wrapper — empty-IR reject reason from a binaural preflight bundle.
HrtfIrRejectReason hrtf_binaural_ir_reject_reason(const HrtfBinauralPreflight& preflight);

/// Composite wrapper — pan-path reject reason from a binaural preflight bundle.
HrtfPanPathRejectReason hrtf_binaural_pan_path_reject_reason(const HrtfBinauralPreflight& preflight);

/// Composite wrapper — attenuation-coupling reject reason from a binaural preflight bundle.
HrtfAttenuationCouplingRejectReason hrtf_binaural_attenuation_coupling_reject_reason(

/// Composite wrapper — returns true when IR reject reason matches \p expected.
bool hrtf_binaural_rejects_ir_for_reason(const HrtfBinauralPreflight& preflight,

/// Composite wrapper — returns true when pan-path reject reason matches \p expected.
bool hrtf_binaural_rejects_pan_path_for_reason(const HrtfBinauralPreflight& preflight,
                                               HrtfPanPathRejectReason expected);

/// Composite wrapper — returns true when attenuation-coupling reject reason matches \p expected.
bool hrtf_binaural_rejects_attenuation_coupling_for_reason(

/// Primary pan bypass reject reason for composite preflight (disabled or co-located).
HrtfBinauralRejectReason hrtf_binaural_pan_reject_reason(const HrtfBinauralPreflight& preflight);

/// Primary convolution skip reject reason for composite preflight (empty or malformed IR).
HrtfBinauralRejectReason hrtf_binaural_convolution_reject_reason(

/// Primary narrowing skip reject reason for composite preflight (bypass path or unity attenuation).
HrtfBinauralRejectReason hrtf_binaural_narrowing_reject_reason(

/// Returns true when composite pan reject reason matches \p expected (B7.2 deepen pass).
bool hrtf_binaural_pan_rejects_for_reason(const HrtfBinauralPreflight& preflight,

/// Returns true when composite convolution reject reason matches \p expected (B7.2 deepen pass).

/// Returns true when composite narrowing reject reason matches \p expected (B7.2 deepen pass).
/// Returns true when composite preflight reject reason matches \p expected.
...
/// Map composite preflight flags to a spatial-pan reject reason.

/// Map composite preflight flags to a convolution reject reason.

/// Map composite preflight flags to a spatial-narrowing reject reason.



/// Classify all binaural/HRTF reject reasons from a composite preflight bundle.
...
/// True when composite preflight allows spatial pan (no pan-path reject reason).
bool hrtf_binaural_rejects_allow_spatial_pan(const HrtfBinauralRejectBundle& rejects);

/// True when composite preflight allows IR convolution (no IR or pan-path reject reason).
...
/// Why composite binaural preflight rejected spatial image narrowing (B7.2 deepen).
enum class HrtfBinauralNarrowRejectReason : u8 {

/// Human-readable labels for composite binaural reject reasons (logging / tests).
const char* hrtf_binaural_convolve_reject_reason_label(HrtfBinauralConvolveRejectReason reason);
const char* hrtf_binaural_narrow_reject_reason_label(HrtfBinauralNarrowRejectReason reason);

/// Classify composite spatial-pan reject from a preflight bundle.

/// Classify composite convolution reject from a preflight bundle.
HrtfBinauralConvolveRejectReason classify_hrtf_binaural_convolve_reject(

/// Classify composite narrowing reject from a preflight bundle.
HrtfBinauralNarrowRejectReason classify_hrtf_binaural_narrow_reject(

/// Composite binaural preflight with optional spatial-pan reject-reason output (B7.2 deepen).

/// Composite binaural preflight when no IR is wired, with optional reject-reason output (B7.2 deepen).

/// Composite binaural preflight with mandatory spatial-pan reject-reason output (B7.2 deepen).
                                 const BinauralPanParams& params,

/// Composite binaural preflight when no IR is wired, with mandatory reject-reason output (B7.2 deepen).
...
/// Composite convolution preflight with mandatory reject-reason output (B7.2 deepen).
...
/// Composite narrowing preflight with mandatory reject-reason output (B7.2 deepen).
bool try_preflight_hrtf_binaural_narrow(const HrtfBinauralPreflight& preflight,
                                          HrtfBinauralNarrowRejectReason& reason);


/// Classify composite reject reason for spatial pan preflight.

/// Classify composite reject reason for IR convolution preflight.
...
/// Returns the first bypass reject reason, or \c None when spatial pan is allowed.
...
/// Composite bypass preflight with optional reject-reason output (B7.2 deepen).
...
/// Composite bypass preflight with optional reject-reason output (IR-aware, B7.2 deepen).
bool preflight_hrtf_binaural_bypass_ready(bool hrtf_enabled, const HrtfIrStub& ir,



/// Composite narrowing preflight with optional reject-reason output (IR-aware, B7.2 deepen).

/// Composite bypass preflight with mandatory reject-reason output (B7.2 deepen).
...
/// Map composite preflight to a spatial-pan reject reason (None when pan is allowed).

/// Map composite preflight to a convolution reject reason (None when convolution is allowed).

/// Map composite attenuation-coupling sub-preflight to a reject reason.
...
/// Composite attenuation-coupling preflight with optional reject-reason output (B7.2 deepen).
                                            HrtfAttenuationCouplingRejectReason* reason = nullptr);
...
/// Classify why composite binaural preflight rejected processing.

/// Composite binaural preflight with optional reject-reason output (IR-aware).

/// Composite binaural preflight with optional reject-reason output (no IR wired).

/// Composite binaural preflight with mandatory reject-reason output (IR-aware).

/// Composite binaural preflight with mandatory reject-reason output (no IR wired).
...
/// Classify a composite binaural/HRTF preflight into a reject reason (B7.2 deepen pass).

/// Composite binaural/HRTF preflight with optional reject-reason output (B7.2 deepen pass).
...
/// Why composite preflight rejects IR convolution while spatial pan may proceed (B7.2 deepen pass).
enum class HrtfConvolutionRejectReason : u8 {
...
/// Map composite preflight flags to bypass and convolution reject reasons (B7.2 deepen pass).
HrtfConvolutionRejectReason classify_hrtf_convolution_reject(const HrtfBinauralPreflight& preflight);
...
/// Composite binaural preflight with optional reject-reason output (B7.2 deepen).
bool should_skip_hrtf_binaural_spatial_preflight(bool hrtf_enabled, const HrtfIrStub& ir,
/// Composite binaural reject-reason classifiers — delegate to sub-preflight bundles (B7.2 deepen pass).
