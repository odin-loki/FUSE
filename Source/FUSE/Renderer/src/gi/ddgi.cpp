...
const char* probeGridSourceLabel(ProbeGridSource source) {
    switch (source) {
    case ProbeGridSource::Invalid:
        return "invalid";
    case ProbeGridSource::FlatIndex:
        return "flat_index";
    case ProbeGridSource::ClampedIndex:
        return "clamped_index";
    case ProbeGridSource::GridCoord:
        return "grid_coord";
    case ProbeGridSource::ClampedGridCoord:
        return "clamped_grid_coord";
    case ProbeGridSource::WorldPosition:
...
bool ProbeGridSource::matchesDesc() const {
    return ddgi_util::isCacheSizedForGrid(desc, cache_count);

bool ProbeGridSource::isAccessible() const {
    return ddgi_util::isProbeCacheAccessible(desc, cache, cache_count);
...
ProbeGridSource ProbeGridLayout::classifyProbeIndexSource(const DDGIDesc& desc, u32 probe_index) {
    if (isEmptyGrid(desc)) {
        return ProbeGridSource::Invalid;
    }
    if (isProbeIndexOutOfRange(probe_index, desc)) {
        return ProbeGridSource::ClampedIndex;
    }
    return ProbeGridSource::FlatIndex;
}

ProbeGridSource ProbeGridLayout::classifyProbeCoordSource(const DDGIDesc& desc, const ProbeGridCoord& coord) {
    if (isEmptyGrid(desc)) {
        return ProbeGridSource::Invalid;
    }
    if (!isValidProbeCoord(desc, coord)) {
        return ProbeGridSource::ClampedGridCoord;
    }
    return ProbeGridSource::GridCoord;
}
...
bool probeDescMatches(const DDGIDesc& lhs, const DDGIDesc& rhs) {
    return lhs.grid_dims.x == rhs.grid_dims.x && lhs.grid_dims.y == rhs.grid_dims.y &&
           lhs.grid_dims.z == rhs.grid_dims.z && lhs.irradiance_res == rhs.irradiance_res &&
           lhs.depth_res == rhs.depth_res;
}
...
bool tryValidateProbeGridSource(const ProbeGridSource& source, ProbeGridSourceRejectReason& outReason) {
    ...
    switch (source.kind) {
    case ProbeGridSourceKind::Desc:
        outReason = ProbeGridSourceRejectReason::None;
        return true;
    case ProbeGridSourceKind::CpuCache:
        if (source.cpu_cache == nullptr) {
            outReason = ProbeGridSourceRejectReason::NullCache;
        if (!isCacheSizedForGrid(*source.desc, source.cpu_cache_count)) {
            outReason = ProbeGridSourceRejectReason::UndersizedCache;
    case ProbeGridSourceKind::ProbeData:
        if (source.probe_data == nullptr) {
            outReason = ProbeGridSourceRejectReason::NullProbeData;
        if (!probeDescMatches(*source.desc, source.probe_data->desc)) {
            outReason = ProbeGridSourceRejectReason::DescProbeDataMismatch;
        if (source.cpu_cache != nullptr && !isCacheSizedForGrid(*source.desc, source.cpu_cache_count)) {
    case ProbeGridSourceKind::Invalid:
        break;
...
bool wouldSkipProbeGridSource(const ProbeVolume& volume, const DDGIDesc& desc) {
    ...
    return !validateProbeGridSource(desc);
}
...
bool validateProbeCacheForDesc(const DDGIDesc& desc, const IrradianceCacheEntry* cache, u32 cache_count) {
    return validateProbeCache(desc, cache, cache_count);
...
bool volumeMatchesDesc(const ProbeVolume& volume, const DDGIDesc& desc) {
    const u32 expectedCount = probeCount(desc);
    if (expectedCount == 0u) {
        return volume.probe_count == 0u;
    return volume.probe_count == expectedCount;
...
bool tryValidateProbeGridSource(const ProbeVolume& volume,
    ...
    if (volume.probe_count != expectedCount) {
        outReason = ProbeGridSourceRejectReason::ProbeCountMismatch;
        return false;
    if (!volume.irradiance_atlas.isValid() || !volume.depth_atlas.isValid() ||
        !volume.probe_offsets.isValid()) {
        outReason = ProbeGridSourceRejectReason::InvalidResources;
    if (desc.irradiance_res == 0u || desc.depth_res == 0u) {
        outReason = ProbeGridSourceRejectReason::DescMismatch;
...
bool validateProbeGridSourceForDesc(const ProbeVolume& volume, const DDGIDesc& desc) {
    return validateProbeGridSource(volume, desc);
...
bool tryCanSampleProbeGrid(const DDGIDesc& desc, ProbeGridRejectReason& outReason) {
    ...
    const u32 expected = probeCount(desc);
    if (probe_count != expected) {
...
bool shouldSkipProbeGridSource(const ProbeGridSource& source) {
    ...
    return !tryValidateProbeGridSource(source, reason);
bool wouldSkipProbeLookup(const DDGIDesc& desc,
...
bool wouldSkipProbeTrilinearSample(const ProbeGridSource& source, const ProbeSampleCoords& coords) {
    return !preflightProbeTrilinearSample(source, coords);
                                   ...
                                   const ProbeSampleCoords& coords,
    return !canTrilinearSampleAtProbeCoords(desc, coords, cache, cache_count);
...
bool wouldSkipProbeLookupAtIndex(const DDGIDesc& desc,
                                 ...
                                 u32 probe_index,
                                 u32 cache_count) {
    if (cache == nullptr || ProbeGridLayout::isEmptyGrid(desc)) {
        return wouldSkipCacheIndexLookup(desc, cache, probe_index, cache_count);
    }
    if (ProbeGridLayout::isProbeIndexOutOfRange(probe_index, desc)) {
        return false;
    }
    return wouldSkipCacheIndexLookup(desc, cache, probe_index, cache_count);
}
...
bool wouldClampCacheIndexCoordForLookup(const DDGIDesc& desc, const ProbeGridCoord& coord) {
    return !ProbeGridLayout::isEmptyGrid(desc) && !ProbeGridLayout::isValidProbeCoord(desc, coord);
...
ProbeTrilinearSampleRejectReason classifyDirectionalTrilinearSampleReject(
    ...
    ProbeTrilinearSampleRejectReason reason = ProbeTrilinearSampleRejectReason::None;
    fuse::math::Vec3 irradiance{};
    tryTrilinearDirectionalProbeIrradiance(
        desc, world_position, direction, cache, cache_count, irradiance, reason);
    return reason;
...
bool tryLaunch_probe_kernels(const DDGIKernelParams& params,
    ...
    const bool traced = launch_probe_trace_kernel(params, cuda_stream);
    const bool blended = launch_probe_blend_kernel(params, cuda_stream);
    return traced && blended;
}
