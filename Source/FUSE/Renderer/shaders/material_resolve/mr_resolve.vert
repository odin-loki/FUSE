...
//   0..3, 5  binned path: one instance per tile of that bin (vkCmdDrawIndirectCount, instanceCount = the
//            classify pass's tile count), 6 vertices = the tile's quad, from the bin's tile list (BDA;
//            the layered bin 5's list is appended after the four feature-bin lists)
...
void main() {
    ...
    const uint tile = FuseMrBinsRef(f.bins).words[fuse_mr_bin_list_word(FUSE_MR_BIN_ID, f.tileCapacity) +
