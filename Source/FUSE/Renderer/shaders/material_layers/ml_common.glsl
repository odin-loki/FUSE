...
vec4 ml_sample_tex(MlView v, uint tex, vec2 uv, bool stochastic, MlStochastic s, vec4 fallback) {
    ...
    const vec4 a = ml_bilinear(v, t, vec2(uv.x + s.offset[0].x, uv.y + s.offset[0].y));
    const vec4 b = ml_bilinear(v, t, vec2(uv.x + s.offset[1].x, uv.y + s.offset[1].y));
    const vec4 c = ml_bilinear(v, t, vec2(uv.x + s.offset[2].x, uv.y + s.offset[2].y));
...
MlTap ml_tap(MlView v, uint albedoTex, uint normalTex, vec2 uv, MlFrame f, uint seed) {
    MlStochastic s;
    s.w = vec3(0.0);
    s.offset[0] = vec2(0.0);
    s.offset[1] = vec2(0.0);
    ...
    MlTap t;
    t.a = ml_sample_tex(v, albedoTex, uv, f.stochastic, s, vec4(1.0, 1.0, 1.0, 0.5));
    t.n = ml_sample_tex(v, normalTex, uv, f.stochastic, s, vec4(0.5, 0.5, 1.0, 1.0));
    return t;
}
...
MlSetSample ml_sample_set(MlView v, uint albedoTex, uint normalTex, float scale, float strength, uint seed, MlFrame f) {
    MlSetSample r;
    if (!f.triplanar) {
        const MlTap t = ml_tap(v, albedoTex, normalTex, vec2(f.uv.x * scale, f.uv.y * scale), f, seed);
        r.albedo = t.a.xyz;
        r.height = t.a.w;
        r.roughness = t.n.z;
        r.ao = t.n.w;
    ...
    const MlTap tx = ml_tap(v, albedoTex, normalTex, vec2(f.P.z * scale, f.P.y * scale), f, seed);
    const MlTap ty = ml_tap(v, albedoTex, normalTex, vec2(f.P.x * scale, f.P.z * scale), f, seed ^ 0x51ed27u);
    const MlTap tz = ml_tap(v, albedoTex, normalTex, vec2(f.P.x * scale, f.P.y * scale), f, seed ^ 0xa3b195u);
