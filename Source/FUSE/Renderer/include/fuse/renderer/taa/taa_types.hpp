...
/// Early-out when history still needs warm-up before temporal reuse (B5.9 deepen).
bool shouldSkipTaaHistoryWarmup(const TaaHistoryBuffer& history);
/// Classify why history warm-up is not yet satisfied (B5.9 deepen).
TaaHistoryReuseBlockReason classifyTaaHistoryWarmupBlock(const TaaHistoryBuffer& history);
/// True when history warm-up is satisfied and temporal reuse may proceed (B5.9 deepen).
...
bool tryPreflightTaaHistoryWarmup(const TaaHistoryBuffer& history, TaaHistoryReuseBlockReason& reason);
/// True when history no longer needs warm-up (B5.9 deepen).
bool taaHistoryWarmupComplete(const TaaHistoryBuffer& history);
/// Classify why history warm-up preflight would reject (B5.9 deepen).
...
/// Why history warm-up completion preflight rejected the request (B5.9 deepen).
enum class TaaHistoryWarmupRejectReason : u8 {
    None = 0,
    NotReady,
    Incomplete,
...
/// Human-readable label for history warm-up reject reasons (B5.9 deepen).
const char* taaHistoryWarmupRejectReasonLabel(TaaHistoryWarmupRejectReason reason);
/// Classify why history warm-up is not yet complete (B5.9 deepen).
TaaHistoryWarmupRejectReason classifyTaaHistoryWarmupReject(const TaaHistoryBuffer& history);
/// True when history warm-up is complete and temporal reuse may proceed (B5.9 deepen).
bool taaHistoryWarmupComplete(const TaaHistoryBuffer& history);
/// True when history warm-up completion preflight passes (B5.9 deepen).
bool preflightTaaHistoryWarmupComplete(const TaaHistoryBuffer& history,
                                       TaaHistoryWarmupRejectReason* reason = nullptr);
/// History warm-up completion preflight with mandatory reject-reason output (B5.9 deepen).
bool tryPreflightTaaHistoryWarmupComplete(const TaaHistoryBuffer& history,
                                          TaaHistoryWarmupRejectReason& reason);
/// Early-out when history warm-up is not yet complete (B5.9 deepen).
bool shouldSkipTaaHistoryWarmupComplete(const TaaHistoryBuffer& history);

/// Warm-up lifecycle for temporal history reuse (B5.9 deepen).
enum class TaaHistoryWarmupState : u8 {
    NotReady = 0,
    NeedsWarmup,
    Complete,
...
/// Human-readable label for history warm-up states (B5.9 deepen).
const char* taaHistoryWarmupStateLabel(TaaHistoryWarmupState state);
/// Classify history warm-up lifecycle state (B5.9 deepen).
TaaHistoryWarmupState classifyTaaHistoryWarmupState(const TaaHistoryBuffer& history);
/// True when history buffers are allocated and warmed for temporal reuse (B5.9 deepen).
bool taaHistoryWarmupComplete(const TaaHistoryBuffer& history);
/// True when history warm-up is complete (B5.9 deepen).
bool preflightTaaHistoryWarmupComplete(const TaaHistoryBuffer& history,
                                       TaaHistoryWarmupState* state = nullptr);
/// History warm-up preflight with mandatory state output (B5.9 deepen).
bool tryPreflightTaaHistoryWarmupComplete(const TaaHistoryBuffer& history, TaaHistoryWarmupState& state);
/// Early-out when history warm-up is not complete (B5.9 deepen).
bool shouldSkipTaaHistoryWarmupComplete(const TaaHistoryBuffer& history);
/// True when history is warmed and generation matches for temporal blending (B5.9 deepen).
bool preflightTaaHistoryForTemporalBlend(const TaaHistoryBuffer& history, u32 observedGeneration,
                                         TaaHistoryReuseBlockReason* reason = nullptr);
/// Temporal-blend preflight with mandatory reject-reason output (B5.9 deepen).
bool tryPreflightTaaHistoryForTemporalBlend(const TaaHistoryBuffer& history, u32 observedGeneration,
                                             TaaHistoryReuseBlockReason& reason);
/// Early-out when temporal history blending should be skipped (B5.9 deepen).
...
/// Classify why combined history-reuse and blend-weight preflight would reject (B5.9 deepen).
                                                                const TaaHistoryBuffer& history,
                                    u32 observedGeneration, TaaResolveTemporalRejectReason& reason);
bool shouldSkipTaaResolveTemporal(const TaaResolveDesc& desc, const TaaHistoryBuffer& history,
/// True when resolve may apply a non-zero temporal history blend this frame (B5.9 deepen).
...
/// True when resolve would sample warmed history with valid blend weights (B5.9 deepen).
...
/// True when resolve blend weights align with history reuse policy (B5.9 deepen).
