...
/// True when the reject reason blocks screen-depth → froxel mapping.
...
/// True when a screen-mapping reject reason would block froxel mapping (B5.11 deepen pass).

/// Classify why screen-depth → sample coords would reject — same ordering as `tryMapScreenDepthToSampleCoords`.
/// True when screen-mapping reject reason blocks the mapping path.
/// Classify screen-depth → froxel mapping reject reason (same ordering as `tryMapScreenDepthToSampleCoords`).
...
/// True when a screen-mapping reject reason would block froxel index/coord mapping (B5.11 deepen).
...
/// True when a screen-mapping reject reason would block froxel index/coord mapping (B5.11 deepen pass).


/// Early-out when screen mapping would be rejected — same ordering as `classifyScreenMappingReject`.
bool wouldSkipScreenMapping(f32 screenX,
/// Screen-depth mapping preflight; optional reject-reason diagnostics.
...
/// True when a screen-mapping reject reason would block froxel screen-depth mapping (B5.11 deepen).

/// True when a screen-mapping reject reason would block froxel lookup (B5.11 deepen pass).

/// True when a screen-mapping reject reason would block depth → froxel mapping (B5.11 deepen).




/// True when screen-mapping preflight would hard-reject the request.

/// True when screen-mapping preflight would hard-reject the request (B5.11 deepen follow-up).
...
/// True when a screen-mapping reject reason would block mapping (B5.11 deepen follow-up).




/// True when screen-mapping reject reason would block guarded sampling.
bool screenMappingRejectReasonIsBlocking(ScreenMappingRejectReason reason);
...
/// True when screen-mapping reject reason blocks the mapping preflight.
bool screenMappingRejectReasonIsBlocking(ScreenMappingRejectReason reason);
...
/// True when a screen-mapping reject reason would block froxel lookup (B5.11 deepen follow-up).
bool screenMappingRejectReasonIsBlocking(ScreenMappingRejectReason reason);
...
/// Human-readable label for sample-coord reject reasons (logging / tests).
const char* sampleCoordRejectReasonLabel(SampleCoordRejectReason reason);
/// True when the reject reason blocks sample-coord preflight (vs. clampable weights).
...
/// True when sample-coord reject reason blocks the preflight path.
/// Classify sample-coord preflight reject reason (same ordering as `tryPreflightSampleCoords`).
SampleCoordRejectReason classifySampleCoordReject(const FroxelSampleCoords& coords, const FroxelGridDesc& desc);
/// Sample-coord preflight; optional reject-reason diagnostics.
...
/// True when a sample-coord reject reason would block density sampling (B5.11 deepen pass).


/// True when bilinear sample reject reason blocks the sample path.
bool froxelBilinearSampleRejectReasonIsBlocking(FroxelBilinearSampleRejectReason reason);


/// True when a sample-coord reject reason would block guarded sampling (B5.11 deepen).



/// True when a sample-coord reject reason would block froxel sampling (B5.11 deepen).
...
/// True when sample-coord preflight would hard-reject the request.
...
/// True when a sample-coord reject reason would block froxel sampling (B5.11 deepen pass).

/// True when a sample-coord reject reason would block preflight (B5.11 deepen follow-up).


/// True when sample-coord preflight would hard-reject the request (B5.11 deepen follow-up).


/// True when a sample-coord reject reason would block sampling (B5.11 deepen follow-up).
/// True when sample-coord reject reason would block guarded sampling.
bool sampleCoordRejectReasonIsBlocking(SampleCoordRejectReason reason);
...
/// True when sample-coord reject reason blocks the sample preflight.
bool sampleCoordRejectReasonIsBlocking(SampleCoordRejectReason reason);
...
/// Classify why sample-coord preflight would reject — same ordering as `tryPreflightSampleCoords`.
SampleCoordRejectReason classifySampleCoordReject(const FroxelSampleCoords& coords,
                                                  const FroxelGridDesc& desc);

/// Non-mutating sample-coord preflight — returns true when preflight would succeed.
bool preflightSampleCoordsReady(const FroxelSampleCoords& coords,
                                const FroxelGridDesc& desc,
                                SampleCoordRejectReason* reason = nullptr);
...
/// True when the reject reason blocks trilinear density sampling (vs. clampable weights).
...
/// True when a trilinear sample reject reason would block density sampling (B5.11 deepen).
/// Why froxel bilinear density sampling preflight rejected the request (B5.11 deepen).
enum class FroxelBilinearSampleRejectReason : u8 {
...
/// Why screen-space froxel density sampling preflight rejected the request (B5.11 deepen).
enum class FroxelScreenSampleRejectReason : u8 {
...
/// True when a trilinear sample reject reason would block density sampling (B5.11 deepen pass).


/// True when trilinear sample reject reason blocks the sample path.
/// Classify trilinear sample preflight reject reason (same ordering as `tryCanTrilinearSampleAtCoords`).
...
/// True when a trilinear sample reject reason would block guarded sampling (B5.11 deepen).


/// True when a trilinear sample reject reason would block density lookup (B5.11 deepen).

/// True when a trilinear sample reject reason would block froxel density sampling (B5.11 deepen).
...
/// True when trilinear sample preflight would hard-reject the request.
...
/// True when a trilinear sample reject reason would block sampling (B5.11 deepen follow-up).


/// True when trilinear sample preflight would hard-reject the request (B5.11 deepen follow-up).


/// True when trilinear sample reject reason would block guarded sampling.
bool froxelTrilinearSampleRejectReasonIsBlocking(FroxelTrilinearSampleRejectReason reason);
...
/// True when trilinear sample reject reason blocks the trilinear preflight.
bool froxelTrilinearSampleRejectReasonIsBlocking(FroxelTrilinearSampleRejectReason reason);
...
/// True when a trilinear sample reject reason would block density lookup (B5.11 deepen pass).
bool froxelTrilinearSampleRejectReasonIsBlocking(FroxelTrilinearSampleRejectReason reason);
...
struct FroxelGridLayout {
    ...
    /// Classify sample-coord preflight reject — same ordering as `tryPreflightSampleCoords`.
    static SampleCoordRejectReason classifySampleCoordReject(const FroxelSampleCoords& coords,
                                                             const FroxelGridDesc& desc);
    /// Non-mutating sample-coord preflight — returns true when sampling would proceed.
    static bool preflightSampleCoords(const FroxelSampleCoords& coords,
    ...
    /// Classify screen-depth → sample-coord rejection — same ordering as `tryMapScreenDepthToSampleCoords`.
    /// Non-mutating screen-mapping preflight — returns true when mapping would proceed (B5.11 deepen).
    /// Grid-only sample-coord preflight without mutating coords (B5.11 deepen).
    /// Screen-depth → sample coords preflight without mutating outputs on rejection (B5.11 deepen).
    /// Non-mutating sample-coord preflight — returns true when coords would pass grid-only preflight.
    /// Non-mutating screen-depth mapping preflight — optionally returns mapped sample coords.
    static bool preflightMapScreenDepthToSampleCoords(f32 screenX,
    /// Classify sample-coord preflight without mutating coords.
    /// Classify screen-depth → sample coords without writing output coords.
    /// Sample-coord preflight with optional reject-reason output.
    /// Screen-depth → sample coords preflight with optional reject-reason output.
    /// Non-mutating grid-only sample-coord preflight — returns true when sampling may proceed.
    /// Classify screen-depth → froxel mapping rejection — same ordering as `tryMapScreenDepthToSampleCoords`.
    /// Non-mutating screen-depth → sample-coord preflight — returns true when mapping may proceed.
    static bool preflightSampleCoordsReady(const FroxelSampleCoords& coords,
    /// Grid-only sample-coord preflight without mutating coords; optional reject-reason diagnostics.
    /// Screen-depth mapping preflight; optional reject-reason diagnostics.
    /// Grid-only sample-coord preflight without mutating coords (B5.11 deepen pass).
    /// Non-mutating screen-depth preflight — returns true when mapping would proceed.
    /// Non-mutating sample-coord preflight — returns true when coords would sample.
    static SampleCoordRejectReason classifyFroxelSampleCoordsReject(const FroxelSampleCoords& coords,
    /// Classify why screen-depth → froxel mapping would reject.
    static ScreenMappingRejectReason classifyScreenDepthToSampleCoordsReject(f32 screenX,
    /// Screen-depth mapping preflight with optional reject-reason diagnostics (B5.11 deepen).
    /// Non-mutating sample-coord preflight — returns true when guarded sampling would proceed.
    ...
    /// Non-mutating sample-coord preflight; returns true when preflight would not block.
    ...
    /// Non-mutating sample-coord preflight — returns true when coords are usable or clampable.
    /// Grid-only sample-coord preflight with optional reject-reason output (B5.11 deepen).
    /// Map screen depth to sample coords with optional reject-reason output (B5.11 deepen).
    /// Non-mutating screen-depth → froxel index preflight — optional index output on success.
    /// Diagnose why screen-depth → sample coords would be rejected — same ordering as `tryMapScreenDepthToSampleCoords`.
    /// Diagnose why sample-coord preflight would reject — same ordering as `tryPreflightSampleCoords`.
    /// Non-mutating screen-mapping preflight — same guards as `tryMapScreenDepthToSampleCoords`.
    static bool preflightScreenMappingReady(f32 screenX,
    /// Non-mutating sample-coord preflight — same guards as `tryPreflightSampleCoords`.
    /// Non-mutating grid-only sample-coord preflight — soft-fails on clampable weights.
    /// Non-mutating screen-depth → sample-coord preflight.
    /// Grid-only sample-coord preflight — non-mutating; optional reject-reason output.
    /// Screen-depth → sample coords preflight — non-mutating; optional coords and reject-reason output.
    /// Classify screen-depth → froxel-index rejection — same ordering as `tryMapScreenDepthToFroxelIndex`.
    ...
    /// Non-mutating screen-mapping preflight — returns true when mapping would proceed without blocking.
    /// Non-mutating sample-coord preflight — returns true when preflight would succeed without blocking.
    /// Non-mutating sample-coord preflight — returns true when coords are sampleable (warnings allowed).
    /// Classify why screen-depth → sample-coord mapping would reject.
    /// Non-mutating screen-mapping preflight — optionally writes mapped sample coords.
    /// Non-mutating screen-depth → froxel index preflight.
    /// Non-mutating sample-coord preflight — returns true when coords would be accepted.
    /// Non-mutating screen-mapping preflight — returns true when mapping would succeed.
    /// Non-mutating screen-mapping preflight — optionally writes mapped sample coords on success.
                                       ScreenMappingRejectReason* reason = nullptr,
                                       FroxelSampleCoords* outCoords = nullptr);
...
/// Human-readable label for density reject reasons (logging / tests).
const char* gridDensityRejectReasonLabel(GridDensityRejectReason reason);
/// True when the reject reason blocks grid density validation.
...
/// True when a grid-density reject reason would block density validation (B5.11 deepen pass).


/// True when grid density reject reason blocks validation.
/// Classify grid density validation reject reason (same ordering as `tryValidateGridDensity`).
...
/// True when a grid-density reject reason would block froxel density validation (B5.11 deepen).
...
/// True when grid-density validation preflight would hard-reject the request.
...
/// Non-mutating grid-density validation preflight — returns true when validation would succeed.
bool preflightGridDensity(const FroxelDensityGrid& grid,
                          GridDensityRejectReason* reason = nullptr,



/// True when a grid-density reject reason would block froxel validation (B5.11 deepen pass).

/// True when a grid-density reject reason would block validation (B5.11 deepen follow-up).


/// True when grid-density validation preflight would hard-reject the request (B5.11 deepen follow-up).


/// True when grid density reject reason would block guarded validation.
bool gridDensityRejectReasonIsBlocking(GridDensityRejectReason reason);
...
/// True when grid-density reject reason blocks the validation preflight.
bool gridDensityRejectReasonIsBlocking(GridDensityRejectReason reason);
...
/// True when populate reject reason blocks meaningful fill.
/// Classify analytic populate reject reason (same ordering as `tryCanPopulateFromAnalyticFog`).
/// Analytic populate preflight; optional reject-reason diagnostics.





/// True when a populate reject reason would block analytic froxel fill (B5.11 deepen).
...
/// True when analytic populate preflight would hard-reject meaningful fill.
...
/// True when a populate reject reason would block meaningful fill (B5.11 deepen follow-up).


/// True when analytic populate preflight would hard-reject meaningful fill (B5.11 deepen follow-up).


/// True when populate reject reason would block guarded fill.
bool froxelPopulateRejectReasonIsBlocking(FroxelPopulateRejectReason reason);
...
/// True when populate reject reason blocks the analytic-fill preflight.
bool froxelPopulateRejectReasonIsBlocking(FroxelPopulateRejectReason reason);
...
/// True when a populate reject reason would block meaningful analytic fill (B5.11 deepen follow-up).
bool froxelPopulateRejectReasonIsBlocking(FroxelPopulateRejectReason reason);
...
/// Non-mutating populate preflight — returns true when populate would fill froxels.
bool preflightFroxelPopulateReady(const FroxelGridDesc& desc,
                                  const FroxelCameraDesc& camera,
                                  const VolumetricFogParams& params,
                                  FroxelPopulateRejectReason* reason = nullptr);
...
/// Human-readable label for density lookup reject reasons (logging / tests).
const char* densityLookupRejectReasonLabel(DensityLookupRejectReason reason);
/// True when the reject reason blocks density lookup (vs. clampable index/coord warning).
bool densityLookupRejectReasonIsBlocking(DensityLookupRejectReason reason);
...
/// True when a density lookup reject reason would block index/coord reads (B5.11 deepen).
...
/// True when a density lookup reject reason would block index-based lookup (B5.11 deepen pass).
...
/// True when density lookup reject reason blocks the lookup path.
/// Classify index-based density lookup reject reason (same ordering as `tryCanLookupAtIndex`).
/// Classify coord-based density lookup reject reason (same ordering as `tryCanLookupAtCoord`).
DensityLookupRejectReason classifyDensityLookupRejectAtCoord(const FroxelDensityGrid& grid,
/// Density lookup preflight; optional reject-reason diagnostics.
bool preflightDensityLookup(const FroxelDensityGrid& grid,
                            u32 index,
                            DensityLookupRejectReason* reason = nullptr);

/// True when a density lookup reject reason would block index/coord lookup (B5.11 deepen).

/// True when a density lookup reject reason would block guarded lookup (B5.11 deepen).



/// True when a density lookup reject reason would block froxel index/coord lookup (B5.11 deepen).
...
/// True when density lookup preflight would hard-reject the request.
...
/// True when a density-lookup reject reason would block index/coord lookup (B5.11 deepen pass).

/// True when a density lookup reject reason would block lookup (B5.11 deepen follow-up).


/// True when density lookup preflight would hard-reject the request (B5.11 deepen follow-up).

/// True when a density lookup reject reason would block index/coord lookup (B5.11 deepen pass).

/// True when density lookup reject reason would block guarded lookup.
bool densityLookupRejectReasonIsBlocking(DensityLookupRejectReason reason);
...
/// True when density lookup reject reason blocks the lookup preflight.
bool densityLookupRejectReasonIsBlocking(DensityLookupRejectReason reason);
...
bool preflightDensityLookupAtCoord(const FroxelDensityGrid& grid,
                                   u32 sliceZ,
/// Early-out when froxel march should be skipped — same ordering as `shouldSkipFroxelMarch`.
/// Classify why index-based density lookup preflight would reject.
/// Classify why coord-based density lookup preflight would reject.
/// Non-mutating index-based density lookup preflight — returns true when lookup would proceed.
bool preflightDensityLookupReady(const FroxelDensityGrid& grid,
/// Non-mutating coord-based density lookup preflight — returns true when lookup would proceed.
...
/// Classify why density lookup would reject — same ordering as `tryCanLookupAtIndex`.
...
/// Non-mutating index-based density lookup preflight — returns true when lookup would proceed.
/// Non-mutating density lookup preflight — returns true when lookup would proceed.
/// Index-based density lookup preflight without mutating storage (B5.11 deepen).
bool preflightDensityLookup(const FroxelDensityGrid& grid,
                            u32 index,
                            DensityLookupRejectReason* reason = nullptr);
/// Non-mutating coord-based density lookup preflight — returns true when lookup would proceed.
...
/// Classify why index-based density lookup would reject — same ordering as `tryCanLookupAtIndex`.
/// Classify why coord-based density lookup would reject — same ordering as `tryCanLookupAtCoord`.
/// Classify index-based density lookup preflight without mutating grid state.
/// Classify coord-based density lookup preflight without mutating grid state.
/// Density lookup preflight with optional reject-reason output.
/// Coord-based density lookup preflight with optional reject-reason output.
/// Classify density lookup rejection — same ordering as `tryCanLookupAtIndex`.
/// Classify coord-based density lookup rejection — same ordering as `tryCanLookupAtCoord`.
DensityLookupRejectReason classifyDensityLookupAtCoordReject(const FroxelDensityGrid& grid,
/// Non-mutating coord-based density lookup preflight — returns true when lookup may proceed.
bool preflightDensityLookupReady(const FroxelDensityGrid& grid,
bool preflightDensityLookupAtCoordReady(const FroxelDensityGrid& grid,
DensityLookupRejectReason classifyDensityLookupCoordReject(const FroxelDensityGrid& grid,
/// Non-mutating density lookup preflight; optional reject-reason diagnostics.
/// Non-mutating coord-based density lookup preflight; optional reject-reason diagnostics.
/// Non-mutating density lookup preflight — returns true when lookup would proceed.
bool preflightDensityLookupCoord(const FroxelDensityGrid& grid,
/// Non-mutating coord-based density lookup preflight (B5.11 deepen).
/// Classify why index-based density lookup preflight would reject — same ordering as `tryCanLookupAtIndex`.
/// Classify why coord-based density lookup preflight would reject — same ordering as `tryCanLookupAtCoord`.
DensityLookupRejectReason classifyFroxelDensityLookupReject(const FroxelDensityGrid& grid,
...
/// Non-mutating coord density-lookup preflight — returns true when lookup would proceed.
/// Classify why density lookup would reject — same ordering as `tryCanLookupAtIndex`.
/// Classify why coord density lookup would reject — same ordering as `tryCanLookupAtCoord`.
/// Non-mutating coord density lookup preflight — returns true when lookup would proceed.
...
DensityLookupRejectReason classifyDensityLookupRejectAtCoord(const FroxelDensityGrid& grid,
                                                             ...
                                                             u32 tileY,
                                                             u32 sliceZ);
/// Non-mutating index-based density lookup preflight — OOB indices that clamp are not rejected.
bool preflightDensityLookup(const FroxelDensityGrid& grid,
                            const FroxelGridDesc& desc,
                            u32 index,
                            DensityLookupRejectReason* reason = nullptr);
/// Non-mutating coord-based density lookup preflight — OOB coords that clamp are not rejected.
bool preflightDensityLookupAtCoord(const FroxelDensityGrid& grid,
                                   const FroxelGridDesc& desc,
                                   u32 tileX,
                                   u32 tileY,
...
/// Classify why coord-based density sampling would reject — same ordering as `tryCanSampleAtCoords`.
/// Non-mutating coord-based density sample preflight — returns true when sampling would proceed.
...
bool preflightFroxelSampleAtCoords(const FroxelDensityGrid& grid,
/// Classify coord-based sample rejection — same ordering as `tryCanSampleAtCoords`.
SampleCoordRejectReason classifyDensitySampleReject(const FroxelDensityGrid& grid,
/// Non-mutating coord-based density sample preflight — returns true when sampling may proceed.
bool preflightDensitySampleAtCoords(const FroxelDensityGrid& grid,
/// Classify bilinear sample preflight reject reason (same ordering as `tryCanBilinearSampleAtCoords`).
...
bool preflightTrilinearSample(const FroxelDensityGrid& grid,
/// Trilinear sample preflight without mutating storage (B5.11 deepen).
/// Non-mutating trilinear sample preflight; returns true when sampling would not block.
/// Non-mutating trilinear density sample preflight — returns true when sampling would proceed.
/// Non-mutating trilinear sample preflight — returns true when sampling would proceed without blocking.
...
FroxelTrilinearSampleRejectReason classifyTrilinearSampleReject(const FroxelDensityGrid& grid,
bool preflightTrilinearSample(const FroxelDensityGrid& grid,
/// Classify trilinear density sample preflight without mutating grid state.
FroxelTrilinearSampleRejectReason classifyFroxelTrilinearSampleReject(
    const FroxelDensityGrid& grid,
/// Trilinear density sample preflight with optional reject-reason output.
/// Classify trilinear sample rejection — same ordering as `tryCanTrilinearSampleAtCoords`.
/// Non-mutating trilinear density sample preflight — returns true when sampling may proceed.
bool preflightDensityTrilinearSample(const FroxelDensityGrid& grid,
bool preflightFroxelTrilinearSampleReady(const FroxelDensityGrid& grid,
/// Non-mutating trilinear sample preflight; optional reject-reason diagnostics.
/// Non-mutating trilinear sample preflight (B5.11 deepen).
/// Classify why trilinear density sampling preflight would reject — same ordering as `tryCanTrilinearSampleAtCoords`.
/// Non-mutating trilinear sample preflight — soft-fails on clampable weights.
bool preflightTrilinearSampleAtCoords(const FroxelDensityGrid& grid,
/// Non-mutating trilinear sample preflight — returns true when sampling would proceed (warnings allowed).
/// Non-mutating trilinear density sample preflight — returns true when sampling would proceed.
bool preflightTrilinearSampleReady(const FroxelDensityGrid& grid,
/// Classify why index-based density lookup would reject — same ordering as `tryCanLookupAtIndex`.
DensityLookupRejectReason classifyDensityLookupReject(const FroxelDensityGrid& grid,
...
bool preflightGridDensity(const FroxelDensityGrid& grid,
                          GridDensityRejectReason* reason = nullptr,
/// Non-mutating analytic populate preflight — returns true when populate would write non-zero density.
bool preflightFroxelPopulate(const FroxelGridDesc& desc,
                             const VolumetricFogParams& params,
                             FroxelPopulateRejectReason* reason = nullptr);
                                   const FroxelGridDesc& desc,
...
/// Classify why trilinear density sampling would reject — same ordering as `tryCanTrilinearSampleAtCoords`.
bool preflightFroxelTrilinearSample(const FroxelDensityGrid& grid,
/// Classify why trilinear density sample preflight would reject — same ordering as `tryCanTrilinearSampleAtCoords`.
/// Non-mutating trilinear sample preflight — returns true when sampling would proceed (soft-fails on clampable weights).
/// Classify why trilinear density sampling preflight would reject — same ordering as `tryCanTrilinearSampleAtCoords`.
/// Non-mutating trilinear density sampling preflight — returns true when sampling would proceed.
/// Classify why index-based density lookup would reject — same ordering as `tryCanLookupAtIndex`.
DensityLookupRejectReason classifyDensityLookupReject(const FroxelDensityGrid& grid,
/// Classify why coord-based density lookup would reject — same ordering as `tryCanLookupAtCoord`.
DensityLookupRejectReason classifyDensityLookupCoordReject(const FroxelDensityGrid& grid,
...
FroxelPopulateRejectReason classifyFroxelPopulateReject(const FroxelGridDesc& desc,
                                                        const FroxelCameraDesc& camera,
/// Non-mutating index-based density lookup preflight.
bool preflightDensityLookup(const FroxelDensityGrid& grid,
                            DensityLookupRejectReason* reason = nullptr);
/// Non-mutating coord-based density lookup preflight.
/// Non-mutating coord-based density sampling preflight.
bool preflightFroxelSample(const FroxelDensityGrid& grid,
/// Non-mutating trilinear density sampling preflight.
/// Non-mutating grid density validation preflight.
bool preflightGridDensity(const FroxelDensityGrid& grid,
                          GridDensityRejectReason* reason = nullptr,
/// Non-mutating analytic populate preflight.
bool preflightFroxelPopulate(const FroxelGridDesc& desc,
                             const VolumetricFogParams& params,
                             FroxelPopulateRejectReason* reason = nullptr);
/// Classify why trilinear sample would reject — same ordering as `tryCanTrilinearSampleAtCoords`.
...
/// Non-mutating grid-density validation preflight — returns true when validation would succeed.
/// Non-mutating grid density preflight — returns true when validation would succeed without blocking.
/// Non-mutating grid density validation preflight — returns true when validation would succeed.
...
/// Non-mutating grid-density preflight — returns true when validation would succeed.
/// Non-mutating grid-density preflight — returns true when validation would succeed (B5.11 deepen).
/// Classify index-based density lookup rejection — same ordering as `tryCanLookupAtIndex`.
DensityLookupRejectReason classifyDensityLookupReject(const FroxelDensityGrid& grid,
/// Classify coord-based density lookup rejection — same ordering as `tryCanLookupAtCoord`.
DensityLookupRejectReason classifyDensityLookupCoordReject(const FroxelDensityGrid& grid,
...
bool preflightTrilinearSample(const FroxelDensityGrid& grid,
                              const FroxelSampleCoords& coords,
                              FroxelTrilinearSampleRejectReason* reason = nullptr);
/// Classify grid density validation without mutating grid state.
/// Grid density validation preflight with optional reject-reason output.
/// Classify grid density validation rejection — same ordering as `tryValidateGridDensity`.
/// Non-mutating grid density validation preflight — returns true when storage matches desc.
/// Classify grid-density validation rejection — same ordering as `tryValidateGridDensity`.
/// Non-mutating grid-density validation preflight; optional reject-reason diagnostics.
/// Non-mutating grid-density validation preflight — returns true when validation would succeed.
...
/// Preflight guard before screen-space density sampling; false on inaccessible grid or mapping failure.
bool tryCanSampleDensityAtScreen(const FroxelDensityGrid& grid,
                                 f32 screenX,
                                 f32 screenY,
                                 f32 viewDepth,
                                 FroxelScreenSampleRejectReason& outReason);
/// Early-out when screen-space density sampling would be rejected — same ordering as `tryCanSampleDensityAtScreen`.
...
/// Non-mutating screen-space density sample preflight — returns true when mapping and lookup may proceed.
bool preflightDensityAtScreen(const FroxelDensityGrid& grid,
/// Screen-space sample with guard preflight and density-lookup reject-reason diagnostics.
...
/// Non-mutating populate preflight — returns true when populate would fill froxels.
/// Non-mutating analytic populate preflight — returns true when populate would fill density.
                             const FroxelCameraDesc& camera,
...
/// Non-mutating populate preflight — returns true when meaningful fill would proceed.
                             ...
                             FroxelPopulateRejectReason* reason = nullptr);
FroxelPopulateRejectReason classifyPopulateReject(const FroxelGridDesc& desc,
/// Non-mutating populate preflight — returns true when analytic fill would proceed (B5.11 deepen).
bool preflightPopulate(const FroxelGridDesc& desc,
/// Classify populate rejection — same ordering as `tryCanPopulateFromAnalyticFog`.
/// Classify analytic populate preflight without mutating grid state.
/// Analytic populate preflight with optional reject-reason output.
/// Classify analytic populate rejection — same ordering as `tryCanPopulateFromAnalyticFog`.
/// Non-mutating analytic populate preflight — returns true when meaningful fill would proceed.
bool preflightFroxelPopulateReady(const FroxelGridDesc& desc,
/// Non-mutating populate preflight; optional reject-reason diagnostics.
/// Non-mutating analytic populate preflight — returns true when populate would write non-zero density.
/// Non-mutating analytic populate preflight — returns true when fill would proceed.
/// Non-mutating analytic populate preflight (B5.11 deepen).
/// Non-mutating populate preflight — returns true when populate would write non-zero density.
/// Diagnose why density lookup preflight would reject — same ordering as `tryCanLookupAtIndex`.
DensityLookupRejectReason classifyDensityLookupReject(const FroxelDensityGrid& grid,
                                                      const FroxelGridDesc& desc,
                                                      u32 index);
/// Diagnose why coord lookup preflight would reject — same ordering as `tryCanLookupAtCoord`.
                                                      u32 tileX,
                                                      u32 tileY,
                                                      u32 sliceZ);
/// Diagnose why grid density validation would reject — same ordering as `tryValidateGridDensity`.
GridDensityRejectReason classifyGridDensityReject(const FroxelDensityGrid& grid,
                                                  f32 epsilon = 1e-6f);
/// Diagnose why analytic populate would skip meaningful fill — same ordering as `tryCanPopulateFromAnalyticFog`.
/// Diagnose why trilinear sample preflight would reject — same ordering as `tryCanTrilinearSampleAtCoords`.
FroxelTrilinearSampleRejectReason classifyFroxelTrilinearSampleReject(const FroxelDensityGrid& grid,
                                                                      const FroxelSampleCoords& coords);
/// Non-mutating density lookup preflight — same guards as `tryCanLookupAtIndex`.
bool preflightDensityLookupReady(const FroxelDensityGrid& grid,
                                 u32 index,
                                 DensityLookupRejectReason* reason = nullptr);
/// Non-mutating coord lookup preflight — same guards as `tryCanLookupAtCoord`.
                                 u32 sliceZ,
/// Non-mutating grid-density preflight — same guards as `tryValidateGridDensity`.
bool preflightGridDensityReady(const FroxelDensityGrid& grid,
                               GridDensityRejectReason* reason = nullptr,
/// Non-mutating trilinear sample preflight — same guards as `tryCanTrilinearSampleAtCoords`.
bool preflightFroxelTrilinearSampleReady(const FroxelDensityGrid& grid,
                                         const FroxelSampleCoords& coords,
                                         FroxelTrilinearSampleRejectReason* reason = nullptr);
/// Non-mutating populate preflight — same guards as `tryCanPopulateFromAnalyticFog`.
/// Non-mutating analytic populate preflight — returns true when populate would fill froxels.
/// Non-mutating populate preflight — returns true when fill would proceed.
...
/// Non-mutating analytic populate preflight — returns true when populate would write non-zero density.
/// Non-mutating analytic populate preflight — returns true when populate would fill froxels.
/// Non-mutating populate preflight — returns true when populate fill would proceed.
                             const FroxelCameraDesc& camera,
                             const VolumetricFogParams& params,
                             FroxelPopulateRejectReason* reason = nullptr);
