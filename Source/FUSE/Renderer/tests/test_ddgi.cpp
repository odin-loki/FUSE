...
void testProbeGridSourceValidation() {
    fuse::renderer::DDGIDesc valid{};
    valid.grid_dims = {2, 2, 2};
    valid.irradiance_res = 8;
    valid.depth_res = 16;

    expectTrue(fuse::renderer::ddgi_util::validateProbeGridSource(valid),
               "validateProbeGridSource succeeds for default desc");
    ...
    fuse::renderer::DDGIDesc empty{};
    empty.grid_dims = {0, 2, 2};
    expectTrue(!fuse::renderer::ddgi_util::validateProbeGridSource(empty),
               "validateProbeGridSource rejects empty grid");
    ...
    fuse::renderer::DDGIDesc zeroSpacing = valid;
    zeroSpacing.probe_spacing = {0.f, 2.f, 2.f};
    ...
    expectTrue(std::strcmp(fuse::renderer::probeGridSourceRejectReasonLabel(
                   fuse::renderer::ProbeGridSourceRejectReason::ZeroSpacing),
               "zero_spacing") == 0,
               "zero_spacing source reject reason label");

    fuse::renderer::DDGIDesc zeroIrradiance = valid;
    ...
    fuse::renderer::DDGIDesc zeroDepth = valid;
    ...
    expectTrue(fuse::renderer::probeGridSourceRejectReasonIsBlocking(
                   fuse::renderer::ProbeGridSourceRejectReason::ZeroDepthRes),
               "zero_depth_res source reject reason is blocking");

    std::vector<fuse::renderer::IrradianceCacheEntry> cache(8);
    expectTrue(fuse::renderer::ddgi_util::validateProbeCacheForGrid(valid, cache.data(), 8u),
               "validateProbeCacheForGrid succeeds for full cache");
    expectTrue(!fuse::renderer::ddgi_util::validateProbeCacheForGrid(valid, nullptr, 8u),
               "validateProbeCacheForGrid rejects null cache");
    expectTrue(!fuse::renderer::ddgi_util::validateProbeCacheForGrid(valid, cache.data(), 4u),
               "validateProbeCacheForGrid rejects undersized cache");
}
...
void testProbeTrilinearPreflightDeepen() {
    ...
    expectTrue(std::strcmp(fuse::renderer::probeTrilinearSampleRejectReasonLabel(reason),
    ...
    expectTrue(!fuse::renderer::probeTrilinearSampleRejectReasonIsBlocking(reason),
               "clampable_sample_coords is not blocking for trilinear preflight");
    ...
    expectTrue(reason == fuse::renderer::ProbeTrilinearSampleRejectReason::InvalidSampleCoords,
               "hard OOB indices report invalid_sample_coords reason");
    expectTrue(fuse::renderer::ddgi_util::wouldSkipProbeTrilinearSample(
...
void testProbeScheduleAtRateTryHelpers() {
    fuse::u32 indices[64]{};
    fuse::u32 count = 0u;
...
void testTryLaunchProbeKernelsCombined() {
    fuse::u32 probeIndices[2] = {0u, 1u};
    fuse::renderer::gi::DDGIKernelParams params{};
    params.probe_indices_to_update = probeIndices;
    ...
    expectTrue(reason == fuse::renderer::gi::ProbeKernelRejectReason::None,
               "successful combined kernel launch reports no reject reason");
    ...
    expectTrue(reason == fuse::renderer::gi::ProbeKernelRejectReason::ZeroUpdateCount,
               "combined kernel launch zero count reports zero_update_count reason");
}
...
int main() {
    ...
    testProbeGridSourceValidation();
    testProbeSampleCoordWouldSkip();
    testProbeTrilinearPreflightDeepen();
    testProbeScheduleAtRateTryHelpers();
    testTryLaunchProbeKernelsCombined();
