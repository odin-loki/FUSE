...
void testDdgiInitUpdateSample() {
    ...
    if (!bootstrap->status().deviceReady) {
        std::printf("SKIP: no Vulkan device — DDGI init/update/sample CPU paths covered elsewhere\n");
        return;
    }
#else
    std::printf("SKIP: Vulkan backend disabled — DDGI GPU allocation test skipped\n");
    return;
#endif
