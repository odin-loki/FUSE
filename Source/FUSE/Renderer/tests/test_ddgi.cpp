...
void testProbeGridSourceValidation() {
    ...
    expectTrue(!fuse::renderer::tryValidateProbeGridSource(zeroSpacing, reason),
               "zero spacing fails source validation");
    ...
    expectTrue(std::strcmp(fuse::renderer::probeGridSourceRejectReasonLabel(reason), "zero_depth_res") == 0,
               "zero_depth_res probe-grid source reject reason label");
}
...
void testTryLaunchCombinedProbeKernels() {
    fuse::u32 indices[2] = {0u, 1u};
    fuse::renderer::gi::DDGIKernelParams validParams{};
    validParams.probe_indices_to_update = indices;
    validParams.probe_update_count = 2u;
...
int main() {
    ...
    testTryLaunchCombinedProbeKernels();
    testSampleGuards();
