...
void testHistoryReusableGuard() {
    ...
    expectTrue(!emptyHistory.canReadForResolve(), "empty history cannot be read for resolve");
    ...
    expectTrue(!history.canReadForResolve(), "unwarmed history cannot be read for resolve");
    ...
    expectTrue(history.canReadForResolve(), "warmed history can be read for resolve");
    expectTrue(fuse::renderer::taaHistoryIsReusable(history, desc),
               "warmed history is reusable with current generation");

    history.invalidateHistory();
    desc.observed_history_generation = 0u;
    expectTrue(!history.canReadForResolve(), "invalidated history cannot be read for resolve");
    expectTrue(!fuse::renderer::taaHistoryIsReusable(history, desc),
...
void testResolveHistoryReusedStat() {
    ...
    expectNear(resolve.lastStats().history_blend, 0.f, 1e-5f, "first resolve has zero history blend");
    ...
    expectNear(resolve.lastStats().history_blend, 0.8f, 1e-5f, "second resolve history blend complements effective");
    ...
    expectNear(resolve.lastStats().effective_blend, 1.f, 1e-5f, "full blend uses full current weight");
...
void testTaaPassHistoryReuseGuards() {
    fuse::renderer::VulkanBootstrapDesc bootstrapDesc{};
    bootstrapDesc.instance.enableValidation = false;
    bootstrapDesc.createSwapchain = false;
    auto bootstrap = fuse::renderer::VulkanBootstrap::create(bootstrapDesc);
    expectTrue(bootstrap != nullptr, "bootstrap allocated for pass history reuse guard test");
    ...
    expectTrue(pass->init(resources), "TaaPass initialized for history reuse guard test");
    expectTrue(!pass->canReadHistory(), "pass history not readable before first resolve");
    expectNear(pass->effectiveBlendForNextResolve(), 1.f, 1e-5f,
               "pass effective blend is full current weight before warmup");
    ...
    desc.surfaces.current_frame = reinterpret_cast<void*>(0x1);
    desc.surfaces.output = reinterpret_cast<void*>(0x2);
    expectTrue(!pass->historyReuseReady(desc), "pass history reuse not ready before first resolve");
    expectTrue(pass->resolveFrame(desc), "initial pass resolve succeeds");
    expectTrue(pass->canReadHistory(), "pass history readable after first resolve");

    pass->stampObservedHistoryGeneration(desc);
    expectTrue(pass->historyReuseReady(desc), "pass history reuse ready after warmup");
    expectNear(pass->effectiveBlendForNextResolve(), 0.25f, 1e-5f,
               "pass effective blend uses configured factor after warmup");
