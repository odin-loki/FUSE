...
void testResolveHistoryReusedStat() {
    ...
    expectNear(resolve.lastStats().history_blend, 0.8f, 1e-5f, "second resolve records history blend complement");
    ...
    expectNear(resolve.lastStats().history_blend, 0.f, 1e-5f, "full blend yields zero history blend");
...
void testTaaPassHistoryReuseAndBlendGuards() {
    fuse::renderer::VulkanBootstrapDesc bootstrapDesc{};
    bootstrapDesc.instance.enableValidation = false;
    bootstrapDesc.createSwapchain = false;
    auto bootstrap = fuse::renderer::VulkanBootstrap::create(bootstrapDesc);
    expectTrue(bootstrap != nullptr, "bootstrap allocated for pass history reuse test");
    ...
    expectTrue(pass->init(resources), "TaaPass initialized for history reuse test");
    ...
    expectTrue(!pass->canReuseHistory(desc), "pass cannot reuse history before first resolve");
    expectNear(pass->effectiveBlendForNextResolve(desc), 1.f, 1e-5f,
               "pass effective blend is full current weight before warmup");

    expectTrue(pass->resolveFrame(desc), "initial pass resolve warms history");
    expectTrue(pass->canReuseHistory(desc), "pass can reuse history after warmup");
    expectNear(pass->effectiveBlendForNextResolve(desc), 0.25f, 1e-5f,
               "pass effective blend uses configured factor when history is reusable");

    pass->invalidateHistory();
    desc.observed_history_generation = 0u;
    expectTrue(!pass->canReuseHistory(desc), "pass cannot reuse history after invalidate");
    expectNear(pass->effectiveBlendForNextResolve(desc), 1.f, 1e-5f,
               "pass effective blend returns to full current weight after invalidate");
...
int main() {
    ...
    testTaaPassHistoryReuseAndBlendGuards();
