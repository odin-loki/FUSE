...
void testFroxelBilinearAndScreenPipelineGuards() {
void testFroxelBilinearAndGridValidationGuards() {
void testFroxelClassifyAndBlockingGuards() {
    ...
    expectTrue(!fuse::renderer::froxel_util::wouldSkipFroxelLookup(grid, desc),
               "accessible grid does not skip froxel lookup");
    expectTrue(fuse::renderer::froxel_util::wouldSkipFroxelLookup(grid, desc) ==
                   fuse::renderer::froxel_util::wouldSkipDensityLookup(grid, desc),
               "wouldSkipFroxelLookup agrees with wouldSkipDensityLookup");

    fuse::renderer::FroxelDensityGrid emptyGrid{};
    expectTrue(fuse::renderer::froxel_util::wouldSkipFroxelLookup(emptyGrid, desc),
               "empty storage skips froxel lookup");
    expectTrue(fuse::renderer::froxel_util::wouldSkipFroxelLookup(emptyGrid, desc) ==
                   fuse::renderer::froxel_util::shouldSkipFroxelLookup(emptyGrid, desc),
               "wouldSkipFroxelLookup agrees with shouldSkipFroxelLookup on empty storage");
    fuse::renderer::FroxelCameraDesc camera{};
    camera.nearPlane = 1.f;
    camera.farPlane = 100.f;
    ...
    expectTrue(fuse::renderer::FroxelGridLayout::classifyScreenMappingReject(
    ...
    fuse::renderer::FroxelGridDesc zeroDesc{};
    zeroDesc.tilesX = 0u;
                   0.5f, 0.5f, 10.f, zeroDesc, camera) ==
                   fuse::renderer::ScreenMappingRejectReason::EmptyGrid,
    ...
    expectTrue(fuse::renderer::froxel_util::tryCanBilinearSampleAtCoords(grid, desc, reversed, bilinearReason),
               "tryCanBilinearSampleAtCoords warns but succeeds for unordered corners");
               "unordered corners map to clampable_weights bilinear reject reason");

    fuse::renderer::FroxelTrilinearSampleRejectReason trilinearReason =
        fuse::renderer::FroxelTrilinearSampleRejectReason::None;
    expectTrue(fuse::renderer::froxel_util::tryCanTrilinearSampleAtCoords(grid, desc, reversed, trilinearReason),
               "tryCanTrilinearSampleAtCoords warns but succeeds for unordered corners");
    expectTrue(trilinearReason == fuse::renderer::FroxelTrilinearSampleRejectReason::ClampableWeights,
               "unordered corners map to clampable_weights trilinear reject reason");
    ...
    expectNear(rejectedBilinear, 0.f, 1e-6f,
               "trySampleDensityBilinear with bilinear reason zeroes output on rejection");
    ...
    expectTrue(densityReason == fuse::renderer::GridDensityRejectReason::UndersizedStorage,
               "undersized storage reports undersized_storage density validation reject reason");
    expectTrue(std::strcmp(fuse::renderer::gridDensityRejectReasonLabel(densityReason), "undersized_storage") == 0,
               "undersized_storage density validation reject reason label");
    ...
    expectTrue(fuse::renderer::froxel_util::tryValidateGridDensityForDesc(emptyGrid, zeroDesc, densityReason),
               ...
               "empty desc density validation reports no reject reason");
    ...
    fuse::f32 screenSample = 0.f;
    fuse::renderer::FroxelTrilinearSampleRejectReason pipelineReason =
    expectTrue(fuse::renderer::froxel_util::trySampleDensityAtScreen(
                   grid, desc, camera, 0.25f, 0.25f, 3.16f, screenSample, pipelineReason),
               "trySampleDensityAtScreen with pipeline reason succeeds on accessible grid");
    expectTrue(pipelineReason == fuse::renderer::FroxelTrilinearSampleRejectReason::None,
               "successful screen sample reports no pipeline reject reason");
    expectNear(screenSample,
               fuse::renderer::froxel_util::sampleDensityAtScreen(grid, desc, camera, 0.25f, 0.25f, 3.16f),
               "trySampleDensityAtScreen with pipeline reason matches unguarded sample");

    fuse::f32 rejectedScreen = 1.f;
    expectTrue(!fuse::renderer::froxel_util::trySampleDensityAtScreen(
                   emptyGrid, desc, camera, 0.5f, 0.5f, 10.f, rejectedScreen, pipelineReason),
               "trySampleDensityAtScreen with pipeline reason rejects empty storage");
    expectTrue(pipelineReason == fuse::renderer::FroxelTrilinearSampleRejectReason::InaccessibleGrid,
               "empty storage reports inaccessible_grid pipeline reject reason");

                   grid, desc, camera, 0.5f, 0.5f, 0.01f, rejectedScreen, pipelineReason),
               "trySampleDensityAtScreen with pipeline reason rejects depth below near plane");
    expectTrue(pipelineReason == fuse::renderer::FroxelTrilinearSampleRejectReason::ScreenMappingFailed,
               "below-near depth reports screen_mapping_failed pipeline reject reason");
    expectTrue(std::strcmp(fuse::renderer::froxelTrilinearSampleRejectReasonLabel(pipelineReason),
                           "screen_mapping_failed") == 0,
               "screen_mapping_failed pipeline reject reason label");
    expectNear(rejectedScreen, 0.f, 1e-6f, "trySampleDensityAtScreen with pipeline reason zeroes output on rejection");

                   grid, zeroDesc, camera, 0.5f, 0.5f, 10.f, rejectedScreen, pipelineReason),
               "trySampleDensityAtScreen with pipeline reason rejects empty froxel desc");
    expectTrue(pipelineReason == fuse::renderer::FroxelTrilinearSampleRejectReason::EmptyGrid,
               "empty froxel desc reports empty_grid pipeline reject reason");
               "tryValidateGridDensityForDesc accepts accessible grid");
               "accessible grid reports no grid-density reject reason");
    ...
    expectTrue(densityReason == fuse::renderer::GridDensityRejectReason::DescMismatch,
               "desc mismatch reports desc_mismatch grid-density reject reason");
    expectTrue(std::strcmp(fuse::renderer::gridDensityRejectReasonLabel(densityReason), "desc_mismatch") == 0,
               "desc_mismatch grid-density reject reason label");

    fuse::renderer::FroxelDensityGrid corrupt{};
    corrupt.allocate(desc);
    corrupt.density[0] = 1.f;
    corrupt.density.pop_back();
    expectTrue(!fuse::renderer::froxel_util::tryValidateGridDensityForDesc(corrupt, desc, densityReason),
               "undersized storage fails tryValidateGridDensityForDesc");
               "undersized storage reports undersized_storage grid-density reject reason");
               "undersized_storage grid-density reject reason label");
...
void testFroxelCoordLookupAndDiagnosticGuards() {
    ...
    expectTrue(fuse::renderer::froxel_util::classifyDensityLookupAtCoord(grid, desc, 1u, 1u, 2u) ==
                   fuse::renderer::DensityLookupRejectReason::None,
               "classifyDensityLookupAtCoord none for in-range coords");
    expectTrue(fuse::renderer::froxel_util::classifyDensityLookupAtCoord(grid, desc, 99u, 99u, 99u) ==
                   fuse::renderer::DensityLookupRejectReason::IndexOutOfRange,
               "classifyDensityLookupAtCoord index_out_of_range for OOB coords");
    expectTrue(fuse::renderer::froxel_util::preflightDensityLookupAtCoord(grid, desc, 99u, 99u, 99u),
...
void testFroxelPreflightHelperGuards() {
    ...
    expectTrue(!fuse::renderer::froxel_util::shouldSkipTrilinearSample(grid, desc, warnWeights),
               "clampable weights do not skip trilinear sample");
    expectTrue(fuse::renderer::froxel_util::wouldClampTrilinearSample(grid, desc, warnWeights),
               "clampable weights would clamp before trilinear sample");
    expectTrue(!fuse::renderer::froxel_util::wouldClampTrilinearSample(grid, desc, inBounds),
               "in-bounds coords would not clamp trilinear sample");
    ...
    expectTrue(!fuse::renderer::froxel_util::tryPreflightTrilinearSample(grid, desc, hardOob, sampleReason),
               "tryPreflightTrilinearSample rejects hard OOB coords");
    expectTrue(fuse::renderer::froxel_util::shouldSkipTrilinearSample(grid, desc, hardOob),
               "hard OOB coords skip trilinear sample");
    expectTrue(fuse::renderer::froxel_util::wouldClampTrilinearSample(grid, desc, hardOob),
               "hard OOB coords would clamp before trilinear sample");
    expectTrue(fuse::renderer::froxel_util::shouldSkipTrilinearSample(emptyGrid, desc, inBounds),
               "empty storage skips trilinear sample");
    expectTrue(!fuse::renderer::froxel_util::wouldClampTrilinearSample(emptyGrid, desc, inBounds),
               "empty storage would not clamp trilinear sample");
...
void testFroxelClassifyBlockingAndPreflightGuards() {
    ...
    fuse::renderer::FroxelSampleCoords clamped = hardOob;
    fuse::renderer::SampleCoordRejectReason clampReason = fuse::renderer::SampleCoordRejectReason::None;
    expectTrue(fuse::renderer::FroxelGridLayout::tryClampSampleCoords(clamped, desc, clampReason),
               "tryClampSampleCoords with reason succeeds on non-empty grid");
    expectTrue(clampReason == fuse::renderer::SampleCoordRejectReason::None,
    ...
    expectTrue(!fuse::renderer::froxelTrilinearSampleRejectReasonIsBlocking(
                   fuse::renderer::FroxelTrilinearSampleRejectReason::None),
               "none trilinear sample reject is not blocking");
    expectTrue(fuse::renderer::froxel_util::classifyFroxelTrilinearSampleReject(grid, desc, warnWeights) ==
                   fuse::renderer::FroxelTrilinearSampleRejectReason::ClampableWeights,
               "classifyFroxelTrilinearSampleReject clampable_weights for OOB weights");
    expectTrue(!fuse::renderer::froxelTrilinearSampleRejectReasonIsBlocking(
                   fuse::renderer::FroxelTrilinearSampleRejectReason::ClampableWeights),
               "clampable_weights trilinear sample reject is not blocking");
    expectTrue(fuse::renderer::froxel_util::classifyFroxelTrilinearSampleReject(grid, desc, hardOob) ==
                   fuse::renderer::FroxelTrilinearSampleRejectReason::InvalidSampleCoords,
               "classifyFroxelTrilinearSampleReject invalid_sample_coords for hard OOB");
    expectTrue(fuse::renderer::froxelTrilinearSampleRejectReasonIsBlocking(
                   fuse::renderer::FroxelTrilinearSampleRejectReason::InvalidSampleCoords),
               "invalid_sample_coords trilinear sample reject is blocking");
    ...
    expectTrue(fuse::renderer::froxelPopulateRejectReasonIsBlocking(
                   fuse::renderer::FroxelPopulateRejectReason::ZeroDensity),
               "zero_density populate reject is blocking");
}
...
void testFroxelPreflightDeepenGuards() {
    ...
    expectTrue(!fuse::renderer::densityLookupRejectReasonIsBlocking(
                   fuse::renderer::DensityLookupRejectReason::None),
               "none lookup reject reason is not blocking");
    expectTrue(fuse::renderer::densityLookupRejectReasonIsBlocking(
                   fuse::renderer::DensityLookupRejectReason::EmptyStorage),
               "empty_storage lookup reject reason is blocking");
    ...
    expectTrue(!fuse::renderer::densityLookupRejectReasonIsBlocking(
                   fuse::renderer::DensityLookupRejectReason::IndexOutOfRange),
               "index_out_of_range lookup reject reason is not blocking");
    expectTrue(fuse::renderer::froxel_util::wouldClampDensityLookupIndex(999u, desc),
               "wouldClampDensityLookupIndex true for OOB index");
    ...
    expectTrue(fuse::renderer::FroxelGridLayout::preflightScreenDepthToSampleCoords(
                   0.25f, 0.25f, 10.f, desc, camera, &mappedCoords, &mapReason),
               "preflightScreenDepthToSampleCoords succeeds for valid screen depth");
    expectTrue(mapReason == fuse::renderer::ScreenMappingRejectReason::None,
               "valid screen mapping reports no reject reason");
    ...
    expectTrue(fuse::renderer::FroxelGridLayout::preflightScreenDepthToFroxelIndex(
                   0.25f, 0.25f, 10.f, desc, camera, &mappedIndex, &mapReason),
               "preflightScreenDepthToFroxelIndex succeeds for valid screen depth");
    expectTrue(mappedIndex == fuse::renderer::FroxelGridLayout::froxelIndex(
                                  mappedCoords.tileX0, mappedCoords.tileY0, mappedCoords.sliceZ0, desc),
    ...
    expectTrue(fuse::renderer::FroxelGridLayout::preflightScreenDepthToSampleCoords(
                   1.5f, 0.25f, 10.f, desc, camera, nullptr, &mapReason),
    ...
    expectTrue(mapReason == fuse::renderer::ScreenMappingRejectReason::ScreenCoordsOutOfRange,
               "OOB screen coords report screen_coords_out_of_range reason");
    expectTrue(!fuse::renderer::screenMappingRejectReasonIsBlocking(mapReason),
               "screen_coords_out_of_range is not blocking for preflight");
    expectTrue(std::strcmp(fuse::renderer::screenMappingRejectReasonLabel(mapReason),
                           "screen_coords_out_of_range") == 0,
               "screen_coords_out_of_range label");

    expectTrue(fuse::renderer::FroxelGridLayout::classifyScreenMappingReject(
                   0.5f, 0.5f, std::numeric_limits<fuse::f32>::quiet_NaN(), desc, camera) ==
                   fuse::renderer::ScreenMappingRejectReason::NonFiniteDepth,
               "classifyScreenMappingReject non_finite_depth for NaN depth");
    expectTrue(!fuse::renderer::FroxelGridLayout::preflightScreenDepthToSampleCoords(
                   0.5f, 0.5f, std::numeric_limits<fuse::f32>::quiet_NaN(), desc, camera, nullptr, &mapReason),
               "preflightScreenDepthToSampleCoords rejects NaN depth");
    expectTrue(fuse::renderer::screenMappingRejectReasonIsBlocking(mapReason),
               "non_finite_depth is blocking for preflight");
    ...
    expectTrue(fuse::renderer::froxel_util::wouldSkipScreenDensitySample(
                   grid, desc, camera, 0.5f, 0.5f, 0.01f),
    ...
    fuse::renderer::FroxelDensityGrid undersized{};
    undersized.density.assign(desc.froxelCount() - 1u, 0.f);
    expectTrue(fuse::renderer::froxel_util::classifyGridDensityReject(undersized, desc) ==
                   fuse::renderer::GridDensityRejectReason::UndersizedStorage,
...
void testFroxelClassifyPreflightAndWouldSkipGuards() {
    ...
    expectTrue(!fuse::renderer::screenMappingRejectReasonIsBlocking(fuse::renderer::ScreenMappingRejectReason::None),
               "none screen-mapping reject is non-blocking");
    expectTrue(fuse::renderer::screenMappingRejectReasonIsBlocking(
                   fuse::renderer::ScreenMappingRejectReason::DepthOutOfRange),
               "depth_out_of_range screen-mapping reject is blocking");
    expectTrue(!fuse::renderer::sampleCoordRejectReasonIsBlocking(fuse::renderer::SampleCoordRejectReason::None),
    ...
    expectTrue(!fuse::renderer::densityLookupRejectReasonIsBlocking(
                   fuse::renderer::DensityLookupRejectReason::IndexOutOfRange),
               "index_out_of_range lookup reject is non-blocking");
    expectTrue(fuse::renderer::densityLookupRejectReasonIsBlocking(
                   fuse::renderer::DensityLookupRejectReason::EmptyStorage),
               "empty_storage lookup reject is blocking");
    ...
    expectTrue(!fuse::renderer::froxelTrilinearSampleRejectReasonIsBlocking(
                   fuse::renderer::FroxelTrilinearSampleRejectReason::None),
               "none trilinear sample reject is non-blocking");
    expectTrue(fuse::renderer::froxelTrilinearSampleRejectReasonIsBlocking(
                   fuse::renderer::FroxelTrilinearSampleRejectReason::InaccessibleGrid),
               "inaccessible_grid trilinear sample reject is blocking");
    expectTrue(std::strcmp(fuse::renderer::froxelTrilinearSampleRejectReasonLabel(
                               fuse::renderer::FroxelTrilinearSampleRejectReason::InvalidSampleCoords),
                           "invalid_sample_coords") == 0,
               "invalid_sample_coords trilinear reject reason label");
    ...
    expectTrue(densityReason == fuse::renderer::GridDensityRejectReason::None,
               "accessible grid classified as none");
    expectTrue(!fuse::renderer::froxel_util::wouldSkipGridDensityValidation(grid, desc),
               "accessible grid does not skip density validation");
    ...
    expectTrue(fuse::renderer::froxel_util::wouldSkipGridDensityValidation(undersized, desc),
               "undersized storage skips density validation");
    ...
    expectTrue(trilinearReason == fuse::renderer::FroxelTrilinearSampleRejectReason::None,
               "successful trilinear preflight reports none");
    expectTrue(!fuse::renderer::froxel_util::wouldSkipTrilinearSample(grid, desc, inBounds),
               "in-bounds trilinear sample does not skip");
    ...
    expectTrue(trilinearReason == fuse::renderer::FroxelTrilinearSampleRejectReason::InaccessibleGrid,
               "empty storage trilinear reject reports inaccessible_grid");

    expectTrue(fuse::renderer::froxel_util::preflightScreenDensitySample(
                   grid, desc, camera, 0.25f, 0.25f, 3.16f, &mapReason),
               "preflightScreenDensitySample succeeds on accessible grid");
    expectTrue(mapReason == fuse::renderer::ScreenMappingRejectReason::None,
               "successful screen density preflight reports none");
    expectTrue(!fuse::renderer::froxel_util::wouldSkipScreenDensitySample(
                   grid, desc, camera, 0.25f, 0.25f, 3.16f),
               "in-range screen density sample does not skip");
    ...
    expectTrue(fuse::renderer::froxel_util::wouldSkipScreenDensitySample(
                   emptyGrid, desc, camera, 0.5f, 0.5f, 10.f),
               "empty storage skips screen density sample");

    fuse::renderer::FroxelPopulateRejectReason populateReason = fuse::renderer::FroxelPopulateRejectReason::None;
    expectTrue(fuse::renderer::froxel_util::preflightFroxelPopulate(desc, camera, params, &populateReason),
               "preflightFroxelPopulate succeeds for valid inputs");
    expectTrue(populateReason == fuse::renderer::FroxelPopulateRejectReason::None,
               "valid populate classified as none");
    expectTrue(!fuse::renderer::froxel_util::wouldSkipFroxelPopulate(desc, camera, params),
               "valid populate does not skip");
    ...
    expectTrue(!fuse::renderer::froxel_util::preflightFroxelPopulate(desc, camera, zeroDensity, &populateReason),
               "preflightFroxelPopulate rejects zero density");
    expectTrue(populateReason == fuse::renderer::FroxelPopulateRejectReason::ZeroDensity,
               "rejected populate classified as zero_density");
    ...
    fuse::renderer::FroxelDensityGrid zeroMarchGrid{};
    zeroMarchGrid.allocate(desc);
...
void testFroxelGridDensityPreflightTrilinearAndPopulateGuards() {
    ...
    expectTrue(fuse::renderer::froxel_util::countNonZeroFroxelsForDesc(grid, desc) == 1u,
               "countNonZeroFroxelsForDesc counts written froxels");
    expectTrue(fuse::renderer::froxel_util::countEmptyFroxelsForDesc(grid, desc) == desc.froxelCount() - 1u,
               "countEmptyFroxelsForDesc partitions remaining froxels");
    expectTrue(!fuse::renderer::froxel_util::isDensityFullyEmpty(grid, desc),
               "partially filled grid is not fully empty");
    ...
    expectTrue(densityReason == fuse::renderer::GridDensityRejectReason::DescMismatch,
               "desc mismatch reports desc_mismatch density reject reason");
    expectTrue(fuse::renderer::froxel_util::countNonZeroFroxelsForDesc(grid, mismatched) == 0u,
               "countNonZeroFroxelsForDesc returns zero on mismatch");
    expectTrue(fuse::renderer::froxel_util::countEmptyFroxelsForDesc(grid, mismatched) == mismatched.froxelCount(),
               "countEmptyFroxelsForDesc returns full count on mismatch");
    ...
    expectTrue(fuse::renderer::froxel_util::tryValidateGridDensityForDesc(grid, zeroDesc, densityReason),
               "tryValidateGridDensityForDesc vacuously succeeds on empty desc");
    expectTrue(!fuse::renderer::froxel_util::isDensityFullyEmpty(grid, mismatched),
               "isDensityFullyEmpty false when grid is inaccessible");

    fuse::renderer::FroxelCameraDesc camera{};
    camera.nearPlane = 1.f;
    camera.farPlane = 100.f;

    expectTrue(fuse::renderer::FroxelGridLayout::wouldClampScreenCoords(-0.5f, 1.5f),
               "wouldClampScreenCoords true for OOB screen coords");
    expectTrue(!fuse::renderer::FroxelGridLayout::wouldClampScreenCoords(0.5f, 0.5f),
               "wouldClampScreenCoords false for in-range screen coords");
    expectTrue(fuse::renderer::FroxelGridLayout::canMapScreenDepthToSampleCoords(
                   0.5f, 0.5f, 10.f, desc, camera),
               "canMapScreenDepthToSampleCoords succeeds in range");
    expectTrue(fuse::renderer::FroxelGridLayout::canMapScreenDepthToFroxelIndex(
                   0.5f, 0.5f, 10.f, desc, camera),
               "canMapScreenDepthToFroxelIndex succeeds in range");
    expectTrue(!fuse::renderer::FroxelGridLayout::canMapScreenDepthToSampleCoords(
                   0.5f, 0.5f, 0.01f, desc, camera),
               "canMapScreenDepthToSampleCoords rejects depth below near plane");
    expectTrue(!fuse::renderer::FroxelGridLayout::canMapScreenDepthToFroxelIndex(
                   0.5f, 0.5f, 200.f, desc, camera),
               "canMapScreenDepthToFroxelIndex rejects depth above far plane");
    ...
    expectTrue(sampleReason == fuse::renderer::SampleCoordRejectReason::InvalidWeights,
               "clampable trilinear weights report invalid_weights reject reason");
    expectTrue(fuse::renderer::froxel_util::wouldClampTrilinearSample(grid, desc, warnWeights),
               "wouldClampTrilinearSample true for clampable weights");
    ...
    expectTrue(!fuse::renderer::froxel_util::tryCanSampleTrilinearAtCoords(grid, desc, hardOob, sampleReason),
               "tryCanSampleTrilinearAtCoords rejects hard OOB tile coord");
    expectTrue(sampleReason == fuse::renderer::SampleCoordRejectReason::OutOfBounds,
               "hard OOB trilinear preflight reports out_of_bounds reject reason");
    expectTrue(fuse::renderer::froxel_util::wouldClampTrilinearSample(grid, desc, hardOob),
               "wouldClampTrilinearSample true for hard OOB coords");

    fuse::renderer::FroxelDensityGrid emptyGrid{};
    expectTrue(!fuse::renderer::froxel_util::canSampleTrilinearAtCoords(emptyGrid, desc, inBounds),
               "canSampleTrilinearAtCoords rejects empty storage");
    expectTrue(!fuse::renderer::froxel_util::wouldClampTrilinearSample(emptyGrid, desc, inBounds),
               "wouldClampTrilinearSample false when grid is inaccessible");
    ...
    fuse::renderer::FroxelDensityGrid populated{};
    fuse::renderer::froxel_util::populateFromAnalyticFog(populated, desc, camera, params);
    expectTrue(fuse::renderer::froxel_util::validatePopulateResult(populated, desc, params),
               "validatePopulateResult succeeds after analytic populate");
    expectTrue(fuse::renderer::froxel_util::countNonZeroFroxelsForDesc(populated, desc) == desc.froxelCount(),
               "populated grid reports all froxels non-zero for valid fill path");

    fuse::renderer::VolumetricFogParams zeroDensity{};
    zeroDensity.density = 0.f;
    fuse::renderer::FroxelDensityGrid skipped{};
    fuse::renderer::froxel_util::populateFromAnalyticFog(skipped, desc, camera, zeroDensity);
    expectTrue(fuse::renderer::froxel_util::validatePopulateResult(skipped, desc, zeroDensity),
               "validatePopulateResult succeeds after skipped populate fill");
    expectTrue(fuse::renderer::froxel_util::isDensityFullyEmpty(skipped, desc),
               "skipped populate leaves fully empty density grid");
}
...
void testFroxelBilinearScreenMappingAndGridDensityGuards() {
    ...
    expectTrue(!fuse::renderer::sampleCoordRejectReasonIsBlocking(
                   fuse::renderer::SampleCoordRejectReason::InvalidWeights),
               "invalid_weights sample reject reason is not blocking");
    expectTrue(fuse::renderer::sampleCoordRejectReasonIsBlocking(
                   fuse::renderer::SampleCoordRejectReason::OutOfBounds),
               "out_of_bounds sample reject reason is blocking");
    expectTrue(!fuse::renderer::densityLookupRejectReasonIsBlocking(
                   fuse::renderer::DensityLookupRejectReason::IndexOutOfRange),
               "index_out_of_range lookup reject reason is not blocking");
    expectTrue(fuse::renderer::densityLookupRejectReasonIsBlocking(
    ...
    expectTrue(!fuse::renderer::gridDensityRejectReasonIsBlocking(
                   fuse::renderer::GridDensityRejectReason::EmptyDesc),
               "empty_desc grid density reject reason is not blocking");
    expectTrue(fuse::renderer::gridDensityRejectReasonIsBlocking(
                   fuse::renderer::GridDensityRejectReason::UndersizedStorage),
               "undersized_storage grid density reject reason is blocking");
    expectTrue(fuse::renderer::froxelPopulateRejectReasonIsBlocking(
    ...
    expectTrue(!fuse::renderer::froxelBilinearSampleRejectReasonIsBlocking(bilinearReason),
               "none bilinear reject reason is not blocking");
    ...
    expectTrue(bilinearReason == fuse::renderer::FroxelBilinearSampleRejectReason::ClampableWeights,
               "clampable weights report clampable_weights bilinear reject reason");
    expectTrue(!fuse::renderer::froxelBilinearSampleRejectReasonIsBlocking(bilinearReason),
               "clampable_weights bilinear reject reason is not blocking");
    expectTrue(!fuse::renderer::froxel_util::wouldSkipDensityBilinearSample(grid, desc, warnWeights),
               "clampable weights do not skip bilinear sample");
    ...
    expectTrue(bilinearReason == fuse::renderer::FroxelBilinearSampleRejectReason::InvalidSampleCoords,
               "hard OOB tile coord reports invalid_sample_coords bilinear reject reason");
    expectTrue(fuse::renderer::froxelBilinearSampleRejectReasonIsBlocking(bilinearReason),
               "invalid_sample_coords bilinear reject reason is blocking");
    expectTrue(fuse::renderer::froxel_util::wouldSkipDensityBilinearSample(grid, desc, hardOob),
               "hard OOB coords skip bilinear sample");
    ...
    expectTrue(!fuse::renderer::FroxelGridLayout::wouldSkipScreenDepthMapping(
                   0.5f, 0.5f, 10.f, desc, camera),
               "in-range screen depth does not skip mapping");
    expectTrue(fuse::renderer::FroxelGridLayout::wouldSkipScreenDepthMapping(
                   0.5f, 0.5f, 0.01f, desc, camera),
               "below-near depth skips screen mapping");
    expectTrue(!fuse::renderer::FroxelGridLayout::wouldSkipScreenDepthToFroxelIndex(
                   0.5f, 0.5f, 10.f, desc, camera),
               "in-range screen depth does not skip froxel index mapping");
    expectTrue(fuse::renderer::FroxelGridLayout::wouldSkipScreenDepthToFroxelIndex(
                   0.5f, 0.5f, 200.f, desc, camera),
               "above-far depth skips froxel index mapping");

    fuse::renderer::FroxelGridDesc zeroDesc{};
    zeroDesc.tilesX = 0u;
    expectTrue(fuse::renderer::FroxelGridLayout::wouldSkipScreenDepthMapping(
                   0.5f, 0.5f, 10.f, zeroDesc, camera),
               "empty grid skips screen depth mapping");
    expectTrue(fuse::renderer::FroxelGridLayout::wouldSkipScreenDepthToFroxelIndex(
                   0.5f, 0.5f, 10.f, zeroDesc, camera),
               "empty grid skips screen depth to froxel index mapping");
    ...
    expectTrue(densityReason == fuse::renderer::GridDensityRejectReason::None,
               "accessible grid reports no grid density reject reason");
    expectTrue(!fuse::renderer::froxel_util::wouldSkipGridDensityValidation(grid, desc),
               "accessible grid does not skip grid density validation");
    ...
    expectTrue(densityReason == fuse::renderer::GridDensityRejectReason::UndersizedStorage,
               "undersized storage reports undersized_storage grid density reject reason");
    expectTrue(fuse::renderer::froxel_util::wouldSkipGridDensityValidation(undersized, desc),
               "undersized storage skips grid density validation");
    ...
    expectTrue(!fuse::renderer::froxel_util::wouldSkipGridDensityValidation(emptyGrid, zeroDesc),
               "empty desc does not skip grid density validation");
}
...
void testFroxelPreflightClassifyAndIsBlockingGuards() {
    ...
    expectTrue(fuse::renderer::froxelPopulateRejectReasonIsBlocking(
                   fuse::renderer::FroxelPopulateRejectReason::EmptyDesc),
               "empty_desc populate reject reason is blocking");
...
void testFroxelClassifyAndIsBlockingGuards() {
    ...
    expectTrue(!fuse::renderer::sampleCoordRejectReasonIsBlocking(
                   fuse::renderer::SampleCoordRejectReason::InvalidWeights),
               "InvalidWeights sample-coord reject reason is not blocking");
    expectTrue(fuse::renderer::sampleCoordRejectReasonIsBlocking(
                   fuse::renderer::SampleCoordRejectReason::EmptyGrid),
               "EmptyGrid sample-coord reject reason is blocking");
    expectTrue(fuse::renderer::sampleCoordRejectReasonIsBlocking(
                   fuse::renderer::SampleCoordRejectReason::OutOfBounds),
               "OutOfBounds sample-coord reject reason is blocking");
    ...
    expectTrue(!fuse::renderer::densityLookupRejectReasonIsBlocking(
                   fuse::renderer::DensityLookupRejectReason::IndexOutOfRange),
               "IndexOutOfRange density lookup reject reason is not blocking");
    expectTrue(fuse::renderer::densityLookupRejectReasonIsBlocking(
                   fuse::renderer::DensityLookupRejectReason::EmptyStorage),
               "EmptyStorage density lookup reject reason is blocking");
    ...
    expectTrue(!fuse::renderer::froxelTrilinearSampleRejectReasonIsBlocking(
                   fuse::renderer::FroxelTrilinearSampleRejectReason::ClampableWeights),
               "ClampableWeights trilinear reject reason is not blocking");
    expectTrue(fuse::renderer::froxelTrilinearSampleRejectReasonIsBlocking(
                   fuse::renderer::FroxelTrilinearSampleRejectReason::InvalidSampleCoords),
               "InvalidSampleCoords trilinear reject reason is blocking");
    ...
    expectTrue(fuse::renderer::screenMappingRejectReasonIsBlocking(
                   fuse::renderer::ScreenMappingRejectReason::DepthOutOfRange),
               "DepthOutOfRange screen-mapping reject reason is blocking");

    expectTrue(!fuse::renderer::gridDensityRejectReasonIsBlocking(fuse::renderer::GridDensityRejectReason::None),
               "None grid-density reject reason is not blocking");
    expectTrue(fuse::renderer::gridDensityRejectReasonIsBlocking(
                   fuse::renderer::GridDensityRejectReason::UndersizedStorage),
               "UndersizedStorage grid-density reject reason is blocking");
    ...
    expectTrue(fuse::renderer::froxelPopulateRejectReasonIsBlocking(
                   fuse::renderer::FroxelPopulateRejectReason::ZeroDensity),
               "ZeroDensity populate reject reason is blocking");
...
void testFroxelClassifyPreflightAndIsBlockingGuards() {
    ...
    expectTrue(!fuse::renderer::froxelTrilinearSampleRejectReasonIsBlocking(
                   fuse::renderer::FroxelTrilinearSampleRejectReason::None),
               "none trilinear sample reject reason is not blocking");

    fuse::renderer::FroxelSampleCoords warnTrilinear = inBounds;
    warnTrilinear.tx = 2.f;
...
void testFroxelClassifyAndBlockingGuards() {
    ...
    expectTrue(fuse::renderer::froxel_util::preflightGridDensity(grid, desc) ==
                   fuse::renderer::froxel_util::validateGridDensity(grid, desc),
...
void testFroxelClassifyPreflightAndIsBlockingGuards() {
    ...
    expectTrue(fuse::renderer::FroxelGridLayout::preflightMapScreenDepthToSampleCoords(
                   0.5f, 0.5f, 10.f, desc, camera),
               "preflightMapScreenDepthToSampleCoords succeeds for valid mapping");
    expectTrue(!fuse::renderer::screenMappingRejectReasonIsBlocking(
                   fuse::renderer::ScreenMappingRejectReason::None),
               "none screen-mapping reject reason is not blocking");
    expectTrue(fuse::renderer::screenMappingRejectReasonIsBlocking(
    ...
    fuse::renderer::ScreenMappingRejectReason mapReason = fuse::renderer::ScreenMappingRejectReason::None;
    expectTrue(fuse::renderer::FroxelGridLayout::preflightMapScreenDepthToSampleCoords(
                   0.25f, 0.5f, 10.f, desc, camera, &mapped, &mapReason),
               "preflightMapScreenDepthToSampleCoords returns mapped coords");
    expectTrue(mapReason == fuse::renderer::ScreenMappingRejectReason::None,
               "preflightMapScreenDepthToSampleCoords reports no reject reason");
    expectTrue(mapped.sliceZ0 < desc.slicesZ, "preflightMapScreenDepthToSampleCoords fills sample coords");
    ...
    expectTrue(!fuse::renderer::FroxelGridLayout::preflightMapScreenDepthToSampleCoords(
                   0.5f, 0.5f, 10.f, zeroDesc, camera),
               "preflightMapScreenDepthToSampleCoords rejects empty desc");
    ...
    expectTrue(fuse::renderer::froxel_util::classifyFroxelPopulateReject(zeroDesc, camera, params) ==
                   fuse::renderer::FroxelPopulateRejectReason::EmptyDesc,
               "classifyFroxelPopulateReject empty_desc for empty froxel desc");
    expectTrue(!fuse::renderer::froxel_util::preflightFroxelPopulate(zeroDesc, camera, params),
               "preflightFroxelPopulate rejects empty froxel desc");
...
void testFroxelClassifyRejectAndPreflightReadyGuards() {
    ...
    fuse::renderer::FroxelSampleCoords warnWeights = inBounds;
    warnWeights.tx = 2.f;
    fuse::renderer::SampleCoordRejectReason sampleReason =
    ...
    expectTrue(!fuse::renderer::gridDensityRejectReasonIsBlocking(
               ...
               "empty desc grid-density validation is not blocking");
...
void testFroxelClassifyAndPreflightGuards() {
    ...
    expectTrue(!fuse::renderer::sampleCoordRejectReasonIsBlocking(
                   fuse::renderer::SampleCoordRejectReason::None),
               "none sample reject reason is not blocking");
    ...
    expectTrue(fuse::renderer::froxelTrilinearSampleRejectReasonIsBlocking(
                   fuse::renderer::FroxelTrilinearSampleRejectReason::InaccessibleGrid),
               "inaccessible_grid trilinear reject reason is blocking");
...
void testFroxelClassifyAndIsBlockingGuards() {
    ...
    expectTrue(fuse::renderer::froxel_util::classifyDensityLookupReject(grid, zeroDesc, 0u) ==
                   fuse::renderer::DensityLookupRejectReason::EmptyGrid,
...
void testFroxelClassifyAndBlockingGuards() {
    ...
    expectTrue(fuse::renderer::densityLookupRejectReasonIsBlocking(fuse::renderer::DensityLookupRejectReason::EmptyGrid),
               "EmptyGrid density lookup reject reason is blocking");
    expectTrue(!fuse::renderer::densityLookupRejectReasonIsBlocking(
                   fuse::renderer::DensityLookupRejectReason::IndexOutOfRange),
               "IndexOutOfRange density lookup reject reason is not blocking");
    ...
    expectTrue(fuse::renderer::froxel_util::classifyDensityLookupAtIndex(grid, desc, 0u) ==
                   fuse::renderer::DensityLookupRejectReason::None,
               "classifyDensityLookupAtIndex none for accessible grid");
    expectTrue(fuse::renderer::froxel_util::classifyDensityLookupAtIndex(grid, desc, 999u) ==
                   fuse::renderer::DensityLookupRejectReason::IndexOutOfRange,
               "classifyDensityLookupAtIndex index_out_of_range for OOB index");
    expectTrue(fuse::renderer::froxel_util::classifyDensityLookupAtCoord(grid, desc, 1u, 1u, 2u) ==
...
void testFroxelClassifyAndIsBlockingGuards() {
    ...
    expectTrue(!fuse::renderer::densityLookupRejectReasonIsBlocking(
                   fuse::renderer::DensityLookupRejectReason::IndexOutOfRange),
               "index_out_of_range lookup reject is not blocking");
    expectTrue(fuse::renderer::densityLookupRejectReasonIsBlocking(
                   fuse::renderer::DensityLookupRejectReason::EmptyStorage),
               "empty_storage lookup reject is blocking");
...
void testFroxelClassifyAndIsBlockingGuards() {
    ...
    expectTrue(fuse::renderer::sampleCoordRejectReasonIsBlocking(SampleCoordRejectReason::OutOfBounds),
               "out_of_bounds sample-coord reject reason is blocking");
    expectTrue(!fuse::renderer::froxelTrilinearSampleRejectReasonIsBlocking(
                   FroxelTrilinearSampleRejectReason::ClampableWeights),
               "clampable_weights trilinear reject reason is not blocking");
    expectTrue(fuse::renderer::froxelTrilinearSampleRejectReasonIsBlocking(
    ...
    inBounds.tz = 0.5f;
    expectTrue(fuse::renderer::FroxelGridLayout::classifySampleCoordReject(inBounds, desc) ==
                   SampleCoordRejectReason::None,
               "classifySampleCoordReject reports none for in-bounds coords");

    fuse::renderer::FroxelSampleCoords warnWeights = inBounds;
    warnWeights.tx = 2.f;
    expectTrue(fuse::renderer::FroxelGridLayout::classifySampleCoordReject(warnWeights, desc) ==
                   SampleCoordRejectReason::InvalidWeights,
               "classifySampleCoordReject reports invalid_weights for clampable weights");
    ...
    expectTrue(fuse::renderer::froxel_util::classifyDensityLookupReject(grid, desc, 0u) ==
                   DensityLookupRejectReason::None,
               "classifyDensityLookupReject reports none for origin index");
    expectTrue(fuse::renderer::froxel_util::classifyDensityLookupReject(grid, desc, 999u) ==
                   DensityLookupRejectReason::IndexOutOfRange,
               "classifyDensityLookupReject reports index_out_of_range for OOB index");
    expectTrue(fuse::renderer::froxel_util::classifyDensityLookupCoordReject(grid, desc, 99u, 99u, 99u) ==
                   DensityLookupRejectReason::IndexOutOfRange,
    ...
    fuse::renderer::FroxelDensityGrid emptyGrid{};
    expectTrue(fuse::renderer::froxel_util::classifyDensityLookupReject(emptyGrid, desc, 0u) ==
                   DensityLookupRejectReason::EmptyStorage,
    ...
    undersized.density.resize(desc.froxelCount() - 1u, 0.f);
    expectTrue(fuse::renderer::froxel_util::classifyGridDensityReject(undersized, desc) ==
                   GridDensityRejectReason::UndersizedStorage,
               "classifyGridDensityReject reports undersized_storage");
    ...
    params.march_steps = 32u;
    expectTrue(fuse::renderer::froxel_util::classifyFroxelPopulateReject(desc, camera, params) ==
                   FroxelPopulateRejectReason::None,
               "classifyFroxelPopulateReject reports none for valid inputs");
    ...
    zeroDensity.density = 0.f;
    expectTrue(fuse::renderer::froxel_util::classifyFroxelPopulateReject(desc, camera, zeroDensity) ==
                   FroxelPopulateRejectReason::ZeroDensity,
               "classifyFroxelPopulateReject reports zero_density");
    expectTrue(!fuse::renderer::froxel_util::preflightFroxelPopulate(desc, camera, zeroDensity, &populateReason),
               "preflightFroxelPopulate rejects zero density");
    ...
    expectTrue(fuse::renderer::FroxelGridLayout::tryPreflightSampleCoords(inBounds, desc, sampleReason) ==
                   fuse::renderer::FroxelGridLayout::preflightSampleCoords(inBounds, desc, &sampleReason),
               "tryPreflightSampleCoords agrees with preflightSampleCoords");
    expectTrue(fuse::renderer::froxel_util::tryCanLookupAtIndex(grid, desc, 5u, lookupReason) ==
                   fuse::renderer::froxel_util::preflightDensityLookup(grid, desc, 5u, &lookupReason),
               "tryCanLookupAtIndex agrees with preflightDensityLookup");
    expectTrue(fuse::renderer::froxel_util::tryCanPopulateFromAnalyticFog(desc, camera, params, populateReason) ==
                   fuse::renderer::froxel_util::preflightFroxelPopulate(desc, camera, params, &populateReason),
...
void testFroxelRejectClassifyAndPreflightGuards() {
    ...
    expectTrue(fuse::renderer::froxel_util::preflightDensityLookup(grid, desc, 0u),
               "preflightDensityLookup succeeds on accessible grid");
    expectTrue(fuse::renderer::froxel_util::preflightDensityLookup(grid, desc, 999u),
               "preflightDensityLookup succeeds when index clamps");
    ...
    expectTrue(fuse::renderer::froxel_util::preflightDensityLookupAtCoord(grid, desc, 1u, 1u, 2u, &lookupReason),
               ...
               "preflightDensityLookupAtCoord reports no reject reason");
    expectTrue(fuse::renderer::froxel_util::preflightDensityLookupAtCoord(grid, desc, 99u, 99u, 99u, &lookupReason),
               "preflightDensityLookupAtCoord succeeds when coords clamp");
               "preflightDensityLookupAtCoord reports index_out_of_range for OOB coords");

               "invalid_weights sample reject is not blocking");
               "out_of_bounds sample reject is blocking");
    ...
    expectTrue(!fuse::renderer::screenMappingRejectReasonIsBlocking(fuse::renderer::ScreenMappingRejectReason::None),
               "none screen-mapping reject is non-blocking");
               "empty_grid screen-mapping reject is blocking");

               "none sample-coord reject is non-blocking");
               "invalid_weights sample-coord reject is non-blocking");
               "out_of_bounds sample-coord reject is blocking");

               "none density lookup reject is non-blocking");
               "index_out_of_range density lookup reject is non-blocking");
    ...
    expectTrue(!fuse::renderer::gridDensityRejectReasonIsBlocking(fuse::renderer::GridDensityRejectReason::EmptyDesc),
               ...
               "none populate reject is non-blocking");
               "zero_density populate reject is blocking");
    ...
    expectTrue(fuse::renderer::froxel_util::wouldSkipFroxelMarch(grid, desc) ==
                   fuse::renderer::froxel_util::shouldSkipFroxelMarch(grid, desc),
    ...
    expectTrue(fuse::renderer::classifyScreenMappingReject(0.5f, 0.5f, 0.01f, desc, camera) ==
                   fuse::renderer::ScreenMappingRejectReason::DepthOutOfRange,
               "classifyScreenMappingReject depth_out_of_range for below-near depth");
    expectTrue(std::strcmp(fuse::renderer::screenMappingRejectReasonLabel(
                               fuse::renderer::ScreenMappingRejectReason::InvalidCamera),
                           "invalid_camera") == 0,
               "invalid_camera screen-mapping reject reason label");
                           "depth_out_of_range") == 0,
               "depth_out_of_range screen-mapping reject reason label");
    expectTrue(!fuse::renderer::wouldSkipScreenMapping(0.5f, 0.5f, 10.f, desc, camera),
               "valid screen mapping does not skip");
    ...
    expectTrue(!fuse::renderer::densityLookupRejectReasonIsBlocking(
                   fuse::renderer::DensityLookupRejectReason::None),
               "none lookup reject is non-blocking");
                   fuse::renderer::DensityLookupRejectReason::IndexOutOfRange),
               "index_out_of_range lookup reject is non-blocking");
    expectTrue(fuse::renderer::densityLookupRejectReasonIsBlocking(
                   fuse::renderer::DensityLookupRejectReason::EmptyStorage),
    ...
    expectTrue(fuse::renderer::classifyFroxelPopulateReject(desc, camera, params) ==
                   fuse::renderer::FroxelPopulateRejectReason::None,
               "classifyFroxelPopulateReject none for valid inputs");
    expectTrue(std::strcmp(fuse::renderer::froxelPopulateRejectReasonLabel(
                               fuse::renderer::FroxelPopulateRejectReason::EmptyDesc),
               "empty_desc populate reject reason label");
                               fuse::renderer::FroxelPopulateRejectReason::ZeroMarchSteps),
                           "zero_march_steps") == 0,
               "zero_march_steps populate reject reason label");
    ...
    expectTrue(fuse::renderer::FroxelGridLayout::classifySampleCoordReject(hardOob, desc) ==
               ...
               "invalid_weights is not blocking for sample preflight");
               "out_of_bounds is blocking for sample preflight");
    ...
    expectTrue(fuse::renderer::FroxelGridLayout::preflightScreenDepthToFroxelIndex(
               ...
               "successful screen preflight reports no reject reason");
    ...
    fuse::u32 froxelIndex = 0u;
                   0.5f, 0.5f, 10.f, desc, camera, &froxelIndex, &mapReason),
    expectTrue(froxelIndex < desc.froxelCount(), "preflight mapped froxel index in bounds");
    expectTrue(fuse::renderer::FroxelGridLayout::preflightScreenMapping(0.5f, 0.5f, 10.f, desc, camera),
               "preflightScreenMapping succeeds for in-range depth");
    expectTrue(!fuse::renderer::FroxelGridLayout::preflightScreenMapping(0.5f, 0.5f, 0.01f, desc, camera),
               "preflightScreenMapping rejects depth below near plane");
    ...
    expectTrue(fuse::renderer::froxel_util::classifyFroxelSampleReject(grid, desc, inBounds) ==
               "classifyFroxelSampleReject none for accessible grid");
    expectTrue(fuse::renderer::froxel_util::preflightFroxelSampleAtCoords(grid, desc, inBounds, &sampleReason),
               "preflightFroxelSampleAtCoords succeeds on accessible grid");
               "preflightFroxelSampleAtCoords reports no reject reason");
    expectTrue(fuse::renderer::froxel_util::classifyFroxelSampleReject(emptyGrid, desc, inBounds) ==
               "classifyFroxelSampleReject out_of_bounds for empty storage");
    expectTrue(!fuse::renderer::froxel_util::preflightFroxelSampleAtCoords(emptyGrid, desc, inBounds),
    ...
    expectTrue(fuse::renderer::FroxelGridLayout::preflightScreenMapping(
                   0.25f, 0.25f, 3.16f, desc, camera, &mapped),
    ...
    expectTrue(fuse::renderer::FroxelGridLayout::classifyScreenDepthMappingReject(
               "classifyScreenDepthMappingReject none for valid mapping");
                   0.5f, 0.5f, 10.f, desc, camera, &mapped),
               "preflightScreenDepthToSampleCoords succeeds for valid mapping");
    expectTrue(mapped.tileX0 < desc.tilesX && mapped.tileY0 < desc.tilesY,
                   0.5f, 0.5f, 0.01f, desc, camera) ==
    ...
    expectTrue(fuse::renderer::froxel_util::classifyDensityLookupAtCoord(emptyGrid, desc, 0u, 0u, 0u) ==
               "classifyDensityLookupAtCoord empty_storage");
    expectTrue(!fuse::renderer::froxel_util::preflightDensityLookupAtCoord(emptyGrid, desc, 0u, 0u, 0u),
               "preflightDensityLookupAtCoord rejects empty storage");
    camera.screenWidth = 1920u;
    camera.screenHeight = 1080u;
    expectTrue(fuse::renderer::isValidFroxelCamera(camera), "valid froxel camera passes validation");
    fuse::renderer::FroxelCameraRejectReason cameraReason = fuse::renderer::FroxelCameraRejectReason::None;
    expectTrue(fuse::renderer::tryValidateFroxelCamera(camera, cameraReason),
               "tryValidateFroxelCamera succeeds for valid camera");
    expectTrue(cameraReason == fuse::renderer::FroxelCameraRejectReason::None,
               "valid camera reports no reject reason");

    fuse::renderer::FroxelCameraDesc badPlanes{};
    badPlanes.nearPlane = 100.f;
    badPlanes.farPlane = 1.f;
    badPlanes.screenWidth = 1920u;
    badPlanes.screenHeight = 1080u;
    ...
    expectTrue(cameraReason == fuse::renderer::FroxelCameraRejectReason::InvalidPlanes,
               "invalid planes report invalid_planes reject reason");
    expectTrue(std::strcmp(fuse::renderer::froxelCameraRejectReasonLabel(cameraReason), "invalid_planes") == 0,
               "invalid_planes camera reject reason label");

    fuse::renderer::FroxelCameraDesc zeroScreen = camera;
    zeroScreen.screenWidth = 0u;
    ...
    expectTrue(cameraReason == fuse::renderer::FroxelCameraRejectReason::ZeroScreenDimensions,
               "zero screen width reports zero_screen_dimensions reject reason");
    expectTrue(fuse::renderer::froxelCameraRejectReasonIsBlocking(cameraReason),
               "zero screen dimensions is blocking for camera validation");
    ...
    expectTrue(lookupReason == fuse::renderer::DensityLookupRejectReason::IndexOutOfRange,
               "OOB index preflight reports index_out_of_range");
    expectTrue(!fuse::renderer::densityLookupRejectReasonIsBlocking(lookupReason),
               "index_out_of_range is not blocking for lookup preflight");

    expectTrue(!fuse::renderer::froxel_util::preflightDensityLookupAtIndex(emptyGrid, desc, 0u),
               "preflightDensityLookupAtIndex rejects empty storage");
               "empty_storage is blocking for lookup preflight");
    ...
    expectTrue(fuse::renderer::FroxelGridLayout::preflightSampleCoords(warnWeights, desc, &sampleReason),
               "preflightSampleCoords reports invalid_weights for clampable weights");
    expectTrue(!fuse::renderer::sampleCoordRejectReasonIsBlocking(sampleReason),
               "invalid_weights is not blocking for sample-coord preflight");
    ...
    expectTrue(!fuse::renderer::FroxelGridLayout::preflightSampleCoords(hardOob, desc, &sampleReason),
    expectTrue(fuse::renderer::sampleCoordRejectReasonIsBlocking(sampleReason),
               "out_of_bounds is blocking for sample-coord preflight");

                   0.5f, 0.5f, 0.01f, desc, camera, &mapReason),
    expectTrue(fuse::renderer::screenMappingRejectReasonIsBlocking(mapReason),
               "depth_out_of_range is blocking for screen mapping");

    expectTrue(fuse::renderer::froxel_util::preflightDensityLookup(grid, desc, 999u, &lookupReason),
               "index_out_of_range is not blocking for density lookup");

    expectTrue(!fuse::renderer::froxel_util::preflightDensityLookup(emptyGrid, desc, 0u, &lookupReason),
    expectTrue(fuse::renderer::densityLookupRejectReasonIsBlocking(lookupReason),
               "empty_storage is blocking for density lookup");
    ...
    expectTrue(!fuse::renderer::froxel_util::preflightFroxelTrilinearSample(emptyGrid, desc, inBounds, &trilinearReason),
               "preflightFroxelTrilinearSample rejects empty storage");
    expectTrue(trilinearReason == fuse::renderer::FroxelTrilinearSampleRejectReason::EmptyStorage,
               "empty storage trilinear preflight reports empty_storage");
    expectTrue(std::strcmp(fuse::renderer::froxelTrilinearSampleRejectReasonLabel(trilinearReason), "empty_storage") ==
                   0,
               "empty_storage trilinear reject reason label");
    expectTrue(!fuse::renderer::froxel_util::preflightFroxelTrilinearSample(grid, desc, hardOob, &trilinearReason),
    expectTrue(trilinearReason == fuse::renderer::FroxelTrilinearSampleRejectReason::InvalidSampleCoords,
               "hard OOB trilinear preflight reports invalid_sample_coords");

    expectTrue(fuse::renderer::froxel_util::isFiniteDensityValue(0.5f), "finite positive density is valid");
    expectTrue(!fuse::renderer::froxel_util::isFiniteDensityValue(-1.f), "negative density is invalid");
    expectTrue(!fuse::renderer::froxel_util::isFiniteDensityValue(
                   std::numeric_limits<fuse::f32>::infinity()),
               "infinite density is invalid");
    expectNear(fuse::renderer::froxel_util::sanitizeDensityValue(-1.f), 0.f, 1e-6f,
               "sanitizeDensityValue clamps negative to zero");
    expectNear(fuse::renderer::froxel_util::sanitizeDensityValue(2.f), 2.f, 1e-6f,
               "sanitizeDensityValue preserves finite positive density");

    fuse::renderer::FroxelDensityGrid nonFiniteGrid{};
    nonFiniteGrid.allocate(desc);
    nonFiniteGrid.density[0] = std::numeric_limits<fuse::f32>::quiet_NaN();
    expectTrue(fuse::renderer::froxel_util::hasNonFiniteDensity(nonFiniteGrid),
               "hasNonFiniteDensity true when NaN present");
    expectTrue(fuse::renderer::froxel_util::countNonFiniteDensities(nonFiniteGrid) == 1u,
               "countNonFiniteDensities counts NaN froxel");
    ...
    expectTrue(fuse::renderer::froxel_util::preflightGridDensity(grid, desc, &densityReason),
               ...
               "finite grid density preflight reports no reject reason");
    expectTrue(std::strcmp(fuse::renderer::gridDensityRejectReasonLabel(
                   fuse::renderer::GridDensityRejectReason::NonFiniteDensity),
               "non_finite_density") == 0,
               "non_finite_density grid reject reason label");
    ...
    expectTrue(fuse::renderer::froxel_util::classifyGridDensityReject(emptyGrid, zeroDesc) ==
               ...
               "classifyGridDensityReject empty_desc for empty froxel desc");
                   fuse::renderer::GridDensityRejectReason::EmptyDesc),
               "empty_desc") == 0,
               "empty_desc grid reject reason label");
    ...
    expectTrue(!fuse::renderer::FroxelGridLayout::preflightScreenMapping(
                   0.5f, 0.5f, 0.01f, desc, camera, nullptr, &mapReason),
    expectTrue(mapReason == fuse::renderer::ScreenMappingRejectReason::DepthOutOfRange,
               "preflightScreenMapping reports depth_out_of_range for below-near depth");

               "empty_desc grid density reject is not blocking");
    ...
    expectTrue(fuse::renderer::froxel_util::preflightGridDensity(emptyGrid, zeroDesc),
    expectTrue(fuse::renderer::gridDensityRejectReasonIsBlocking(densityReason),
               "undersized_storage is blocking for grid density validation");
    ...
    expectTrue(sampleReason == fuse::renderer::SampleCoordRejectReason::None,
               "successful trilinear preflight reports no sample reject reason");
    ...
    expectTrue(!fuse::renderer::froxel_util::preflightDensityAtScreen(
                   emptyGrid, desc, camera, 0.5f, 0.5f, 10.f, &mapReason),
               "preflightDensityAtScreen rejects empty storage");
    expectTrue(mapReason == fuse::renderer::ScreenMappingRejectReason::EmptyGrid,
               "empty storage maps to empty_grid screen preflight reject reason");
                   grid, desc, camera, 0.5f, 0.5f, 0.01f, &mapReason),
               "preflightDensityAtScreen rejects depth below near plane");
               "below-near depth reports depth_out_of_range screen preflight reject reason");
               "classifyFroxelPopulateReject zero_density for zero density");
               "zero_density is blocking for populate preflight");
    expectTrue(!fuse::renderer::froxel_util::preflightFroxelPopulate(desc, camera, zeroDensity, &populateReason),
               "preflightFroxelPopulate reports zero_density reject reason");
    ...
    expectTrue(!fuse::renderer::froxel_util::preflightFroxelPopulate(desc, badCamera, params),
               "preflightFroxelPopulate rejects invalid camera");
                   fuse::renderer::FroxelPopulateRejectReason::InvalidCamera),
               "invalid_camera populate reject reason is blocking");
    ...
    expectTrue(fuse::renderer::FroxelGridLayout::classifySampleCoordReject(inBounds, zeroDesc) ==
               "classifySampleCoordReject empty_grid for empty desc");
    expectTrue(!fuse::renderer::FroxelGridLayout::preflightSampleCoords(inBounds, zeroDesc),
               "preflightSampleCoords rejects empty desc");
    ...
    fuse::f32 rejectedScreen = 1.f;
    expectTrue(!fuse::renderer::froxel_util::trySampleDensityAtScreen(
                   grid, mismatched, camera, 0.5f, 0.5f, 10.f, rejectedScreen, lookupReason),
               "trySampleDensityAtScreen with lookup reason rejects desc mismatch");
    expectTrue(lookupReason == fuse::renderer::DensityLookupRejectReason::DescMismatch,
               "desc mismatch reports desc_mismatch lookup reject reason on screen sample");
    expectNear(rejectedScreen, 0.f, 1e-6f, "trySampleDensityAtScreen with lookup reason zeroes output on rejection");

    expectTrue(!fuse::renderer::froxel_util::tryWriteDensityAtCoord(grid, mismatched, 0u, 0u, 0u, 9.f, lookupReason),
               "tryWriteDensityAtCoord with reason rejects desc mismatch");
               "rejected coord write reports desc_mismatch lookup reject reason");
...
void testDeferredPipelinePassHooks() {
    ...
    if (bootstrap == nullptr) {
        expectTrue(false, "bootstrap allocated for B5.11 graph test");
...
int main() {
    ...
    testFroxelBilinearAndScreenPipelineGuards();
    testFroxelBilinearAndGridValidationGuards();
    testFroxelClassifyAndBlockingGuards();
    ...
    testFroxelVolumetricDeepenGuards();
    testFroxelBilinearScreenMappingAndGridDensityGuards();
