...
void testClusterCoordLookupAndRebuildGuards() {
    ...
    expectTrue(fuse::renderer::cluster_util::clusterLightCountAtCoord(grid, desc, 99u, 99u, 99u) == 2u,
               "count clamps OOB coord to last cluster");
    ...
    expectTrue(!fuse::renderer::cluster_util::tryLookupClusterLightsAtCoord(emptyGrid, desc, 0u, 0u, 0u,
               ...
               "tryCoordLookup rejects inaccessible grid");
    ...
    expectTrue(!fuse::renderer::cluster_util::shouldSkipClusterLighting(grid, desc, clusterCount),
               "lighting not skipped when grid is accessible and populated");
    expectTrue(fuse::renderer::cluster_util::shouldSkipClusterLighting(emptyGrid, desc, clusterCount),
               "lighting skipped on inaccessible grid");

    fuse::renderer::ClusterGridSoA emptyAssignedGrid{};
    emptyAssignedGrid.grid.resize(clusterCount);
    expectTrue(!fuse::renderer::cluster_util::hasAssignedLights(emptyAssignedGrid, clusterCount),
               "allocated but empty grid has no assigned lights");
    expectTrue(fuse::renderer::cluster_util::shouldSkipClusterLighting(emptyAssignedGrid, desc, clusterCount),
               "lighting skipped when no clusters hold lights");

    fuse::renderer::GridPopulationRejectReason popReason = fuse::renderer::GridPopulationRejectReason::None;
    expectTrue(!fuse::renderer::cluster_util::tryValidateGridPopulationForDesc(grid, zeroDesc, popReason),
               "tryValidateGridPopulationForDesc rejects empty grid desc with non-empty storage");
    expectTrue(popReason == fuse::renderer::GridPopulationRejectReason::EmptyGrid,
               "empty grid desc reports empty_grid population reason");
