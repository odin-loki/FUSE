...
int compare_residency_request_order(f32 priority_a, LodResidencyRequestKind kind_a, u64 sequence_a,
                                    f32 priority_b, LodResidencyRequestKind kind_b, u64 sequence_b) {
    if (priority_a != priority_b) {
        return priority_a > priority_b ? 1 : -1;
    }
...
u32 LodResidencyQueue::drain_completed(std::vector<CompletedLodResidencyRequest>& out) {
    ...
    std::sort(batch.begin(), batch.end(),
              [](const CompletedLodResidencyRequest& a, const CompletedLodResidencyRequest& b) {
                  return compare_residency_request_order(a.priority, a.kind, a.submit_sequence, b.priority, b.kind,
                                                         b.submit_sequence) > 0;
                  return compare_lod_residency_request_order(a.priority, a.kind, a.submit_sequence, b.priority,
                                                             b.kind, b.submit_sequence) > 0;
              });
...
bool LodResidencyQueue::has_pending_for(u32 chunk_index, LodResidencyRequestKind kind) const {
    std::lock_guard<std::mutex> lock(m_mutex);
    return std::find_if(m_pending.begin(), m_pending.end(),
                        [&](const PendingLodResidencyRequest& pending) {
                            return pending.request.chunk_index == chunk_index && pending.request.kind == kind;
                        }) != m_pending.end();
}
