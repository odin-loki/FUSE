...
void testParticleGpuMirrorPreflight() {
    ...
    expectTrue(synced.packed_layout_ok, "synced mirror preflight reports packed layout ok");
...
void testParticleGpuEmitSimEmptyBufferGuards() {
    const fuse::vfx::ParticleGpuBuffers empty = fuse::vfx::ParticleGpuBuffers::forCapacity(0u);
    expectTrue(empty.isEmpty(), "zero-capacity buffer is empty");
    expectTrue(!empty.isBound(), "unbound buffer reports not bound");
    expectTrue(!empty.bytesMatchLayout(), "empty buffer bytes do not match layout");
    expectTrue(fuse::vfx::should_skip_empty_buffer(empty), "should_skip_empty_buffer for zero capacity");

    const fuse::vfx::ParticleGpuBufferPreflight empty_preflight = empty.preflight();
    expectTrue(empty_preflight.is_empty(), "empty buffer preflight is empty");
    expectTrue(!empty_preflight.can_upload(), "empty buffer preflight blocks upload");
    expectTrue(!empty_preflight.can_bind_device(), "empty buffer preflight blocks device bind");

    const fuse::vfx::ParticleGpuBuffers sized = fuse::vfx::ParticleGpuBuffers::forCapacity(64u);
    expectTrue(!sized.isEmpty(), "non-zero capacity buffer is not empty");
    expectTrue(sized.bytesMatchLayout(), "sized buffer bytes match layout");
    expectTrue(!fuse::vfx::should_skip_empty_buffer(sized), "sized buffer is not skipped");

    const fuse::vfx::ParticleGpuBufferPreflight sized_preflight = sized.preflight();
    expectTrue(!sized_preflight.is_empty(), "sized buffer preflight is not empty");
    expectTrue(sized_preflight.can_upload(), "sized buffer preflight allows upload");
    expectTrue(!sized_preflight.can_bind_device(), "unbound sized buffer blocks device bind");

    fuse::vfx::ParticleGpuBuffers bound = sized;
    bound.packedSoa = 0x5000u;
    expectTrue(bound.preflight().can_bind_device(), "bound sized buffer can bind device");

    fuse::vfx::ParticleGpuBuffers mismatched = bound;
    mismatched.deviceBytes -= 16u;
    expectTrue(!mismatched.bytesMatchLayout(), "undersized deviceBytes fail layout match");
    expectTrue(fuse::vfx::should_skip_empty_buffer(mismatched), "layout mismatch skips buffer");

    expectTrue(fuse::vfx::should_skip_gpu_emit(0u, 64u, 10u), "should_skip_gpu_emit for zero emit count");
    expectTrue(!fuse::vfx::should_skip_gpu_emit(4u, 64u, 10u), "should_skip_gpu_emit false with free slots");
    expectTrue(fuse::vfx::should_skip_gpu_emit(4u, 64u, 64u), "should_skip_gpu_emit at capacity");
    expectTrue(fuse::vfx::should_skip_gpu_emit(4u, 0u, 0u), "should_skip_gpu_emit on zero capacity");

    expectTrue(fuse::vfx::should_skip_gpu_simulate(0u, 0u, 0u), "should_skip_gpu_simulate for zero capacity");
    expectTrue(fuse::vfx::should_skip_gpu_simulate(64u, 0u, 0u),
               "should_skip_gpu_simulate when no live particles and no emit");
    expectTrue(!fuse::vfx::should_skip_gpu_simulate(64u, 8u, 0u),
               "should_skip_gpu_simulate false with live particles");
    expectTrue(!fuse::vfx::should_skip_gpu_simulate(64u, 0u, 4u),
               "should_skip_gpu_simulate false when emit scheduled");

    const fuse::vfx::ParticleGpuFramePlan active =
        fuse::vfx::ParticleGpuFramePlan::forStub(64u, 8u, 48u);
    expectEq(active.remainingEmitSlots(), 16u, "frame plan remaining emit slots");
    expectEq(active.clampedEmitCount(), 8u, "frame plan emit count within free slots");
    expectTrue(!active.shouldSkipGpuEmit(), "active frame plan emits");
    expectTrue(!active.shouldSkipGpuSimulate(), "active frame plan simulates");

    const fuse::vfx::ParticleGpuEmitPreflight emit_preflight = active.preflightEmit();
    expectEq(emit_preflight.requested, 8u, "emit preflight carries requested count");
    expectEq(emit_preflight.allowed, 8u, "emit preflight allows full request");
    expectTrue(!emit_preflight.would_clamp, "emit preflight not clamped within free slots");
    expectTrue(emit_preflight.can_emit(), "emit preflight can emit");

    const fuse::vfx::ParticleGpuSimPreflight sim_preflight = active.preflightSimulate();
    expectTrue(sim_preflight.has_live_particles, "sim preflight reports live particles");
    expectTrue(sim_preflight.can_simulate(), "sim preflight can simulate");

    const fuse::vfx::ParticleGpuFramePlan overflow =
        fuse::vfx::ParticleGpuFramePlan::forStub(64u, 32u, 48u);
    expectEq(overflow.remainingEmitSlots(), 16u, "overflow frame remaining slots");
    expectEq(overflow.clampedEmitCount(), 16u, "overflow frame clamps emit count");
    const fuse::vfx::ParticleGpuEmitPreflight overflow_preflight = overflow.preflightEmit();
    expectTrue(overflow_preflight.would_clamp, "overflow emit preflight marks clamp");
    expectTrue(overflow_preflight.can_emit(), "clamped overflow emit still can emit");

    const fuse::vfx::ParticleGpuFramePlan full =
        fuse::vfx::ParticleGpuFramePlan::forStub(32u, 8u, 32u);
    expectEq(full.remainingEmitSlots(), 0u, "full frame has no remaining emit slots");
    expectTrue(full.shouldSkipGpuEmit(), "full frame skips gpu emit");
    expectTrue(!full.preflightEmit().can_emit(), "full frame emit preflight cannot emit");

    const fuse::vfx::ParticleGpuFramePlan dead =
        fuse::vfx::ParticleGpuFramePlan::forStub(128u, 0u, 0u);
    expectTrue(dead.shouldSkipGpuSimulate(), "dead frame skips gpu simulate");
    expectTrue(dead.preflightSimulate().skipped, "dead frame sim preflight skipped");
    expectTrue(!dead.preflightSimulate().has_live_particles, "dead frame has no live particles");

    const fuse::vfx::ParticleGpuFramePlan emit_only =
        fuse::vfx::ParticleGpuFramePlan::forStub(128u, 16u, 0u);
    expectTrue(!emit_only.shouldSkipGpuSimulate(), "emit-only frame still simulates");
    expectTrue(!emit_only.shouldSkipGpuEmit(), "emit-only frame emits");

    const fuse::vfx::ParticleGpuFramePreflight active_frame_preflight = active.preflight();
    expectTrue(active_frame_preflight.can_emit(), "active frame preflight can emit");
    expectTrue(active_frame_preflight.can_simulate(), "active frame preflight can simulate");
    expectTrue(!active_frame_preflight.buffers_empty, "active frame buffers not empty");
    expectTrue(active_frame_preflight.emit_within_capacity, "active frame emit within capacity");
    expectTrue(active_frame_preflight.ready_for_stub(), "active frame preflight still stub-ready");

    const fuse::vfx::ParticleGpuFramePreflight overflow_frame_preflight = overflow.preflight();
    expectTrue(!overflow_frame_preflight.emit_within_capacity, "overflow frame emit exceeds capacity");
    expectTrue(!overflow_frame_preflight.can_emit(), "overflow frame preflight cannot emit unchecked");

    const fuse::vfx::ParticleGpuFramePreflight idle_frame_preflight =
        fuse::vfx::ParticleGpuFramePlan::forStub(0u, 0u, 0u).preflight();
    expectTrue(idle_frame_preflight.skip_sim, "idle frame preflight skips sim");
    expectTrue(idle_frame_preflight.skip_emit, "idle frame preflight skips emit");
    expectTrue(idle_frame_preflight.buffers_empty, "idle frame preflight marks empty buffers");
    expectTrue(!idle_frame_preflight.can_emit(), "idle frame preflight cannot emit");
    expectTrue(!idle_frame_preflight.can_simulate(), "idle frame preflight cannot simulate");
    expectTrue(idle_frame_preflight.ready_for_stub(), "idle frame preflight remains stub-ready");
}
...
void testParticleGpuEmptyBufferGuards() {
    using fuse::vfx::ParticleGpuBufferPreflight;
    using fuse::vfx::ParticleGpuBuffers;

    const fuse::vfx::ParticleGpuBuffers zero_cap = fuse::vfx::ParticleGpuBuffers::forCapacity(0u);
    expectTrue(zero_cap.isEmpty(), "zero capacity buffer is empty");
    expectTrue(!zero_cap.hasDeviceBinding(), "unbound buffer has no device binding");
    expectTrue(fuse::vfx::should_skip_empty_buffer(zero_cap), "should_skip_empty_buffer for zero capacity");

    const ParticleGpuBufferPreflight zero_preflight = zero_cap.preflight();
    expectTrue(zero_preflight.is_empty, "zero capacity preflight marks empty");
    expectTrue(!zero_preflight.can_allocate(), "zero capacity preflight cannot allocate");
    expectTrue(!zero_preflight.ready_for_stub(), "zero capacity preflight is not stub-ready");

    const fuse::u32 capacity = 64u;
    const fuse::vfx::ParticleGpuBuffers live = fuse::vfx::ParticleGpuBuffers::forCapacity(capacity);
    expectTrue(!live.isEmpty(), "non-zero capacity buffer is not empty");
    expectTrue(!fuse::vfx::should_skip_empty_buffer(live), "should_skip_empty_buffer false for live buffer");

    const ParticleGpuBufferPreflight live_preflight = live.preflight();
    expectTrue(live_preflight.capacity_ok, "live buffer preflight capacity ok");
    expectTrue(live_preflight.device_bytes_ok, "live buffer preflight device bytes ok");
    expectTrue(!live_preflight.is_empty, "live buffer preflight is not empty");
    expectTrue(live_preflight.can_allocate(), "live buffer preflight can allocate");
    expectTrue(live_preflight.ready_for_stub(), "live buffer preflight is stub-ready");

    fuse::vfx::ParticleGpuBuffers undersized = live;
    undersized.deviceBytes -= 16u;
    const ParticleGpuBufferPreflight undersized_preflight = undersized.preflight();
    expectTrue(!undersized_preflight.device_bytes_ok, "undersized buffer fails device byte check");
    expectTrue(!undersized_preflight.can_allocate(), "undersized buffer cannot allocate");

    fuse::vfx::ParticleGpuBuffers bound = live;
    bound.packedSoa = 0x5000u;
    expectTrue(bound.hasDeviceBinding(), "bound buffer reports device binding");
    expectTrue(bound.preflight().has_binding, "bound buffer preflight reports binding");

void testParticleGpuFrameEmissionGuards() {
    const fuse::vfx::ParticleGpuFramePlan partial =
        fuse::vfx::ParticleGpuFramePlan::forStub(100u, 30u, 70u);
    expectEq(partial.remainingEmitSlots(), 30u, "remaining emit slots subtract alive count");
    expectEq(partial.clampedEmitCount(), 30u, "clamped emit count fits remaining slots");
    expectTrue(!partial.shouldSkipEmitAtCapacity(), "partial frame is not at capacity");
    expectTrue(!partial.shouldSkipEmitOverflow(), "partial emit fits within capacity");
    expectTrue(!fuse::vfx::should_skip_frame_emit(partial), "partial frame does not skip emit");

    const fuse::vfx::ParticleGpuFramePreflight partial_preflight = partial.preflight();
    expectTrue(partial_preflight.emit_slots_ok, "partial frame emit slots validate");
    expectTrue(partial_preflight.can_emit(), "partial frame can emit");
    expectTrue(!partial_preflight.skip_emit_at_capacity, "partial frame not at capacity");

    const fuse::vfx::ParticleGpuFramePlan overflow =
        fuse::vfx::ParticleGpuFramePlan::forStub(100u, 50u, 80u);
    expectEq(overflow.remainingEmitSlots(), 20u, "overflow frame reports remaining slots");
    expectEq(overflow.clampedEmitCount(), 20u, "overflow frame clamps emit to remaining slots");
    expectTrue(overflow.shouldSkipEmitOverflow(), "overflow frame reports emit overflow");
    expectTrue(!overflow.preflight().emit_slots_ok, "overflow frame fails emit slot check");
    expectTrue(!overflow.preflight().can_emit(), "overflow frame cannot emit without clamp");

    const fuse::vfx::ParticleGpuFramePlan full =
        fuse::vfx::ParticleGpuFramePlan::forStub(64u, 10u, 64u);
    expectEq(full.remainingEmitSlots(), 0u, "full frame has zero remaining slots");
    expectEq(full.clampedEmitCount(), 0u, "full frame clamps emit to zero");
    expectTrue(full.shouldSkipEmitAtCapacity(), "full frame skips emit at capacity");
    expectTrue(fuse::vfx::should_skip_frame_emit(full), "should_skip_frame_emit at capacity");
    expectTrue(full.preflight().skip_emit_at_capacity, "full frame preflight marks at capacity");

    const fuse::vfx::ParticleGpuFramePlan zero_emit =
        fuse::vfx::ParticleGpuFramePlan::forStub(128u, 0u, 16u);
    expectTrue(fuse::vfx::should_skip_frame_emit(zero_emit), "should_skip_frame_emit for zero emit count");
    expectTrue(zero_emit.preflight().emit_slots_ok, "zero emit passes slot check");

void testParticleGpuFrameSimulationGuards() {
    const fuse::vfx::ParticleGpuFramePlan live =
        fuse::vfx::ParticleGpuFramePlan::forStub(256u, 32u, 48u);
    expectTrue(!live.shouldSkipSimWithZeroAlive(), "live frame does not skip sim for zero alive");
    expectTrue(!fuse::vfx::should_skip_frame_sim(live), "live frame does not skip sim");
    expectTrue(live.preflight().sim_alive_ok, "live frame sim alive count validates");
    expectTrue(live.preflight().can_simulate(), "live frame can simulate");

    const fuse::vfx::ParticleGpuFramePlan zero_alive =
        fuse::vfx::ParticleGpuFramePlan::forStub(256u, 32u, 0u);
    expectTrue(zero_alive.shouldSkipSimWithZeroAlive(), "zero alive frame skips sim integration");
    expectTrue(fuse::vfx::should_skip_frame_sim(zero_alive), "should_skip_frame_sim for zero alive");
    expectTrue(!zero_alive.preflight().can_simulate(), "zero alive frame cannot simulate");
    expectTrue(zero_alive.preflight().skip_sim_zero_alive, "zero alive preflight marks skip");
    expectTrue(!zero_alive.skipSimLaunch(), "zero alive still has sim dispatch for capacity");

    const fuse::vfx::ParticleGpuFramePlan over_alive =
        fuse::vfx::ParticleGpuFramePlan::forStub(64u, 0u, 80u);
    expectTrue(!over_alive.preflight().sim_alive_ok, "over-capacity alive fails sim alive check");
    expectTrue(!over_alive.preflight().can_simulate(), "over-capacity alive cannot simulate");

    expectTrue(idle.preflight().empty_buffer, "idle frame preflight marks empty buffer");
    expectTrue(idle.preflight().skip_sim_zero_alive, "idle frame skips sim for zero alive");
    expectTrue(idle.preflight().skip_emit_at_capacity, "idle frame skips emit at capacity");
    expectTrue(fuse::vfx::should_skip_frame_sim(idle), "should_skip_frame_sim for idle frame");
    expectTrue(fuse::vfx::should_skip_frame_emit(idle), "should_skip_frame_emit for idle frame");
    expectTrue(idle.preflight().ready_for_stub(), "idle frame dispatch preflight still stub-ready");
...
void testParticleGpuDispatchPreflight() {
    ...
    const fuse::vfx::ParticleGpuDispatch active = ParticleGpuDispatch::forFrame(100u, 200u);
    const fuse::vfx::ParticleGpuDispatchPreflight active_preflight = active.preflight(100u, 200u);
    expectTrue(active_preflight.ready_for_stub(), "active dispatch preflight is ready");
    expectTrue(active_preflight.sim_covers_capacity, "active dispatch covers capacity");
    expectTrue(active_preflight.emit_covers_count, "active dispatch covers emit count");
    expectTrue(active_preflight.sim_padding_ok, "active dispatch sim padding accounts");
    expectTrue(active_preflight.emit_padding_ok, "active dispatch emit padding accounts");
    expectTrue(!active_preflight.sim_will_skip_launch, "active dispatch does not skip sim");
    expectTrue(!active_preflight.emit_will_skip_launch, "active dispatch does not skip emit");

    const fuse::vfx::ParticleGpuDispatch idle = ParticleGpuDispatch::forFrame(0u, 0u);
    const fuse::vfx::ParticleGpuDispatchPreflight idle_preflight = idle.preflight(0u, 0u);
    expectTrue(idle_preflight.ready_for_stub(), "idle dispatch preflight is ready");
    expectTrue(idle_preflight.sim_will_skip_launch, "idle dispatch skips sim launch");
    expectTrue(idle_preflight.emit_will_skip_launch, "idle dispatch skips emit launch");

    const fuse::vfx::ParticleGpuDispatch sim_only = ParticleGpuDispatch::forFrame(256u, 0u);
    const fuse::vfx::ParticleGpuDispatchPreflight sim_only_preflight = sim_only.preflight(256u, 0u);
    expectTrue(sim_only_preflight.ready_for_stub(), "sim-only dispatch preflight is ready");
    expectTrue(sim_only_preflight.emit_will_skip_launch, "sim-only dispatch skips emit launch");
    expectTrue(sim_only_preflight.emit_padding_ok, "sim-only dispatch emit padding passes");
}
...
void testParticleGpuDispatchThreadSlotGuards() {
    ...
    const fuse::vfx::ParticleGpuDispatch partial = ParticleGpuDispatch::forSimulate(100u);
    expectEq(partial.simSlotForThread(0u, 100u), 0u, "first sim thread maps to slot zero");
    expectEq(partial.simSlotForThread(99u, 100u), 99u, "last covered sim thread maps to last slot");
    expectEq(partial.simSlotForThread(100u, 100u), 100u, "first padding sim thread returns invalid slot marker");
    expectTrue(partial.simThreadHasValidSlot(50u, 100u), "in-range sim thread has valid slot");
    expectTrue(!partial.simThreadHasValidSlot(100u, 100u), "padding sim thread has no valid slot");
    expectTrue(!partial.simThreadHasValidSlot(0u, 0u), "zero capacity sim thread has no valid slot");

    const fuse::vfx::ParticleGpuDispatch emit = ParticleGpuDispatch::forEmit(200u);
    expectEq(emit.emitIndexForThread(199u, 200u), 199u, "last covered emit thread maps to last index");
    expectEq(emit.emitIndexForThread(200u, 200u), 200u, "first padding emit thread returns invalid index marker");
    expectTrue(emit.emitThreadHasValidIndex(0u, 200u), "in-range emit thread has valid index");
    expectTrue(!emit.emitThreadHasValidIndex(200u, 200u), "padding emit thread has no valid index");
    expectTrue(!emit.emitThreadHasValidIndex(0u, 0u), "zero emit has no valid index");
}
...
void testParticleGpuSlotLocateAndIndexGuards() {
    ...
    const fuse::u32 capacity = 65u;
    expectTrue(ParticleGpuBufferLayout::validateSlotDeviceOffset(ParticleGpuColumn::Positions, 3u, capacity),
               "valid slot device offset passes guard");
    expectTrue(!ParticleGpuBufferLayout::validateSlotDeviceOffset(ParticleGpuColumn::Positions, capacity, capacity),
               "out-of-range slot fails device offset guard");
    expectTrue(!ParticleGpuBufferLayout::validateSlotDeviceOffset(ParticleGpuColumn::Positions, 0u, 0u),
               "zero capacity fails device offset guard");

    const fuse::usize positions_slot_3 =
        ParticleGpuBufferLayout::slotDeviceOffset(ParticleGpuColumn::Positions, 3u, capacity);
    expectEq(ParticleGpuBufferLayout::slotIndexFromColumnOffset(ParticleGpuColumn::Positions, 3u * sizeof(fuse::math::Vec3),
                                                              capacity),
             3u, "slotIndexFromColumnOffset recovers slot from column offset");
    expectEq(ParticleGpuBufferLayout::slotIndexFromColumnOffset(ParticleGpuColumn::Positions, 1u, capacity), capacity,
             "misaligned column offset returns invalid slot marker");

    fuse::vfx::ParticleGpuColumn located_column = ParticleGpuColumn::Positions;
    fuse::u32 located_slot = 0u;
    expectTrue(ParticleGpuBufferLayout::locateSlotAtOffset(positions_slot_3, capacity, &located_column, &located_slot),
               "locateSlotAtOffset finds positions slot");
    expectEq(static_cast<fuse::u32>(located_column), static_cast<fuse::u32>(ParticleGpuColumn::Positions),
             "located column is positions");
    expectEq(located_slot, 3u, "located slot matches requested index");

    const fuse::usize padding_offset =
        ParticleGpuBufferLayout::columnDeviceOffset(ParticleGpuColumn::Positions, capacity) +
        ParticleGpuBufferLayout::columnByteSize(ParticleGpuColumn::Positions, capacity);
    fuse::vfx::ParticleGpuColumn padding_column = ParticleGpuColumn::Positions;
    fuse::u32 padding_slot = 0u;
    expectTrue(!ParticleGpuBufferLayout::locateSlotAtOffset(padding_offset, capacity, &padding_column, &padding_slot),
               "inter-column padding is not a slot offset");
    ...
    mirror.alive_count = 1u;
    const std::vector<fuse::u8> packed = mirror.packToDeviceLayout();
    expectTrue(ParticleGpuBufferLayout::packedBytesFitCapacity(packed, capacity),
               "packedBytesFitCapacity accepts full layout buffer");
    expectTrue(!ParticleGpuBufferLayout::packedBytesFitCapacity(std::vector<fuse::u8>(packed.size() / 2u, 0u), capacity),
               "packedBytesFitCapacity rejects undersized buffer");
}
...
void testParticleGpuMirrorPackUnpackGuards() {
    ...
    mirror.syncFromCpuSoA(cpu);
    const fuse::vfx::ParticleGpuMirrorPreflight synced = mirror.preflightFromCpu(cpu);
    expectTrue(synced.packed_layout_ok, "synced mirror reports packed layout ok");
    expectTrue(synced.can_pack(), "synced mirror preflight allows pack");
    ...
    expectTrue(synced.can_unpack(packed, 8u), "synced mirror preflight allows unpack of full layout");
    ...
    expectEq(static_cast<fuse::u32>(fuse::vfx::ParticleGpuMirror::unpackGuardForLayout(packed, 8u)),
             static_cast<fuse::u32>(ParticleGpuSyncGuard::Ok),
             "unpackGuardForLayout accepts full layout");
    expectEq(static_cast<fuse::u32>(
                 fuse::vfx::ParticleGpuMirror::unpackGuardForLayout(std::vector<fuse::u8>(packed.size() / 2u, 0u), 8u)),
             static_cast<fuse::u32>(ParticleGpuSyncGuard::MirrorUninitialized),
             "unpackGuardForLayout rejects undersized buffer");
    expectEq(static_cast<fuse::u32>(fuse::vfx::ParticleGpuMirror::unpackGuardForLayout(packed, 0u)),
             static_cast<fuse::u32>(ParticleGpuSyncGuard::CpuUninitialized),
             "unpackGuardForLayout rejects zero capacity");
}
...
void testParticleGpuDispatchPreflight() {
    ...
    expectTrue(partial_preflight.ready_for_stub(100u, 0u), "partial sim dispatch preflight is ready");
    expectTrue(partial_preflight.sim_covers, "partial sim dispatch covers slot count");
    expectTrue(partial_preflight.sim_padding_ok, "partial sim dispatch padding accounts");
    expectTrue(partial_preflight.emit_skip_ok, "zero emit dispatch skip is valid");

    const fuse::vfx::ParticleGpuDispatch emit = ParticleGpuDispatch::forEmit(200u);
    const fuse::vfx::ParticleGpuDispatchPreflight emit_preflight = emit.preflight(0u, 200u);
    expectTrue(emit_preflight.ready_for_stub(0u, 200u), "emit-only dispatch preflight is ready");
    expectTrue(emit_preflight.emit_covers, "emit dispatch covers emit count");
    expectTrue(emit_preflight.emit_padding_ok, "emit dispatch padding accounts");
    expectTrue(emit_preflight.sim_skip_ok, "zero slot sim skip is valid");

    const fuse::vfx::ParticleGpuDispatch frame = ParticleGpuDispatch::forFrame(4096u, 200u);
    const fuse::vfx::ParticleGpuDispatchPreflight frame_preflight = frame.preflight(4096u, 200u);
    expectTrue(frame_preflight.ready_for_stub(4096u, 200u), "frame dispatch preflight is ready");
    expectTrue(frame_preflight.sim_covers && frame_preflight.emit_covers,
               "frame dispatch covers sim and emit");

    const fuse::vfx::ParticleGpuDispatch idle = ParticleGpuDispatch::forFrame(0u, 0u);
    expectTrue(idle.preflight(0u, 0u).ready_for_stub(0u, 0u), "idle dispatch preflight is ready");

    const fuse::vfx::ParticleGpuDispatch exact =
        ParticleGpuDispatch::forSimulate(ParticleGpuBufferLayout::kSimBlockSize);
    const fuse::vfx::ParticleGpuDispatchPreflight exact_preflight =
        exact.preflight(ParticleGpuBufferLayout::kSimBlockSize, 0u);
    expectEq(exact.simPaddingThreads(ParticleGpuBufferLayout::kSimBlockSize), 0u,
             "exact sim grid has no padding threads");
    expectTrue(exact_preflight.sim_padding_ok, "exact sim dispatch padding accounts");
}
...
void testParticleGpuSlotAddressGuards() {
    ...
    const fuse::u32 capacity = 65u;
    const fuse::u64 base = 0x8000u;

    const fuse::u64 positions_slot_3 =
        ParticleGpuBufferLayout::slotDeviceAddress(ParticleGpuColumn::Positions, base, 3u, capacity);
    expectTrue(positions_slot_3 > base, "slotDeviceAddress advances from packed base");
    expectTrue(ParticleGpuBufferLayout::validateSlotDeviceAddress(positions_slot_3, base,
                                                                  ParticleGpuColumn::Positions, 3u, capacity),
               "validateSlotDeviceAddress accepts valid positions slot");

    expectTrue(!ParticleGpuBufferLayout::slotDeviceAddress(ParticleGpuColumn::Positions, 0u, 0u, capacity),
               "slotDeviceAddress rejects null base");
    expectTrue(!ParticleGpuBufferLayout::validateSlotDeviceAddress(positions_slot_3 + 1u, base,
                                                                   ParticleGpuColumn::Positions, 3u, capacity),
               "validateSlotDeviceAddress rejects mismatched address");

    fuse::vfx::ParticleGpuColumn located_column = ParticleGpuColumn::Positions;
    fuse::u32 located_slot = 0u;
    const fuse::usize byte_offset =
        ParticleGpuBufferLayout::slotDeviceOffset(ParticleGpuColumn::Ages, 7u, capacity);
    expectTrue(ParticleGpuBufferLayout::locateSlotAtByteOffset(byte_offset, capacity, &located_column,
                                                               &located_slot),
               "locateSlotAtByteOffset finds ages slot");
    expectEq(static_cast<fuse::u32>(located_column), static_cast<fuse::u32>(ParticleGpuColumn::Ages),
             "located column is ages");
    expectEq(located_slot, 7u, "located slot index matches ages slot");
    ...
    fuse::vfx::ParticleGpuColumn padding_column = ParticleGpuColumn::Positions;
    fuse::u32 padding_slot = 0u;
    expectTrue(!ParticleGpuBufferLayout::locateSlotAtByteOffset(padding_offset, capacity, &padding_column,
                                                                &padding_slot),
               "locateSlotAtByteOffset rejects inter-column padding");
}

void testParticleGpuThreadSlotMapping() {
    using fuse::vfx::particle_gpu_util::globalThreadCoversSlot;
    using fuse::vfx::particle_gpu_util::slotIndexFromGlobalThread;

    expectEq(slotIndexFromGlobalThread(42u, 100u), 42u, "in-range thread maps to slot index");
    expectEq(slotIndexFromGlobalThread(100u, 100u), 100u, "padding thread maps to element count sentinel");
    expectEq(slotIndexFromGlobalThread(255u, 100u), 100u, "last padding thread maps to sentinel");
    expectTrue(globalThreadCoversSlot(99u, 100u), "last covered thread covers a slot");
    expectTrue(!globalThreadCoversSlot(100u, 100u), "first padding thread does not cover a slot");
    expectTrue(!globalThreadCoversSlot(0u, 0u), "zero elements has no covered threads");
}

void testParticleGpuMirrorPackGuards() {
    using fuse::vfx::ParticleGpuBufferLayout;
    using fuse::vfx::ParticleGpuPackGuard;

    fuse::vfx::ParticleSoA cpu{};
    fuse::vfx::particle_soa::init(cpu, 8u);

    fuse::vfx::ParticleGpuMirror mirror{};
    expectEq(static_cast<fuse::u32>(mirror.packGuard()),
             static_cast<fuse::u32>(ParticleGpuPackGuard::MirrorUninitialized),
             "uninitialized mirror pack guard blocks pack");
    ...
    mirror.syncFromCpuSoA(cpu);
    expectEq(static_cast<fuse::u32>(mirror.packGuard()),
             static_cast<fuse::u32>(ParticleGpuPackGuard::Ok),
             "synced mirror pack guard is ok");

    const std::vector<fuse::u8> packed = mirror.tryPackToDeviceLayout();
    expectEq(static_cast<fuse::u32>(packed.size()),
             static_cast<fuse::u32>(ParticleGpuBufferLayout::packedDeviceBytes(8u)),
             "tryPackToDeviceLayout returns full layout on ok guard");

    const fuse::vfx::ParticleGpuMirrorPreflight synced = mirror.preflightFromCpu(cpu);
    expectTrue(synced.can_pack(), "synced mirror preflight allows pack");
    expectTrue(synced.can_unpack(packed, 8u), "synced mirror preflight allows unpack of full buffer");
    expectTrue(!synced.can_unpack(std::vector<fuse::u8>(packed.size() / 2u, 0u), 8u),
               "synced mirror preflight rejects undersized unpack buffer");
    expectTrue(mirror.canUnpackFromDeviceLayout(packed, 8u), "canUnpackFromDeviceLayout accepts full buffer");
    expectTrue(!mirror.canUnpackFromDeviceLayout(packed, 4u), "canUnpackFromDeviceLayout rejects capacity mismatch");

    mirror.alive_count = 99u;
    expectEq(static_cast<fuse::u32>(mirror.packGuard()),
             static_cast<fuse::u32>(ParticleGpuPackGuard::AliveCountMismatch),
             "stale alive_count fails pack guard");
    ...
    expectTrue(!stale.can_pack(), "stale alive_count blocks pack preflight");

    expectTrue(std::strcmp(ParticleGpuBufferLayout::packGuardName(ParticleGpuPackGuard::AliveCountMismatch),
                           "alive_count_mismatch") == 0,
               "pack guard name matches alive count mismatch reason");
}

void testParticleGpuSlotPreflightGuards() {
    using fuse::vfx::ParticleGpuBufferLayout;
    using fuse::vfx::ParticleGpuColumn;

    const fuse::u32 capacity = 65u;

    const fuse::vfx::ParticleGpuSlotPreflight valid =
        ParticleGpuBufferLayout::preflightSlotAccess(ParticleGpuColumn::Ages, 4u, capacity);
    expectTrue(valid.slot_in_bounds, "valid slot is in bounds");
    expectTrue(valid.offset_in_column, "valid slot offset stays inside column span");
    expectTrue(valid.offset_aligned, "valid slot offset is element-aligned");
    expectTrue(valid.ready_for_stub_access(), "valid slot passes stub access preflight");

    const fuse::vfx::ParticleGpuSlotPreflight out_of_bounds =
        ParticleGpuBufferLayout::preflightSlotAccess(ParticleGpuColumn::Positions, capacity, capacity);
    expectTrue(!out_of_bounds.slot_in_bounds, "slot equal to capacity is out of bounds");
    expectTrue(!out_of_bounds.ready_for_stub_access(), "out-of-bounds slot fails stub access preflight");

    expectTrue(ParticleGpuBufferLayout::validateSlotDeviceOffset(ParticleGpuColumn::Positions, 0u, capacity),
               "positions slot zero validates");
    expectTrue(!ParticleGpuBufferLayout::validateSlotDeviceOffset(ParticleGpuColumn::Positions, capacity, capacity),
               "invalid slot fails validateSlotDeviceOffset");
    expectTrue(ParticleGpuBufferLayout::isSlotOffsetAligned(ParticleGpuColumn::Colors, 2u, capacity),
               "vec3 column slot offsets stay aligned");

    const fuse::usize ages_offset =
        ParticleGpuBufferLayout::slotDeviceOffset(ParticleGpuColumn::Ages, 3u, capacity);
    expectEq(ParticleGpuBufferLayout::slotIndexAtColumnOffset(ParticleGpuColumn::Ages, ages_offset -
                                                                                      ParticleGpuBufferLayout::columnDeviceOffset(
                                                                                          ParticleGpuColumn::Ages, capacity),
                                                              capacity),
             3u, "slotIndexAtColumnOffset inverts slotDeviceOffset");

    fuse::vfx::ParticleGpuColumn located_column = ParticleGpuColumn::Positions;
    fuse::u32 located_slot = 0u;
    expectTrue(ParticleGpuBufferLayout::locateSlotAtOffset(ages_offset, capacity, &located_column, &located_slot),
               "locateSlotAtOffset resolves ages slot");
    expectEq(static_cast<fuse::u32>(located_column), static_cast<fuse::u32>(ParticleGpuColumn::Ages),
             "locateSlotAtOffset reports ages column");
    expectEq(located_slot, 3u, "locateSlotAtOffset reports slot index");

    const fuse::usize padding_offset =
        ParticleGpuBufferLayout::columnDeviceOffset(ParticleGpuColumn::Positions, capacity) +
        ParticleGpuBufferLayout::columnByteSize(ParticleGpuColumn::Positions, capacity);
    fuse::u32 padding_slot = 0u;
    expectTrue(!ParticleGpuBufferLayout::locateSlotAtOffset(padding_offset, capacity, &located_column, &padding_slot),
               "inter-column padding is not a slot offset");
}

void testParticleGpuMirrorPackPreflight() {
    fuse::vfx::ParticleSoA cpu{};
    fuse::vfx::particle_soa::init(cpu, 8u);
    (void)fuse::vfx::particle_soa::burst_emit(cpu, {}, {}, 2u, 5u);

    fuse::vfx::ParticleGpuMirror mirror{};
    const fuse::vfx::ParticleGpuMirrorPackPreflight uninitialized = mirror.preflightPack();
    expectTrue(!uninitialized.can_pack(), "uninitialized mirror blocks guarded pack");
    expectTrue(mirror.tryPackToDeviceLayout().empty(), "tryPackToDeviceLayout returns empty when blocked");

    mirror.syncFromCpuSoA(cpu);
    const fuse::vfx::ParticleGpuMirrorPackPreflight ready = mirror.preflightPack();
    expectTrue(ready.can_pack(), "synced mirror passes pack preflight");
    expectTrue(ready.layout_bytes_ok, "synced mirror validates layout bytes");
    expectTrue(ready.mirror.alive_count_matches_flags, "synced mirror alive count matches flags");

    const std::vector<fuse::u8> packed = mirror.tryPackToDeviceLayout();
    expectTrue(!packed.empty(), "tryPackToDeviceLayout succeeds after sync");
    expectEq(static_cast<fuse::u32>(packed.size()),
             static_cast<fuse::u32>(fuse::vfx::ParticleGpuBufferLayout::packedDeviceBytes(cpu.capacity)),
             "guarded pack matches layout size");

    mirror.alive_count = 99u;
    expectTrue(!mirror.preflightPack().can_pack(), "stale alive_count blocks pack preflight");
    expectTrue(mirror.tryPackToDeviceLayout().empty(), "tryPackToDeviceLayout blocks stale mirror");

    expectTrue(mirror.validateSlotAccess(0u), "mirror validates in-range slot");
    expectTrue(!mirror.validateSlotAccess(cpu.capacity), "mirror rejects out-of-range slot");
    const fuse::vfx::ParticleGpuSlotPreflight slot = mirror.preflightSlot(1u, fuse::vfx::ParticleGpuColumn::Alphas);
    expectTrue(slot.ready_for_stub_access(), "mirror slot preflight allows valid alpha slot");
}

void testParticleGpuFramePlanDispatchPreflight() {
    const fuse::vfx::ParticleGpuFramePlan active =
        fuse::vfx::ParticleGpuFramePlan::forStub(100u, 200u, 48u);
    const fuse::vfx::ParticleGpuDispatchPreflight dispatch_preflight = active.dispatchPreflight();
    expectTrue(dispatch_preflight.ready_for_stub(), "frame dispatch preflight is stub-ready");
    expectTrue(dispatch_preflight.can_launch_sim(), "frame dispatch preflight launches sim");
    expectTrue(dispatch_preflight.can_launch_emit(), "frame dispatch preflight launches emit");

    const fuse::vfx::ParticleGpuFramePreflight frame_preflight = active.preflight();
    expectTrue(frame_preflight.dispatch_preflight_ok, "frame preflight includes dispatch preflight");
    expectTrue(frame_preflight.ready_for_stub(), "active frame preflight remains stub-ready");

    fuse::vfx::ParticleGpuFramePlan broken = active;
    broken.dispatch = fuse::vfx::ParticleGpuDispatch::forSimulate(0u);
    broken.dispatch.emitBlockCount = active.dispatch.emitBlockCount;
    broken.dispatch.emitThreadCount = active.dispatch.emitThreadCount;
    expectTrue(!broken.dispatchPreflight().can_launch_sim(), "broken dispatch preflight blocks sim launch");
    expectTrue(!broken.preflight().dispatch_preflight_ok, "broken frame fails dispatch preflight flag");
}

void testParticleGpuDeviceUploadSkipGuard() {
    fuse::vfx::ParticleSoA cpu{};
    fuse::vfx::particle_soa::init(cpu, 8u);
    (void)fuse::vfx::particle_soa::burst_emit(cpu, {}, {}, 2u, 11u);

    fuse::vfx::ParticleGpuMirror mirror{};
    expectTrue(!fuse::vfx::should_skip_device_upload(mirror, cpu),
               "uninitialized mirror cannot skip device upload");

    mirror.syncFromCpuSoA(cpu);
    expectTrue(fuse::vfx::should_skip_device_upload(mirror, cpu),
               "synced matching mirror skips redundant upload");

    mirror.alive_count = 0u;
    mirror.alive_flags[0] = 0u;
    mirror.alive_flags[1] = 0u;
    expectTrue(!fuse::vfx::should_skip_device_upload(mirror, cpu),
               "stale mirror blocks upload skip guard");
}

void testParticleGpuEmitGuard() {
    using fuse::vfx::ParticleGpuEmitGuard;

    expectEq(fuse::vfx::gpu_free_slot_count(64u, 48u), 16u, "gpu_free_slot_count subtracts alive count");
    expectEq(fuse::vfx::gpu_free_slot_count(0u, 0u), 0u, "gpu_free_slot_count zero capacity");
    expectEq(fuse::vfx::clamp_gpu_emit_count(32u, 64u, 48u), 16u, "clamp_gpu_emit_count respects free slots");
    expectEq(fuse::vfx::clamp_gpu_emit_count(10u, 64u, 48u), 10u, "clamp_gpu_emit_count passes through when under free");
    expectEq(fuse::vfx::clamp_gpu_emit_count(5u, 64u, 64u), 0u, "clamp_gpu_emit_count zero at capacity");

    const ParticleGpuEmitGuard within = fuse::vfx::emit_guard_for_frame(10u, 64u, 48u);
    expectEq(within.requested, 10u, "emit guard carries requested count");
    expectEq(within.clamped, 10u, "emit guard passes through under free slots");
    expectTrue(!within.at_capacity, "emit guard not at capacity with free slots");
    expectTrue(within.can_emit(), "emit guard can emit when slots remain");
    expectTrue(!within.emit_clamped(), "emit guard not clamped under free slots");

    const ParticleGpuEmitGuard clamped = fuse::vfx::emit_guard_for_frame(32u, 64u, 48u);
    expectEq(clamped.clamped, 16u, "emit guard clamps to free slots");
    expectTrue(clamped.emit_clamped(), "emit guard reports clamping");
    expectTrue(clamped.can_emit(), "clamped emit guard still can emit partial count");

    const ParticleGpuEmitGuard full = fuse::vfx::emit_guard_for_frame(8u, 64u, 64u);
    expectTrue(full.at_capacity, "emit guard at capacity when full");
    expectTrue(full.skip_emit, "emit guard skips when at capacity");
    expectTrue(!full.can_emit(), "emit guard cannot emit at capacity");

    const ParticleGpuEmitGuard zero = fuse::vfx::emit_guard_for_frame(0u, 64u, 0u);
    expectTrue(zero.skip_emit, "emit guard skips zero request");
    expectTrue(!zero.can_emit(), "emit guard cannot emit zero request");
}
...
void testParticleGpuBuffersPreflight() {
    ...
    const ParticleGpuBuffers buffers = ParticleGpuBuffers::forCapacity(128u);
    expectTrue(!buffers.isBound(), "unbound buffer reports not bound");
    expectTrue(buffers.hasDeviceAllocation(), "forCapacity yields device allocation");
    expectTrue(!buffers.isEmptyCapacity(), "forCapacity is not empty capacity");

    const ParticleGpuBuffersPreflight preflight = buffers.preflight();
    expectTrue(preflight.capacity_ok, "buffer preflight validates capacity");
    expectTrue(preflight.bytes_ok, "buffer preflight validates device bytes");
    expectTrue(!preflight.bound, "unbound buffer preflight reports unbound");
    expectTrue(preflight.can_bind(), "sized buffer can bind");
    expectTrue(!preflight.is_empty_capacity(), "non-zero capacity is not empty");

    const ParticleGpuBuffers empty = ParticleGpuBuffers::forCapacity(0u);
    expectTrue(empty.isEmptyCapacity(), "zero capacity buffer is empty");
    expectTrue(!empty.hasDeviceAllocation(), "zero capacity has no device allocation");
    expectTrue(fuse::vfx::is_empty_device_buffer(empty), "is_empty_device_buffer for zero capacity");
    expectTrue(empty.preflight().is_empty_capacity(), "empty buffer preflight reports empty capacity");

    ParticleGpuBuffers bound = buffers;
    bound.packedSoa = 0x10000u;
    expectTrue(bound.isBound(), "bound buffer reports bound");
    expectTrue(bound.preflight().bound, "bound buffer preflight reports bound");
}

void testParticleGpuFrameLaunchGuards() {
    const fuse::vfx::ParticleGpuFramePlan active =
        fuse::vfx::ParticleGpuFramePlan::forStub(64u, 20u, 48u);
    expectEq(active.freeSlotCount(), 16u, "frame plan free slot count");
    expectEq(active.clampedEmitCount(), 16u, "frame plan clamps emit to free slots");
    expectTrue(!active.shouldSkipSimWhenEmpty(), "active frame does not skip sim when alive");
    expectTrue(active.canSimulate(), "active frame can simulate");
    expectTrue(active.canEmit(), "active frame can emit within free slots");
    expectTrue(active.requiresBufferBind(), "unbound frame requires buffer bind");
    expectTrue(!active.buffersBound(), "stub frame buffers are unbound");

    const fuse::vfx::ParticleGpuFrameLaunchPreflight launch = active.launchPreflight();
    expectTrue(launch.ready_for_stub(), "active frame launch preflight is ready");
    expectTrue(launch.sim_can_launch, "active frame sim can launch with alive particles");
    expectTrue(launch.emit_can_launch, "active frame emit can launch within free slots");
    expectTrue(launch.emit.emit_clamped(), "launch preflight reports emit clamping");
    expectTrue(launch.buffers.bytes_ok, "launch preflight validates buffer bytes");

    const fuse::vfx::ParticleGpuFramePlan empty_alive =
        fuse::vfx::ParticleGpuFramePlan::forStub(64u, 0u, 0u);
    expectTrue(empty_alive.shouldSkipSimWhenEmpty(), "zero alive skips sim when empty");
    expectTrue(fuse::vfx::should_skip_sim_when_empty(0u), "should_skip_sim_when_empty for zero alive");
    expectTrue(!empty_alive.canEmit(), "zero emit request cannot emit");
    const fuse::vfx::ParticleGpuFrameLaunchPreflight empty_launch = empty_alive.launchPreflight();
    expectTrue(empty_launch.skip_sim_when_empty, "empty alive launch skips sim");
    expectTrue(!empty_launch.sim_can_launch, "empty alive launch cannot sim");
    expectTrue(!empty_launch.emit_can_launch, "zero emit launch cannot emit");

    const fuse::vfx::ParticleGpuFramePlan full =
        fuse::vfx::ParticleGpuFramePlan::forStub(64u, 32u, 64u);
    expectEq(full.freeSlotCount(), 0u, "full frame has zero free slots");
    expectTrue(full.emitGuard().at_capacity, "full frame emit guard at capacity");
    expectTrue(!full.canEmit(), "full frame cannot emit");
...
void testParticleGpuEmptyBufferPackGuards() {
    fuse::vfx::ParticleGpuMirror mirror{};
    expectTrue(mirror.shouldSkipPack(), "default mirror skips pack");
    ...
    mirror.alive_flags[0] = 1u;
    mirror.alive_count = 1u;
    expectTrue(!mirror.shouldSkipPack(), "synced mirror does not skip pack");
    ...
    expectTrue(mirror.packedBytesFit(packed), "packed bytes fit layout");

    mirror.alive_count = 3u;
    expectTrue(mirror.shouldSkipPack(), "stale alive_count skips pack");
...
void testParticleGpuDispatchPreflight() {
    ...
    expectEq(sim_from_dispatch.padding_threads, partial.padding_threads,
    ...
    expectEq(emit_from_dispatch.padding_threads, emit.padding_threads,
...
void testParticleGpuMirrorSyncPreflight() {
    ...
    expectEq(static_cast<fuse::u32>(uninitialized.guard),
             static_cast<fuse::u32>(ParticleGpuSyncGuard::MirrorUninitialized),
    ...
    expectEq(static_cast<fuse::u32>(mismatch.guard),
             static_cast<fuse::u32>(ParticleGpuSyncGuard::CapacityMismatch),
             "preflightWriteToCpu reports capacity mismatch");
    expectTrue(!mismatch.can_write, "preflightWriteToCpu rejects write on mismatch");

    expectTrue(ParticleGpuBufferLayout::syncGuardName(ParticleGpuSyncGuard::Ok) != nullptr,
               "sync guard name remains defined");
}

void testParticleGpuSlotOffsetGuards() {
    using fuse::vfx::ParticleGpuBufferLayout;
    using fuse::vfx::ParticleGpuColumn;
    using fuse::vfx::ParticleGpuSlotGuard;

    expectEq(static_cast<fuse::u32>(ParticleGpuBufferLayout::slotGuard(0u, 0u)),
             static_cast<fuse::u32>(ParticleGpuSlotGuard::InvalidCapacity),
             "slotGuard rejects zero capacity");
    expectEq(static_cast<fuse::u32>(ParticleGpuBufferLayout::slotGuard(8u, 8u)),
             static_cast<fuse::u32>(ParticleGpuSlotGuard::OutOfRange),
             "slotGuard rejects out-of-range slot");
    expectTrue(ParticleGpuBufferLayout::isSlotInRange(7u, 8u), "isSlotInRange accepts last slot");
    expectTrue(!ParticleGpuBufferLayout::isSlotInRange(8u, 8u), "isSlotInRange rejects past-end slot");

    const fuse::u32 capacity = 64u;
    const fuse::usize positions_slot_3 =
        ParticleGpuBufferLayout::slotColumnByteOffset(ParticleGpuColumn::Positions, 3u, capacity);
    expectEq(positions_slot_3, 3u * sizeof(fuse::math::Vec3), "slotColumnByteOffset for positions slot 3");

    const fuse::usize velocities_slot_3 =
        ParticleGpuBufferLayout::slotColumnByteOffset(ParticleGpuColumn::Velocities, 3u, capacity);
    expectTrue(velocities_slot_3 > positions_slot_3, "velocities slot offset follows column base");
    expectEq(velocities_slot_3,
             ParticleGpuBufferLayout::columnDeviceOffset(ParticleGpuColumn::Velocities, capacity) +
                 3u * sizeof(fuse::math::Vec3),
             "slotColumnByteOffset matches column base plus element offset");

    expectEq(ParticleGpuBufferLayout::slotElementByteOffset(ParticleGpuColumn::Positions, 99u, capacity),
             0u,
             "slotElementByteOffset returns zero for out-of-range slot");
    expectEq(ParticleGpuBufferLayout::slotColumnByteOffset(ParticleGpuColumn::AliveFlags, 99u, capacity),
             0u,
             "slotColumnByteOffset returns zero for out-of-range slot");

    expectTrue(std::strcmp(ParticleGpuBufferLayout::slotGuardName(ParticleGpuSlotGuard::OutOfRange),
                           "out_of_range") == 0,
               "slot guard name matches out-of-range reason");

    fuse::vfx::ParticleGpuMirror mirror{};
    mirror.reserve(16u);
    expectTrue(mirror.isSlotInRange(15u), "mirror isSlotInRange accepts last slot");
    expectTrue(!mirror.isSlotInRange(16u), "mirror isSlotInRange rejects past-end slot");
    expectEq(static_cast<fuse::u32>(mirror.slotGuard(16u)),
             static_cast<fuse::u32>(ParticleGpuSlotGuard::OutOfRange),
             "mirror slotGuard rejects out-of-range slot");
}

void testParticleGpuFramePlanPreflight() {
    const fuse::vfx::ParticleGpuFramePlanPreflight active =
        fuse::vfx::ParticleGpuFramePlan::preflightStub(100u, 200u, 48u);
    expectEq(active.capacity, 100u, "frame preflight carries capacity");
    expectEq(active.emit_count, 200u, "frame preflight carries emit count");
    expectEq(active.alive_count, 48u, "frame preflight carries alive count");
    expectTrue(active.buffers_ok, "frame preflight buffers_ok for stub plan");
    expectTrue(active.sim_covers_capacity, "frame preflight sim covers capacity");
    expectTrue(active.emit_covers_count, "frame preflight emit covers count");
    expectTrue(!active.skip_sim, "frame preflight sim not skipped");
    expectTrue(!active.skip_emit, "frame preflight emit not skipped");
    expectTrue(active.ready(), "active frame preflight is ready");
    expectTrue(!active.isIdle(), "active frame preflight is not idle");
    expectEq(active.sim_padding_threads, 156u, "frame preflight reports sim padding");
    expectEq(active.emit_padding_threads, 56u, "frame preflight reports emit padding");

    const fuse::vfx::ParticleGpuFramePlan plan = fuse::vfx::ParticleGpuFramePlan::forStub(128u, 0u, 10u);
    const fuse::vfx::ParticleGpuFramePlanPreflight sim_only = plan.preflight();
    expectTrue(sim_only.ready(), "sim-only frame preflight is ready");
    expectTrue(sim_only.skip_emit, "sim-only frame preflight skips emit");
    expectTrue(!sim_only.skip_sim, "sim-only frame preflight runs sim");
    expectTrue(sim_only.emit_covers_count, "skipped emit still covers zero count");

    const fuse::vfx::ParticleGpuFramePlanPreflight idle =
        fuse::vfx::ParticleGpuFramePlan::preflightStub(0u, 0u, 0u);
    expectTrue(idle.isIdle(), "idle frame preflight is idle");
    expectTrue(idle.ready(), "idle frame preflight is ready");
    expectTrue(idle.skip_sim && idle.skip_emit, "idle frame preflight skips both launches");
    expectEq(idle.sim_padding_threads, 0u, "idle frame preflight has zero sim padding");
    expectEq(idle.emit_padding_threads, 0u, "idle frame preflight has zero emit padding");

    fuse::vfx::ParticleGpuFramePlan broken = plan;
    broken.buffers.deviceBytes -= 32u;
    const fuse::vfx::ParticleGpuFramePlanPreflight broken_preflight = broken.preflight();
    expectTrue(!broken_preflight.buffers_ok, "undersized buffers fail frame preflight");
    expectTrue(!broken_preflight.ready(), "broken frame preflight is not ready");
}
...
int main() {
    ...
    testParticleGpuEmitSimEmptyBufferGuards();
    testParticleGpuDispatchPreflight();
    testParticleGpuMirrorSyncSkipGuards();
    testParticleGpuEmptyBufferGuards();
    testParticleGpuFrameEmissionGuards();
    testParticleGpuFrameSimulationGuards();
    testParticleGpuMirrorSyncPreflight();
    testParticleGpuSlotOffsetGuards();
    testParticleGpuDispatchThreadSlotGuards();
    testParticleGpuSlotLocateAndIndexGuards();
    testParticleGpuMirrorPackUnpackGuards();
    testParticleGpuSlotAddressGuards();
    testParticleGpuThreadSlotMapping();
    testParticleGpuMirrorPackGuards();
    testParticleGpuSlotPreflightGuards();
    testParticleGpuMirrorPackPreflight();
    testParticleGpuFramePlanDispatchPreflight();
    testParticleGpuDeviceUploadSkipGuard();
    testParticleGpuEmitGuard();
    testParticleGpuBuffersPreflight();
    testParticleGpuFrameLaunchGuards();
    testParticleGpuEmptyBufferPackGuards();
